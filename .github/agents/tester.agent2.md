...
- Lua integration tests go in `tests/lua/` and run via `cargo test --test lua_tests <dispatcher-name>`
- Rust integration tests may use local helper constructors when a stateful scenario needs one
- Test names: `<subject>_<scenario>_<expected_outcome>`

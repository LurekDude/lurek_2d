...
function detectLunaCalls(text: string): DetectedCall[] {
...
function generateTestContent(sourceFileName: string, calls: DetectedCall[]): string {
  ...
  for (const [mod, modCalls] of modules) {
    ...
    for (const call of modCalls) {
      lines.push(
        `test("luna.${mod}.${call.func} works", function()`,
        `  -- Source line ${call.line}: ${call.text}`,
        `  -- TODO: Add proper test assertion`,
        `  local result = luna.${mod}.${call.func}()`,
        `  assert(result ~= nil, "luna.${mod}.${call.func} should return a value")`,
        `end)`,
        "",
      );
    }
...
function generateFunctionTestContent(
  ...
  if (calls.length > 0) {
    ...
    for (const call of calls) {
      lines.push(
        `test("${funcName} uses luna.${call.module}.${call.func}", function()`,
        `  -- Verify luna.${call.module}.${call.func} is available`,
        `  assert(type(luna.${call.module}.${call.func}) == "function",`,
        `    "luna.${call.module}.${call.func} should be available")`,
        `end)`,
        "",
      );
    }
...
export function registerTestCommands(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("luna.test.generateForFile", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("luna.test.generateForFunction", async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor || editor.document.languageId !== "lua") {
        vscode.window.showWarningMessage("Open a Lua file first.");
        return;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("luna.test.runCurrent", async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor || editor.document.languageId !== "lua") {
      ...
      const lunaPath = vscode.workspace.getConfiguration("luna").get<string>("lunaPath", "luna");
  ...
  // ── luna.test.runAll ─────────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand("luna.test.runAll", async () => {
      const editor = vscode.window.activeTextEditor;
  ...
  // ── luna.test.coverage ───────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand("luna.test.coverage", async () => {
      const editor = vscode.window.activeTextEditor;
      const gameRoot = editor
        ? getGameRoot(editor.document.fileName)
        : vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;

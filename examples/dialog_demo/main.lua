...
function luna.draw()
    -- Background
    luna.graphics.setBackgroundColor(0.12, 0.12, 0.18, 1)

    -- Title
    luna.graphics.setColor(0.6, 0.8, 1.0, 1)
    luna.graphics.print("Dialog Demo", 20, 15)
    luna.graphics.setColor(0.5, 0.5, 0.6, 1)
    luna.graphics.print("Space=advance  1-9=choose  S=skip  R=restart", 20, 40)

    -- Dialog box
    local bx, by, bw, bh = 40, 80, 720, 160
    luna.graphics.setColor(0.15, 0.15, 0.22, 0.95)
    luna.graphics.rectangle("fill", bx, by, bw, bh)
    luna.graphics.setColor(0.4, 0.5, 0.7, 1)
    luna.graphics.rectangle("line", bx, by, bw, bh)
    ...
    if seq then
        ...
        if state == "typing" or state == "waiting" then
            ...
            luna.graphics.print(seq:currentSpeaker(), bx + 15, by + 10)
            -- Revealed text
            luna.graphics.setColor(1, 1, 1, 1)
            luna.graphics.print(seq:revealedText(), bx + 15, by + 40)
            -- State indicator
            luna.graphics.setColor(0.5, 0.5, 0.5, 1)
            if state == "typing" then
                luna.graphics.print("...", bx + bw - 40, by + bh - 25)
            else
                luna.graphics.print("[Space to continue]", bx + bw - 180, by + bh - 25)
            end
        elseif state == "choice" then
            luna.graphics.setColor(1, 0.85, 0.3, 1)
            luna.graphics.print(seq:getChoiceText(), bx + 15, by + 10)
            local labels = seq:getChoiceLabels()
            for i, label in ipairs(labels) do
                luna.graphics.setColor(0.7, 0.9, 1, 1)
                luna.graphics.print(i .. ". " .. label, bx + 30, by + 30 + i * 25)
            end
        elseif state == "paused" then
            luna.graphics.setColor(0.7, 0.7, 0.7, 1)
            luna.graphics.print("(waiting...)", bx + 15, by + 40)
        elseif state == "done" then
            luna.graphics.setColor(0.5, 0.7, 0.5, 1)
            luna.graphics.print("Dialog complete. Press R to restart.", bx + 15, by + 40)
        elseif state == "idle" then
            luna.graphics.setColor(0.5, 0.5, 0.5, 1)
            luna.graphics.print("Press Space to start.", bx + 15, by + 40)
        end
    end

    -- Event log
    luna.graphics.setColor(0.3, 0.3, 0.4, 1)
    luna.graphics.rectangle("fill", 40, 260, 720, 310)
    luna.graphics.setColor(0.5, 0.5, 0.6, 1)
    luna.graphics.print("Event Log:", 50, 265)
    luna.graphics.setColor(0.8, 0.8, 0.8, 1)
    for i, line in ipairs(log_lines) do
        luna.graphics.print(line, 50, 280 + i * 20)
    end
end

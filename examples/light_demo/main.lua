...
function luna.draw()
    -- Dark background
    luna.graphics.clear(0.02, 0.02, 0.03)

    -- Draw walls as dark rectangles
    luna.graphics.setColor(0.15, 0.12, 0.1)
    -- Pillar 1
    luna.graphics.rectangle("fill", 250, 180, 30, 200)
    -- Pillar 2
    luna.graphics.rectangle("fill", 620, 180, 30, 200)
    -- Horizontal wall
    luna.graphics.rectangle("fill", 350, 250, 150, 20)
    -- L-shape
    luna.graphics.rectangle("fill", 150, 300, 50, 20)
    luna.graphics.rectangle("fill", 180, 320, 20, 100)
    -- Diamond obstacle
    luna.graphics.polygon("fill", 700, 300, 730, 280, 760, 300, 730, 320)

    -- Draw player marker
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.circle("fill", player_x, player_y, 8)
    ...
    for _, t in ipairs(torch_lights) do
        if t.light:isValid() then
            ...
            luna.graphics.circle("fill", tx, ty, 5)
        end
    end
    ...
    luna.graphics.print("Luna2D Light Demo", 10, 10)
    luna.graphics.print(string.format("Lights: %d  Occluders: %d", lights_count, occluder_count), 10, 30)
    luna.graphics.print(string.format("Blend: %s  Falloff: %s  Shadows: %s", blend, falloff, tostring(shadows)), 10, 50)
    luna.graphics.print(string.format("Ambient: %.2f", ambient_level), 10, 70)
    luna.graphics.print("WASD=move  1/2/3=blend  F/G/H=falloff  SPACE=shadows  +/-=ambient  T=torches  C=clear", 10, H - 25)
end

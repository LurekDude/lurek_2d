...
def main() -> int:
    ...
    # Always write JSON metadata to docs/API/test_coverage.json (unless --json stdout mode)
    if not args.json:
        json_payload = {
            "rust": {
                "total": rust_total,
        ...
        print(f"[OK] Coverage JSON written to docs/API/test_coverage.json", file=sys.stderr)

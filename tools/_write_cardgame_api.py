"""Write src/lua_api/cardgame_api.rs"""
import os, sys

content = r"""//! Lua bindings for `luna.cardgame.*`.
//!
...
pub fn register(lua: &Lua, luna: &LuaTable) -> LuaResult<()> {
    ...
    module.set(
        ...
        lua.create_function(|_, (name, opts): (String, Option<LuaTable>)| {
            ...
            if let Some(t) = opts {
                if let Ok(cat) = t.get::<String>("category") { def.category = cat; }
                if let Ok(n) = t.get::<String>("name") { def.name = n; }
                if let Ok(stats) = t.get::<LuaTable>("stats") {
                    for pair in stats.pairs::<String, f64>() {
                        let (k, v) = pair?;
                        def.stats.insert(k, v);
                    }
                }
                if let Ok(tags) = t.get::<LuaTable>("tags") {
...
with open('src/lua_api/cardgame_api.rs', 'w', encoding='utf-8') as f:
    f.write(content)
print('cardgame_api.rs written')

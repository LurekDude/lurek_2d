#!/usr/bin/env python3
"""
gen_coverage_gaps.py — Generate an API gap report for Luna2D.
...
Usage:
    python tools/gen_coverage_gaps.py                    # -> docs/API/coverage_gaps.md
...
def generate_report(rust_data: dict, lua_data: dict) -> str:
    ...
    lines: list[str] = []
    lines.append("# Luna2D API Coverage Gaps")
    lines.append("")
    ...
    lines.append("")
    lines.append("These public Rust functions are **not exposed** to the `luna.*` Lua API.")
    ...
    lines.append("These appear as `// (undocumented)` in `docs/API/rust-api.md`.")
    lines.append("")
    ...
    lines.append("These appear without documentation in `docs/API/lua-api.md` and IntelliSense.")
    lines.append("")
    ...
    lines.append("")
    lines.append("**Lua docstrings:** Add `/// description` above the `luna.set(\"name\", ...)` call")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Luna2D API coverage gap report.")

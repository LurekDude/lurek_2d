...
use crate::particle::{
    AreaDistribution, EmissionShape, InsertMode, ParticleConfig, ParticleSystem, RelativeMode,
};
use slotmap::{Key, SlotMap};

fn invalid_particle_handle(function_name: &str) -> LuaError {
...
fn particle_key_from_value(val: &LuaValue) -> LuaResult<ParticleKey> {
    match val {
...
fn parse_color(c: &LuaTable, default_a: f32) -> LuaResult<[f32; 4]> {
...
fn parse_emission_shape(shape: &str, args: Option<&LuaTable>) -> EmissionShape {
...
fn emission_shape_to_lua<'lua>(lua: &'lua Lua, shape: &EmissionShape) -> LuaResult<LuaTable<'lua>> {
    let t = lua.create_table()?;
    match shape {
        EmissionShape::Point => {
            t.set("type", "point")?;
...
pub fn register(lua: &Lua, luna: &LuaTable, state: Rc<RefCell<SharedState>>) -> LuaResult<()> {
        ...
        particle.set(
            ...
            lua.create_function(move |_, config: Option<LuaTable>| {
                ...
                if let Some(t) = config {
                    ...
                    // Multi-stop sizes — prefer `sizes` table, fall back to sizeStart/sizeEnd
                    ...
                    // Multi-stop colors — prefer `colors` table, fall back to colorStart/colorEnd
    ...
    // ── setEmissionRate / getEmissionRate ───────────────────────────────────
    {
        let s = Rc::clone(&state);
        particle.set(
            "setEmissionRate",
    ...
    // ── setEmissionArea / getEmissionArea ───────────────────────────────────
    {
        let s = Rc::clone(&state);
        particle.set(
            "setEmissionArea",
    ...
    // ── setParticleLifetime / getParticleLifetime ───────────────────────────
    {
        let s = Rc::clone(&state);
        particle.set(
            "setParticleLifetime",
    ...
    // ── setEmitterLifetime / getEmitterLifetime ────────────────────────────
    {
        let s = Rc::clone(&state);
        particle.set(
            "setEmitterLifetime",
    ...
    // ── setRadialAcceleration / getRadialAcceleration ──────────────────────
    {
        let s = Rc::clone(&state);
        particle.set(
    ...
    // ── setTangentialAcceleration / getTangentialAcceleration ───────────────
    {
        let s = Rc::clone(&state);
        particle.set(
    ...
    // ── setSizeVariation / getSizeVariation ─────────────────────────────────
    {
        let s = Rc::clone(&state);
        particle.set(
            "setSizeVariation",
    ...
    // ── setSpinVariation / getSpinVariation ─────────────────────────────────
    {
        let s = Rc::clone(&state);
        particle.set(
            "setSpinVariation",
    ...
    // ── setRelativeRotation / hasRelativeRotation ───────────────────────────
    {
        let s = Rc::clone(&state);
        particle.set(
            "setRelativeRotation",
...
fn lua_value_to_f64(v: &LuaValue) -> Option<f64> {
    match v {

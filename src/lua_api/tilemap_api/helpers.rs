...
//!
//! Contains `LuaUserData` implementations for tilemap wrapper types.
...
use crate::tilemap::mapgen::{
    Edge, LayerMode, MapBlock, MapGen, MapGroup, MapOrientation, MapScript, ScriptStep, StepType,
};
use crate::tilemap::tilemap::TileMap;
use crate::tilemap::tileset::{TileAnimFrame, TileSet};

use super::super::lua_types::{add_type_methods, LunaType};
...
///
/// # Fields
/// - `inner` ÔÇö `Rc<RefCell<TileMap>>`.
#[derive(Clone)]
pub(crate) struct LuaTileMap {
...
pub(super) struct LuaMapGen {
    ...
    pub(super) group: Rc<RefCell<MapGroup>>,
}
...
// ---------------------------------------------------------------------------
// LuaMapScript UserData ÔÇö helpers
// ---------------------------------------------------------------------------
...
pub(super) fn parse_script_step(t: &LuaTable) -> LuaResult<ScriptStep> {
    ...
    // Helper for 1-based Ôćĺ 0-based conversion on coordinate/index fields
...
pub(super) fn step_to_table<'lua>(lua: &'lua Lua, step: &ScriptStep) -> LuaResult<LuaTable<'lua>> {
    ...
    /// # Returns
    /// The result.
    t.set("x", step.x + 1)?; // 0-based Ôćĺ 1-based
    /// Y on this MapGroup.
    ///
    /// # Returns
    /// The result.
    ...
    /// # Returns
    /// The result.
    t.set("tileLayer", step.tile_layer + 1)?; // 0-based Ôćĺ 1-based
    t.set(
        "zoneStartY",
        ...
        methods.add_method("getZone", |lua, this, index: usize| {
            ...
            match inner.get_zone(index - 1) {
                Some(zone) => {
                    ...
                    /// # Returns
                    /// The result.
                    t.set("startRow", zone.start_row + 1)?; // 0-based Ôćĺ 1-based

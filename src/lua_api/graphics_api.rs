...
use crate::graphics::renderer::{BlendMode, DrawCommand, DrawMode, StencilAction, TextAlign};
...
fn texture_key_from_value(val: &LuaValue) -> LuaResult<TextureKey> {
    match val {
...
fn font_key_from_value(val: &LuaValue) -> LuaResult<FontKey> {
    match val {
        LuaValue::UserData(ud) => {
...
fn batch_key_from_value(val: &LuaValue) -> LuaResult<SpriteBatchKey> {
    match val {
        LuaValue::UserData(ud) => {
...
fn canvas_key_from_value(val: &LuaValue) -> LuaResult<CanvasKey> {
    match val {
        LuaValue::UserData(ud) => {
...
fn invalid_texture_handle(function_name: &str) -> LuaError {
...
fn invalid_font_handle(function_name: &str) -> LuaError {
    LuaError::RuntimeError(format!(
...
fn invalid_batch_handle(function_name: &str) -> LuaError {
    LuaError::RuntimeError(format!(
...
fn invalid_canvas_handle(function_name: &str) -> LuaError {
    LuaError::RuntimeError(format!(
...
fn invalid_mesh_handle(function_name: &str) -> LuaError {
    LuaError::RuntimeError(format!(
...
fn require_mesh_key(state: &SharedState, id: u64, function_name: &str) -> LuaResult<MeshKey> {
...
pub fn register(lua: &Lua, luna: &LuaTable, state: Rc<RefCell<SharedState>>) -> LuaResult<()> {
    ...
    // ── Feature 4: getColor – read back the current draw colour ─────────────
    ...
    ///
    /// Lua API: luna.graphics.rotate(angle)
    // luna.graphics.rotate(angle)  — radians
    let s = state.clone();
    graphics.set(
        "rotate",
    ...
    // luna.graphics.applyTransform(transform) — apply a Transform object to the current matrix
    ...
    // luna.graphics.reset() — reset all graphics state to defaults
    ...
    // ── Feature 3: Quads (sprite-sheet regions) ───────────────────────────
    ...
    ///
    /// Lua API: luna.graphics.polyline(x1, y1, x2, y2, ...)
    // luna.graphics.polyline(x1, y1, x2, y2, ...)  — 2+ point pairs
    let s = state.clone();
    graphics.set(
        "polyline",
        lua.create_function(move |lua_ref, args: LuaMultiValue| {
    ...
    // luna.graphics.newShader(wgslCode) → shader_id
    ...
    // luna.graphics.setShader(shader_id?) — set active shader or reset to default
    ...
    // luna.graphics.getShader() → shader_id or nil
    ...
    // luna.graphics.hasShaderUniform(shader_id, name) → bool
    ...
    // luna.graphics.releaseShader(shader_id) — release a shader
    ...
    // luna.graphics.newMesh(vertexTable, mode?) → mesh_id
    ...
    // luna.graphics.getMeshVertex(mesh_id, index) → x,y,u,v,r,g,b,a
    ...
    // luna.graphics.getMeshVertexCount(mesh_id) → number
    ...
    // luna.graphics.getMeshTexture(mesh_id) → texture_id or nil

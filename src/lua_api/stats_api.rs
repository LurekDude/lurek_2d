...
impl LuaUserData for LuaSheet {
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method(
            "define",
            |_, this, (name, base, opts): (String, f64, Option<LuaTable>)| {
        ...
        methods.add_method(
            ...
            |_, this, (name, opts): (String, Option<LuaTable>)| {
                ...
                if let Some(opts) = opts {
                    if let Ok(v) = opts.get::<_, u32>("maxLevel") {
                        skill.max_level = v;
                    }
                    if let Ok(v) = opts.get::<_, String>("resource") {
                        skill.resource = v;
                    }
                    if let Ok(v) = opts.get::<_, f64>("cost") {
                        skill.cost = v;
                    }
                    if let Ok(v) = opts.get::<_, f64>("cooldown") {
                        skill.cooldown = v;
                    }
                }
        ...
        methods.add_method("getCooldownRemaining", |_, this, name: String| {
            ...
            s.skills
                .get(&name)
                .map(|sk| sk.cooldown_remaining)
                .ok_or_else(|| {
                    LuaError::RuntimeError(format!(
                        "sheet:getCooldownRemaining: unknown skill '{}'",
                        name
                    ))
                })
        });
        ...
        methods.add_method(
            "definePerk",
            |_, this, (name, opts): (String, Option<LuaTable>)| {
                let require_level = opts
                    .as_ref()
                    .and_then(|o| o.get::<_, u32>("requireLevel").ok())
                    .unwrap_or(0);
                let trait_name = opts
                    .as_ref()
                    .and_then(|o| o.get::<_, String>("traitName").ok());
        ...
        methods.add_method("acquirePerk", |_, this, name: String| {
            ...
            if perk.acquired {
                return Err(LuaError::RuntimeError(format!(
                    "sheet:acquirePerk: '{}' already acquired",
                    name
                )));
            }
        ...
        methods.add_method("setLevelThresholds", |_, this, t: LuaTable| {
            ...
            if let Ok(base) = maybe_base {
                ...
                this.inner.borrow_mut().level_thresholds =
                    LevelThresholds::Linear { base, increment };
            } else {
        ...
        methods.add_method("spendActionPoints", |_, this, n: f64| {
            ...
            let ap = s.action_points.as_mut().ok_or_else(|| {
                LuaError::RuntimeError(
                    "sheet:spendActionPoints: action points not initialised".to_string(),
                )
            })?;
            if ap.current < n {
                return Err(LuaError::RuntimeError(format!(
                    "sheet:spendActionPoints: not enough AP ({:.1} < {:.1})",
                    ap.current, n
                )));
            }
            ap.current -= n;
        ...
        methods.add_method("setPanicThreshold", |_, this, val: f64| {
            ...
            let m = s.morale.as_mut().ok_or_else(|| {
                LuaError::RuntimeError(
                    "sheet:setPanicThreshold: morale not initialised".to_string(),
                )
            })?;
        ...
        methods.add_method("setBerserkThreshold", |_, this, val: f64| {
            ...
            let m = s.morale.as_mut().ok_or_else(|| {
                LuaError::RuntimeError(
                    "sheet:setBerserkThreshold: morale not initialised".to_string(),
                )
            })?;
        ...
        methods.add_method("setResistance", |_, this, (dtype, val): (String, f64)| {
            this.inner
                .borrow_mut()
                .resistances
                .insert(dtype, val.clamp(0.0, 1.0));
            Ok(())
        });
        ...
        methods.add_method(
            "applyDamage",
            |_, this, (stat, amount, dtype): (String, f64, Option<String>)| {
                let actual = this
                    .inner
                    .borrow_mut()
                    .apply_damage(&stat, amount, dtype.as_deref());
                Ok(actual)
            },
        );
        ...
        methods.add_method("snapshot", |lua, this, ()| {
            ...
            for (i, (name, base, min, max, regen, growth)) in
                s.snapshot_attributes().into_iter().enumerate()
            {
                let entry = lua.create_table()?;
                /// Name on this Sheet.
                ///
        ...
        methods.add_method("restore", |_, this, snap: LuaTable| {
            ...
            if let Ok(lvl) = snap.get::<_, u32>("level") {
                s.level = lvl;
            }
    ...
    // luna.stats.defineClass(name, opts)
    {
        ...
        module.set(
            ...
            lua.create_function(move |_, (name, opts): (String, LuaTable)| {
                ...
                registry
                    .borrow_mut()
                    .define_class(&name, bases, trait_names);
                Ok(())
            })?,
        )?;
    }

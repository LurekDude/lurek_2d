...
impl Overlay {
    ...
    fn update_weather(&mut self, dt: f32) {
        // Cap live particle count proportional to intensity (0.0–1.0 → 0–200).
        let max_particles = (self.weather.intensity * 200.0) as usize;
        let w = self.width as f32;
        let h = self.height as f32;

        // Spawn new particles at a rate proportional to intensity.
        // The timer accumulates dt; each time it exceeds the spawn interval
        // a new particle is emitted until the cap is reached.

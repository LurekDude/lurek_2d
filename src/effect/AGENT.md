...
| `stack.rs` | `PostFxStack` — ordered chain of effect indices with parallel `enabled` flags. Manages ping-pong canvas dimensions. 1-based position insertion, per-index enable/disable, and `enabled_effects()` for the GPU layer. || `render.rs` | `generate_render_commands()`, `begin_capture_command()`, `end_capture_command()`, `apply_command()` on `PostFxStack`; emits `BeginPostFx` / `EndPostFx` / `ApplyPostFx` render commands for the scene capture/apply pipeline. || `draw.rs` | CPU headless drawing — `draw_to_image()` on `PostFxStack` for headless testing. || `weather.rs` | `WeatherType` enum (8 variants: None, Rain, Snow, Hail, Dust, Leaves, Ash, Pollen), `WeatherParticle` (position, velocity, size, alpha), and `WeatherState` (spawn timer, wind, intensity, live particle pool). |
| `mod.rs` | — |

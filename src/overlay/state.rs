use super::atmosphere::{CloudState, FilmGrainState, FogState, HeatHazeState, LightningState, VignetteState};
...
///
/// # Fields
/// - `width` — `u32` — Overlay canvas width in pixels.
/// - `height` — `u32` — Overlay canvas height in pixels.
/// - `weather` — Weather particle subsystem.
/// - `ambient` — Ambient lighting with time-of-day colour cycling.
/// - `flash` — One-shot screen flash effect.
/// - `shake` — Camera-shake effect with decaying PRNG offsets.
/// - `fade` — Full-screen colour fade effect.
/// - `clouds` — Scrolling cloud shadow overlay.
/// - `fog` — Uniform atmospheric fog tint.
/// - `heat_haze` — Distortion strength for shimmer effects.
/// - `vignette` — Screen-edge darkening.
/// - `film_grain` — Per-frame random noise overlay.
/// - `lightning` — Hard single-frame lightning flash.
pub struct Overlay {
...
impl Overlay {
    ...
    ///
    /// All subsystems are initialised to their `Default` state — inactive,
    ...
    ///
    /// # Parameters
    /// - `width` — `u32` — Overlay width in pixels.
    /// - `height` — `u32` — Overlay height in pixels.
    ///
    /// # Returns
    /// `Self`.
    ...
    fn update_weather(&mut self, dt: f32) {
        ...
        self.weather.particles.retain(|p| {
            p.x > -50.0 && p.x < w + 50.0 && p.y > -50.0 && p.y < h + 50.0
        });
    }
    ...
    ///
    /// # Parameters
    /// - `r` — `f32` — Red channel (0.0–1.0).
    /// - `g` — `f32` — Green channel (0.0–1.0).
    /// - `b` — `f32` — Blue channel (0.0–1.0).
    ...
    ///
    /// # Parameters
    /// - `r` — `f32` — Red channel of the fill colour (0.0–1.0).
    /// - `g` — `f32` — Green channel (0.0–1.0).
    /// - `b` — `f32` — Blue channel (0.0–1.0).
    /// - `target_alpha` — `f32` — Alpha to reach at the end (0.0–1.0).
    /// - `duration` — `f32` — Fade duration in seconds.
    ...
    /// visible area. Does not discard existing particles — they will be
    /// culled naturally on the next `update` call if they are off-screen.
    ///
    /// # Parameters
    /// - `width` — `u32` — New overlay width in pixels.
    /// - `height` — `u32` — New overlay height in pixels.
    ...
    ///
    /// # Returns
    /// `u32` — Current canvas width in pixels.
    ...
    ///
    /// # Returns
    /// `u32` — Current canvas height in pixels.
    ...
    ///
    /// # Returns
    /// `(u32, u32)` — Width and height in pixels.
    ...
    ///
    /// # Returns
    /// `f32` — Current flash alpha in [0.0, flash.color[3]].
    ...
    ///
    /// # Returns
    /// `f32` — Current lightning alpha in [0.0, lightning.color[3]].

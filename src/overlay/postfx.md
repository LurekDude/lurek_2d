...
- Multi-pass effects (e.g., two-pass Gaussian blur) are handled internally by the C++ Effect
...
- **W1 (MVP)**: Effect chain — ping-pong Canvas pipeline; addEffect(shader) / removeEffect(id)
- **W1**: Built-in bloom effect (threshold + gaussian blur pass)
- **W2**: Built-in effects: vignette, chromatic aberration, CRT scanlines, color grading LUT
- **W2**: Per-effect parameter animation — tween effect uniforms via luna.timer
- **W3**: Temporal anti-aliasing (TAA) — accumulation buffer for smooth edges without MSAA
- **W3**: Screen-space reflections — deferred reflection pass for water/floor surfaces

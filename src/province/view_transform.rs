//! Province map view transform helpers shared by strategy-style map scripts.

/// Computes camera transform that fits the full province map inside the screen.
...
/// Converts a screen-space position to map-space pixel coordinates.
...
/// Converts map-space coordinates to a 0-based integer cell when inside bounds.
...
/// Recomputes camera origin so zooming keeps a screen anchor fixed.

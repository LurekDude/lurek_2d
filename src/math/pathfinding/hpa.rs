...
use crate::math::pathfinding::astar;
...
pub fn hpa_star(
        ...
        if let Some(chunk) = abstract_graph.chunks.get(&chunk_key) {
            for &ent_idx in &chunk.entrance_indices {
                ...
                if complete {
                    if let Some(ref p) = path {
                        let cost = path_cost_f32(p);
                        edges[temp_idx].push(AbstractEdge {
                            to: ent_idx,
                            cost,
                        });
                        edges[ent_idx].push(AbstractEdge {
                            to: temp_idx,
                            cost,
                        });
                    }
    ...
    if start_chunk == goal_chunk {
        ...
        if complete {
            if let Some(ref p) = path {
                let cost = path_cost_f32(p);
                edges[start_idx].push(AbstractEdge {
                    to: goal_idx,
                    cost,
                });
            }

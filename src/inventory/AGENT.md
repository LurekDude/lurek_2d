...
| **Lua API** | `luna.inventory` |
| **Source** | `src/inventory/` |
| **Tests** | `tests/inventory_tests.rs` |
| **Lua Tests** | `tests/lua/unit/test_inventory.lua` |

## Summary

Equipment and item inventory with typed container slots, item stacking,
named equipment attachment points, optional item-set bonuses, and
configurable subsystem toggles. `Container` holds items in one of four
layout modes: Slots (linear list), FixedGrid (every cell always present),
FreeGrid (sparse positional), or Bag (unordered). Each `Slot` carries an
optional type/tag filter, a state flag (Empty, Occupied, Locked, Reserved),
and an optional `ItemStack` payload. `Inventory.equip_slots` maps named
attachment points like "weapon", "head", "chest" to individual Slots,
decoupling rendered equipment from bag contents. `ItemSet` and
`SetRequirement` define which slots must be filled with matching tagged
items to activate a set bonus, enabling classic 2-of-4 or 5-of-5 mechanics.
`SubsystemFlags` acts as a feature toggle: weight simulation, size-based
stacking, item-set detection, and stacking rules can each be independently
enabled or disabled without code changes, making the same inventory system
work for a minimalist platformer or a deep RPG.

## Architecture

```
Inventory (top-level item storage)
  │
  ├── SubsystemFlags { weight, size, stacking, item_sets }
  │     └── enables/disables optional inventory semantics
  │
  ├── containers: HashMap<String, Container>
  │     ├── ContainerMode: Slots | FixedGrid | FreeGrid | Bag
  │     └── slots: Vec<Slot>
  │           ├── SlotState: Empty | Occupied | Locked | Reserved
  │           ├── filter: required item type/tag for this slot
  │           └── item_stack: Option<ItemStack>
  │
  ├── equip_slots: HashMap<String, Slot>
  │     └── Named attachment points ("head", "chest", "weapon"...)
  │
  ├── ItemSet / SetRequirement
  │     ├── defines which items complete the set
  │     └── set_bonus applied when all required slots are filled
  │
  └── Callbacks: on_add, on_remove, on_equip, on_item_set_complete
```
...
| File | Purpose |
|------|---------|
| `container.rs` | InventoryEntry containers with configurable modes |
| `inventory.rs` | Inventory: the top-level item storage with subsystem management |
| `item.rs` | Item definition and item stack |
| `item_set.rs` | Item sets, set requirements, and subsystem flags |
| `slot.rs` | Inventory slots and slot state |
...
- **`ContainerMode`** (enum): Container storage mode. Consult the module-level documentation for the broader usage context and preconditions.
- **`Container`** (struct): A named collection of `Slot`s. Consult the module-level documentation for the broader usage context and preconditions.

### `inventory::inventory`

Inventory: the top-level item storage with subsystem management.

- **`SubsystemFlags`** (struct): Active subsystem flags for an `Inventory`.
- **`Inventory`** (struct): Top-level inventory managing containers, equip slots, item sets, and callbacks.

### `inventory::item`

Item definition and item stack.

- **`InventoryEntry`** (struct): A single item definition with type, tags, weight, size, and stack limit.
- **`ItemStack`** (struct): A counted stack of a single Item type.

### `inventory::item_set`

Item sets, set requirements, and subsystem flags.

- **`SetRequirement`** (struct): A single requirement in an `ItemSet`. Consult the module-level documentation for the broader usage context and...
- **`ItemSet`** (struct): A named set defining bonus conditions: all requirements must be satisfied simultaneously.

### `inventory::slot`

Inventory slots and slot state.

- **`SlotState`** (enum): Valid state strings for a slot. Consult the module-level documentation for the broader usage context and preconditions.
- **`Slot`** (struct): A single inventory position that holds an optional `ItemStack`.

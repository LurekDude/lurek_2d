...
mod tests {
    ...
    fn chunk_allocated_on_write() {
        ...
        m.set_tile(16, 0, 2); // second chunk
    ...
    fn load_and_unload_chunk() {
        ...
        m.unload_chunk(0, 0);
        assert!(!m.is_chunk_loaded(0, 0));
        assert_eq!(m.get_tile(5, 5), 0); // reads from unloaded chunk
    }
    ...
    fn chunks_in_view() {
        let m = ChunkMap::new(16);
        // Viewport 0..64 × 0..64 pixels, 32×32 tiles, chunk 16×16 tiles → chunks 0,1
        let chunks = m.get_chunks_in_view(0.0, 0.0, 511.0, 511.0, 32.0, 32.0);
        // chunk width = 16*32 = 512px; viewport ends just before 512

...
/// - `quit_requested` — Set to `true` by `luna.signal.quit()` to end the game loop.
/// - `exit_code` — Exit code to return when `quit_requested` is `true`.
/// - `restart_requested` — Set to `true` by `luna.signal.restart()` to trigger engine restart.
...
pub struct SharedState {
    ...
    pub exit_code: i32,
    /// Whether a restart was requested via `luna.signal.restart()`.

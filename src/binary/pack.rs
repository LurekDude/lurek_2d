...
//!
//! Provides `write`, `read`, and `measure_size` for the `luna.binary` module.
...
use crate::binary::byte_data::ByteData;
...
fn parse_format(fmt: &str) -> Result<(Endian, Vec<Token>), String> {
    ...
    for token in fmt.split_whitespace() {
        match token {
            ...
            other => return Err(format!("luna.binary: unknown format token '{other}'")),
        }
    }
    Ok((endian, tokens))
}
    ...
    for token in &tokens {
        match token {
            ...
            Token::CStr => {
                ...
                if pos >= data.len() {
                    return Err(format!(
                        "luna.binary read 'cstr': null terminator not found starting at offset {start}"
                    ));
                }
...
pub fn measure_size(format: &str) -> Result<usize, String> {
    ...
    for token in &tokens {
        match token {
            ...
            Token::Str => {
                return Err(
                    "luna.binary size: 'str' token has variable length; cannot compute static size".to_string(),
                )
            }
            Token::CStr => {
                return Err(
                    "luna.binary size: 'cstr' token has variable length; cannot compute static size".to_string(),
                )
            }
        }
    }
...
fn check_bounds(data: &[u8], pos: usize, count: usize, token: &str) -> Result<(), String> {
    if pos + count > data.len() {
        Err(format!(
            "luna.binary read '{token}': buffer underflow at offset {pos} (need {count}, have {})",
            data.len()
        ))
    } else {
        Ok(())
...
fn coerce_u64(values: &[BinValue], idx: usize, token: &str) -> Result<u64, String> {
    match values.get(idx) {
        ...
        Some(_) => Err(format!("luna.binary write '{token}': expected integer at value index {idx}")),
        None    => Err(format!("luna.binary write '{token}': not enough values (expected index {idx})")),
    }
}
...
fn coerce_f64(values: &[BinValue], idx: usize, token: &str) -> Result<f64, String> {
    match values.get(idx) {
        ...
        Some(_) => Err(format!("luna.binary write '{token}': expected numeric at value index {idx}")),
        None    => Err(format!("luna.binary write '{token}': not enough values (expected index {idx})")),
    }
}
...
fn coerce_bool(values: &[BinValue], idx: usize, token: &str) -> Result<bool, String> {
    match values.get(idx) {
        ...
        Some(_) => Err(format!("luna.binary write '{token}': expected bool at value index {idx}")),
        None    => Err(format!("luna.binary write '{token}': not enough values (expected index {idx})")),
    }
}
...
fn coerce_str(values: &[BinValue], idx: usize, token: &str) -> Result<String, String> {
    match values.get(idx) {
        ...
        Some(BinValue::Bytes(b)) => String::from_utf8(b.clone()).map_err(|e| {
            format!("luna.binary write '{token}': bytes at index {idx} are not valid UTF-8: {e}")
        }),
        Some(_) => Err(format!("luna.binary write '{token}': expected string at value index {idx}")),
        None    => Err(format!("luna.binary write '{token}': not enough values (expected index {idx})")),
    }
}

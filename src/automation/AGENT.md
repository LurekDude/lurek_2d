...
| **Tests** | `tests/automation_tests.rs` |

## Summary

Input replay and automated testing via TOML-defined timed step scripts.
The `Simulator` loads named `Script` objects — each containing an ordered
list of timed `Step` records — and plays them back by injecting synthetic
input events into the Luna2D event queue on each `update(dt)` call. A
`Step` records a wall-clock offset in seconds from script start and an
action type (KeyPressed, KeyReleased, MouseMoved, MousePressed,
MouseReleased, TextInput, or Wait) plus action-specific fields such as key
name, scancode, x/y coordinates, button index, and text string. Scripts are
loaded as Lua tables or from TOML files via the bundled `loadFile(path,
name)` helper that delegates TOML parsing to `luna.data.decodeToml()`.
Multiple scripts can be registered simultaneously; `start(name)` selects
which one plays. Step count is capped at 100,000 per script to prevent
unbounded allocation. Primary use-cases are headless integration tests, QA
regression replay, speedrun verification, and recorded developer input
sessions.
...
Simulator (script playback engine)
  ...
  └── update(dt)
        ├── elapsed += dt
        ├── while steps[next].time <= elapsed: dispatch step
        └── dispatch → inject into luna input event queue
              (KeyPressed / MouseMoved / TextInput / Wait)

Dependencies: luna.data.decodeToml (TOML helper), luna.filesystem (file loader)
```

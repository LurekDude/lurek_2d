...
Directed acyclic graph (DAG) pipeline orchestrator for composing
multi-step workflows with dependency tracking, conditional execution,
per-step delays, parallel coroutine dispatch, and result propagation. A
`Pipeline` is a collection of named `PipelineStep` nodes; each step declares
upstream dependencies via an `inputs` list, an `execute` Lua function that
receives resolved upstream results, an optional `condition` guard (step is
skipped if it returns false), an optional `delay` in seconds, and an
`on_error` policy — abort, continue, or retry with configurable attempts.
Setting `optional: true` lets downstream steps proceed even if this step was
skipped. Scheduling is cooperative single-threaded: `start()` performs a
topological sort and rejects cycles; `update(dt)` ticks delay timers and
resumes all independent ready-step coroutines in the same frame for logical
parallelism without OS threads. Results propagate by value so later steps
can combine or transform outputs from earlier ones. Use-cases include
cutscene sequencing, multi-stage quest triggers, procedural content
pipelines, and test automation workflows.

## Architecture

```
Pipeline (DAG of named steps)
  ├── name: String
  ├── steps: HashMap<name, PipelineStep>
  │     ├── inputs: Vec<step_name>    (dependency list)
  │     ├── execute: Lua fn          (received inputs, returns result)
  │     ├── condition: Option<Lua fn> → bool  (skip if false)
  │     ├── delay: f32               (seconds before execution after deps done)
  │     ├── on_error: "abort"|"continue"|"retry"
  │     ├── retry_attempts: u32
  │     └── optional: bool           (dependents proceed if skipped)
  │
  ├── status: HashMap<step, StepStatus { Pending|Running|Done(result)|Skipped|Error }>
  │
  ├── Scheduling (cooperative single-threaded)
  │     ├── topological sort at start() → reject cycles
  │     ├── parallel dispatch via Lua coroutines (no OS threads)
  │     └── update(dt) → tick delays, resume ready coroutines
  │
  └── Serialization: to_json() / from_json() (via luna.data)

Dependencies: luna.data (JSON serialization, optional)
```

...
> **Target:** Luna2D reusable library `luna.sim` backed by a Rust simulation kernel

---

## What This Idea Is

The goal is to port the core of a declarative, graph-based process-flow simulator into Luna2D as an engine-level reusable library. Instead of Python + Flask + YAML, the target stack is:
...
| Flask REST API | `luna.sim.*` Lua API (synchronous from script) |
| DuckDB analytics | `luna.dataframe` post-run analytics in Lua |
| React dashboard | `luna.gui` / `luna.terminal` or external frontend |
...
| [LUA-API-DESIGN.md](LUA-API-DESIGN.md) | Full proposed `luna.sim.*` API surface with function signatures and examples |
...
- ✅ Deterministic Rust kernel, headless testing, Lua authoring, TOML data — all good Luna2D fit.
- ✅ Monitor stream, post-run analytics via `luna.dataframe`, and external visualization — all cleanly separable.

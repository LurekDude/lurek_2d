# Feasibility Study — Block Simulation Library for Luna2D
...
3. Post-run analytics must stay outside the tick loop. Lua plus `luna.dataframe` is the right post-run layer.
...
- **YAML is not used in Luna2D at all.** The migration is total, not partial.

### 2.4 Backend surface

The source project exposes a Flask REST API for start, stop, state inspection, repair, DLQ drain, approval resolution, and speed control. The Luna2D equivalent exposes `luna.sim.*`: a synchronous Lua API that wraps the Rust kernel for all the same operations. A separate web/external wrapper can sit above the Lua API if REST is still needed for a particular deployment.

---

## 3. Luna2D Capability Evidence

### 3.1 What Luna2D already has that helps

| Capability | Luna2D source | Benefit |
|---|---|---|
| TOML parse/encode | `luna.data.parseToml`, `luna.data.encodeToml` | Direct config import; no external parser needed |
| Graph flow simulation | `src/graph/`, `luna.graph.*` | Conceptual reference for flow, queue, capacity patterns |
| Dense arrays | `src/compute/`, `luna.compute.*` | Substrate for numeric value fields in block state |
| Tabular analytics | `src/dataframe/`, `luna.dataframe.*` | Post-run KPI, aggregation, comparison, CSV/JSON export |
| Background workers | `src/thread/`, `luna.thread.*` | Off-main-thread bulk runs and monitor export |
| Charts and widgets | `src/gui/`, `luna.ui.*` | In-engine monitoring dashboard surface |
| Character cell UI | `src/terminal/`, `luna.terminal.*` | Text-mode inspector and debug REPL |
...
### 3.2 What Luna2D's tier rules require
...
| `lua_api` is the bridge layer | Peer composition across Tier 2 modules happens by calling `luna.graph`, `luna.dataframe`, `luna.ui` from Lua |
| `library/` is pure Lua | A Lua helper library can compose across the full public `luna.*` surface |
...
Build everything in `library/blocksim.lua` on top of `luna.graph`, `luna.thread`, `luna.data`, and `luna.dataframe`.
...
| | |
|---|---|
| **Pros** | Correct tier placement; can use `data`, `compute`, `thread`; headless testable; reusable by other frontends through the `luna.sim.*` API; Lua library can compose across it and all other Tier 2 APIs at once |
...
Build the simulation kernel as a separate Rust crate outside Luna2D's `src/` tree and link it as a Cargo dependency.

| | |
|---|---|
| **Pros** | Maximum backend neutrality; easier to publish independently |
| **Cons** | Requires publishing or workspace-linking; integration with `luna.*` API becomes a secondary layer; not the natural home for a "Luna2D library" — the user wants it inside the engine |
...
3. **TOML for static data.** Block definitions and catalogs, thresholds, schedules, presets, and anomaly profiles live in TOML files loaded by `luna.data.parseToml` and passed into `luna.sim.create(spec)`.
...
5. **Analytics are post-run.** KPI computation, comparison across runs, detection correlation, and reporting are done after the run using `luna.dataframe` or an external consumer tool. Nothing analytical runs inside the tick loop.
...
- A built-in web server or REST API endpoint (an external wrapper can add that above `luna.sim`)
- A graph layout engine (visualization is a separate UI layer)
- A DuckDB or SQL driver (use `luna.dataframe` for in-engine analytics)
- Real-time network-distributed simulation
- YAML parsing (YAML is not part of Luna2D's design — migrate to TOML/Lua)
- WASM or browser runtime (Luna2D is desktop-only per constraint A-02)
- Any Python interop
...
| Can TOML and Lua replace YAML for authored graphs? | High — `luna.data.parseToml` exists; Lua DSL is well-suited for wiring composites |
| Can `luna.dataframe` replace DuckDB for post-run analytics? | Medium — it can do grouping, joins, stats, export; may hit limits for very large monitor logs |
...
| Can approval workflows and DLQ/replay be modelled without service-layer assumptions? | Medium — requires some design work on the Luna2D-specific interop model |

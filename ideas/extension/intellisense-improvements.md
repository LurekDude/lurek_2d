...
- **Per-frame allocation warning** — flag `luna.graphics.newImage()` or `luna.audio.newSource()` inside `luna.draw`/`luna.update` callbacks
...
- **Easing function names** when typing `luna.timer.tween(_, _, _, "` → show all easing names with preview
- **Blend mode names** when typing `luna.graphics.setBlendMode("` → show modes with visual description
...
- **Draw modes** when typing `luna.graphics.circle("` → "fill", "line" with explanation
- **Event names** when typing `luna.event.on("` → list all engine event names
...
- Support `luna.filesystem.load()` as a module import path
...
- Flag `luna.graphics.newImage()` called every frame

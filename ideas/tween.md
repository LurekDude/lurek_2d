...
- All active tweens are tracked globally by the `TweenModule`. `luna.animation.update(dt)` advances all registered tweens. Individual tweens auto-register on creation.
...
| Method | Signature | Returns | Description |
|--------|-----------|---------|-------------|
| `tween` | `tween(duration, target, fields [, easing])` | `TweenSequence` | Append a tween step. Same params as `luna.animation.tween()`. Returns self. |
...
local t = luna.animation.tween(2.0, obj, { x = 100, y = 200 }, "cubicOut")
t:onComplete(function() print("done!") end)

function luna.process(dt)
    luna.animation.update(dt)
...
```lua
local t1 = luna.animation.tween(1.0, sprite, { x = 100 })
local t2 = luna.animation.tween(1.5, sprite, { alpha = 0 }, "expoOut")

local par = luna.animation.parallel()
par:add(t1)
par:add(t2)
...
```lua
local pulse = luna.animation.tween(0.5, light, { intensity = 2.0 }, "sineInOut")
...
```lua
luna.animation.registerEasing("bounceCustom", function(t)
    return 1 - math.abs(math.sin(t * math.pi * 3)) * (1 - t)
end)

luna.animation.tween(2.0, obj, { scale = 2.0 }, "bounceCustom")
```
...
**vs luna.gfx** � Graphics renders visuals. Tween modifies numeric fields (x, y, alpha, scale) on data tables; Graphics draws whatever those values say each frame.

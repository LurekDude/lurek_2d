...
1. **Add animation state machine**: `lurek.animation.newStateMachine({idle=clipA, walk=clipB, jump=clipC}, transitions)` � declarative state transitions with optional blend times.
2. **Add crossfade**: `anim:crossfade(targetClip, duration)` � smooth transition between clips.
3. **Add Aseprite import**: `lurek.animation.fromAseprite(jsonPath)` � load frame data from Aseprite export. Very common pixel art workflow.

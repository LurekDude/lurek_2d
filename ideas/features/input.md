...
- 4 input namespaces: `luna.keyboard`, `luna.mouse`, `luna.gamepad`, `luna.touch`
...
- Text input events via `luna.textInput` callback
...
5. **No IME support**: Text input via `luna.textInput` callback works for ASCII but complex input methods (CJK, accented characters) may not be handled.
...
- **Four namespaces vs one**: Having `luna.keyboard`, `luna.mouse`, `luna.gamepad`, `luna.touch` as separate namespaces is clear but verbose. Consider also exposing `luna.input` as a unified query facade.

## Suggestions

1. **Add input action mapping** (high priority): `luna.input.bind("jump", {"space", "gamepad:a"})` → `luna.input.isActionDown("jump")`. This is the single most impactful missing input feature. Every competitor either has it built-in or it's the first thing users build.
2. **Add input recording**: `luna.input.startRecording()` / `luna.input.stopRecording()` → returns a replayable input log. Transforms testing and replay systems.
3. **Add gamepad vibration**: `luna.gamepad.vibrate(pad, low, high, duration)` — requires winit vibration API.
4. **Add custom cursor**: `luna.mouse.setCursor(image, hotX, hotY)` — set cursor from ImageData or system cursor name.
5. **Add input buffer**: `luna.input.wasActionPressedWithin("jump", frames)` — returns true if action was pressed within N frames. Essential for responsive platformer controls.

...
1. **Consider moving to Tier 3 library**: `library/dataframe/` as pure Lua � no Rust overhead. Use `lurek.codec` for CSV import and `lurek.compute` for numerics.
2. **Add window functions**: `df:rollingMean("column", windowSize)` � enables time series analysis, rolling averages for game stats.
3. **Add SQLite import**: `lurek.dataframe.fromSQLite(path, query)` � useful for modders and data-driven game design.
...
| Feature | Lurek2D | Engine A | Engine B | Engine D | Pandas (ref) |
|---|---|---|---|---|---|
| DataFrame | ? | ? | ? | ? | ? |

...
use lurek2d::sprite::NineSlice;
use lurek2d::sprite::SpriteBatch;
...
fn test_phase01_released_texture_handle_reuse_reports_invalid_texture() {
    ...
    let result = lua
        .load(
            r#"
            local released = lurek.graphic.newImage("assets/icon.png")
            assert(type(released) == "userdata")
            assert(released:release() == true)

            local replacement = lurek.graphic.newImage("assets/splash.png")
            assert(type(replacement) == "userdata")
            assert(replacement:getWidth() > 0)

            lurek.graphic.draw(released, 10, 20)
            "#,
        )
        .exec();
    ...
    assert_lua_error_contains(
        result,
        "Image handle is not valid or was released",
    );
}
...
fn test_phase01_released_numeric_texture_handle_reports_invalid_texture() {
    ...
    lua.load(
        r#"
        _phase01_texture = lurek.graphic.newImage("assets/icon.png")
    ...
    let script = format!(
        r#"
        assert(lurek.graphic.release({released_texture_id}) == true)

        local replacement = lurek.graphic.newImage("assets/splash.png")
        assert(type(replacement) == "userdata")
        assert(replacement:getWidth() > 0)

        lurek.graphic.draw({released_texture_id}, 10, 20)
        "#,
    );
...
fn test_phase01_released_sprite_batch_handle_reuse_reports_invalid_batch() {
    ...
    let result = lua
        .load(
            r#"
            local image = lurek.graphic.newImage("assets/icon.png")
            local released = lurek.graphic.newSpriteBatch(image, 4)
            assert(type(released) == "userdata")
            assert(released:release() == true)

            local replacement = lurek.graphic.newSpriteBatch(image, 4)
            assert(type(replacement) == "userdata")
            assert(replacement:getCount() == 0)

            lurek.graphic.spriteBatchAdd(released, 1, 2)
            "#,
        )
        .exec();
    ...
    assert_lua_error_contains(
        result,
        "SpriteBatch handle is not valid or was released",
    );
}
...
fn test_transform_push_queues_push_transform() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.push()");
    assert!(matches!(
...
fn test_transform_pop_queues_pop_transform() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.pop()");
    assert!(matches!(
        state.borrow().render_commands.last(),
...
fn test_transform_translate_queues_correct_values() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.translate(30, 45)");
    let st = state.borrow();
...
fn test_transform_rotate_queues_angle() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.rotate(1.5707963)");
    let st = state.borrow();
...
fn test_transform_scale_uniform_defaults_sy_to_sx() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.scale(2)");
    let st = state.borrow();
...
fn test_transform_scale_nonuniform_queues_distinct_sx_sy() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.scale(3, 0.5)");
    let st = state.borrow();
    if let Some(RenderCommand::Scale { sx, sy }) = st.render_commands.last() {
...
fn test_arc_fill_mode_queues_arc_command_with_default_segments() {
    ...
    run_draw(
        &lua,
        r#"lurek.graphic.arc("fill", 100, 200, 50, 0, 3.14159265)"#,
    );
    let st = state.borrow();
...
fn test_arc_line_mode_queues_line_draw_mode() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, r#"lurek.graphic.arc("line", 0, 0, 10, 0, 1)"#);
    let st = state.borrow();
...
fn test_arc_explicit_segments_overrides_default() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, r#"lurek.graphic.arc("fill", 0, 0, 20, 0, 6.28, 16)"#);
    let st = state.borrow();
...
fn test_drawex_defaults_rotation_scale_origin_to_identity() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.draw(0, 100, 200)");
    let st = state.borrow();
...
fn test_drawex_with_all_params_queues_correct_values() {
    ...
    run_draw(
        &lua,
        "lurek.graphic.draw(1, 50, 60, 0.785, 2.0, 1.5, 10, 20)",
    );
    let st = state.borrow();
    if let Some(RenderCommand::DrawImageEx {
        texture_key,
...
fn test_newquad_and_drawquad_queue_draw_quad_command() {
    ...
    run_draw(
        &lua,
        r#"
        local q = lurek.graphic.newQuad(10, 20, 64, 64, 256, 256)
        lurek.graphic.drawq(0, q, 100, 200)
        "#,
    );
    let st = state.borrow();
...
fn test_get_color_returns_set_color_values() {
    ...
    lua.load(
        r#"
        lurek.graphic.setColor(0.5, 0.25, 0.75, 1.0)
        _r, _g, _b, _a = lurek.graphic.getColor()
        "#,
    )
    .exec()
    .unwrap();
...
fn test_get_color_default_is_white() {
    let (_state, lua) = make_graphics_vm();
    lua.load("_r, _g, _b, _a = lurek.graphic.getColor()")
        .exec()
        .unwrap();
    let r: f32 = lua.globals().get("_r").unwrap();
    let g: f32 = lua.globals().get("_g").unwrap();
...
fn test_polyline_two_points_queues_polyline_command() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.polyline(10, 20, 30, 40)");
    let st = state.borrow();
...
fn test_polyline_three_points_queues_six_coordinates() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.polyline(0, 0, 50, 100, 100, 0)");
    let st = state.borrow();
    if let Some(RenderCommand::Polyline { points }) = st.render_commands.last() {
...
fn test_draw_batch_lua_command() {
    ...
    lua.load(
        r#"
        local batch = lurek.graphic.newSpriteBatch(_tex_id, 100)
        lurek.graphic.spriteBatchAdd(batch, 10, 20)
        lurek.graphic.drawBatch(batch)
        "#,
    )
    .exec()
    .unwrap();
...
fn test_set_blend_mode_lua() {
    ...
    lua.load(
        r#"
        lurek.graphic.setBlendMode("add")
        assert(lurek.graphic.getBlendMode() == "add")
        "#,
    )
    .exec()
    .unwrap();
...
fn test_set_blend_mode_invalid() {
    let (_state, lua) = make_graphics_vm();
    let result = lua.load(r#"lurek.graphic.setBlendMode("invalid")"#).exec();
...
fn test_set_blend_mode_pushes_draw_command() {
    ...
    lua.load(
        r#"
        lurek.graphic.setBlendMode("multiply")
        lurek.graphic.rectangle("fill", 10, 20, 100, 100)
        lurek.graphic.setBlendMode("alpha")
        "#,
    )
    .exec()
    .unwrap();
...
fn test_get_blend_mode_default_is_alpha() {
    let (_state, lua) = make_graphics_vm();
    let mode: String = lua
        .load("return lurek.graphic.getBlendMode()")
        .eval()
        .unwrap();
...
fn test_set_blend_mode_additive_alias() {
    let (state, lua) = make_graphics_vm();
    lua.load(r#"lurek.graphic.setBlendMode("additive")"#)
        .exec()
        .unwrap();
    let st = state.borrow();
    assert!(st
...
fn test_blend_mode_all_variants() {
    ...
    for mode in &["alpha", "add", "multiply", "replace", "screen"] {
        lua.load(&format!(
            r#"
            lurek.graphic.setBlendMode("{}")
            assert(lurek.graphic.getBlendMode() == "{}")
            "#,
            mode, mode
        ))
        .exec()
...
fn test_animation_lua_create_and_query() {
    ...
    lua.load(
        r#"
        local id = lurek.graphic.newAnimation(32, 32, 4, 8, 0.1)
        assert(type(id) == "number")
        assert(lurek.graphic.isAnimationPlaying(id) == true)
        assert(lurek.graphic.getAnimationFrame(id) == 0)
    "#,
    )
    .exec()
    .unwrap();
...
fn test_animation_lua_update_and_draw() {
    ...
    lua.load(
        r#"
        local anim = lurek.graphic.newAnimation(32, 32, 2, 4, 0.1)
        lurek.graphic.updateAnimation(anim, 0.15)
        assert(lurek.graphic.getAnimationFrame(anim) == 1)
        lurek.graphic.drawAnimation(anim, 0, 100, 200)
    "#,
    )
    .exec()
    .unwrap();
...
fn test_animation_lua_pause_resume_reset() {
    ...
    lua.load(
        r#"
        local id = lurek.graphic.newAnimation(32, 32, 4, 4, 0.1)
        lurek.graphic.pauseAnimation(id)
        assert(lurek.graphic.isAnimationPlaying(id) == false)
        lurek.graphic.resumeAnimation(id)
        assert(lurek.graphic.isAnimationPlaying(id) == true)
        lurek.graphic.updateAnimation(id, 0.2)
        assert(lurek.graphic.getAnimationFrame(id) > 0)
        lurek.graphic.resetAnimation(id)
        assert(lurek.graphic.getAnimationFrame(id) == 0)
    "#,
    )
    .exec()
...
fn test_canvas_new_returns_userdata_handles() {
    ...
    lua.load(
        r#"
        local c0 = lurek.graphic.newCanvas(256, 128)
        local c1 = lurek.graphic.newCanvas(64, 64)
...
fn test_canvas_get_size_accepts_canvas_userdata() {
    ...
    lua.load(
        r#"
        local canvas = lurek.graphic.newCanvas(320, 240)
        local w, h = lurek.graphic.getCanvasSize(canvas)
...
fn test_canvas_set_canvas_pushes_command_and_get_canvas_round_trips() {
    ...
    lua.load(
        r#"
        local canvas = lurek.graphic.newCanvas(100, 100)
        lurek.graphic.setCanvas(canvas)
        local active = lurek.graphic.getCanvas()
        assert(active ~= nil)
        assert(active:type() == "Canvas")
        assert(active:getWidth() == 100)
        assert(active:getHeight() == 100)
        lurek.graphic.rectangle("fill", 0, 0, 50, 50)
        lurek.graphic.setCanvas()
        assert(lurek.graphic.getCanvas() == nil)
    "#,
    )
    .exec()
    .unwrap();
...
fn test_canvas_draw_pushes_command() {
    ...
    lua.load(
        r#"
        local canvas = lurek.graphic.newCanvas(200, 200)
        lurek.graphic.draw(canvas, 10, 20)
    "#,
    )
    .exec()
    .unwrap();
...
fn test_canvas_draw_with_transform() {
    ...
    lua.load(
        r#"
        local canvas = lurek.graphic.newCanvas(100, 100)
        lurek.graphic.draw(canvas, 50, 60, 1.57, 2.0, 3.0, 10, 20)
    "#,
    )
    .exec()
    .unwrap();
...
fn test_canvas_release_active_canvas_clears_target_and_invalidates_handle() {
    ...
    let result = lua
        .load(
            ...
            lurek.graphic.setCanvas(canvas)
            assert(lurek.graphic.getCanvas() ~= nil)
            assert(lurek.graphic.releaseCanvas(canvas) == true)
            assert(lurek.graphic.getCanvas() == nil)

            lurek.graphic.draw(canvas, 1, 2)
            "#,
        )
        .exec();
    ...
    assert_lua_error_contains(
        result,
        "lurek.graphic.draw: invalid or already-released canvas handle",
    );
...
fn test_phase02_shear_queues_command_with_expected_factors() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.shear(0.5, -0.25)");
...
fn test_phase02_origin_queues_origin_command() {
    let (state, lua) = make_graphics_vm();
    run_draw(&lua, "lurek.graphic.origin()");
...
fn test_phase02_get_stack_depth_tracks_push_pop_origin_and_reset() {
    ...
    lua.load(
        r#"
        assert(lurek.graphic.getStackDepth() == 1)
        lurek.graphic.push()
        lurek.graphic.push()
        assert(lurek.graphic.getStackDepth() == 3)
        lurek.graphic.origin()
        assert(lurek.graphic.getStackDepth() == 3)
        lurek.graphic.pop()
        assert(lurek.graphic.getStackDepth() == 2)
        lurek.graphic.reset()
        assert(lurek.graphic.getStackDepth() == 1)
        "#,
    )
...
fn test_phase02_scissor_round_trips_and_intersects() {
    ...
    lua.load(
        r#"
        lurek.graphic.setScissor(10, 20, 100, 50)
        local x, y, w, h = lurek.graphic.getScissor()
        assert(x == 10 and y == 20 and w == 100 and h == 50)

        lurek.graphic.intersectScissor(50, 30, 100, 100)
        x, y, w, h = lurek.graphic.getScissor()
...
fn test_phase02_color_mask_round_trips_and_resets() {
    ...
    lua.load(
        r#"
        lurek.graphic.setColorMask(false, true, false, true)
        local r, g, b, a = lurek.graphic.getColorMask()
        assert(r == false and g == true and b == false and a == true)

        lurek.graphic.setColorMask()
        r, g, b, a = lurek.graphic.getColorMask()
...
fn test_phase02_default_filter_round_trips_min_and_mag() {
    ...
    lua.load(
        r#"
        lurek.graphic.setDefaultFilter("linear", "nearest")
        local min, mag = lurek.graphic.getDefaultFilter()
        assert(min == "linear")
        assert(mag == "nearest")
        "#,
    )
...
fn test_phase02_default_filter_round_trips_anisotropy() {
    ...
    let result = lua
        .load(
            r#"
            lurek.graphic.setDefaultFilter("linear", "nearest", 4)
            local min, mag, anisotropy = lurek.graphic.getDefaultFilter()
            assert(min == "linear")
            assert(mag == "nearest")
...
fn test_phase02_stats_report_resource_counts_and_render_counters() {
    ...
    lua.load(
        r#"
        _phase02_image = lurek.graphic.newImage("assets/icon.png")
        _phase02_canvas = lurek.graphic.newCanvas(32, 16)
        _phase02_font = lurek.graphic.newFont("assets/fonts/bitmap_8x14.png", 14)
        "#,
    )
    .exec()
    .unwrap();
    ...
    lua.load(
        r#"
        local stats = lurek.graphic.getStats()
...
fn test_phase02_stats_texture_memory_reflects_loaded_images() {
    ...
    let result = lua
        .load(
            r#"
            lurek.graphic.newImage("assets/icon.png")
            local stats = lurek.graphic.getStats()
...
fn test_phase02_stencil_queues_begin_geometry_end_and_test_commands() {
    ...
    lua.load(
        r#"
        lurek.graphic.stencil(function()
            lurek.graphic.rectangle("fill", 1, 2, 3, 4)
        end, "increment", 7, true)
        lurek.graphic.setStencilTest("greater", 7)
        lurek.graphic.setStencilTest()
        "#,
    )
    .exec()
    .unwrap();
...
fn test_phase02_shader_round_trips_active_shader_and_uniforms() {
    ...
    let script = format!(
        ...
        assert(lurek.graphic.getShader() == shader)
        lurek.graphic.sendShader(shader, "tint", {{1.0, 0.5, 0.25, 1.0}})
        assert(lurek.graphic.hasShaderUniform(shader, "tint"))
        lurek.graphic.setShader()
        assert(lurek.graphic.getShader() == nil)
        "#,
...
fn test_phase02_shader_compile_rejects_invalid_wgsl() {
    let (_state, lua) = make_graphics_vm();
    let result = lua
        .load(r#"lurek.graphic.newShader("not valid wgsl")"#)
        .exec();
...
fn test_phase02_released_mesh_handle_reports_invalid_mesh_and_skips_queueing_draw() {
    ...
    let result = lua
        .load(
            r#"
            local released = lurek.graphic.newMesh({
                {0, 0, 0, 0, 1, 0, 0, 1},
                {8, 0, 1, 0, 0, 1, 0, 1},
                {0, 8, 0, 1, 0, 0, 1, 1},
            })
            ...
            local replacement = lurek.graphic.newMesh({
                {1, 1, 0, 0, 1, 1, 1, 1},
                {9, 1, 1, 0, 1, 1, 1, 1},
                {1, 9, 0, 1, 1, 1, 1, 1},
            })
            assert(lurek.graphic.getMeshVertexCount(replacement) == 3)

            lurek.graphic.draw(released, 5, 6)
            "#,
        )
        .exec();
    ...
    assert_lua_error_contains(
        result,
        "lurek.graphic.draw: invalid or already-released mesh handle",
    );
...
fn test_phase02_mesh_round_trips_vertex_state_texture_and_draw_command() {
    ...
    lua.load(
        ...
        assert(lurek.graphic.getMeshVertexCount(mesh) == 3)

        lurek.graphic.setMeshVertex(mesh, 2, {10, 0, 1, 0, 1, 1, 0, 1})
        local x, y, u, v, r, g, b, a = lurek.graphic.getMeshVertex(mesh, 2)
        assert(x == 10 and y == 0 and u == 1 and v == 0)
        assert(r == 1 and g == 1 and b == 0 and a == 1)

        lurek.graphic.setMeshTexture(mesh, _phase02_mesh_texture)
        lurek.graphic.setMeshVertexMap(mesh, {1, 2, 3})
        lurek.graphic.draw(mesh, 5, 6, 0.5, 2, 3, 4, 5)
        "#,
    )
    .exec()
    .unwrap();
...
fn test_phase02_printf_pushes_formatted_text_command_with_alignment() {
    ...
    lua.load(
        ...
        local font = lurek.graphic.newFont("assets/fonts/bitmap_8x14.png", 14)
        lurek.graphic.setFont(font)
        lurek.graphic.printf("wrapped text", 12, 34, 56, "center")
        "#,
    )
    .exec()
    .unwrap();
...
fn nine_slice_lua_method_draw() {
    ...
    lua.load(
        ...
        lurek.graphic.drawNineSlice(ns, 10, 20, 400, 300)
        "#,
    )
    .exec()
    .unwrap();
...
fn nine_slice_lua_negative_insets_error() {
    ...
    let result = lua
        .load(
            r#"
            local img = lurek.graphic.newImage("assets/icon.png")
            local ns = lurek.graphic.newNineSlice(img, -5, 10, 10, 10)
            "#,
        )
        .exec();
...
fn nine_slice_lua_excessive_insets_error() {
    ...
    let result = lua
        .load(
            r#"
            local img = lurek.graphic.newImage("assets/icon.png")
            local ns = lurek.graphic.newNineSlice(img, 500, 500, 500, 500)
            "#,
        )
        .exec();
...
fn test_phase02_reset_restores_phase02_state_defaults() {
    ...
    let setup = format!(
        r#"
        local shader = lurek.graphic.newShader([=[{}]=])
        local canvas = lurek.graphic.newCanvas(8, 8)

        lurek.graphic.push()
        lurek.graphic.push()
        lurek.graphic.setScissor(1, 2, 3, 4)
        lurek.graphic.setColorMask(false, true, false, true)
        lurek.graphic.setPointSize(5)
        lurek.graphic.setShader(shader)
        lurek.graphic.setCanvas(canvas)
        lurek.graphic.reset()
        "#,
    ...
    let scissor_arity: i64 = lua
        .load("return select('#', lurek.graphic.getScissor())")
        .eval()
        .unwrap();
    let color_mask: (bool, bool, bool, bool) = lua
        .load("return lurek.graphic.getColorMask()")
        .eval()
        .unwrap();
    let point_size: f32 = lua
        .load("return lurek.graphic.getPointSize()")
        .eval()
        .unwrap();
    let shader_value: mlua::Value = lua.load("return lurek.graphic.getShader()").eval().unwrap();
    let canvas_value: mlua::Value = lua.load("return lurek.graphic.getCanvas()").eval().unwrap();
...
fn test_phase02_wireframe_round_trips_and_queues_command() {
    ...
    lua.load(
        r#"
        assert(lurek.graphic.isWireframe() == false)
        lurek.graphic.setWireframe(true)
        assert(lurek.graphic.isWireframe() == true)
        lurek.graphic.setWireframe(false)
        assert(lurek.graphic.isWireframe() == false)
        "#,
    )
    .exec()
...
fn graphics_draw_dispatch_image_with_rotation_pushes_draw_image_ex() {
    ...
    run_draw(
        ...
        r#"
        local img = lurek.graphic.newImage("assets/icon.png")
        lurek.graphic.draw(img, 5, 10, 1.57)
        "#,
    );
    let st = state.borrow();
    let found = st.render_commands.iter().any(|cmd| {
...
fn graphics_draw_dispatch_canvas_userdata_pushes_draw_canvas() {
    ...
    run_draw(
        &lua,
        r#"
        local canvas = lurek.graphic.newCanvas(64, 64)
        lurek.graphic.draw(canvas, 30, 40)
        "#,
    );
    let st = state.borrow();
    let found = st.render_commands.iter().any(|cmd| {
...
fn graphics_draw_dispatch_sprite_batch_userdata_pushes_draw_batch() {
    ...
    run_draw(
        ...
        r#"
        local img = lurek.graphic.newImage("assets/icon.png")
        local batch = lurek.graphic.newSpriteBatch(img, 10)
        lurek.graphic.draw(batch, 0, 0)
        "#,
    );
    let st = state.borrow();
    let found = st
...
fn graphics_draw_dispatch_nil_returns_error() {
    let (_state, lua) = make_graphics_vm();
    let result = lua.load("lurek.graphic.draw(nil, 0, 0)").exec();
...
fn graphics_draw_dispatch_string_returns_error() {
    let (_state, lua) = make_graphics_vm();
    let result = lua.load("lurek.graphic.draw('not_drawable', 0, 0)").exec();
...
fn graphics_draw_ex_dispatch_image_userdata_pushes_draw_image_ex() {
    ...
    run_draw(
        ...
        r#"
        local img = lurek.graphic.newImage("assets/icon.png")
        lurek.graphic.draw(img, 15, 25, 0.5, 2.0, 2.0, 0, 0)
        "#,
    );
    let st = state.borrow();
    let found = st.render_commands.iter().any(|cmd| {
...
fn graphics_draw_ex_dispatch_canvas_userdata_pushes_draw_canvas() {
    ...
    run_draw(
        &lua,
        r#"
        local canvas = lurek.graphic.newCanvas(32, 32)
        lurek.graphic.draw(canvas, 5, 10, 0, 1, 1, 0, 0)
        "#,
    );
    let st = state.borrow();
    let found = st
...
fn graphics_draw_ex_sy_defaults_to_sx() {
    ...
    run_draw(
        ...
        r#"
        local img = lurek.graphic.newImage("assets/icon.png")
        lurek.graphic.draw(img, 0, 0, 0, 3.0)
        "#,
    );
    let st = state.borrow();
...
fn graphics_save_screenshot_queues_save_path() {
    let (state, lua) = make_graphics_vm();
    lua.load(r#"lurek.graphic.saveScreenshot('save/test_frame.png')"#)
        .exec()
...
fn graphics_save_screenshot_rejects_non_save_path() {
    let (_state, lua) = make_graphics_vm();
    let result = lua
        .load(r#"lurek.graphic.saveScreenshot('frame.png')"#)
        .exec();
    assert!(
        result.is_err(),
...
fn graphics_stencil_mode_round_trip() {
    let (state, lua) = make_graphics_vm();
    lua.load(r#"lurek.graphic.setStencilMode("replace", "always", 1)"#)
        .exec()
...
fn graphics_depth_mode_round_trip() {
    let (state, lua) = make_graphics_vm();
    lua.load(r#"lurek.graphic.setDepthMode("less", true)"#)
        .exec()
...
fn graphics_stencil_mode_defaults_in_get() {
    ...
    let result: (String, String, u8) = lua
        .load(r#"return lurek.graphic.getStencilMode()"#)
        .eval()
...
fn graphics_depth_mode_defaults_in_get() {
    ...
    let result: (String, bool) = lua
        .load(r#"return lurek.graphic.getDepthMode()"#)
        .eval()
...
fn graphics_clear_stencil_resets_to_default() {
    ...
    lua.load(
        r#"
        lurek.graphic.setStencilMode("replace", "equal", 7)
...
fn graphics_stencil_mode_unknown_action_errors() {
    let (_state, lua) = make_graphics_vm();
    let result = lua
        .load(r#"lurek.graphic.setStencilMode("explode")"#)
        .exec();
...
fn graphics_depth_mode_unknown_mode_errors() {
    let (_state, lua) = make_graphics_vm();
    let result = lua.load(r#"lurek.graphic.setDepthMode("turbo")"#).exec();

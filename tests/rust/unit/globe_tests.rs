...
mod topology_tests {
    ...
    // ProvinceGraph::find_path_default

    #[test]
    fn find_path_default_connected_provinces() {
        let mut g = ProvinceGraph::new();
        // Build a chain: 0 — 1 — 2
        let mut p0 = Province::new(0, vec![(0.0_f32, 0.0_f32)]);
        p0.neighbors = vec![1];
        let mut p1 = Province::new(1, vec![(1.0_f32, 0.0_f32)]);
        p1.neighbors = vec![0, 2];
        let mut p2 = Province::new(2, vec![(2.0_f32, 0.0_f32)]);
        p2.neighbors = vec![1];
        g.insert(p0).unwrap();
        g.insert(p1).unwrap();
        g.insert(p2).unwrap();
        let path = g.find_path_default(0, 2);
        assert!(path.is_some(), "path from 0 to 2 should exist");
    }

    #[test]
    fn find_path_default_disconnected_returns_none() {
        let mut g = ProvinceGraph::new();
        g.insert(Province::new(0, vec![])).unwrap();
        g.insert(Province::new(1, vec![])).unwrap();
        // No adjacency — path should not exist.
        let path = g.find_path_default(0, 1);
        assert!(path.is_none(), "disconnected provinces should have no path");
    }

    #[test]
    fn find_path_default_self_path() {
        let mut g = ProvinceGraph::new();
        g.insert(Province::new(0, vec![])).unwrap();
        // Path from a province to itself should be Some (zero cost).
        let path = g.find_path_default(0, 0);
        // Some implementations return Some([0]), others differ — just no panic.
        let _ = path;
    }
    ...
    #[test]
    fn reachable_default_no_neighbors_only_start() {
        let mut g = ProvinceGraph::new();
        g.insert(Province::new(0, vec![])).unwrap();
        let reachable = g.reachable_default(0, 5.0);
        // Start is always in the result (cost 0).
        assert!(reachable.contains_key(&0));
    }

    #[test]
    fn reachable_default_chain_within_budget() {
        let mut g = ProvinceGraph::new();
        let mut p0 = Province::new(0, vec![(0.0_f32, 0.0_f32)]);
        p0.neighbors = vec![1];
        let mut p1 = Province::new(1, vec![(1.0_f32, 0.0_f32)]);
        ...
        g.insert(p1).unwrap();
        g.insert(p2).unwrap();
        let reachable = g.reachable_default(0, 3.0);
        assert!(reachable.contains_key(&1));
        assert!(reachable.contains_key(&2));
    }

    #[test]
    fn reachable_default_zero_budget_only_start() {
        let mut g = ProvinceGraph::new();
        let mut p0 = Province::new(0, vec![(0.0_f32, 0.0_f32)]);
        p0.neighbors = vec![1];
        let mut p1 = Province::new(1, vec![(0.0_f32, 0.0_f32)]);
        p1.neighbors = vec![0];
        g.insert(p0).unwrap();
        g.insert(p1).unwrap();
        let reachable = g.reachable_default(0, 0.0);
        // With zero budget only the start should be in the result.
        assert!(reachable.contains_key(&0));
        assert!(!reachable.contains_key(&1));
    }

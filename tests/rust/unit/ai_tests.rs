...
#[test]
fn aiworld_add_agent_duplicate_errors() {
    let mut world = AIWorld::new();
...
#[test]
fn agent_decision_model_as_str() {
...
#[test]
fn fsm_current_state_none() {
    let fsm = StateMachine::new();
    assert!(fsm.current_state().is_none());
}

#[test]
fn fsm_time_in_state() {
    let fsm = StateMachine::new();
...
#[test]
fn steering_combine_mode_parse() {
...
#[test]
fn steering_disabled_returns_zero() {
    let seek = SteeringBehaviorType::Seek {
        target: (100.0, 0.0),
        base: SteeringBase {
            weight: 1.0,
...
#[test]
fn steering_manager_calculate_weighted() {
...
#[test]
fn qlearner_serialize_deserialize() {
    let mut ql = QLearner::new(2, 3);
    ql.set_q(0, 0, 1.5);
    ql.set_q(0, 1, 2.5);
    ql.set_q(0, 2, 3.5);
...
#[test]
fn qlearner_deserialize_dimension_mismatch() {
...
#[test]
fn qlearner_end_episode_decays_epsilon() {
    let mut ql = QLearner::new(2, 2);
...
#[test]
fn goap_plan_already_satisfied() {
    let mut planner = GOAPPlanner::new();
    let mut goal_state = HashMap::new();
...
#[test]
fn squad_get_formation_position() {
...
#[test]
fn command_queue_enqueue_dequeue() {
    let lua = make_lua();
...
#[test]
fn command_queue_cancel_current() {
    let lua = make_lua();
    let mut cq = CommandQueue::new();

...
// ─── New tests: getMod, emitHook, releaseRefs ───

#[test]
fn test_lua_mod_manager_get_mod() {
    let (_state, lua) = make_vm();
    lua.load(
        r#"
        local mgr = luna.modding.newModManager()
        local m = luna.modding.newMod({ id = "findme", name = "Findable" })
        mgr:registerMod(m)
        local found = mgr:getMod("findme")
        assert(found ~= nil, "getMod should return the mod")
        assert(found:getName() == "Findable")
        local absent = mgr:getMod("nope")
        assert(absent == nil, "getMod returns nil for missing mod")
    "#,
    )
    .exec()
    .unwrap();
...
#[test]
fn test_lua_mod_manager_get_mod_after_unregister() {
    let (_state, lua) = make_vm();
    lua.load(
        r#"
        local mgr = luna.modding.newModManager()
        mgr:registerMod(luna.modding.newMod({ id = "temp" }))
        assert(mgr:getMod("temp") ~= nil)
        mgr:unregisterMod("temp")
        assert(mgr:getMod("temp") == nil)
    "#,
    )
    .exec()
...
#[test]
fn test_lua_mod_manager_emit_hook() {
    let (_state, lua) = make_vm();
    lua.load(
        r#"
        local mgr = luna.modding.newModManager()
        local log = {}
        local m1 = luna.modding.newMod({ id = "mod1" })
        m1:setHook("onInit", function(ctx) table.insert(log, "m1:" .. ctx) end)
        local m2 = luna.modding.newMod({ id = "mod2" })
        m2:setHook("onInit", function(ctx) table.insert(log, "m2:" .. ctx) end)
        mgr:registerMod(m1)
        mgr:registerMod(m2)
        local count = mgr:emitHook("onInit", "hello")
        assert(count == 2, "emitHook should return count of mods that handled")
        assert(#log == 2)
        -- Both mods received the hook
        local found_m1, found_m2 = false, false
        for _, v in ipairs(log) do
            if v == "m1:hello" then found_m1 = true end
            if v == "m2:hello" then found_m2 = true end
        end
        assert(found_m1 and found_m2, "both hooks should fire")
    "#,
    )
    .exec()
    .unwrap();
...
#[test]
fn test_lua_mod_manager_emit_hook_skips_disabled() {
    let (_state, lua) = make_vm();
    lua.load(
        r#"
        local mgr = luna.modding.newModManager()
        local called = false
        local m = luna.modding.newMod({ id = "disabled-mod" })
        m:setHook("tick", function() called = true end)
        m:setEnabled(false)
        mgr:registerMod(m)
        local count = mgr:emitHook("tick")
        assert(count == 0, "disabled mod hooks should not fire")
        assert(called == false)
    "#,
    )
    .exec()
...
#[test]
fn test_lua_mod_manager_emit_hook_no_such_hook() {
    let (_state, lua) = make_vm();
    lua.load(
        r#"
        local mgr = luna.modding.newModManager()
        mgr:registerMod(luna.modding.newMod({ id = "a" }))
        local count = mgr:emitHook("nonexistent")
        assert(count == 0)
    "#,
    )
    .exec()
...
#[test]
fn test_lua_mod_manager_release_refs() {
    let (_state, lua) = make_vm();
    lua.load(
        r#"
        local mgr = luna.modding.newModManager()
        local m = luna.modding.newMod({ id = "clean" })
        m:setHook("x", function() end)
        m:setConfig({ a = 1 })
        mgr:registerMod(m)
        assert(mgr:getMod("clean") ~= nil)
        mgr:releaseRefs()
        -- After release, all mods are removed
        assert(mgr:getModCount() == 0)
        assert(mgr:getMod("clean") == nil)
    "#,
    )

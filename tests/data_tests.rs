...
#[test]
fn toml_encode_requires_table_root() {
    let result = toml_convert::encode_toml(&toml::Value::String("hello".into()));
    assert!(result.is_err());
    assert!(result
        .unwrap_err()
        .contains("TOML documents must be tables at the root"));
}
...
#[test]
fn test_lua_parse_toml_datetime_decodes_to_string() {
    let (_state, lua) = make_vm();
    lua.load(
        r#"
        local t = luna.data.parseToml('when = 1979-05-27T07:32:00Z')
        assert(type(t.when) == "string")
        assert(t.when == "1979-05-27T07:32:00Z")
        "#,
    )
    .exec()
    .unwrap();
...
#[test]
fn test_lua_encode_toml_requires_table_root_error_prefix() {
    let (_state, lua) = make_vm();
    let result = lua
        .load(
            r#"
    ...
    assert!(error.contains("luna.data.encodeToml:"));
    assert!(error.contains("TOML documents must be tables at the root"));
}
...
fn test_lua_parse_toml_error() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("luna.data.parseToml:"));
}

#[test]
fn test_lua_encode_toml_rejects_mixed_table() {
    let (_state, lua) = make_vm();
    let result = lua
        .load(
            r#"
    ...
    let error = result.unwrap_err().to_string();
    assert!(error.contains("luna.data.encodeToml:"));
    assert!(error.contains("tables cannot mix string keys and array-style indices"));
}

#[test]
fn test_lua_encode_toml_rejects_sparse_array() {
    let (_state, lua) = make_vm();
    let result = lua
        .load(
            r#"
            luna.data.encodeToml({ [1] = "first", [3] = "third" })
            "#,
        )
        .exec();
    assert!(result.is_err());
    let error = result.unwrap_err().to_string();
    assert!(error.contains("luna.data.encodeToml:"));
    assert!(error.contains("contiguous integer keys starting at 1"));
}

#[test]
fn test_lua_encode_toml_rejects_unsupported_value() {
    let (_state, lua) = make_vm();
    let result = lua
        .load(
            r#"
            luna.data.encodeToml({ callback = function() end })
            "#,
        )
        .exec();
    assert!(result.is_err());
    let error = result.unwrap_err().to_string();
    assert!(error.contains("luna.data.encodeToml:"));
    assert!(error.contains("unsupported function value"));
}

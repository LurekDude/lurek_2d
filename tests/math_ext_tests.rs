...
use luna2d::math::noise_generator::{DistType, FractalType, MapGenOptions, NoiseGenerator, NoiseKind};
use luna2d::math::procgen::{self, CellularOpts, VoronoiOpts};
use luna2d::math::raycaster2d::Raycaster2D;
use luna2d::math::raycasting::{self, Segment};
use luna2d::math::spatial_hash::SpatialHash;
use luna2d::math::tile_walker::{Facing, TileWalker};
...
fn test_noise_generator_fbm() {
    ...
    assert!(
        v.abs() < 5.0,
        "fbm value {v} seems unreasonably large"
    );
}
...
fn test_noise_generator_generate_map() {
    ...
    assert_eq!(map.len(), 32 * 16, "Map should have width * height elements");
...
fn test_grid_flow_field() {
    ...
    assert_eq!(field.len(), 25, "Flow field should have width*height entries");
...
fn test_spatial_hash_update() {
    ...
    assert!(!old.contains(&"c".to_string()), "Should not find at old pos");
...
fn test_raycaster_project_sprite() {
    ...
    let proj = rc.project_sprite(
        4.5, 6.5, // sprite position (ahead in +Y)
        4.5, 2.5, // player position
...
fn test_tile_walker_move_forward() {
    ...
    assert!(moved, "Should move forward when no raycaster (no collision)");
    assert_eq!(tw.x(), 6);
    assert_eq!(tw.y(), 5);
}
...
fn test_angle_between() {
    ...
    assert!(a.abs() < 1e-4, "Angle to (1,0) from origin should be ~0, got {a}");
...
fn test_circle_intersects_circle() {
    // Overlapping circles
    assert!(geometry::circle_intersects_circle(0.0, 0.0, 5.0, 3.0, 0.0, 5.0));
    // Non-overlapping circles
    assert!(!geometry::circle_intersects_circle(0.0, 0.0, 2.0, 10.0, 0.0, 2.0));
    // Touching circles (distance = r1 + r2)
    assert!(geometry::circle_intersects_circle(0.0, 0.0, 3.0, 6.0, 0.0, 3.0));
}

#[test]
fn test_circle_intersects_line() {
    // Line through center of circle
    let (hit, p1, p2) = geometry::circle_intersects_line(
        0.0, 0.0, 5.0, -10.0, 0.0, 10.0, 0.0,
    );
...
fn test_circle_intersects_segment() {
    ...
    let (hit, p1, p2) = geometry::circle_intersects_segment(
        0.0, 0.0, 5.0, -10.0, 0.0, 10.0, 0.0,
    );
    ...
    let (miss, _, _) = geometry::circle_intersects_segment(
        0.0, 0.0, 2.0, 10.0, 10.0, 20.0, 10.0,
    );
...
fn test_segment_intersects_segment() {
    ...
    let (hit, pt) = geometry::segment_intersects_segment(
        0.0, 0.0, 4.0, 4.0, 4.0, 0.0, 0.0, 4.0,
    );
    assert!(hit, "Crossing segments should intersect");
    let (ix, iy) = pt.unwrap();
    assert!((ix - 2.0).abs() < 1e-4, "Intersection x should be 2, got {ix}");
    assert!((iy - 2.0).abs() < 1e-4, "Intersection y should be 2, got {iy}");
}

#[test]
fn test_segment_no_intersection() {
    // Parallel segments: (0,0)-(4,0) and (0,2)-(4,2)
    let (hit, _) = geometry::segment_intersects_segment(
...
fn test_closest_point_on_segment() {
    ...
    assert!((px2 - 4.0).abs() < 1e-4, "Should clamp to endpoint, got {px2}");
...
fn test_point_in_polygon() {
    ...
    let verts = [0.0f32, 0.0, 4.0, 0.0, 4.0, 4.0, 0.0, 4.0];
    assert!(geometry::point_in_polygon(&verts, 2.0, 2.0), "Center should be inside");
    assert!(!geometry::point_in_polygon(&verts, 5.0, 5.0), "Outside should not be inside");
}
...
fn test_convex_hull() {
    ...
    assert_eq!(hull.len(), 8, "Hull should have 4 vertices (8 floats), got {}", hull.len());
}
...
fn test_cast_ray_2d() {
    let segments = vec![
        Segment { x1: 5.0, y1: -5.0, x2: 5.0, y2: 5.0 }, // vertical wall at x=5
    ];
...
fn test_field_of_view() {
    // Simple box room
    let segments = vec![
        Segment { x1: -5.0, y1: -5.0, x2: 5.0, y2: -5.0 },
        Segment { x1: 5.0, y1: -5.0, x2: 5.0, y2: 5.0 },
        Segment { x1: 5.0, y1: 5.0, x2: -5.0, y2: 5.0 },
        Segment { x1: -5.0, y1: 5.0, x2: -5.0, y2: -5.0 },
    ];
    ...
    assert!(vis.len() >= 6, "Visibility polygon needs at least 3 vertices (6 floats), got {}", vis.len());
    assert_eq!(vis.len() % 2, 0, "Should have even number of floats (x,y pairs)");
}
...
fn test_distance_shade() {
    ...
    assert!(shade_near >= 0.0 && shade_near <= 1.0, "Shade should be [0,1], got {shade_near}");
    assert!(shade_far >= 0.0 && shade_far <= 1.0, "Shade should be [0,1], got {shade_far}");

    let shade_zero = raycasting::distance_shade(0.0, 20.0);
    assert!((shade_zero - 1.0).abs() < 1e-4, "Distance 0 → full brightness");
}
...
fn test_voronoi_diagram() {
    ...
    assert_eq!(distances.len(), 20 * 20, "Distance map should be width*height");

...
#[test]
fn test_cellvalue_cmp_for_sort_nil_goes_last() {
...
#[test]
fn test_dataframe_default_is_empty() {
...
#[test]
fn test_dataframe_from_raw() {
    let df = sample_df();
    assert_eq!(df.nrows(), 3);
    assert_eq!(df.ncols(), 2);
    assert_eq!(df.columns(), &["name", "score"]);
}

#[test]
fn test_dataframe_count_alias() {
    let df = sample_df();
    assert_eq!(df.count(), df.nrows());
}
...
#[test]
fn test_dataframe_add_column_duplicate_errors() {
    let mut df = sample_df();
...
#[test]
fn test_dataframe_remove_column_by_name() {
    let mut df = sample_df();
    df.remove_column(ColRef::Name("score".to_string())).unwrap();
    assert_eq!(df.ncols(), 1);
    assert_eq!(df.columns(), &["name"]);
}

#[test]
fn test_dataframe_remove_column_not_found_errors() {
    let mut df = sample_df();
...
fn test_dataframe_rename_column() {
    ...
    df.rename_column(ColRef::Name("score".to_string()), "points")
        .unwrap();
    assert_eq!(df.columns(), &["name", "points"]);
}

#[test]
fn test_dataframe_rename_column_duplicate_errors() {
    let mut df = sample_df();
...
#[test]
fn test_dataframe_get_column_by_name() {
    let df = sample_df();
...
#[test]
fn test_dataframe_get_column_by_index() {
    let df = sample_df();
...
#[test]
fn test_dataframe_resolve_col_index_zero_errors() {
    let df = sample_df();
...
#[test]
fn test_dataframe_resolve_col_index_out_of_range_errors() {
    let df = sample_df();
...
#[test]
fn test_dataframe_add_row_returns_index() {
    let mut df = sample_df();
    let idx = df.add_row(&[
...
#[test]
fn test_dataframe_add_row_missing_column_fills_nil() {
    let mut df = sample_df();
...
#[test]
fn test_dataframe_remove_row_out_of_range_errors() {
    let mut df = sample_df();
...
#[test]
fn test_dataframe_get_row_out_of_range_errors() {
    let df = sample_df();
...
#[test]
fn test_dataframe_get_value_out_of_range_errors() {
    let df = sample_df();
    assert!(df.get_value(99, ColRef::Name("name".to_string())).is_err());
}

#[test]
fn test_dataframe_set_value_out_of_range_errors() {
    let mut df = sample_df();
    assert!(df
...
fn test_filter_equals() {
    ...
    assert_eq!(
        result
            .get_value(0, ColRef::Name("name".to_string()))
            .unwrap(),
        CellValue::Text("Bob".to_string())
    );
}
...
#[test]
fn test_filter_unsupported_op_errors() {
    let df = sample_df();
...
fn test_sort_ascending() {
    ...
    let last = sorted
        ...
        .unwrap();
    assert_eq!(first, CellValue::Number(75.0));
    assert_eq!(last, CellValue::Number(90.0));
}
...
fn test_sort_descending() {
    ...
    let first = sorted
        .get_value(0, ColRef::Name("score".to_string()))
        .unwrap();
    assert_eq!(first, CellValue::Number(90.0));
}
...
fn test_head() {
    ...
    assert_eq!(h.nrows(), 2);
    assert_eq!(
        h.get_value(0, ColRef::Name("name".to_string())).unwrap(),
        CellValue::Text("Alice".to_string())
    );
}
...
fn test_tail() {
    ...
    assert_eq!(t.nrows(), 1);
    assert_eq!(
        t.get_value(0, ColRef::Name("name".to_string())).unwrap(),
        CellValue::Text("Charlie".to_string())
    );
}
...
#[test]
fn test_slice_out_of_range_start_errors() {
    let df = sample_df();
...
fn test_select_columns() {
    ...
    assert_eq!(projected.columns(), &["score"]);
...
fn test_count_by() {
    ...
    assert_eq!(counts.columns(), &["value", "count"]);
}
...
#[test]
fn test_analytics_empty_column_errors() {
    let df = DataFrame::from_raw(
        vec!["x".to_string()],
...
#[test]
fn test_describe_returns_stats_rows() {
    let df = numeric_df();
...
#[test]
fn test_describe_empty_no_numeric_returns_empty() {
    let df = DataFrame::from_raw(
        vec!["label".to_string()],
...
fn test_json_roundtrip() {
    ...
    assert_eq!(
        restored
            .get_value(1, ColRef::Name("name".to_string()))
            .unwrap(),
        CellValue::Text("Bob".to_string())
    );
}
...
fn test_binary_roundtrip() {
    ...
    assert_eq!(
        restored
            .get_value(2, ColRef::Name("name".to_string()))
            .unwrap(),
        CellValue::Text("Charlie".to_string())
    );
}
...
fn test_binary_roundtrip_all_types() {
    ...
    assert_eq!(
        restored
            .get_value(0, ColRef::Name("c".to_string()))
            .unwrap(),
        CellValue::Text("hello".to_string())
    );
    assert_eq!(
        restored
            .get_value(0, ColRef::Name("d".to_string()))
            .unwrap(),
...
#[test]
fn test_database_add_and_get_table() {
...
#[test]
fn test_database_get_table_mut() {
    let mut db = Database::new();
    db.add_table("users", sample_df());
...
#[test]
fn test_database_remove_table_not_found_errors() {
    let mut db = Database::new();
    assert!(db.remove_table("nonexistent").is_err());
}

#[test]
fn test_database_list_tables_sorted() {
    let mut db = Database::new();
...
#[test]
fn test_dataframe_merge_appends_rows() {
...
fn test_random_deterministic_with_seed() {
    ...
    for row in 0..5 {
        assert_eq!(
            df1.get_value(row, ColRef::Name("name".to_string()))
                .unwrap(),

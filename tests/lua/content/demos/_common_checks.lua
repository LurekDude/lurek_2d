...
--   4. No banned render API calls (lurek.render.rectangle, lurek.graphic.*).
...
--   7. No old namespace names (lurek.graphic, lurek.keyboard, lurek.mouse).
--
...
function demo_common_checks(demo_name, demo_path)
    ...
    describe(demo_name .. ": API correctness", function()
        ...
        it("does not use old lurek.graphic namespace", function()
            if not src then pending("source missing") return end
            -- Allow 'lurek.graphic' only if it's the globe 'graphic' property name.
            -- We target 'lurek.graphic.' method calls specifically.
            refute_pattern(src, "lurek%.graphic%.",
                "old namespace lurek.graphic.* found — render API is lurek.render.*")
        end)

        it("does not use old lurek.keyboard namespace", function()
            if not src then pending("source missing") return end
            refute_pattern(src, "lurek%.keyboard%.",
                "old namespace lurek.keyboard found — use lurek.input")
        end)

        it("does not use old lurek.mouse namespace", function()
            if not src then pending("source missing") return end
            -- Allow lurek.mouse only as a string argument like input.bind("mouse1")
            -- which won't match lurek.mouse.* anyway.
            refute_pattern(src, "lurek%.mouse%.",
                "old namespace lurek.mouse found — use lurek.input")
        end)

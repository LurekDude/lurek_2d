-- Luna2D Golden File Test: Math Constants
...
describe("golden: math constants", function()
    it("generates constant table", function()
        local output = {}
        output[#output + 1] = "pi=" .. string.format("%.15f", luna.math.pi)
        output[#output + 1] = "e=" .. string.format("%.15f", luna.math.exp(1))
        output[#output + 1] = "sqrt2=" .. string.format("%.15f", luna.math.sqrt(2))

        -- Trig identity: sin^2 + cos^2 = 1
        for deg = 0, 360, 15 do
            local rad = luna.math.rad(deg)
            local s = luna.math.sin(rad)
            local c = luna.math.cos(rad)

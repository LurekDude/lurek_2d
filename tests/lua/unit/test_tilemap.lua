-- Luna2D Tilemap API Tests
-- Covers luna.tilemap module: factory functions, TileSet, TileMap,
...
describe("luna.tilemap module exists", function()
    it("luna.tilemap is a table", function()
        expect_type("table", luna.tilemap)
    end)

    it("exposes factory functions", function()
        expect_type("function", luna.tilemap.newTileSet)
        expect_type("function", luna.tilemap.newTileMap)
        expect_type("function", luna.tilemap.newAutoTileSheet)
        expect_type("function", luna.tilemap.newChunkMap)
        expect_type("function", luna.tilemap.newIsoMap)
        expect_type("function", luna.tilemap.newMapBlock)
        expect_type("function", luna.tilemap.newMapGroup)
        expect_type("function", luna.tilemap.newMapScript)
        expect_type("function", luna.tilemap.newMapGen)
    end)

    it("exposes coordinate helper functions", function()
        expect_type("function", luna.tilemap.toScreenIso)
        expect_type("function", luna.tilemap.fromScreenIso)
        expect_type("function", luna.tilemap.toScreenHex)
        expect_type("function", luna.tilemap.fromScreenHex)
        expect_type("function", luna.tilemap.hexNeighbors)
        expect_type("function", luna.tilemap.hexDistance)
        expect_type("function", luna.tilemap.hexRound)
        expect_type("function", luna.tilemap.hexLine)
        expect_type("function", luna.tilemap.hexRing)
        expect_type("function", luna.tilemap.hexSpiral)
        expect_type("function", luna.tilemap.hexArea)
        expect_type("function", luna.tilemap.hexRotate)
        expect_type("function", luna.tilemap.hexReflect)
        expect_type("function", luna.tilemap.isoRotate)
        expect_type("function", luna.tilemap.isoDirectionName)
        expect_type("function", luna.tilemap.isoDirectionFromAngle)
    end)

    it("exposes TMX loader", function()
        expect_type("function", luna.tilemap.loadTMX)
    end)

    it("exposes IsoMap tile-part constants", function()
        expect_equal(1, luna.tilemap.FLOOR)
        expect_equal(2, luna.tilemap.NORTH_WALL)
        expect_equal(3, luna.tilemap.WEST_WALL)
        expect_equal(4, luna.tilemap.OBJECT)
    end)
end)
...
describe("luna.tilemap isometric coordinates", function()
    it("toScreenIso converts (1,1) to expected screen position", function()
        local sx, sy = luna.tilemap.toScreenIso(1, 1, 32, 16)
    ...
    it("fromScreenIso round-trips with toScreenIso", function()
        local tx, ty = 3, 5
        local sx, sy = luna.tilemap.toScreenIso(tx, ty, 32, 16)
        local rx, ry = luna.tilemap.fromScreenIso(sx, sy, 32, 16)
        expect_near(tx, rx, 0.01)
        expect_near(ty, ry, 0.01)
    end)

    it("toScreenIso at origin returns 0,0 for tile (0,0)", function()
        local sx, sy = luna.tilemap.toScreenIso(0, 0, 32, 16)
        expect_near(0, sx, 0.001)
        expect_near(0, sy, 0.001)
    end)

    it("isoRotate direction 1 by 1 step = direction 2", function()
        local dir = luna.tilemap.isoRotate(1, 1)
        expect_equal(2, dir)
    end)

    it("isoRotate wraps at 4 back to 1", function()
        local dir = luna.tilemap.isoRotate(4, 1)
        expect_equal(1, dir)
    end)

    it("isoRotate by 4 steps returns same direction", function()
        for d = 1, 4 do
            local rotated = luna.tilemap.isoRotate(d, 4)
            expect_equal(d, rotated)
        end
    end)

    it("isoDirectionName returns expected strings", function()
        expect_equal("south", luna.tilemap.isoDirectionName(1))
        expect_equal("west",  luna.tilemap.isoDirectionName(2))
        expect_equal("north", luna.tilemap.isoDirectionName(3))
        expect_equal("east",  luna.tilemap.isoDirectionName(4))
    end)

    it("isoDirectionFromAngle snaps to nearest direction", function()
        local dir = luna.tilemap.isoDirectionFromAngle(0)
...
describe("luna.tilemap hexagonal coordinates", function()
    it("toScreenHex and fromScreenHex round-trip", function()
        local q, r = 2, 3
        local sx, sy = luna.tilemap.toScreenHex(q, r, 16)
        local rq, rr = luna.tilemap.fromScreenHex(sx, sy, 16)
        expect_near(q, rq, 0.5)
        expect_near(r, rr, 0.5)
    end)

    it("hexDistance between same cell is 0", function()
        expect_equal(0, luna.tilemap.hexDistance(2, 3, 2, 3))
    end)

    it("hexDistance between adjacent cells is 1", function()
        expect_equal(1, luna.tilemap.hexDistance(0, 0, 1, 0))
    end)

    it("hexDistance is symmetric", function()
        local d1 = luna.tilemap.hexDistance(1, 2, 4, 5)
        local d2 = luna.tilemap.hexDistance(4, 5, 1, 2)
        expect_equal(d1, d2)
    end)

    it("hexRound returns integers", function()
        local q, r = luna.tilemap.hexRound(1.4, 2.7)
    ...
    it("hexNeighbors returns 6 cells", function()
        local neighbors = luna.tilemap.hexNeighbors(0, 0)
...
describe("luna.tilemap.newTileSet", function()
    it("creates a TileSet userdata", function()
        local ts = luna.tilemap.newTileSet(1, 16, 4, 32, 32)
    ...
    it("getFirstGid non-1 start", function()
        local ts = luna.tilemap.newTileSet(17, 8, 4, 16, 16)
    ...
    it("getTileDimensions returns width and height", function()
        local ts = luna.tilemap.newTileSet(1, 16, 4, 32, 48)
    ...
    it("getSpacing returns spacing passed to constructor", function()
        local ts = luna.tilemap.newTileSet(1, 16, 4, 32, 32, 2)
    ...
    it("getMargin returns margin passed to constructor", function()
        local ts = luna.tilemap.newTileSet(1, 16, 4, 32, 32, 0, 4)
...
describe("luna.tilemap.newTileMap", function()
    ...
    it("getChunkSize uses custom value", function()
        local tm = luna.tilemap.newTileMap(32, 32, 8)
...
describe("TileMap collision", function()
    it("isSolid returns false for GID 0", function()
        local tm = luna.tilemap.newTileMap(32, 32)
        local ts = luna.tilemap.newTileSet(1, 4, 2, 32, 32)
        tm:addTileSet(ts)
...
describe("luna.tilemap.newAutoTileSheet", function()
    ...
    it("creates AutoTileSheet for composite48 layout", function()
        local sheet = luna.tilemap.newAutoTileSheet(16, 16, "composite48")
        expect_type("userdata", sheet)
    end)

    it("creates AutoTileSheet for minimal16 layout", function()
        local sheet = luna.tilemap.newAutoTileSheet(16, 16, "minimal16")
        expect_type("userdata", sheet)
    end)

    it("rejects invalid layout name", function()
        expect_error(function()
            luna.tilemap.newAutoTileSheet(16, 16, "invalid_layout")
        end)
    end)
    ...
    it("getTileWidth returns value from constructor", function()
        local sheet = luna.tilemap.newAutoTileSheet(24, 32, "minimal16")
    ...
    it("applyToTileSet attaches rules to a TileSet", function()
        local sheet = luna.tilemap.newAutoTileSheet(16, 16, "minimal16")
        local ts = luna.tilemap.newTileSet(1, 16, 4, 16, 16)
        expect_no_error(function()
...
describe("luna.tilemap.newChunkMap", function()
    it("creates a ChunkMap userdata", function()
        local cm = luna.tilemap.newChunkMap()
    ...
    it("getChunkSize uses custom value", function()
        local cm = luna.tilemap.newChunkMap(8)
...
describe("luna.tilemap.newIsoMap", function()
    ...
    it("getWidth and getHeight return constructor values", function()
        local iso = luna.tilemap.newIsoMap(8, 6, 64, 32, 24)
    ...
    it("addLevel increases level count", function()
        local iso = luna.tilemap.newIsoMap(4, 4, 64, 32, 24)
        iso:addLevel()
        expect_equal(1, iso:getLevelCount())
    end)

    it("setTilePart and getTilePart round-trip for floor part", function()
        local iso = luna.tilemap.newIsoMap(5, 5, 64, 32, 24)
        iso:addLevel()
        iso:setTilePart(1, 1, 1, luna.tilemap.FLOOR, 3)
        expect_equal(3, iso:getTilePart(1, 1, 1, luna.tilemap.FLOOR))
    end)
    ...
    it("setTilePart rejects index 0 for level", function()
        ...
        iso:addLevel()
        expect_error(function()
            iso:setTilePart(0, 1, 1, luna.tilemap.FLOOR, 3)  -- level must be >= 1
        end)
    end)
end)
...
describe("luna.tilemap.newMapScript", function()
...
describe("luna.tilemap.newMapGen", function()
    it("creates a MapGen from small preset", function()
        local group = luna.tilemap.newMapGroup("world")
        local gen = luna.tilemap.newMapGen(group, "small", 4)
        expect_type("userdata", gen)
    end)

    it("creates a MapGen from medium preset", function()
        local group = luna.tilemap.newMapGroup("world")
        local gen = luna.tilemap.newMapGen(group, "medium", 4)
        expect_type("userdata", gen)
    end)

    it("creates a MapGen from large preset", function()
        local group = luna.tilemap.newMapGroup("world")
        local gen = luna.tilemap.newMapGen(group, "large", 4)
        expect_type("userdata", gen)
    end)

    it("creates a MapGen from numeric dimensions", function()
        local group = luna.tilemap.newMapGroup("world")
        local gen = luna.tilemap.newMapGen(group, 4, 4, 4)
        expect_type("userdata", gen)
    end)
    ...
    it("rejects invalid size string", function()
        local group = luna.tilemap.newMapGroup("world")
        expect_error(function()
            luna.tilemap.newMapGen(group, "huge", 4)
        end)
    end)
...
describe("luna.tilemap.loadTMX", function()
    ...
    it("loadTMX throws an error for invalid XML", function()
        expect_error(function()
            luna.tilemap.loadTMX("not xml at all")
        end)
    end)
...
<map version="1.10" tiledversion="1.10.0" orientation="orthogonal"
 ...
 <layer id="1" name="Ground" width="2" height="2">
  <data encoding="csv">1,1,1,1</data>
        ...
        local result, err = luna.tilemap.loadTMX(minimal_tmx)
...
<map version="1.10" tiledversion="1.10.0" orientation="orthogonal"
 ...
 <layer id="1" name="Tiles" width="3" height="2">
  <data encoding="csv">1,1,1,1,1,1</data>
        ...
        local result = luna.tilemap.loadTMX(minimal_tmx)
        expect_not_nil(result)

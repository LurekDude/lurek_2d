-- test_evidence_graphic_draw_cmds.lua
-- Evidence test: new lurek.graphic draw commands (bezier curves, gradient rect,
-- colored polygon, iso cube tile, hex tile, sort group, bevel rect, layers, path).

-- @description Covers suite: Evidence: lurek.graphic new GPU draw commands.
describe("Evidence: lurek.graphic new GPU draw commands", function()

    -- ── Quad Bézier ──────────────────────────────────────────────────────────
    -- @covers lurek.graphic.drawQuadBezier
    it("drawQuadBezier: no error on valid call", function()
        local ok = pcall(lurek.graphic.drawQuadBezier,
            0, 0,     -- start
            100, -50, -- control
            200, 0,   -- end
    ...
    -- ── Cubic Bézier ─────────────────────────────────────────────────────────
    -- @covers lurek.graphic.drawCubicBezier
    it("drawCubicBezier: no error on valid call", function()
        local ok = pcall(lurek.graphic.drawCubicBezier,
            0, 0,
            33, -60, 66, -60,
            100, 0,
            {r=1, g=0, b=1, a=1}, 1.5
    ...
    -- ── Draw Path ────────────────────────────────────────────────────────────
    -- @covers lurek.graphic.drawPath
    it("drawPath: open path no error", function()
        local ok = pcall(lurek.graphic.drawPath,
    ...
    -- ── Gradient Rect ────────────────────────────────────────────────────────
    -- @covers lurek.graphic.drawGradientRect
    it("drawGradientRect: vertical gradient no error", function()
        local ok = pcall(lurek.graphic.drawGradientRect,
            10, 10, 200, 100,
    ...
    -- ── Colored Polygon ──────────────────────────────────────────────────────
    -- @covers lurek.graphic.drawColoredPolygon
    it("drawColoredPolygon: triangle no error", function()
        ...
        local ok = pcall(lurek.graphic.drawColoredPolygon, verts)
        expect_equal(ok, true)
    end)

    -- ── Isometric Cube Tile ──────────────────────────────────────────────────
    -- @covers lurek.graphic.drawIsoCubeTile
    it("drawIsoCubeTile: no error", function()
        local ok = pcall(lurek.graphic.drawIsoCubeTile,
            200, 150, 32,
    ...
    -- ── Hex Tile ─────────────────────────────────────────────────────────────
    -- @covers lurek.graphic.drawHexTile
    it("drawHexTile: flat-top no error", function()
        local ok = pcall(lurek.graphic.drawHexTile,
            150, 150, 30,
            "flat",
    ...
    -- ── Sort Group ───────────────────────────────────────────────────────────
    -- @covers lurek.graphic.beginSortGroup
    -- @covers lurek.graphic.pushSortKey
    -- @covers lurek.graphic.flushSortGroup
    it("sort group: begin / pushKey / flush no error", function()
        local ok = pcall(function()
            lurek.graphic.beginSortGroup(42)
            lurek.graphic.pushSortKey(0.5)
            lurek.graphic.flushSortGroup(42)
        end)
        expect_equal(ok, true)
    end)

    -- ── Bevel Rect ───────────────────────────────────────────────────────────
    -- @covers lurek.graphic.drawBevelRect
    it("drawBevelRect: minimal call no error", function()
        local ok = pcall(lurek.graphic.drawBevelRect,
            50, 50, 120, 80
        )
        expect_equal(ok, true)
    end)
    ...
    -- ── Layer Push / Pop ─────────────────────────────────────────────────────
    -- @covers lurek.graphic.pushLayer
    -- @covers lurek.graphic.popLayer
    it("pushLayer / popLayer: no error", function()
        local ok = pcall(function()
            lurek.graphic.pushLayer("add", 0.8)
            lurek.graphic.popLayer()
        end)
        expect_equal(ok, true)
    end)

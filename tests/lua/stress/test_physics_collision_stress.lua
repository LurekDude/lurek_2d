-- Luna2D Stress Test: Physics Collision Storm
...
describe("physics stress: collision storm", function()
    it("creates 500 bodies in a confined space", function()
        ...
        -- Ground
        luna.physics.newBody(world, 250, 500, "static")
        ...
        for i = 1, 500 do
            local x = 50 + (i % 20) * 20
            local y = -i * 5
            bodies[i] = luna.physics.newBody(world, x, y, "dynamic")
        end
        ...
        for step = 1, 300 do
            luna.physics.step(world, 1.0 / 60.0)
        end

        -- Check that a body moved due to gravity
        local x, y = luna.physics.getBody(world, bodies[1])
    ...
    it("detects collisions between moving bodies", function()
        ...
        local a = luna.physics.newBody(world, 100, 100, "dynamic")
        local b = luna.physics.newBody(world, 200, 100, "dynamic")

        -- Give them opposing velocities
        luna.physics.setBodyVelocity(world, a, 50, 0)
        luna.physics.setBodyVelocity(world, b, -50, 0)

        -- Step until they might collide
        local collisions_detected = false
        for step = 1, 120 do
            luna.physics.step(world, 1.0 / 60.0)
            local events = luna.physics.getCollisions(world)
    ...
    it("circle bodies handle mass collision", function()
        ...
        for i = 1, 200 do
            ...
            luna.physics.newCircleBody(world, x, y, 5, "dynamic")
        end
...
describe("physics stress: determinism", function()
    it("same initial state produces same result", function()
        local function run_simulation()
            local world = luna.physics.newWorld(0, 100)
            local body = luna.physics.newBody(world, 100, 0, "dynamic")

            for step = 1, 60 do
                luna.physics.step(world, 1.0 / 60.0)
            end

            local x, y = luna.physics.getBody(world, body)
            return x, y
        end

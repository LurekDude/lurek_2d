-- Luna2D Stress Test: Math Operations
...
describe("math stress: trigonometry throughput", function()
    it("10000 sin/cos pairs", function()
        ...
        for i = 1, 10000 do
            local angle = i * 0.001
            sum = sum + luna.math.sin(angle) + luna.math.cos(angle)
        end
...
describe("math stress: vector operations", function()
    it("10000 distance calculations", function()
        ...
        for i = 1, 10000 do
            ...
            sum = sum + luna.math.sqrt((x2 - x1)^2 + (y2 - y1)^2)
        end
    ...
    it("10000 normalize operations", function()
        ...
        for i = 1, 10000 do
            local x, y = i, i * 2
            local len = luna.math.sqrt(x * x + y * y)
            if len > 0 then
                local nx, ny = x / len, y / len
                local check_len = luna.math.sqrt(nx * nx + ny * ny)
                if luna.math.abs(check_len - 1.0) < 0.001 then
                    count = count + 1
                end
            end
        end

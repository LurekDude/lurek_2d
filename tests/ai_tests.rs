...
fn test_agent_decision_model_parse() {
    ...
    assert_eq!(DecisionModel::parse_str("steering"), Some(DecisionModel::Steering));
    assert_eq!(DecisionModel::parse_str("fsm+steering"), Some(DecisionModel::FsmSteering));
    assert_eq!(DecisionModel::parse_str("bt+steering"), Some(DecisionModel::BtSteering));
...
fn test_bt_node_selector_count() {
    let node = BTNode::Selector {
        children: vec![
            BTNode::Selector { children: vec![], running_idx: 0 },
            BTNode::Sequence { children: vec![], running_idx: 0 },
            BTNode::Selector { children: vec![], running_idx: 0 },
        ],
        running_idx: 0,
    };
    ...
    // Empty selector
    let empty = BTNode::Selector { children: vec![], running_idx: 0 };
...
fn test_bt_node_decorator_count() {
    let child = BTNode::Selector { children: vec![], running_idx: 0 };
    let inverter = BTNode::Inverter { child: Box::new(child) };
...
fn test_bt_node_reset() {
    let mut node = BTNode::Sequence {
        children: vec![
            BTNode::Repeater {
                child: Box::new(BTNode::Selector { children: vec![], running_idx: 0 }),
                count: 5,
                done: 3,
            },
        ],
    ...
    node.reset();
    if let BTNode::Sequence { running_idx, children, .. } = &node {
...
fn test_steering_disabled_returns_zero() {
    let seek = SteeringBehaviorType::Seek {
        target: (100.0, 0.0),
        base: SteeringBase { weight: 1.0, enabled: false },
    };
    let force = seek.calculate((0.0, 0.0), (0.0, 0.0), 100.0, 0.016);
...
fn test_steering_manager_calculate_weighted() {
    ...
    mgr.behaviors.push(SteeringBehaviorType::Seek {
        target: (100.0, 0.0),
        base: SteeringBase { weight: 1.0, enabled: true },
    });
...
fn test_response_curve_parse() {
    ...
    assert_eq!(ResponseCurve::parse_str("quadratic"), ResponseCurve::Quadratic);
    assert_eq!(ResponseCurve::parse_str("logistic"), ResponseCurve::Logistic);

...
function lurek.init()
    ...
    lurek.render.setBackgroundColor(0.2, 0.45, 0.15)
    initTiles()
end
...
function lurek.render()
    ...
    lurek.render.setBackgroundColor(sr * dnf * 0.5, sg * dnf * 0.5, sb * dnf * 0.5)
    -- Draw ground
    for y = 1, MAP_H do
        for x = 1, MAP_W do
            local t = tiles[y][x]
            ...
            if t.soil then
                lurek.render.setColor(0.35 * dnf, 0.22 * dnf, 0.1 * dnf)
                lurek.render.rectangle("fill", px, py, TILE - 1, TILE - 1)
                -- Crops
                if t.crop then
                    local c = crops[t.crop.type]
                    if t.crop.grown then
                        lurek.render.setColor(c.color[1] * dnf, c.color[2] * dnf, c.color[3] * dnf)
                        lurek.render.circle("fill", px + TILE / 2, py + TILE / 2, 10)
                        lurek.render.setColor(0, 0.4 * dnf, 0)
                        lurek.render.rectangle("fill", px + TILE / 2 - 1, py + 4, 2, TILE / 2 - 4)
                    else
                        local progress = (day - t.crop.plantDay) / c.growTime
                        local sz = lerp(3, 10, clamp(progress, 0, 1))
                        lurek.render.setColor(0.2 * dnf, 0.6 * dnf, 0.15 * dnf)
                        lurek.render.circle("fill", px + TILE / 2, py + TILE / 2, sz)
                        lurek.render.setColor(0.3 * dnf, 0.4 * dnf, 0.1 * dnf)
                        lurek.render.rectangle("fill", px + TILE / 2 - 1, py + TILE / 2, 2, TILE / 2 - 2)
                    end
                end
            else
                lurek.render.setColor(sr * dnf, sg * dnf, sb * dnf)
                lurek.render.rectangle("fill", px, py, TILE - 1, TILE - 1)
            end
        end
    end
    -- Farm border
    lurek.render.setColor(0.6 * dnf, 0.4 * dnf, 0.2 * dnf)
    lurek.render.rectangle("line", (FARM_X - 1) * TILE - 2, (FARM_Y - 1) * TILE - 2, FARM_W * TILE + 4, FARM_H * TILE + 4)
    -- Farmer
    lurek.render.setColor(0.2 * dnf, 0.5 * dnf, 0.9 * dnf)
    local fpx = (farmer.x - 1) * TILE
    local fpy = (farmer.y - 1) * TILE
    lurek.render.rectangle("fill", fpx + 4, fpy + 4, TILE - 8, TILE - 8)
    -- Head
    lurek.render.setColor(0.9 * dnf, 0.75 * dnf, 0.55 * dnf)
    lurek.render.circle("fill", fpx + TILE / 2, fpy + 6, 6)
    -- Hat
    lurek.render.setColor(0.6 * dnf, 0.3 * dnf, 0.1 * dnf)
    lurek.render.rectangle("fill", fpx + 6, fpy, TILE - 12, 5)
    -- HUD Panel
    lurek.render.setColor(0, 0, 0, 0.75)
    lurek.render.rectangle("fill", 0, MAP_H * TILE, 800, 600 - MAP_H * TILE)
    -- Day/Season info
    lurek.render.setColor(1, 1, 0.8)
    lurek.render.print("Day " .. day .. " | " .. seasonNames[season] .. " (Day " .. (((day - 1) % seasonLen) + 1) .. "/" .. seasonLen .. ")", 10, MAP_H * TILE + 6)
    lurek.render.print("Money: $" .. money, 10, MAP_H * TILE + 24)
    -- Time bar
    local tbx, tby = 350, MAP_H * TILE + 6
    lurek.render.setColor(0.2, 0.2, 0.3); lurek.render.rectangle("fill", tbx, tby, 120, 12)
    lurek.render.setColor(1, 0.8, 0.2); lurek.render.rectangle("fill", tbx, tby, 120 * (dayTimer / dayDuration), 12)
    lurek.render.setColor(1, 1, 1); lurek.render.print(autoTime and "Auto" or "Manual (N)", tbx + 130, tby)
    -- Inventory
    lurek.render.setColor(1, 1, 1)
    lurek.render.print("Seeds:", 10, MAP_H * TILE + 44)
    for i, c in ipairs(crops) do
        local sel = (i == selectedCrop)
        if sel then lurek.render.setColor(1, 1, 0.3) else lurek.render.setColor(0.7, 0.7, 0.7) end
        lurek.render.print("[" .. i .. "] " .. c.name .. ": " .. inventory.seeds[i], 10 + (i - 1) * 160, MAP_H * TILE + 60)
    end
    lurek.render.setColor(1, 1, 1)
    lurek.render.print("Harvest:", 10, MAP_H * TILE + 78)
    for i, c in ipairs(crops) do
        lurek.render.setColor(c.color[1], c.color[2], c.color[3])
        lurek.render.print(c.name .. ": " .. inventory.harvest[i], 80 + (i - 1) * 160, MAP_H * TILE + 78)
    end
    -- Controls hint
    lurek.render.setColor(0.5, 0.5, 0.6)
    lurek.render.print("B=Buy seeds  S=Sell harvest  T=Toggle auto-time  1/2/3=Select crop  Click=Plant/Harvest", 10, MAP_H * TILE + 98)
    -- Shop overlay
    if shopOpen then
        lurek.render.setColor(0, 0, 0, 0.85); lurek.render.rectangle("fill", 200, 150, 400, 250)
        lurek.render.setColor(0.8, 0.7, 0.3); lurek.render.print("SEED SHOP", 340, 160, 1.3)
        lurek.render.setColor(1, 1, 1); lurek.render.print("Money: $" .. money, 340, 185)
        for i, c in ipairs(crops) do
            local by = 210 + (i - 1) * 55
            lurek.render.setColor(0.15, 0.15, 0.2); lurek.render.rectangle("fill", 220, by, 360, 45)
            lurek.render.setColor(c.color[1], c.color[2], c.color[3])
            lurek.render.print(c.name .. " Seeds", 235, by + 5, 1.1)
            lurek.render.setColor(0.7, 0.7, 0.7)
            lurek.render.print("Cost: $" .. c.seedCost .. "/seed  |  Grow: " .. c.growTime .. " days  |  Sells: $" .. c.sellPrice, 235, by + 24)
            lurek.render.setColor(0.3, 0.8, 0.3)
            lurek.render.print("[" .. i .. "] Buy", 520, by + 10)
        end
        lurek.render.setColor(0.6, 0.6, 0.6); lurek.render.print("Press B to close shop", 320, 380)
    end

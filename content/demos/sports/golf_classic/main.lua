...
function lurek.init()
    lurek.render.setBackgroundColor(0.18, 0.55, 0.18)
    reset()
end
...
function lurek.render()
    -- Fairway
    lurek.render.setColor(0.18, 0.55, 0.18)
    lurek.render.rectangle("fill", 0, 0, W, H)

    local h = COURSE[hole_idx]

    -- Rough border
    lurek.render.setColor(0.1, 0.4, 0.1)
    lurek.render.rectangle("fill", 0, 0, W, 50)
    lurek.render.rectangle("fill", 0, H-50, W, 50)
    lurek.render.rectangle("fill", 0, 0, 50, H)
    lurek.render.rectangle("fill", W-50, 0, 50, H)

    -- Obstacles
    for _, obs in ipairs(h.obs) do
        if obs.type == "water" then
            lurek.render.setColor(0.1, 0.3, 0.8, 0.8)
        elseif obs.type == "bunker" then
            lurek.render.setColor(0.88, 0.8, 0.55, 0.9)
        else  -- tree
            lurek.render.setColor(0.05, 0.25, 0.05)
        end
        if obs.type == "tree" then
            lurek.render.rectangle("fill", obs.x - 2, obs.y, 4, obs.h)
            lurek.render.circle("fill", obs.x, obs.y, obs.w/2)
        else
    ...
    -- Tee
    lurek.render.setColor(0.65, 0.55, 0.3)
    lurek.render.circle("fill", h.tee[1], h.tee[2], 12)
    lurek.render.setColor(1, 1, 1)
    lurek.render.print("T", h.tee[1] - 5, h.tee[2] - 8, 1.5)

    -- Cup / hole
    lurek.render.setColor(0.05, 0.05, 0.05)
    lurek.render.circle("fill", h.cup[1], h.cup[2], 10)
    lurek.render.setColor(1, 0.2, 0)
    lurek.render.rectangle("fill", h.cup[1], h.cup[2] - 28, 3, 28)  -- flag pole
    lurek.render.rectangle("fill", h.cup[1] + 3, h.cup[2] - 28, 14, 10)  -- flag

    -- Ball trail
    for i, pt in ipairs(ball_trail) do
        local a = i / #ball_trail * 0.6
        lurek.render.setColor(1, 1, 1, a * 0.5)
        lurek.render.circle("fill", pt.x, pt.y, 2)
    end
    ...
    if game_state == "aim" then
        ...
        local len = 50
        lurek.render.setColor(1, 0.9, 0, 0.8)
        lurek.render.line(ball.x, ball.y, ball.x + math.cos(aim_angle)*len, ball.y + math.sin(aim_angle)*len)
        -- Wind arrow (top right)
        local wlen = math.sqrt(wx^2 + wy^2) * 60
        if wlen > 2 then
            local wangle = math.atan(wy, wx)
            lurek.render.setColor(0.5, 0.8, 1, 0.8)
            lurek.render.line(W - 70, 70, W - 70 + math.cos(wangle)*wlen, 70 + math.sin(wangle)*wlen)
            lurek.render.circle("fill", W - 70 + math.cos(wangle)*wlen, 70 + math.sin(wangle)*wlen, 5)
        end
    end
    ...
    if game_state == "swing" then
        ...
        lurek.render.rectangle("fill", W/2 - 80, H - 30, 160, 18)
        ...
        lurek.render.rectangle("fill", W/2 - 80, H - 30, 160 * pct, 18)
        lurek.render.setColor(1, 1, 1)
        lurek.render.print("POWER — Release SPACE", W/2 - 80, H - 48, 1.4)
    end

    -- HUD
    lurek.render.setColor(0, 0, 0, 0.65)
    lurek.render.rectangle("fill", 0, 0, W, 46)
    lurek.render.setColor(1, 0.9, 0.3)
    lurek.render.print("HOLE " .. hole_idx .. "/" .. HOLES .. "  PAR " .. h.par, 14, 8, 2.2)
    lurek.render.setColor(1, 1, 1)
    lurek.render.print("Shots: " .. shot_count, 260, 8, 2)
    ...
    lurek.render.setColor(sc[1], sc[2], sc[3])
    lurek.render.print("Score: " .. scorestr, 440, 8, 2)
    lurek.render.setColor(0.6, 0.8, 1, 0.7)
    lurek.render.print("Wind →", W - 90, 8, 1.4)

    lurek.render.setColor(0.7, 0.9, 0.7, 0.7)
    ...
    lurek.render.print(hint, 14, H - 18, 1.4)

    -- Message
    if msg_timer > 0 then
        local alpha = math.min(1, msg_timer)
        lurek.render.setColor(0, 0, 0, alpha * 0.7)
        lurek.render.rectangle("fill", W/2 - 200, H/2 - 25, 400, 50)
        lurek.render.setColor(1, 1, 0.3, alpha)
        lurek.render.print(msg, W/2 - #msg * 7, H/2 - 20, 2)
    end

    -- Hole transition
    if game_state == "holed" then
        lurek.render.setColor(0.5, 0.9, 0.5, 0.7)
        lurek.render.print("Press SPACE for next hole", W/2 - 130, H - 20, 2)
    end

    -- Game over
    if game_state == "gameover" then
        lurek.render.setColor(0, 0, 0, 0.82)
        lurek.render.rectangle("fill", 0, 0, W, H)
        lurek.render.setColor(1, 0.9, 0.2)
        lurek.render.print("ROUND COMPLETE!", W/2 - 105, 60, 2.8)
        for i = 1, HOLES do
            local par_i = COURSE[i].par
            local s_i = scores[i] or 0
            local d = s_i - par_i
            local cl = d < 0 and {0.3,1,0.3} or (d == 0 and {1,1,0.3} or {1,0.4,0.4})
            lurek.render.setColor(cl[1], cl[2], cl[3])
            local xs = 70 + ((i-1) % 5) * 135
            local ys = 130 + math.floor((i-1)/5) * 55
            lurek.render.print("H" .. i .. ": " .. (s_i > 0 and s_i or "—"), xs, ys, 1.8)
        end
        lurek.render.setColor(1, 1, 1)
        lurek.render.print("TOTAL: " .. (total_score > 0 and "+" or "") .. total_score, W/2 - 60, 360, 3)
        lurek.render.setColor(0.6, 0.6, 0.6)
        lurek.render.print("Press R to play again", W/2 - 110, 420, 2)
    end
end

...
function lurek.init()
    lurek.render.setBackgroundColor(0.15, 0.5, 0.15)
    reset()
end
...
function lurek.render()
    -- Green court
    lurek.render.setColor(0.12, 0.45, 0.12)
    lurek.render.rectangle("fill", COURT_X, COURT_Y, COURT_W, COURT_H)

    -- Court lines
    lurek.render.setColor(1, 1, 1, 0.9)
    lurek.render.rectangle("line", COURT_X, COURT_Y, COURT_W, COURT_H)
    -- Net
    lurek.render.rectangle("fill", COURT_X, NET_Y - 3, COURT_W, 6)
    -- Service boxes
    local mid_x = COURT_X + COURT_W/2
    lurek.render.line(mid_x, COURT_Y, mid_x, COURT_Y + COURT_H)
    lurek.render.line(COURT_X, NET_Y - COURT_H/4, COURT_X + COURT_W, NET_Y - COURT_H/4)
    lurek.render.line(COURT_X, NET_Y + COURT_H/4, COURT_X + COURT_W, NET_Y + COURT_H/4)

    -- CPU player
    lurek.render.setColor(0.9, 0.2, 0.2)
    lurek.render.rectangle("fill", cpu.x, cpu.y, cpu.w, cpu.h)
    lurek.render.setColor(0.85, 0.65, 0.4)
    lurek.render.circle("fill", cpu.x + cpu.w/2, cpu.y - 8, 9)

    -- Player
    local plr = 0.9 + 0.1 * math.sin(anim * 6)
    lurek.render.setColor(0.2, 0.5, plr)
    lurek.render.rectangle("fill", pl.x, pl.y, pl.w, pl.h)
    lurek.render.setColor(0.85, 0.65, 0.4)
    lurek.render.circle("fill", pl.x + pl.w/2, pl.y + pl.h + 8, 9)

    -- Ball
    lurek.render.setColor(0.9, 0.9, 0.1)
    lurek.render.circle("fill", ball.x, ball.y, 7)
    lurek.render.setColor(0, 0.3, 0)
    lurek.render.circle("line", ball.x, ball.y, 7)

    -- HUD
    lurek.render.setColor(0, 0, 0, 0.65)
    lurek.render.rectangle("fill", 0, 0, COURT_X, H)
    lurek.render.rectangle("fill", COURT_X + COURT_W, 0, W - COURT_X - COURT_W, H)
    ...
    lurek.render.print("CPU", 10, 130, 1.4)
    lurek.render.print(tostring(games.cpu), 12, 150, 2.5)
    lurek.render.print(cpu_score, 10, 185, 1.8)

    lurek.render.setColor(0.4, 0.7, 1)
    lurek.render.print("YOU", 10, 360, 1.4)
    lurek.render.print(tostring(games.pl), 12, 380, 2.5)
    lurek.render.print(pl_score, 10, 415, 1.8)

    lurek.render.setColor(0.9, 0.9, 0.9)
    lurek.render.print("SET", 14, 110, 1.3)
    lurek.render.print("pts:", 12, 170, 1.1)
    ...
    lurek.render.circle("fill", ball.x, ball.y + shadow_y_offset + 6, 5)

    lurek.render.setColor(0.6, 0.8, 0.6, 0.6)
    lurek.render.print("[WASD/Arrows] Move  [Space] Topspin", COURT_X + 5, COURT_Y + COURT_H + 8, 1.3)

    -- Serve prompt
    if game_state == "serve_prep" then
        lurek.render.setColor(1, 1, 0.5, 0.8)
        lurek.render.print("Press SPACE to serve", COURT_X + COURT_W/2 - 100, NET_Y - 18, 1.8)
    end
    ...
    if game_state == "gameover" then
        ...
        lurek.render.print(winner, W/2 - 80, H/2 - 25, 3.2)
        lurek.render.setColor(1, 1, 1)
        lurek.render.print(games.pl .. " — " .. games.cpu, W/2 - 28, H/2 + 18, 2.5)

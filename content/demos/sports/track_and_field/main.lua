...
local function draw_athlete(x, y, running)
    ...
    lurek.gfx.rectangle("fill", x - 6, y - 30, 12, 18)
    -- Head
    lurek.gfx.circle("fill", x, y - 36, 7)
    -- Legs
    lurek.gfx.setColor(0.2, 0.3, 0.8)
    lurek.gfx.rectangle("fill", x - 5, y - 12, 5, 18 + leg_off)
    lurek.gfx.rectangle("fill", x,     y - 12, 5, 18 - leg_off)
    -- Arms
    lurek.gfx.setColor(0.9, 0.6, 0.3)
    lurek.gfx.line(x - 6, y - 26, x - 14, y - 20 + leg_off/2)
    lurek.gfx.line(x + 6, y - 26, x + 14, y - 20 - leg_off/2)
end
...
function lurek.render()
    ...
    -- Crowd stands
    lurek.gfx.setColor(0.6, 0.3, 0.1)
    lurek.gfx.rectangle("fill", 0, 0, W, 60)
    for i = 0, 15 do
        local cx = 24 + i * 50
        lurek.gfx.setColor(0.2 + (i % 3)*0.2, 0.1, 0.4 + (i % 2)*0.2)
        lurek.gfx.circle("fill", cx, 35 + (i % 3) * 7, 10)
    end
    -- Ground / track
    lurek.gfx.setColor(0.85, 0.65, 0.3)
    lurek.gfx.rectangle("fill", 0, H - 130, W, 130)
    lurek.gfx.setColor(0.8, 0.6, 0.25)
    for i = 0, 3 do
        lurek.gfx.rectangle("fill", 0, H - 130 + i * 32, W, 2)
    end
    ...
    if ev == 1 then
        ...
        lurek.gfx.rectangle("line", 80, H - 120, 640, 20)
        lurek.gfx.line(720, H - 130, 720, H)  -- finish line
        draw_athlete(athlete_x, H - 90, game_state == "running" or game_state == "ready")
        lurek.gfx.setColor(1, 1, 0.5)
        lurek.gfx.print("SPEED: " .. string.format("%.1f", speed), 20, 70, 2.2)
        lurek.gfx.setColor(1, 1, 1)
        lurek.gfx.print("Time: " .. string.format("%.2f", run_timer) .. "s", 20, 96, 1.8)

    -- ── Long Jump ────────────────────────────────────────────────────────
    elseif ev == 2 then
        lurek.gfx.setColor(0.85, 0.75, 0.5)
        lurek.gfx.rectangle("fill", 420, H - 98, 260, 30)   -- sand pit
        lurek.gfx.setColor(1, 1, 1)
        lurek.gfx.rectangle("fill", 420, H - 115, 8, 30)    -- take-off board
        draw_athlete(athlete_x, athlete_y, game_state == "running")
        lurek.gfx.setColor(1, 1, 0.5)
        lurek.gfx.print("SPEED: " .. string.format("%.1f", speed), 20, 70, 2.2)
        if game_state == "result" then
            lurek.gfx.setColor(0.2, 1, 0.5)
            lurek.gfx.print(string.format("%.1f m", result), 20, 96, 2.5)
        end
    ...
    elseif ev == 3 then
        for _, hrd in ipairs(hurdles) do
            if not hrd.cleared then
                ...
                lurek.gfx.rectangle("fill", hrd.x - 3, H - 130, 6, 30)
                lurek.gfx.rectangle("fill", hrd.x - 15, H - 100, 30, 5)
            end
        end
    ...
    elseif ev == 4 then
        ...
        lurek.gfx.setColor(0.9, 0.6, 0.3)
        lurek.gfx.circle("fill", W/2, H - 90, 12)
        lurek.gfx.setColor(0.3, 0.4, 0.8)
        lurek.gfx.circle("fill", W/2, H - 70, 9)
        -- Hammer chain
        local hx = W/2 + math.cos(hammer_spin) * 45
        local hy = H - 80 + math.sin(hammer_spin) * 20
        lurek.gfx.setColor(0.7, 0.7, 0.7)
        lurek.gfx.line(W/2, H - 80, hx, hy)
        lurek.gfx.setColor(0.3, 0.3, 0.3)
        lurek.gfx.circle("fill", hx, hy, 10)

        -- Power bar
        lurek.gfx.setColor(0.3, 0.3, 0.3)
        lurek.gfx.rectangle("fill", W - 60, H - 200, 30, 160)
        local bar_r = power >= 70 and 1 or 0.3
        local bar_g = power <= 30 and 0.6 or (power <= 70 and 1 or 0.2)
        lurek.gfx.setColor(bar_r, bar_g, 0.1)
        lurek.gfx.rectangle("fill", W - 60, H - 200 + 160 * (1 - power/100), 30, 160 * power/100)
        lurek.gfx.setColor(1, 1, 1)
        lurek.gfx.print("POWER", W - 70, H - 210, 1.3)
        lurek.gfx.print("Release:", W - 110, H/2, 1.5)
        lurek.gfx.print("[Space]", W - 100, H/2 + 22, 1.5)

        if game_state == "result" then
            lurek.gfx.setColor(0.2, 1, 0.6)
            lurek.gfx.print(tostring(result) .. " m", W/2 - 30, 70, 3)
        end
    end
    ...
    lurek.gfx.print(EVENTS[event_idx], W/2 - 100, 12, 2.5)
    local instr = { "[A] [D] Alternate fast!", "[A/D] Run then [Space] Jump", "[A/D] Run [Space] Leap hurdle", "[A/D] Spin then [Space] Release" }
    lurek.gfx.setColor(0.8, 0.9, 1, 0.8)
    lurek.gfx.print(instr[event_idx], W/2 - 150, 36, 1.4)

    -- Best scores
    lurek.gfx.setColor(0.2, 0.2, 0.2, 0.65)
    lurek.gfx.rectangle("fill", 0, H - 26, W, 26)
    for i, nm in ipairs(EVENTS) do
        lurek.gfx.setColor(i == event_idx and 0.9 or 0.5, 0.9, 0.5)
        local bstr = best[i] > 0 and (i >= 2 and best[i] .. "m" or best[i] .. "s") or "—"
        lurek.gfx.print(nm .. ": " .. bstr, (i-1)*200 + 8, H - 22, 1.3)
    end

    -- Result overlay
    if game_state == "result" then
        lurek.gfx.setColor(0, 0, 0, 0.6)
        lurek.gfx.rectangle("fill", W/2 - 180, H/2 - 45, 360, 90)
        lurek.gfx.setColor(1, 1, 0)
        lurek.gfx.print("RESULT", W/2 - 50, H/2 - 40, 2)
        local rstr = (ev == 1 or ev == 3) and (string.format("%.2f", result) .. " s") or (tostring(result) .. " m")
        lurek.gfx.setColor(1, 1, 1)
        lurek.gfx.print(rstr, W/2 - 40, H/2 - 5, 2.5)
    end
    ...
    if game_state == "gameover" then
        ...
        lurek.gfx.print("ALL EVENTS COMPLETE!", W/2 - 140, H/2 - 60, 2.5)
        for i, nm in ipairs(EVENTS) do
            lurek.gfx.setColor(0.8, 1, 0.8)
            local bstr = best[i] > 0 and (i >= 2 and best[i] .. " m" or best[i] .. " s") or "—"
            lurek.gfx.print(nm .. ": " .. bstr, W/2 - 100, H/2 - 10 + i * 28, 1.7)
        end
        lurek.gfx.setColor(0.6, 0.6, 0.6)
        lurek.gfx.print("Press R to try again", W/2 - 110, H/2 + 130, 2)
    end
end

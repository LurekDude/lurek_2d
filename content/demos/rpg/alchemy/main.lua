...
function luna.render()
    luna.gfx.setBackgroundColor(0.12, 0.1, 0.15)

    -- Title and gold
    luna.gfx.setColor(1, 0.85, 0.4, 1)
    luna.gfx.print("~ Alchemy Workshop ~", 220, 10, 1.3)
    luna.gfx.setColor(1, 1, 0.5, 1)
    luna.gfx.print("Gold: " .. gold, 620, 15, 1)

    -- Ingredient shelf
    luna.gfx.setColor(0.7, 0.6, 0.4, 1)
    luna.gfx.print("Ingredients:", SHELF_X, SHELF_Y - 20, 1)
    for i, ing in ipairs(ingredients) do
        local iy = SHELF_Y + (i - 1) * 30
        local c = ing.color
        luna.gfx.setColor(c[1], c[2], c[3], 1)
        luna.gfx.rectangle("fill", SHELF_X, iy, 14, 14)
        luna.gfx.setColor(0.9, 0.9, 0.8, 1)
        luna.gfx.print(ing.name .. " x" .. ing.owned, SHELF_X + 20, iy, 0.8)
    end

    -- Mortar
    luna.gfx.setColor(0.5, 0.4, 0.3, 1)
    luna.gfx.rectangle("fill", MORTAR_X - 30, MORTAR_Y - 20, 60, 40)
    luna.gfx.setColor(0.3, 0.25, 0.2, 1)
    luna.gfx.rectangle("fill", MORTAR_X - 25, MORTAR_Y - 15, 50, 30)
    luna.gfx.setColor(0.9, 0.85, 0.7, 1)
    luna.gfx.print("Mortar", MORTAR_X - 20, MORTAR_Y - 40, 0.8)
    if #mortar.items > 0 then
        local mc = mix_color(mortar.items)
        luna.gfx.setColor(mc[1], mc[2], mc[3], 1)
        local sz = mortar.ground and 20 or 12
        luna.gfx.circle("fill", MORTAR_X, MORTAR_Y, sz)
        if mortar.grinding > 0 then
            luna.gfx.setColor(1, 1, 1, 0.7)
            luna.gfx.print("Grinding...", MORTAR_X - 25, MORTAR_Y + 25, 0.7)
        elseif mortar.ground then
            luna.gfx.setColor(0.5, 1, 0.5, 1)
            luna.gfx.print("Ground!", MORTAR_X - 20, MORTAR_Y + 25, 0.7)
        end
    end

    -- Cauldron
    luna.gfx.setColor(0.3, 0.3, 0.35, 1)
    luna.gfx.circle("fill", CAULDRON_X, CAULDRON_Y, 38)
    luna.gfx.setColor(0.15, 0.15, 0.2, 1)
    luna.gfx.circle("fill", CAULDRON_X, CAULDRON_Y, 30)
    luna.gfx.setColor(0.9, 0.85, 0.7, 1)
    luna.gfx.print("Cauldron", CAULDRON_X - 25, CAULDRON_Y - 55, 0.8)
    if #cauldron.items > 0 then
        local cc = cauldron.result and cauldron.result.color or mix_color(cauldron.items)
        luna.gfx.setColor(cc[1], cc[2], cc[3], 0.8)
        luna.gfx.circle("fill", CAULDRON_X, CAULDRON_Y, 25)
    end
    -- Temperature bar
    if #cauldron.items > 0 then
        luna.gfx.setColor(0.3, 0.3, 0.3, 1)
        luna.gfx.rectangle("fill", CAULDRON_X - 5, CAULDRON_Y + 42, 70, 12)
        local tr = cauldron.temperature / 100
        local tg = 1 - tr
        luna.gfx.setColor(tr, tg, 0.1, 1)
        luna.gfx.rectangle("fill", CAULDRON_X - 5, CAULDRON_Y + 42, 70 * (cauldron.temperature / 100), 12)
        luna.gfx.setColor(1, 1, 1, 1)
        luna.gfx.print(cauldron.temperature .. "C", CAULDRON_X + 68, CAULDRON_Y + 42, 0.7)
    end

    -- Bottle
    luna.gfx.setColor(0.6, 0.8, 0.9, 0.5)
    luna.gfx.rectangle("fill", BOTTLE_X - 12, BOTTLE_Y - 35, 24, 50)
    luna.gfx.rectangle("fill", BOTTLE_X - 5, BOTTLE_Y - 42, 10, 10)
    luna.gfx.setColor(0.9, 0.85, 0.7, 1)
    luna.gfx.print("Bottle", BOTTLE_X - 18, BOTTLE_Y - 58, 0.8)
    if bottle.potion then
        local pc = bottle.potion.color
        luna.gfx.setColor(pc[1], pc[2], pc[3], 0.9)
        luna.gfx.rectangle("fill", BOTTLE_X - 10, BOTTLE_Y - 20, 20, 30)
        luna.gfx.setColor(1, 1, 1, 1)
        luna.gfx.print(bottle.potion.name, BOTTLE_X - 40, BOTTLE_Y + 20, 0.7)
    end

    -- Recipe book
    luna.gfx.setColor(0.8, 0.7, 0.5, 1)
    luna.gfx.print("Recipes:", 600, 80, 1)
    for i, recipe in ipairs(recipes) do
        local ry = 105 + (i - 1) * 22
        if discovered[recipe.name] then
            local rc = recipe.color
            luna.gfx.setColor(rc[1], rc[2], rc[3], 1)
            luna.gfx.rectangle("fill", 600, ry + 2, 10, 10)
            luna.gfx.setColor(0.9, 0.9, 0.8, 1)
            luna.gfx.print(recipe.name .. " (" .. recipe.value .. "g)", 615, ry, 0.7)
        else
            luna.gfx.setColor(0.4, 0.4, 0.4, 1)
            luna.gfx.print("???", 615, ry, 0.7)
        end
    end

    -- Controls
    luna.gfx.setColor(0.6, 0.6, 0.5, 1)
    luna.gfx.print("Click shelf=add | Click mortar=grind | Click cauldron=transfer", 30, 420, 0.65)
    luna.gfx.print("Up/Down=heat | Space=brew | B=bottle | S=sell | R=reset | Esc=quit", 30, 438, 0.65)

    -- Message
    if message_timer > 0 then
        luna.gfx.setColor(1, 1, 0.7, message_timer)
        luna.gfx.print(message, 200, 460, 1)
    end

    luna.gfx.setColor(0.5, 0.5, 0.5, 1)
    luna.gfx.print("FPS: " .. luna.time.getFPS(), 5, 5, 0.6)
end

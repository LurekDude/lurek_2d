...
- Reaction-time measurement using `lurek.time.getTime()` with a random delay before the signal
...
- `lurek.keyboard.isDown()` and `lurek.mouse.getPosition()` used in different minigames

## How to Run

```powershell
cargo run -- content/demos/party_games
```

...
A showcase of the Lurek2D particle system featuring six hand-crafted presets covering every available particle shape (square, spark, circle, point). Cycle through fire, explosion, snow, galaxy, magic, and fountain effects, toggle gravity, and trigger burst emissions to explore the full range of `lurek.particles` parameters.

## What It Demonstrates

- `lurek.particles.newSystem()` with a full parameter table: `maxParticles`, `emissionRate`, `lifetimeMin/Max`, `speedMin/Max`, `direction`, `spread`, `gravityX/Y`, `spinMin/Max`, `turbulence`, `sizes`, `colors`
...
- Mouse-follow: all systems update their emitter position to `lurek.mouse.getPosition()` every frame

## How to Run

```powershell
cargo run -- content/demos/particles_demo
```

...
-- Run with: cargo run -- content/demos/showcase/light_demo
...
local function create_scene()
    ...
    lurek.light.setAmbient(ambient_level, ambient_level, ambient_level * 1.2)

    -- Player light — warm white, smooth falloff, shadows
    player_light = lurek.light.newLight(player_x, player_y, 250, {
        color = {1.0, 0.95, 0.85},
        intensity = 1.3,
        energy = 1.0,
        falloff = "smooth",
    ...
    for _, pos in ipairs(torch_positions) do
        local t = lurek.light.newLight(pos[1], pos[2], 140, {
            color = {1.0, 0.55, 0.15},
            intensity = 0.7,
            falloff = "linear",
            shadowEnabled = true,
    ...
    for _, verts in ipairs(wall_rects) do
        table.insert(walls, lurek.light.newOccluder(verts))
    end
end
...
function lurek.process(dt)
    -- Player movement
    local speed = 250 * dt
    if lurek.keyboard.isDown("w") then player_y = player_y - speed end
    if lurek.keyboard.isDown("s") then player_y = player_y + speed end
    if lurek.keyboard.isDown("a") then player_x = player_x - speed end
    if lurek.keyboard.isDown("d") then player_x = player_x + speed end

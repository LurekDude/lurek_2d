...
if not lurek.devtools then
    function lurek.init()
        ...
        lurek.render.print("lurek.devtools is not available in this build", 180, 270)
    end
    return
end
...
function lurek.render()
    ...
    lurek.render.print("Devtools Demo", 20, y)
    y = y + 30

    -- Frame stats
    local stats = dt.getFrameStats()
    lurek.render.setColor(0.7, 1, 0.7)
    lurek.render.print(string.format("FPS: %.1f  |  dt: %.3fms", stats.fps or 0, (stats.avg or 0) * 1000), 20, y)
    y = y + 20
    lurek.render.print(string.format("p50: %.3fms  p95: %.3fms  p99: %.3fms", (stats.p50 or 0) * 1000, (stats.p95 or 0) * 1000, (stats.p99 or 0) * 1000), 20, y)
    y = y + 30

    -- Profile data (most recent frame)
    lurek.render.setColor(0.7, 0.8, 1)
    lurek.render.print("Profile (last frame):", 20, y)
    y = y + 20
    ...
    if zones then
        for i = 1, #zones do
            ...
            lurek.render.print(string.format("  %s: %.3fms (self: %.3fms)", z.name, z.time * 1000, z.selfTime * 1000), 20, y)
            y = y + 18
            -- Children
            if z.children then
                for j = 1, #z.children do
                    ...
                    lurek.render.print(string.format("    %s: %.3fms", c.name, c.time * 1000), 20, y)
                    y = y + 18
                end
            end
        end
    ...
    lurek.render.print("Recent logs:", 20, y)
    y = y + 20
    ...
    for i = 1, #history do
        ...
        lurek.render.print(string.format("[%s] %s", entry.level, entry.message), 30, y)
        y = y + 16
    end
    y = y + 20

    -- Eval demo
    lurek.render.setColor(0.6, 1, 0.9)
    local ok, result = dt.eval("return 2 + 2")
    lurek.render.print(string.format("eval('return 2+2') = %s (ok=%s)", tostring(result), tostring(ok)), 20, y)
    y = y + 20

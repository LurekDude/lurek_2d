...
local function draw_base_scene(time)
    ...
    for i = 0, 19 do
        ...
        lurek.render.drawRect("fill", 0, i * 30, SCREEN_W, 30)
    end
    ...
    for i, c in ipairs(colors) do
        ...
        lurek.render.drawRect("fill", sx - sz / 2, sy - sz / 2, sz, sz)
        lurek.render.setColor(1, 1, 1, 0.3)
        lurek.render.drawRect("line", sx - sz / 2, sy - sz / 2, sz, sz)
    end
    ...
    for bx = 0, SCREEN_W - 1, 4 do
        ...
        lurek.render.drawRect("fill", bx, bar_y, 4, 20)
    end
end
...
local function apply_blur(intensity)
    ...
    for _, ox in ipairs(offsets) do
        for _, oy in ipairs(offsets) do
            ...
            lurek.render.drawRect("fill", ox, oy, SCREEN_W, SCREEN_H)
        end
    end
end
...
local function apply_crt(intensity, time)
    ...
    for y = 0, SCREEN_H - 1, 3 do
        ...
        lurek.render.drawRect("fill", 0, y, SCREEN_W, 1)
    end
    ...
    for s = 0, steps - 1 do
        ...
        lurek.render.drawRect("fill", 0, 0, inset, SCREEN_H)
        lurek.render.drawRect("fill", SCREEN_W - inset, 0, inset, SCREEN_H)
        lurek.render.drawRect("fill", 0, 0, SCREEN_W, inset * 0.6)
        lurek.render.drawRect("fill", 0, SCREEN_H - inset * 0.6, SCREEN_W, inset * 0.6)
    end
    -- Color fringing (red/blue offset lines)
    local fringe = 1.5 * intensity
    lurek.render.setColor(1, 0, 0, 0.03 * intensity)
    lurek.render.drawRect("fill", -fringe, 0, SCREEN_W, SCREEN_H)
...
local function apply_vignette(intensity)
    ...
    for r = 0, rings - 1 do
        ...
        lurek.render.drawRect("fill", 0, 0, inset, SCREEN_H)
        lurek.render.drawRect("fill", SCREEN_W - inset, 0, inset, SCREEN_H)
        lurek.render.drawRect("fill", 0, 0, SCREEN_W, inset * 0.75)
        lurek.render.drawRect("fill", 0, SCREEN_H - inset * 0.75, SCREEN_W, inset * 0.75)
    end
...
local function apply_chromatic(intensity, time)
    ...
    lurek.render.drawRect("fill", -offset, -offset * 0.5, SCREEN_W, SCREEN_H)
...
local function apply_pixelate(intensity)
    ...
    for gx = 0, SCREEN_W - 1, block * 2 do
        for gy = 0, SCREEN_H - 1, block * 2 do
            ...
            lurek.render.drawRect("fill", gx, gy, block, block)
        end
    end
end
...
local function apply_sepia(intensity)
    ...
    lurek.render.drawRect("fill", 0, 0, SCREEN_W, SCREEN_H / 2)
end
...
local function apply_invert(intensity)
    ...
    for y = 0, SCREEN_H - 1, 6 do
        lurek.render.drawRect("fill", 0, y, SCREEN_W, 2)
    end
end
...
local function apply_grain(intensity, seed)
    ...
    for i = 1, count do
        ...
        lurek.render.drawRect("fill", rx, ry, 2, 2)
    end
end
...
function lurek.render_ui()
    ...
    lurek.render.drawRect("fill", panel_x, panel_y, panel_w, panel_h)
    lurek.render.setColor(0.3, 0.7, 0.9, 0.4)
    lurek.render.drawRect("line", panel_x, panel_y, panel_w, panel_h)
    ...
    for i, def in ipairs(EFFECT_DEFS) do
        ...
        if is_selected and e.enabled then
            ...
            lurek.render.drawRect("fill", panel_x + 2, ey - 1, panel_w - 4, 17)
        end
        ...
        if e.enabled then
            ...
            lurek.render.drawRect("fill", bar_x, ey + 2, bar_w, bar_h)
            lurek.render.setColor(0.3, 0.8, 0.5, 0.9)
            lurek.render.drawRect("fill", bar_x, ey + 2, bar_w * e.intensity, bar_h)
            lurek.render.setColor(0.5, 1.0, 0.7, 0.5)
            lurek.render.drawRect("line", bar_x, ey + 2, bar_w, bar_h)
        end
    end

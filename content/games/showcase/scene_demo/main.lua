...
scenes.menu = {
    ...
    render = function()
        ...
        lurek.render.drawRect("fill", 0, 0, 60, SCREEN_H)
        lurek.render.drawRect("fill", SCREEN_W - 60, 0, 60, SCREEN_H)
    end,
    render_ui = function()
        lurek.render.setColor(COL_TITLE[1], COL_TITLE[2], COL_TITLE[3], 1)
...
scenes.settings = {
    ...
    render_ui = function()
        ...
        for d = 1, 3 do
            ...
            lurek.render.drawRect("fill", bar_x + (d - 1) * 22, bar_y, 18, 14)
        end
        ...
        lurek.render.setColor(0.25, 0.25, 0.25, 1)
        lurek.render.drawRect("fill", bar_x, bar_y + 50, vol_w, 12)
        lurek.render.setColor(COL_HIGHLIGHT[1], COL_HIGHLIGHT[2], COL_HIGHLIGHT[3], 1)
        lurek.render.drawRect("fill", bar_x, bar_y + 50, vol_fill, 12)
    end,
}
...
local function draw_transition(progress)
    ...
    elseif transition_type == TRANS_SLIDE then
        ...
        lurek.render.drawRect("fill", -SCREEN_W + offset, 0, SCREEN_W, SCREEN_H)
    ...
    elseif transition_type == TRANS_DISSOLVE then
        ...
        for i = 1, reveal do
            ...
            lurek.render.drawRect("fill", g.c * cell_w, g.r * cell_h, cell_w + 1, cell_h + 1)
        end
    end
...
local function draw_debug()
    ...
    lurek.render.drawRect("fill", SCREEN_W - 260, 0, 260, 160 + #scene_history * 16)

...
function lurek.process(dt)
    ...
    if current_state == STATE.PAUSED then
        ...
        for idx = 1, 5 do
            ...
            if lurek.input.wasActionPressed(action) then
                ...
                lurek.tween.to(entity_highlight, 0.6, { alpha = 0.3 })
    ...
    if math.random() < 0.15 then
        ...
        ps_log_pulse:emit(PANEL_X_LEFT + 10, SCREEN_H - 60, 5)
    end
    ...
    if lurek.input.wasActionPressed("breakpoint") then
        ...
        ps_breakpoint:emit(SCREEN_W / 2, SCREEN_H / 2, 40)
    end
end
...
function lurek.render()
    ...
    lurek.render.drawRect("fill", gx - 2, gy - 2, GRAPH_W + 4, GRAPH_H + 4)

    -- Target line (16.67ms = 60fps)
    local target_y = gy + GRAPH_H - (16.67 / 50 * GRAPH_H)
    lurek.render.setColor(0.3, 0.3, 0.4, 0.5)
    lurek.render.drawLine(gx, target_y, gx + GRAPH_W, target_y)
    ...
    for i = 1, GRAPH_FRAMES do
        ...
        lurek.render.drawRect("fill", bx, by, math.max(bar_w - 1, 1), h)
    end
    ...
    lurek.render.drawRect("fill", gx, mem_y, mem_w, mem_h)
    lurek.render.setColor(COL_MEM_USED[1], COL_MEM_USED[2], COL_MEM_USED[3], 0.8)
    lurek.render.drawRect("fill", gx, mem_y, mem_w * used_frac, mem_h)
...
function lurek.render_ui()
    ...
    lurek.render.drawRect("fill", 0, 0, SCREEN_W, 40)
    ...
    lurek.render.drawRect("fill", lx, ly, PANEL_LEFT_W, 200)

    -- Panel header
    lurek.render.setColor(COL_PANEL_HEAD[1], COL_PANEL_HEAD[2], COL_PANEL_HEAD[3], 1)
    lurek.render.drawRect("fill", lx, ly, PANEL_LEFT_W, 20)
    lurek.render.setColor(COL_ACCENT[1], COL_ACCENT[2], COL_ACCENT[3], 1)
    ...
    lurek.render.setColor(COL_PANEL[1], COL_PANEL[2], COL_PANEL[3], 0.92)
    lurek.render.drawRect("fill", lx, log_y, PANEL_LEFT_W, log_h)

    lurek.render.setColor(COL_PANEL_HEAD[1], COL_PANEL_HEAD[2], COL_PANEL_HEAD[3], 1)
    lurek.render.drawRect("fill", lx, log_y, PANEL_LEFT_W, 20)
    lurek.render.setColor(COL_ACCENT[1], COL_ACCENT[2], COL_ACCENT[3], 1)
    ...
    -- Panel header
    lurek.render.setColor(COL_PANEL_HEAD[1], COL_PANEL_HEAD[2], COL_PANEL_HEAD[3], 1)
    lurek.render.drawRect("fill", rx, ry, PANEL_RIGHT_W, 20)
    lurek.render.setColor(COL_ACCENT[1], COL_ACCENT[2], COL_ACCENT[3], 1)
    ...
    lurek.render.setColor(COL_PANEL[1], COL_PANEL[2], COL_PANEL[3], 0.92)
    lurek.render.drawRect("fill", rx, insp_y, PANEL_RIGHT_W, insp_h)

    lurek.render.setColor(COL_PANEL_HEAD[1], COL_PANEL_HEAD[2], COL_PANEL_HEAD[3], 1)
    lurek.render.drawRect("fill", rx, insp_y, PANEL_RIGHT_W, 20)
    lurek.render.setColor(COL_ACCENT[1], COL_ACCENT[2], COL_ACCENT[3], 1)
    ...
    if selected_entity >= 1 and selected_entity <= #entities then
        ...
        lurek.render.drawRect("fill", rx + 4, iy - 2, PANEL_RIGHT_W - 8, 18)

...
lurek.render(function()
    ...
    for col = 1, COLS do
      ...
      if tx > -TILE and tx < 820 then
        if grid[row][col] == "1" then
          ...
          lurek.render.drawRect(tx, ty, TILE, TILE, shade, 0.28, 0.14, 1)
          lurek.render.drawRect(tx + 2, ty + 2, TILE - 4, TILE - 4, shade + 0.1, 0.33, 0.18, 1)
        end
      end
    end
  end
  ...
  local ey = exit_pos.y
  lurek.render.drawRect(ex, ey, TILE, TILE, 0.1, 0.7, 0.2, 1)
  lurek.render.drawRect(ex + 4, ey + 4, TILE - 8, TILE - 8, 0.2, 0.9, 0.3, 1)
  lurek.render.drawRect(ex + 6, ey, TILE - 12, 6, 0.2, 0.9, 0.3, 1)
  ...
  for _, gem in pairs(gems_map) do
    if gem.alive then
      ...
      lurek.render.drawRect(gx - s / 2, gy - s / 2, s, s, 1.0, 0.85, 0.1, 1)
      lurek.render.drawRect(gx - s / 4, gy - s / 4, s / 2, s / 2, 1.0, 1.0, 0.5, 1)
    end
  end
  ...
  for _, mon in ipairs(monsters) do
    if mon.alive then
      ...
      local my = mon.y
      -- Body
      lurek.render.drawRect(mx + 2, my + 2, mon.w - 4, mon.h - 4, 0.9, 0.25, 0.1, 1)
      lurek.render.drawRect(mx + 4, my + 4, mon.w - 8, mon.h - 8, 1.0, 0.4, 0.15, 1)
      -- Eyes
      lurek.render.drawRect(mx + 6, my + 6, 5, 5, 1, 1, 1, 1)
      lurek.render.drawRect(mx + 17, my + 6, 5, 5, 1, 1, 1, 1)
      lurek.render.drawRect(mx + 7, my + 8, 3, 3, 0, 0, 0, 1)
      lurek.render.drawRect(mx + 18, my + 8, 3, 3, 0, 0, 0, 1)
    end
  end
  ...
  for _, star in ipairs(stars) do
    if star.alive then
      ...
      local sy = star.y
      local s = 12
      lurek.render.drawRect(sx - s / 2, sy - 2, s, 4, 1, 1, 0.8, 1)
      lurek.render.drawRect(sx - 2, sy - s / 2, 4, s, 1, 1, 0.8, 1)
      lurek.render.drawRect(sx - 3, sy - 3, 6, 6, 1, 1, 1, 1)
    end
  end
  ...
  if player.alive then
    ...
    if not blink then
      ...
      lurek.render.drawRect(px, py, player.w, player.h, pr, pg, pb, 1)
      -- Hair (pink top)
      lurek.render.drawRect(px + 2, py, player.w - 4, 8, 1.0, 0.45, 0.6, 1)
      -- Face
      lurek.render.drawRect(px + 5, py + 10, 4, 4, 1, 1, 1, 1)
      lurek.render.drawRect(px + 15, py + 10, 4, 4, 1, 1, 1, 1)
      -- Feet
      lurek.render.drawRect(px + 2, py + player.h - 4, 8, 4, 0.2, 0.2, 0.6, 1)
      lurek.render.drawRect(px + 14, py + player.h - 4, 8, 4, 0.2, 0.2, 0.6, 1)
    end
  end
  ...
  for _, p in ipairs(particles) do
    ...
    local ps = 3 + alpha * 3
    lurek.render.drawRect(p.x + ox - ps / 2, p.y - ps / 2, ps, ps, p.r, p.g, p.b, alpha)
  end

  -- Level complete flash overlay
  if flash_alpha > 0 then
    lurek.render.drawRect(0, 0, 800, 600, 1, 1, 1, flash_alpha * 0.5)
  end
end)
...
lurek.render_ui(function()
  -- ---- TITLE SCREEN ----
  if state == TITLE then
    lurek.render.drawRect(0, 0, 800, 600, 0.05, 0.05, 0.2, 1)
    -- Title text background
    lurek.render.drawRect(100, 150, 600, 60, 0.2, 0.1, 0.4, 0.8)
    lurek.render.drawText("THE GREAT GIANA SISTERS", 160, 165, 28, 1.0, 0.85, 0.2, 1)
    lurek.render.drawText("A Lurek2D Retro Tribute", 260, 210, 16, 0.7, 0.7, 0.7, 1)

    -- Controls
    lurek.render.drawText("A/D or Arrows - Move", 280, 300, 16, 0.8, 0.8, 0.8, 1)
    lurek.render.drawText("Space/W/Up - Jump", 290, 325, 16, 0.8, 0.8, 0.8, 1)
    lurek.render.drawText("Stomp monsters from above!", 250, 365, 16, 1.0, 0.5, 0.3, 1)

    -- Blink prompt
    local blink = math.floor(lurek.timer.getTime() * 2) % 2 == 0
    if blink then
      lurek.render.drawText("PRESS ENTER TO START", 270, 440, 22, 1, 1, 1, 1)
    end

    -- Decorative gems
    for i = 0, 7 do
      local gx = 120 + i * 80
      lurek.render.drawRect(gx, 500, 10, 10, 1.0, 0.85, 0.1, 0.6)
    end
    return
  end
  ...
  -- ---- GAME OVER ----
  if state == GAME_OVER then
    lurek.render.drawRect(0, 0, 800, 600, 0.1, 0.0, 0.0, 0.85)
    lurek.render.drawText("GAME OVER", 280, 200, 36, 1.0, 0.2, 0.2, 1)
    lurek.render.drawText("Final Score: " .. score, 300, 280, 22, 1, 1, 1, 1)
    lurek.render.drawText("Levels Cleared: " .. (current_level - 1) .. " / " .. #levels, 270, 320, 18, 0.8, 0.8, 0.8, 1)
    local blink = math.floor(lurek.timer.getTime() * 2) % 2 == 0
    if blink then
      lurek.render.drawText("PRESS ENTER", 320, 420, 22, 1, 1, 1, 1)
    end
    return
  end
  ...
  -- ---- LEVEL COMPLETE ----
  if state == LEVEL_COMPLETE then
    lurek.render.drawRect(200, 200, 400, 120, 0.1, 0.3, 0.1, 0.9)
    lurek.render.drawText("LEVEL " .. current_level .. " COMPLETE!", 270, 230, 26, 0.2, 1.0, 0.3, 1)
    lurek.render.drawText("Score: " .. score, 330, 280, 20, 1, 1, 1, 1)
  end

  -- ---- HUD ----
  -- Background bar
  lurek.render.drawRect(0, 0, 800, 28, 0, 0, 0, 0.6)

  -- Score
  lurek.render.drawText("SCORE: " .. score, 10, 5, 18, 1, 1, 1, 1)

  -- Gems with pulse effect
  local gem_scale = 1.0 + gem_pulse * 0.3
  local gem_text_size = math.floor(18 * gem_scale)
  lurek.render.drawText("GEMS: " .. gem_count, 200, 5, gem_text_size, 1.0, 0.85, 0.1, 1)

  -- Level
  lurek.render.drawText("LEVEL: " .. current_level .. "/" .. #levels, 400, 5, 18, 0.6, 0.8, 1.0, 1)

  -- Lives
  lurek.render.drawText("LIVES: ", 580, 5, 18, 1, 1, 1, 1)
  for i = 1, lives do
    lurek.render.drawRect(648 + (i - 1) * 22, 6, 16, 16, 1.0, 0.45, 0.6, 1)
  end

  -- FPS
  local fps = lurek.timer.getFPS()
  lurek.render.drawText("FPS: " .. fps, 730, 5, 14, 0.5, 0.5, 0.5, 1)

  -- Invincibility indicator
  if player.invincible then
    local remaining = math.ceil(player.inv_timer)
    lurek.render.drawRect(300, 35, 200, 20, 0.2, 0.2, 0.0, 0.7)
    local bar_w = (player.inv_timer / STAR_DURATION) * 196
    lurek.render.drawRect(302, 37, bar_w, 16, 1.0, 0.9, 0.2, 0.9)
    lurek.render.drawText("STAR POWER " .. remaining .. "s", 340, 38, 14, 1, 1, 1, 1)
  end
end)

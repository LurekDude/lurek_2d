...
function lurek.draw()
    ...
    for d = 1, 4 do
        ...
        local c = di.color
        -- Background
        rect(di.x, di.y, di.w, di.h, c[1] * 0.3, c[2] * 0.3, c[3] * 0.3, 0.8)
        -- Border
        rect(di.x, di.y, di.w, 2, c[1], c[2], c[3], 1)
        rect(di.x, di.y + di.h - 2, di.w, 2, c[1], c[2], c[3], 1)
        rect(di.x, di.y, 2, di.h, c[1], c[2], c[3], 1)
        rect(di.x + di.w - 2, di.y, 2, di.h, c[1], c[2], c[3], 1)

        -- Department name
        text_(di.name, di.x + 6, di.y + 6, 14, c[1], c[2], c[3], 1)

        -- Equipment level indicator
        for e = 1, departments[d].equip_level do
            rect(di.x + di.w - 12 * e, di.y + 6, 8, 8, 1.0, 0.85, 0.2, 1)
        end
        ...
        for j, p in ipairs(departments[d].patients) do
            ...
            local py = di.y + 30
            rect(px, py, 14, 14, 0.9, 0.9, 0.3, 0.8)
            text_(p.condition.name:sub(1, 1), px + 2, py + 1, 10, 0, 0, 0, 1)
        end
        ...
        for j, doc in ipairs(departments[d].doctors) do
            ...
            rect(dx, dy, 20, 20, 0.2, 0.6, 0.9, da)
            text_("D", dx + 5, dy + 3, 12, 1, 1, 1, da)

            -- Treatment progress bar
            if doc.busy and doc.patient then
                local prog = doc.patient.displayProgress or doc.patient.treatProgress
                rect(dx, dy - 8, 20, 4, 0.3, 0.3, 0.3, 1)
                rect(dx, dy - 8, 20 * prog, 4, 0.2, 1.0, 0.3, 1)
            end

            -- Selection highlight
            if doc == selectedDoctor then
                rect(dx - 2, dy - 2, 24, 24, 1, 1, 0, 0.5)
            end
        end
    end

    -- Draw waiting area background
    rect(WAIT_X - 4, WAIT_Y - 24, WAIT_COLS * WAIT_CELL + 8, 90, 0.15, 0.15, 0.18, 0.8)
    text_("WAITING AREA", WAIT_X, WAIT_Y - 20, 12, 0.7, 0.7, 0.7, 1)
    ...
    for i, p in ipairs(patients) do
        if not p.assigned then
            ...
            rect(px + 2, py + 2, WAIT_CELL - 4, WAIT_CELL - 4, r, g, b, 0.9)
            text_(p.condition.name:sub(1, 1), px + 8, py + 8, 12, 0, 0, 0, 1)

            -- Selection highlight
            if p == selectedPatient then
                rect(px, py, WAIT_CELL, WAIT_CELL, 1, 1, 0, 0.4)
            end
        end
    end
    ...
    for _, p in ipairs(particles) do
        ...
        rect(p.x - p.size * 0.5, p.y - p.size * 0.5, p.size, p.size, p.r, p.g, p.b, a)
    end
end
...
function lurek.draw_ui()
    -- ---- TITLE SCREEN ----
    if current_state == STATE.TITLE then
        text_("MEDICAL SIM", 200, 180, 48, 0.9, 0.3, 0.3, 1)
        text_("SAVE LIVES", 260, 240, 28, 0.8, 0.8, 0.8, 1)
        text_("Click to start", 310, 340, 16, 0.6, 0.6, 0.6, 1)
        text_("1-4: Assign dept | H: Hire | E: Equip", 200, 400, 14, 0.5, 0.5, 0.5, 1)
        return
    end

    -- ---- VICTORY SCREEN ----
    if current_state == STATE.VICTORY then
        text_("VICTORY!", 280, 200, 44, 0.2, 1.0, 0.3, 1)
        local msg = string.format("Treated %d patients — Rating: %.1f stars", totalTreated, calcRating())
        text_(msg, 180, 270, 18, 0.9, 0.9, 0.9, 1)
        text_("Click to return to title", 280, 360, 16, 0.6, 0.6, 0.6, 1)
        return
    end

    -- ---- GAME OVER SCREEN ----
    if current_state == STATE.GAME_OVER then
        text_("GAME OVER", 260, 200, 44, 0.9, 0.2, 0.2, 1)
        local msg = string.format("Treated %d / %d — Rating: %.1f stars", totalTreated, GOAL_TREATED, calcRating())
        text_(msg, 190, 270, 18, 0.9, 0.9, 0.9, 1)
        text_("Click to return to title", 280, 360, 16, 0.6, 0.6, 0.6, 1)
        return
    end
    ...
    -- ---- HUD ----
    -- Top bar background
    rect(0, 0, SCREEN_W, 28, 0.08, 0.08, 0.10, 0.9)

    -- Gold
    text_(string.format("Gold: %d", gold), 10, 6, 14, 1.0, 0.85, 0.2, 1)

    -- Rating (stars)
    local r = calcRating()
    local starStr = string.format("Rating: %.1f", r)
    local starColor = r >= 4 and {0.2, 1.0, 0.3} or (r >= 2.5 and {0.9, 0.8, 0.2} or {0.9, 0.2, 0.2})
    text_(starStr, 160, 6, 14, starColor[1], starColor[2], starColor[3], 1)

    -- Patients treated
    text_(string.format("Treated: %d / %d", totalTreated, GOAL_TREATED), 340, 6, 14, 0.8, 0.8, 0.8, 1)

    -- Doctors count
    text_(string.format("Doctors: %d", #doctors), 530, 6, 14, 0.5, 0.7, 1.0, 1)

    -- FPS
    text_(string.format("FPS: %d", fps), SCREEN_W - 80, 6, 14, 0.4, 0.4, 0.4, 1)

    -- Bottom instructions
    rect(0, SCREEN_H - 22, SCREEN_W, 22, 0.08, 0.08, 0.10, 0.8)
    text_("Click patient/doctor then 1-4 assign | H: Hire (100g) | E: Equip (200g) | Esc: Quit",
    ...
    if selectedPatient then
        ...
        text_(info, 10, SCREEN_H - 42, 14, 1.0, 1.0, 0.4, 1)
    elseif selectedDoctor then
        ...
        text_(info, 10, SCREEN_H - 42, 14, 0.4, 0.8, 1.0, 1)
    end
end

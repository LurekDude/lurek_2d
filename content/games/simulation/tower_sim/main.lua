...
function lurek.draw()
    ...
    rect(0, -camera_y, 800, 300, 0.4, 0.6, 0.9, 1.0)
    rect(0, 300 - camera_y, 800, 300, 0.6, 0.8, 1.0, 1.0)

    -- Draw ground
    rect(0, 560 - camera_y, 800, 40, 0.3, 0.6, 0.2, 1.0)
    ...
    for _, fl in ipairs(floors) do
        ...
        rect((FLOOR_WIDTH - sw) / 2, fy, sw, FLOOR_HEIGHT, 0.85, 0.82, 0.78, 1.0)
        rect((FLOOR_WIDTH - sw) / 2, fy, sw, 3, 0.5, 0.5, 0.5, 1.0)
        ...
        if scale >= 1.0 then
            for slot = 1, SLOTS_PER_FLOOR do
                ...
                if rm and rm.type ~= "lobby" then
                    ...
                    rect(rx, ry, rw, rh, c[1], c[2], c[3], 0.9)
                    rect(rx, ry, rw, 2, c[1] * 0.7, c[2] * 0.7, c[3] * 0.7, 1.0)
                    text_(rm.label or "?", rx + 4, ry + 4, 14)
                    ...
                    if rm.type == "apartment" and rm.tenants_in then
                        for t = 1, rm.tenants_in do
                            ...
                            local ty2 = ry + rh - 14
                            circ(tx, ty2, 4, 0.1, 0.1, 0.8, 1.0)
                        end
                    end
                elseif rm and rm.type == "lobby" then
                    rect(rx, ry, rw * 3, rh, 0.5, 0.5, 0.5, 0.7)
                    text_("LOBBY", rx + 20, ry + 20, 20)
                elseif not rm then
                    -- Empty slot outline
                    rect(rx, ry, rw, rh, 0.9, 0.9, 0.9, 0.3)
                end
            end
        end

        -- Floor number
        text_(tostring(fl.index), 4, fy + FLOOR_HEIGHT / 2 - 7, 14)
    end

    -- Draw elevator shaft
    local shaft_top = get_floor_y(top_floor_index()) - camera_y
    local shaft_bottom = get_floor_y(0) + FLOOR_HEIGHT - camera_y
    rect(ELEVATOR_X, shaft_top, ELEVATOR_WIDTH, shaft_bottom - shaft_top, 0.3, 0.3, 0.35, 0.8)
    -- Elevator car
    local ecy = elev_pos_y - camera_y
    if ecy == 0 then ecy = shaft_bottom - 30 end
    rect(ELEVATOR_X + 5, ecy - 15, ELEVATOR_WIDTH - 10, 30, 0.8, 0.75, 0.2, 1.0)
    text_("E:" .. elevators, ELEVATOR_X + 8, ecy - 10, 12)

    -- Particles (dust)
    for _, p in ipairs(particles) do
        local alpha = p.life / p.max_life
        circ(p.x, p.y - camera_y, 3, p.r, p.g, p.b, alpha)
    end
    ...
    for _, s in ipairs(sparkles) do
        ...
        circ(s.x, s.y - camera_y, sz, s.r, s.g, s.b, alpha)
    end

    -- Elevator glow
    for _, g in ipairs(elev_glow) do
        local alpha = g.life / g.max_life * 0.5
        circ(g.x, g.y - camera_y, g.radius, 1.0, 0.9, 0.3, alpha)
    end
end

function lurek.draw_ui()
    if state == "TITLE" then
        rect(0, 0, 800, 600, 0.1, 0.1, 0.2, 1.0)
        text_("TOWER SIM", 240, 160, 48)
        text_("BUILD HIGHER", 270, 240, 28)
        text_("Click to start", 310, 350, 18)
        text_("[F] Floor  [O] Office  [A] Apt  [S] Shop", 140, 420, 14)
        text_("[R] Restaurant  [G] Gym  [E] Elevator", 160, 445, 14)
        text_("[ESC] Quit", 350, 480, 14)
        return
    end

    if state == "VICTORY" then
        rect(0, 0, 800, 600, 0.05, 0.15, 0.05, 0.85)
        text_("VICTORY!", 280, 180, 48)
        text_("Tower complete: " .. floor_count() .. " floors", 240, 270, 22)
        text_("Final gold: " .. math.floor(gold), 280, 310, 22)
        text_("Click to return to title", 270, 400, 18)
        return
    end

    -- HUD
    rect(0, 0, 800, 36, 0.1, 0.1, 0.15, 0.85)
    text_("Gold: " .. math.floor(display_gold), 10, 8, 18)
    text_("Floors: " .. floor_count() .. "/" .. MAX_FLOORS, 200, 8, 18)
    text_("Tenants: " .. count_tenants(), 380, 8, 18)
    text_("Elevators: " .. elevators, 530, 8, 18)

    -- Satisfaction bar
    local sat_pct = display_satisfaction / 100
    local bar_w = 120
    rect(660, 8, bar_w, 18, 0.3, 0.3, 0.3, 1.0)
    local sr = satisfaction < 40 and 0.8 or 0.2
    local sg = satisfaction >= 40 and 0.7 or 0.3
    rect(660, 8, bar_w * sat_pct, 18, sr, sg, 0.2, 1.0)
    text_("Sat:" .. math.floor(satisfaction) .. "%", 665, 9, 13)

    -- Revenue timer bar
    local rev_pct = revenue_timer / REVENUE_INTERVAL
    rect(0, 36, 800 * rev_pct, 3, 1.0, 0.85, 0.2, 0.7)
    ...
    text_(mode_text, 10, 580, 14)

    -- FPS
    text_("FPS: " .. lurek.timer.getFPS(), 730, 580, 12)
end

...
local function buy_item(idx)
    ...
    gold = gold - item.cost
    add_tween(_G, "gold_display_tween_target", gold_display, gold, 0.3)
...
local function serve_customer()
    ...
    if (inventory[item.id] or 0) > 0 then
        ...
        gold = gold + price
        total_earned = total_earned + price
        add_tween(_G, "gold_display_tween_target", gold_display, gold, 0.4)
        inventory[item.id] = inventory[item.id] - 1
...
local function update_tweens(dt)
    ...
    -- gold display tween
    if _G.gold_display_tween_target then
        gold_display = math.floor(_G.gold_display_tween_target)
    else
        gold_display = gold
    end
end

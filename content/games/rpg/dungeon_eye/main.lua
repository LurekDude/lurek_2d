...
function lurek.draw()
    ...
    rect("fill", VIEW_X, VIEW_Y + VIEW_H/2, VIEW_W, VIEW_H/2)
    -- Ceiling
    lurek.render.setColor(0.06, 0.06, 0.12)
    rect("fill", VIEW_X, VIEW_Y, VIEW_W, VIEW_H/2)
    ...
    for ry = 1, MAP_H do
        for rx = 1, MAP_W do
            ...
            rect("fill", mm_ox + (rx-1)*mm_scale, mm_oy + (ry-1)*mm_scale, mm_scale-1, mm_scale-1)
        end
    end
    -- Player dot
    ...
    for _, e in ipairs(enemies) do
        if not e.dead then
            ...
            circ("fill", mm_ox + (e.mx-0.5)*mm_scale, mm_oy + (e.my-0.5)*mm_scale, 2)
        end
    end

    -- ── Right panel ─────────────────────────────────────────────────────
    lurek.render.setColor(0.06, 0.04, 0.10)
    rect("fill", PANEL_X, 0, W - PANEL_X, VIEW_H)
    lurek.render.setColor(0.4, 0.3, 0.6)
    rect("line", PANEL_X, 0, W - PANEL_X, VIEW_H)

    -- HP bar
    lurek.render.setColor(0.6, 0.1, 0.1)
    rect("fill", PANEL_X + 8, 10, 120, 14)
    local hp_frac = player.hp / player.max_hp
    lurek.render.setColor(0.1, 0.8, 0.2)
    rect("fill", PANEL_X + 8, 10, 120 * hp_frac, 14)
    lurek.render.setColor(1, 1, 1)
    text_(string.format("HP %d/%d", player.hp, player.max_hp), PANEL_X + 12, 10)

    -- Direction
    lurek.render.setColor(0.8, 0.75, 0.55)
    text_("Facing: " .. DIR_NAME[player.dir], PANEL_X + 8, 32)
    text_(string.format("Pos: %d,%d", player.mx, player.my), PANEL_X + 8, 48)

    -- Inventory panel header
    lurek.render.setColor(0.9, 0.8, 0.5)
    text_("Inventory [I]", PANEL_X + 8, 72)
    local slots = inventory.getSlots(player.inv)
    local shown = 0
    for i, slot in ipairs(slots) do
        if slot then
            local item_color = show_inv and (i == inv_cursor and {1,1,0} or {0.8,0.8,0.7}) or {0.6,0.6,0.55}
            lurek.render.setColor(item_color[1], item_color[2], item_color[3])
            text_(string.format("%d. %s", i, slot.type_id), PANEL_X + 12, 86 + shown * 16)
            shown = shown + 1
        end

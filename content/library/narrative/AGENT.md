# `narrative` — Agent Reference (Lunasome)
...
### Story lifecycle
- `story:start(knot?)` — reset to entry knot (default `START` or first knot)
...
`partial` — implements a usable subset that covers ~90% of typical branching
...
See `content/library/narrative/example.lua` for a multi-knot story exercising

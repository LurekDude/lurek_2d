...
local clamped = lurek.math.clamp(1.5, 0, 1)   -- → 1
local clamped2 = lurek.math.clamp(-5, 0, 100) -- → 0

-- Linear interpolation between two values
local blend = lurek.math.lerp(0, 100, 0.25)   -- → 25

-- Distance between two points
local dist = lurek.math.distance(0, 0, 3, 4)  -- → 5

-- Angle from point A to point B (in radians)
local ang = lurek.math.angle(0, 0, 1, 0)      -- → 0 (facing right)
local ang2 = lurek.math.angle(0, 0, 0, 1)     -- → math.pi/2 (facing down)

-- Normalize a 2D vector (returns unit vector components)
local nx, ny = lurek.math.normalize(3, 4)     -- → 0.6, 0.8

-- Dot product of two 2D vectors
local d = lurek.math.dot(1, 0, 0, 1)          -- → 0 (perpendicular)

-- Convert from polar (magnitude, angle) to Cartesian (x, y)
local x, y = lurek.math.fromPolar(10, math.pi / 4) -- → 7.07, 7.07

-- Convert from Cartesian to polar (magnitude, angle)
local r, theta = lurek.math.toPolar(1, 1)     -- → 1.41, 0.785

-- ─────────────────────────────────────────────────────────────────────────────
-- Standard Math (also available as normal Lua math.*)
-- ─────────────────────────────────────────────────────────────────────────────
local a1 = lurek.math.abs(-7)        -- → 7
local a2 = lurek.math.floor(3.9)     -- → 3
local a3 = lurek.math.ceil(3.1)      -- → 4
local a4 = lurek.math.sqrt(16)       -- → 4
local a5 = lurek.math.sin(math.pi)   -- → ~0
local a6 = lurek.math.cos(0)         -- → 1
local a7 = lurek.math.atan2(1, 0)    -- → pi/2
local a8 = lurek.math.max(3, 7, 2)   -- → 7
local a9 = lurek.math.min(3, 7, 2)   -- → 2
local pi  = lurek.math.pi            -- → 3.14159...
...
-- Linear (no shaping)
local v_linear    = lurek.math.linear(t)          -- → 0.6  (identity; use when no easing is needed)

-- Quadratic — gentle, almost imperceptible at low t
local v_inQuad    = lurek.math.inQuad(t)          -- → 0.36  gentle acceleration
local v_outQuad   = lurek.math.outQuad(t)         -- → 0.84  gentle deceleration; most common for UI
local v_inOutQuad = lurek.math.inOutQuad(t)       -- → 0.72  smooth symmetric feel

-- Cubic — more pronounced than quadratic
local v_inCubic    = lurek.math.inCubic(t)        -- → 0.216 sharper acceleration
local v_outCubic   = lurek.math.outCubic(t)       -- → 0.936 crisp deceleration
local v_inOutCubic = lurek.math.inOutCubic(t)     -- → 0.648 balanced snap

-- Quartic — strong, cinematic feel
local v_inQuart    = lurek.math.inQuart(t)        -- → 0.130 dramatic slow start
local v_outQuart   = lurek.math.outQuart(t)       -- → 0.974 very fast start, then sudden stop
local v_inOutQuart = lurek.math.inOutQuart(t)     -- → 0.741 punchy middle

-- Sinusoidal — the softest of the power easings; follows a cosine curve
local v_inSine    = lurek.math.inSine(t)          -- → subtle acceleration; good for breathing animations
local v_outSine   = lurek.math.outSine(t)         -- → subtle deceleration
local v_inOutSine = lurek.math.inOutSine(t)       -- → very gentle overall arc

-- Exponential — nearly flat then explosive (or vice versa)
local v_inExpo    = lurek.math.inExpo(t)          -- → barely moves then launches hard
local v_outExpo   = lurek.math.outExpo(t)         -- → explosive start, nearly stops; good for projectiles
local v_inOutExpo = lurek.math.inOutExpo(t)       -- → symmetric exponential snap

-- Elastic — spring-like oscillation past the end value
local v_inElastic  = lurek.math.inElastic(t)      -- → wobbles before leaving; unusual but dramatic
local v_outElastic = lurek.math.outElastic(t)     -- → overshoots then settles; great for UI pop-in

-- Bounce — simulates a physical bounce (multiple sub-bounces)
local v_inBounce  = lurek.math.inBounce(t)        -- → bounces before the move begins; rarely used
local v_outBounce = lurek.math.outBounce(t)       -- → bounces after arriving; classic ball-drop feel

-- Back — slight overshoot, like a rubber-band pull or snap
local v_inBack  = lurek.math.inBack(t)            -- → pulls back briefly then launches forward
local v_outBack = lurek.math.outBack(t)           -- → flies past target then snaps back; good for menu items
...
local shaped = lurek.math.applyEasing(easing_name, t)   -- → same as outQuad(t); name looked up dynamically
...
local aim_angle = lurek.math.angleBetween(player_x, player_y, enemy_x, enemy_y)  -- → radians

-- Circle-vs-point: is the mouse cursor inside a circular button hitzone?
local inside = lurek.math.circleContainsPoint(200, 175, 40, player_x, player_y)  -- → true/false

-- Squared distance: fast broad-phase range check without a sqrt
local dsq = lurek.math.distanceSq(player_x, player_y, enemy_x, enemy_y)          -- → squared pixels
...
local hit_line, lx1, ly1, lx2, ly2 =
    lurek.math.circleIntersectsLine(200, 150, 50,  50, 50, 350, 50)               -- → hit, pt1?, pt2?

-- Circle-vs-segment: same test but capped to the actual wall segment length
local hit_seg, sx1, sy1, sx2, sy2 =
    lurek.math.circleIntersectsSegment(200, 150, 50,  50, 50, 350, 50)            -- → hit, pt1?, pt2?
...
local hull = lurek.math.convexHull(spawn_cloud)        -- → flat {x,y,...} in CCW order

-- isConvex: verify that a hand-authored polygon has no concave notches
local convex = lurek.math.isConvex(hull)               -- → true if the hull is convex (it always will be here)
...
local tri_list  = lurek.math.delaunayTriangulate(waypoints)  -- → table of flat 6-float triangle tables
...
local in_room = lurek.math.pointInPolygon(room, player_x, player_y)   -- → true/false

-- Polygon area: scale loot drop probability by room size
local area = lurek.math.polygonArea(room)              -- → signed area in px² (negative = CW winding)

-- Polygon centroid: place the boss or a spotlight at the room's centre of mass
local cent_x, cent_y = lurek.math.polygonCentroid(room)                -- → (cx, cy)
...
local cells = lurek.math.bresenham(3, 2, 10, 7)        -- → {{x=3,y=2}, {x=4,y=2}, ..., {x=10,y=7}}
...
local s   = lurek.math.sin(math.pi / 6)        -- → 0.5
local c   = lurek.math.cos(math.pi / 3)        -- → 0.5
local tg  = lurek.math.tan(math.pi / 4)        -- → 1.0
local as  = lurek.math.asin(0.5)               -- → π/6  (inverse sine)
local ac  = lurek.math.acos(0.5)               -- → π/3  (inverse cosine)
local at  = lurek.math.atan(1.0)               -- → π/4  single-argument form
local at2 = lurek.math.atan(1.0, 1.0)          -- → π/4  two-argument form (y, x); use for direction vectors

-- Angle conversion
local deg_val = lurek.math.deg(math.pi)        -- → 180.0  radians → degrees
local rad_val = lurek.math.rad(90)             -- → π/2    degrees → radians

-- Algebra
local sq  = lurek.math.sqrt(144)               -- → 12.0
local ab  = lurek.math.abs(-7.5)               -- → 7.5
local ep  = lurek.math.exp(1)                  -- → e ≈ 2.71828
local ln  = lurek.math.log(math.exp(1))        -- → 1.0    natural log (base e)
local lg  = lurek.math.log(1024, 2)            -- → 10.0   log base 2
local pw  = lurek.math.pow(2, 10)              -- → 1024
local sg  = lurek.math.sign(-42)               -- → -1    ; sign(0) → 0 ; sign(pos) → 1
local fm  = lurek.math.fmod(7.5, 2.0)          -- → 1.5   remainder, same sign as dividend

-- Rounding
local fl  = lurek.math.floor(3.9)              -- → 3
local cl  = lurek.math.ceil(3.1)               -- → 4
local ro  = lurek.math.round(3.5)              -- → 4  (half-up rounding)

-- Scalar utilities
local lo  = lurek.math.min(3, 7)               -- → 3
local hi  = lurek.math.max(3, 7)               -- → 7
local clv = lurek.math.clamp(15, 0, 10)        -- → 10  caps the value to [0, 10]
...
local pn2 = lurek.math.perlin2d(0.5, 0.3)           -- → [-1, 1]  2D Perlin
local pn3 = lurek.math.perlin3d(0.5, 0.3, 1.2)      -- → [-1, 1]  3D Perlin (e.g. animated terrain)
local sx2 = lurek.math.simplex2d(0.5, 0.3)           -- → [-1, 1]  2D Simplex; faster than Perlin
local sfbm = lurek.math.fbm(0.5, 0.3)               -- → layered fractal Brownian motion noise
local snz = lurek.math.simplexNoise(0.5, 0.3)        -- → aliased form of simplex2d with alternate signature

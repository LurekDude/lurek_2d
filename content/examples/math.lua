...
-- if false then -- lurek.math.newRandomGenerator
...
-- if false then -- lurek.math.newTransform
...
-- if false then -- lurek.math.newBezierCurve
...
-- if false then -- lurek.math.newTween
...
-- if false then -- lurek.math.newSpatialHash
...
-- if false then -- lurek.math.newNoiseGenerator
...
-- if false then -- lurek.math.perlin2d
...
-- if false then -- lurek.math.perlin3d
--   local t = 0
...
-- if false then -- lurek.math.simplex2d
...
-- if false then -- lurek.math.applyEasing
...
-- Identity easing â€” useful as a default tween shape that is later swapped for a curved one.
-- if false then -- lurek.math.linear
--   local t = 0.42
...
-- if false then -- lurek.math.inQuad
--   local progress = 0.3
...
-- if false then -- lurek.math.outQuad
...
-- if false then -- lurek.math.inOutQuad
--   local t = 0.5
...
-- if false then -- lurek.math.inCubic
...
-- Strong deceleration â€” ideal for tooltips that snap into view.
-- if false then -- lurek.math.outCubic
...
-- if false then -- lurek.math.inOutCubic
...
-- if false then -- lurek.math.inQuart
...
-- if false then -- lurek.math.outQuart
...
-- Long flat ends with a steep middle â€” good for dramatic camera dolly-ins.
-- if false then -- lurek.math.inOutQuart
...
-- if false then -- lurek.math.inSine
...
-- Soft deceleration based on cosine â€” good fit for icons fading in.
-- if false then -- lurek.math.outSine
...
-- if false then -- lurek.math.inOutSine
...
-- if false then -- lurek.math.inExpo
...
-- Snaps to peak then settles â€” feels like a sword swing recoiling.
-- if false then -- lurek.math.outExpo
...
-- if false then -- lurek.math.inOutExpo
...
-- if false then -- lurek.math.inElastic
...
-- if false then -- lurek.math.outElastic
...
-- if false then -- lurek.math.outBounce
...
-- Reverse bounce â€” the object hops up before launching.
-- if false then -- lurek.math.inBounce
...
-- if false then -- lurek.math.inBack
...
-- Overshoots target then settles â€” a classic for HUD pop-in.
-- if false then -- lurek.math.outBack
...
-- if false then -- lurek.math.inOutElastic
...
-- if false then -- lurek.math.inOutBounce
...
-- if false then -- lurek.math.inOutBack
...
-- if false then -- lurek.math.triangulate
...
-- if false then -- lurek.math.isConvex
--   local poly = {0, 0, 100, 0, 100, 100, 0, 100}
...
-- if false then -- lurek.math.gammaToLinear
...
-- if false then -- lurek.math.linearToGamma
...
-- if false then -- lurek.math.angleBetween
...
-- if false then -- lurek.math.circleContainsPoint
...
-- if false then -- lurek.math.circleIntersectsCircle
...
-- if false then -- lurek.math.circleIntersectsLine
...
-- Like circleIntersectsLine but bounded to the segment endpoints â€” use for bullet vs target.
-- if false then -- lurek.math.circleIntersectsSegment
...
-- if false then -- lurek.math.closestPointOnSegment
...
-- if false then -- lurek.math.convexHull
...
-- if false then -- lurek.math.delaunayTriangulate
...
-- if false then -- lurek.math.lineIntersect
...
-- if false then -- lurek.math.pointInPolygon
--   local poly = {0, 0, 100, 0, 100, 100, 0, 100}
...
-- if false then -- lurek.math.polygonArea
--   local poly = {0, 0, 100, 0, 100, 100, 0, 100}
...
-- if false then -- lurek.math.polygonCentroid
--   local poly = {0, 0, 100, 0, 100, 100, 0, 100}
...
-- Bounded version of lineIntersect â€” use for laser-beam-vs-wall or sword swing arcs.
-- if false then -- lurek.math.segmentIntersectsSegment
...
-- if false then -- lurek.math.bresenham
...
-- Use for oscillating bobs, springs, or waveform generation â€” period is 2*pi.
-- if false then -- lurek.math.sin
--   local t = 0
--   function lurek.process(dt)
--     t = t + dt
...
-- if false then -- lurek.math.atan2
...
-- Strip the sign before comparing magnitudes â€” handy when reading axis input.
-- if false then -- lurek.math.abs
--   local axis = -0.7
...
-- if false then -- lurek.math.floor
--   local raw_x = 123.7
...
-- if false then -- lurek.math.round
...
-- Use to enforce a floor â€” e.g. damage taken cannot drop below 1.
-- if false then -- lurek.math.max
...
-- if false then -- lurek.math.clamp
...
-- if false then -- lurek.math.distance
...
-- if false then -- lurek.math.distanceSq
...
-- if false then -- lurek.math.random
...
-- if false then -- lurek.math.randomInt
...
-- if false then -- lurek.math.simplexNoise
...
-- if false then -- lurek.math.catmullRom
  ---@type LCatmullRom
...
-- if false then -- lurek.math.hermite
  ---@type LHermite
...
-- Maps a value across two ranges in one call â€” handy for scaling sensor input to UI sliders.
-- if false then -- lurek.math.remap
...
--@api-stub: lurek.math.clamp
-- Clamps `v` between `min` and `max`.
-- f32 overload â€” pin a control value into [min, max] without branching.
-- if false then -- lurek.math.clamp
...
-- if false then -- lurek.math.smoothstep
...
-- Inverse of lerp â€” recover the t parameter that interpolates a to b into v.
-- if false then -- lurek.math.inverseLerp
...
-- if false then -- lurek.math.hslToRgb
...
-- if false then -- lurek.math.fromHex
...
-- if false then -- lurek.math.rgbToHsl
...
-- if false then -- lurek.math.rectUnion
...
-- if false then -- lurek.math.rectFromCenter
...
-- if false then -- lurek.math.polygonClip
--   local poly = {0, 0, 100, 0, 100, 100, 0, 100}
...
-- if false then -- lurek.math.aabbTree
...
-- if false then -- lurek.math.newCircle
...
-- if false then -- lurek.math.polygonIntersection
...
-- Approximated as the convex hull of all vertices â€” fine for visualisations, not for area math.
-- if false then -- lurek.math.polygonUnion
--   local a = {{x=0,y=0},{x=100,y=0},{x=100,y=100},{x=0,y=100}}
...
-- if false then -- lurek.math.polygonDifference
--   local a = {{x=0,y=0},{x=100,y=0},{x=100,y=100},{x=0,y=100}}
...
-- if false then -- lurek.math.voronoi
...
-- O(sqrt) â€” prefer lengthSquared for ordering comparisons.
-- if false then -- Vec2:length
...
-- if false then -- Vec2:lengthSquared
--   local v = lurek.math.vec2(3, 4)
...
-- Returns (-y, x) â€” a 90-degree CCW rotation; double-apply to get the negation.
-- if false then -- Vec2:perpendicular
...
-- Reflects self off a surface with the given (unit) normal â€” used for bouncing balls.
-- if false then -- Vec2:reflect
...
-- Squared length â€” avoid the sqrt when sorting things by distance.
-- if false then -- Vec3:lengthSquared
  ---@type LVec3
...
-- Returns a new Vec3 â€” does not mutate self.
-- if false then -- Vec3:add
  ---@type LVec3
...
-- if false then -- CatmullRom:sample
  ---@type LCatmullRom
--   local cr = lurek.math.catmullRom({{x=0,y=0},{x=100,y=200},{x=300,y=200},{x=400,y=0}})
...
-- if false then -- CatmullRom:sampleSegment
  ---@type LCatmullRom
...
-- if false then -- CatmullRom:addPoint
  ---@type LCatmullRom
...
-- if false then -- CatmullRom:removePoint
  ---@type LCatmullRom
--   local cr = lurek.math.catmullRom({{x=0,y=0},{x=50,y=50},{x=100,y=0},{x=150,y=50}})
...
-- Uniform [0,1) â€” bedrock for everything else; combine with multipliers for ranges.
-- if false then -- RandomGenerator:random
...
-- Inclusive lower bound, exclusive upper â€” matches Python's random.uniform half-open.
-- if false then -- RandomGenerator:randomFloat
...
-- Inclusive on both ends â€” perfect for d20 rolls and grid coordinates.
-- if false then -- RandomGenerator:randomInt
...
-- if false then -- RandomGenerator:getSeed
...
-- Resets internal state â€” call before deterministic sequences like proc-gen rooms.
-- if false then -- RandomGenerator:setSeed
...
-- if false then -- RandomGenerator:getState
...
-- if false then -- RandomGenerator:setState
--   local rng = lurek.math.newRandomGenerator(77)
...
-- Mutates the transform â€” apply each frame to drift a sprite's local origin.
-- if false then -- Transform:translate
...
-- if false then -- Transform:rotate
--   local t = lurek.math.newTransform()
--   t:rotate(math.pi / 4)
...
-- Drops back to identity â€” call to recycle a transform across frames without re-allocating.
-- if false then -- Transform:reset
...
-- if false then -- Transform:transformPoint
...
-- if false then -- Transform:inverseTransformPoint
...
-- if false then -- Transform:inverse
...
-- if false then -- Transform:getMatrix
...
-- if false then -- Transform:decompose
--   local t = lurek.math.newTransform(100, 50, math.pi / 4, 2, 2)
...
-- if false then -- BezierCurve:evaluate
...
-- if false then -- BezierCurve:render
--   local c = lurek.math.newBezierCurve({0, 0, 100, 200, 300, 200, 400, 0})
...
-- if false then -- BezierCurve:getDerivative
--   local c = lurek.math.newBezierCurve({0, 0, 100, 200, 300, 200, 400, 0})
...
-- if false then -- BezierCurve:getControlPoint
--   local c = lurek.math.newBezierCurve({0, 0, 100, 200, 300, 200, 400, 0})
...
-- if false then -- BezierCurve:removeControlPoint
--   local c = lurek.math.newBezierCurve({0, 0, 100, 200, 300, 200, 400, 0})
...
-- if false then -- BezierCurve:getControlPointCount
--   local c = lurek.math.newBezierCurve({0, 0, 100, 200, 300, 200, 400, 0})
...
-- if false then -- BezierCurve:length
--   local c = lurek.math.newBezierCurve({0, 0, 100, 200, 300, 200, 400, 0})
--   local len = c:length()
...
-- if false then -- BezierCurve:translate
--   local c = lurek.math.newBezierCurve({0, 0, 100, 200, 300, 200, 400, 0})
--   c:translate(10, 5)
...
-- if false then -- BezierCurve:rotate
--   local c = lurek.math.newBezierCurve({0, 0, 100, 200, 300, 200, 400, 0})
...
-- if false then -- BezierCurve:scale
--   local c = lurek.math.newBezierCurve({0, 0, 100, 200, 300, 200, 400, 0})
--   c:scale(2.0, 0, 0)
...
-- if false then -- Tween:getAllValues
--   local tw = lurek.math.newTween(0.5)
--   tw:addValue(0, 1)
...
-- if false then -- Tween:isComplete
...
-- if false then -- Tween:getValueCount
--   local tw = lurek.math.newTween(0.5)
--   tw:addValue(0, 10)
...
-- Returns the easing string passed to newTween â€” useful in debug overlays.
-- if false then -- Tween:getEasingName
...
-- if false then -- Tween:getDuration
...
-- if false then -- SpatialHash:remove
...
-- Drops every entry â€” useful at scene transitions to start with a fresh grid.
-- if false then -- SpatialHash:clear
--   local h = lurek.math.newSpatialHash(64)
...
-- if false then -- SpatialHash:getCellSize
...
-- if false then -- SpatialHash:getItemCount
--   local h = lurek.math.newSpatialHash(64)
--   h:insert("a", 0, 0, 16, 16)
...
-- if false then -- NoiseGenerator:perlin1d
...
-- if false then -- NoiseGenerator:perlin2d
...
-- if false then -- NoiseGenerator:perlin3d
...
-- if false then -- NoiseGenerator:perlin4d
...
-- if false then -- NoiseGenerator:simplex1d
...
-- if false then -- NoiseGenerator:simplex2d
...
-- if false then -- NoiseGenerator:simplex3d
...
-- if false then -- NoiseGenerator:getSeed
...
-- Re-seeds AND rebuilds the permutation table â€” costlier than a number assignment.
-- if false then -- NoiseGenerator:setSeed
...
-- if false then -- Circle:perimeter
...
-- Inclusive of the boundary â€” a point exactly on the rim returns true.
-- if false then -- Circle:contains
...
-- if false then -- Circle:intersects
...
-- if false then -- AabbTree:queryPoint
--   local t = lurek.math.aabbTree()
--   t:insert(1, 0, 0, 32, 32)
...
-- if false then -- AabbTree:contains
--   local t = lurek.math.aabbTree()
...
-- if false then -- AabbTree:isEmpty
--   local t = lurek.math.aabbTree()
--   if t:isEmpty() then
...
-- if false then -- NoiseGenerator:fbm
...
-- if false then -- NoiseGenerator:generateMap
...
-- if false then -- SpatialHash:insert
...
-- if false then -- BezierCurve:insertControlPoint
...
-- if false then -- SpatialHash:queryCircle
...
-- if false then -- SpatialHash:queryRect
--   local sh = lurek.math.newSpatialHash(64)
...
-- if false then -- SpatialHash:querySegment
--   local sh = lurek.math.newSpatialHash(64)
...
-- if false then -- RandomGenerator:randomNormal
...
-- if false then -- NoiseGenerator:ridged
...
-- if false then -- BezierCurve:setControlPoint
...
-- if false then -- Transform:setTransformation
--   local t = lurek.math.newTransform()
...
-- if false then -- NoiseGenerator:turbulence
...
-- if false then -- SpatialHash:update
--   local sh = lurek.math.newSpatialHash(64)
--   sh:insert("player", 100, 100, 32, 32)
...
-- if false then -- NoiseGenerator:warpDomain
...
-- if false then -- NoiseGenerator:worley2d
...
-- if false then -- NoiseGenerator:worley3d
...
-- =============================================================================
-- STUBS: 24 uncovered lurek.math API item(s)
...
-- =============================================================================
-- STUBS: 26 uncovered lurek.math API item(s)
-- Generated by tools/audit/example_add_missing.py
-- REQUIRED: replace every --@api-stub: block below with a real scenario.
-- Run .github/prompts/flesh-out-example.prompt.md for instructions.
-- The final committed file must contain ZERO --@api-stub: lines.
...
-- if false then -- LAabbTree:typeOf
--   local aabb_tree_obj = lurek.math.aabbTree()
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LBezierCurve:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LBezierCurve:typeOf
--   local bezier_curve_obj = lurek.math.newBezierCurve({0,0, 100,50, 200,0})
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LCatmullRom:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LCatmullRom:typeOf
--   local catmull_rom_obj = lurek.math.catmullRom({{0,0},{100,50},{200,0},{300,50}})
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LNoiseGenerator:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LNoiseGenerator:typeOf
--   local noise_generator_obj = lurek.math.newNoiseGenerator(42)
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LRandomGenerator:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LRandomGenerator:typeOf
--   local random_generator_obj = lurek.math.newRandomGenerator(42)
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LSpatialHash:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LSpatialHash:typeOf
--   local spatial_hash_obj = lurek.math.newSpatialHash(64)
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LTransform:typeOf
--   local transform_obj = lurek.math.newTransform()

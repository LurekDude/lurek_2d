...
-- if false then -- lurek.docs.scanModule
...
-- if false then -- lurek.docs.loadToml
...
-- if false then -- lurek.docs.loadAll
...
-- if false then -- lurek.docs.describe
--   lurek.docs.scan()
...
-- if false then -- lurek.docs.setParamInfo
--   lurek.docs.scan()
...
-- if false then -- lurek.docs.setReturnInfo
--   lurek.docs.scan()
...
-- if false then -- lurek.docs.getCatalog
--   lurek.docs.scan()
...
-- if false then -- lurek.docs.resetCatalog
--   lurek.docs.scan()
...
-- if false then -- lurek.docs.validate
...
-- if false then -- lurek.docs.validateModule
--   local ok, catalog = pcall(lurek.docs.loadToml, "docs/api/audio.toml")
--   if ok then
...
-- if false then -- lurek.docs.checkStaleness
--   local catalog = lurek.docs.loadAll("docs/api")
...
-- if false then -- lurek.docs.quality
--   local catalog = lurek.docs.loadAll("docs/api")
...
-- if false then -- lurek.docs.qualityModule
--   local catalog = lurek.docs.loadAll("docs/api")
...
-- if false then -- lurek.docs.coverage
--   local catalog = lurek.docs.loadAll("docs/api")
...
-- if false then -- lurek.docs.coverageModule
--   local ok, catalog = pcall(lurek.docs.loadToml, "docs/api/audio.toml")
--   if ok then
...
-- if false then -- lurek.docs.exportCompletions
--   local catalog = lurek.docs.scan()
...
-- if false then -- lurek.docs.exportHover
--   local catalog = lurek.docs.scan()
...
-- if false then -- lurek.docs.exportSignatures
--   local catalog = lurek.docs.scan()
...
-- if false then -- lurek.docs.exportAll
--   local catalog = lurek.docs.scan()
...
-- if false then -- lurek.docs.exportMarkdown
--   local catalog = lurek.docs.loadAll("docs/api")
...
-- if false then -- lurek.docs.exportCheatsheet
--   local catalog = lurek.docs.scan()
...
-- if false then -- lurek.docs.schema
...
-- if false then -- lurek.docs.reflectLive
...
-- if false then -- lurek.docs.reflectTable
...
-- if false then -- Schema:getFields
...
-- if false then -- DocEntry:getName
...
-- if false then -- DocEntry:getQualifiedName
--   local catalog = lurek.docs.scan()
...
-- if false then -- DocEntry:getModule
--   local catalog = lurek.docs.scan()
...
-- if false then -- DocEntry:getKind
--   local catalog = lurek.docs.scan()
--   for _, e in ipairs(catalog:getEntries()) do
...
-- if false then -- DocEntry:getDescription
--   local ok, catalog = pcall(lurek.docs.loadToml, "docs/api/audio.toml")
--   if ok then
...
-- if false then -- DocEntry:getParameters
--   local ok, catalog = pcall(lurek.docs.loadToml, "docs/api/audio.toml")
--   if ok then
--     local entry = catalog:getEntry("lurek.audio.play")
--     if entry then
...
-- if false then -- DocEntry:getReturns
--   local ok, catalog = pcall(lurek.docs.loadToml, "docs/api/audio.toml")
--   if ok then
--     local entry = catalog:getEntry("lurek.audio.play")
--     if entry then
...
-- if false then -- DocEntry:getExample
--   local catalog = lurek.docs.loadAll("docs/api")
--   local entry = catalog:getEntry("lurek.audio.play")
...
-- if false then -- DocEntry:getSince
--   local catalog = lurek.docs.loadAll("docs/api")
--   for _, e in ipairs(catalog:getEntries()) do
--     local v = e:getSince()
...
-- if false then -- DocEntry:getDeprecated
--   local catalog = lurek.docs.loadAll("docs/api")
--   for _, e in ipairs(catalog:getEntries()) do
...
-- if false then -- DocEntry:getScore
--   local catalog = lurek.docs.loadAll("docs/api")
--   for _, e in ipairs(catalog:getEntries()) do
...
-- if false then -- DocEntry:hasDescription
--   local catalog = lurek.docs.scan()
--   local missing = 0
--   for _, e in ipairs(catalog:getEntries()) do
...
-- if false then -- DocEntry:hasParameters
--   local catalog = lurek.docs.loadAll("docs/api")
--   for _, e in ipairs(catalog:getEntries()) do
...
-- if false then -- DocEntry:hasReturnType
--   local catalog = lurek.docs.loadAll("docs/api")
--   for _, e in ipairs(catalog:getEntries()) do
...
-- if false then -- DocEntry:hasExample
--   local catalog = lurek.docs.loadAll("docs/api")
--   local without = 0
--   for _, e in ipairs(catalog:getEntries()) do
...
-- if false then -- ApiCatalog:getModules
--   local catalog = lurek.docs.scan()
...
-- if false then -- ApiCatalog:getEntries
--   local catalog = lurek.docs.scan()
...
-- if false then -- ApiCatalog:getEntry
--   local catalog = lurek.docs.scan()
--   local entry = catalog:getEntry("lurek.audio.play")
...
-- if false then -- ApiCatalog:getTypes
--   local catalog = lurek.docs.loadAll("docs/api")
...
-- if false then -- ApiCatalog:getTypeMethods
--   local catalog = lurek.docs.loadAll("docs/api")
...
-- if false then -- ApiCatalog:entryCount
--   local catalog = lurek.docs.scan()
...
-- if false then -- ApiCatalog:merge
...
-- if false then -- ApiCatalog:filter
--   local catalog = lurek.docs.loadAll("docs/api")
...
-- if false then -- ApiCatalog:search
--   local catalog = lurek.docs.scan()
...
-- if false then -- ApiCatalog:toTable
--   local catalog = lurek.docs.scan()
...
-- if false then -- ApiCatalog:toJSON
--   local catalog = lurek.docs.scan()
...
-- if false then -- ValidationReport:isValid
--   local catalog = lurek.docs.loadAll("docs/api")
--   local report = lurek.docs.validate(catalog)
...
-- if false then -- ValidationReport:getMissing
...
-- if false then -- ValidationReport:getPhantom
--   local report = lurek.docs.validate(lurek.docs.loadAll("docs/api"))
...
-- if false then -- ValidationReport:getIncomplete
--   local report = lurek.docs.validate(lurek.docs.loadAll("docs/api"))
...
-- if false then -- ValidationReport:missingCount
--   local report = lurek.docs.validate(lurek.docs.loadAll("docs/api"))
...
-- if false then -- ValidationReport:phantomCount
--   local report = lurek.docs.validate(lurek.docs.loadAll("docs/api"))
...
-- if false then -- ValidationReport:incompleteCount
--   local report = lurek.docs.validate(lurek.docs.loadAll("docs/api"))
...
-- if false then -- ValidationReport:getSummary
--   local report = lurek.docs.validate(lurek.docs.loadAll("docs/api"))
...
-- if false then -- ValidationReport:toTable
--   local report = lurek.docs.validate(lurek.docs.loadAll("docs/api"))
...
-- if false then -- ValidationReport:toJSON
--   local report = lurek.docs.validate(lurek.docs.loadAll("docs/api"))
...
-- if false then -- QualityReport:getOverallScore
...
-- if false then -- QualityReport:getGrade
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
...
-- if false then -- QualityReport:getModuleScores
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
...
-- if false then -- QualityReport:getWorst
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
...
-- if false then -- QualityReport:getBest
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
...
-- if false then -- QualityReport:getByGrade
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
...
-- if false then -- QualityReport:getSummary
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
...
-- if false then -- QualityReport:toTable
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
...
-- if false then -- QualityReport:toJSON
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
--   pcall(function() lurek.fs.write("build/docs-quality.json", q:toJSON()) end)
-- end

-- =============================================================================
-- STUBS: 7 uncovered lurek.docs API item(s)
...
-- =============================================================================
-- STUBS: 17 uncovered lurek.docs API item(s)
-- Generated by tools/audit/example_add_missing.py
-- REQUIRED: replace every --@api-stub: block below with a real scenario.
-- Run .github/prompts/flesh-out-example.prompt.md for instructions.
-- The final committed file must contain ZERO --@api-stub: lines.
...
-- if false then -- LApiCatalog:type
...
-- if false then -- LApiCatalog:typeOf
--   local api_catalog_obj = lurek.docs.scan(nil)
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LDocEntry:typeOf
--   local catalog = lurek.docs.scanModule("audio")
--     local entry = catalog:getEntries()[1]
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LQualityReport:type
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
--   local t = q:type()
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LQualityReport:typeOf
--   local q = lurek.docs.quality(lurek.docs.loadAll("docs/api"))
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LValidationReport:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LValidationReport:typeOf
--   local validation_report_obj = lurek.docs.validate(nil)

...
-- quads to lurek.render.draw â€” the snippets here focus on the
...
-- if false then -- lurek.sprite.newSheet
...
-- if false then -- lurek.sprite.newRPGMakerSheet
...
-- if false then -- lurek.sprite.parseAtlas
--   pcall(function()
...
-- if false then -- lurek.sprite.newAtlasSheet
--   pcall(function()
...
-- if false then -- lurek.sprite.parseAsepriteAtlas
--   pcall(function()
...
-- if false then -- SpriteSheet:getFrame
...
-- if false then -- SpriteSheet:getFrameCount
...
-- Handy for direction strips: row 0 = walk-down, row 1 = walk-left, etc. â€” pass the row a character is facing.
-- if false then -- SpriteSheet:getRow
...
-- if false then -- SpriteSheet:getColumn
...
-- Returns nil if the group was never registered â€” guard with `if frames then` before iterating.
-- if false then -- SpriteSheet:getGroupFrames
...
-- if false then -- SpriteSheet:getGroupNames
--   local sheet = lurek.sprite.newRPGMakerSheet(96, 128)
...
-- if false then -- SpriteSheet:getFrameSize
...
-- if false then -- SpriteSheet:getGridSize
--   local sheet = lurek.sprite.newSheet(96, 128, 32, 32)
--   local cols, rows = sheet:getGridSize()
...
-- if false then -- SpriteSheet:drawToImage
--   local sheet = lurek.sprite.newRPGMakerSheet(96, 128)
...
-- The `rotated` flag tells the renderer to draw the source quad 90Â° CCW (TexturePacker rotation packing).
-- if false then -- SpriteAtlas:getEntry
...
-- if false then -- SpriteAtlas:getByIndex
...
-- if false then -- SpriteAtlas:entryCount
--   local json = tryRead("img/ui.json")
--   if json then
--     local atlas = lurek.sprite.parseAtlas(json)
...
-- if false then -- SpriteAtlas:entryNames
--   local json = tryRead("img/ui.json")
--   if json then
--     local atlas = lurek.sprite.parseAtlas(json)
--     local icons = {}
...
-- if false then -- SpriteAtlas:getFlipped
...
-- if false then -- SpriteSheet:nameGroup
...
-- ---- Stub: LSpriteAtlas:type ---------------------------------------------
--@api-stub: LSpriteAtlas:type
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LSpriteAtlas:type
...
-- if false then -- LSpriteAtlas:typeOf
--   local ok_a, sprite_atlas_obj = pcall(lurek.sprite.parseAtlas, "sprites/atlas.png")
--   if not ok_a then sprite_atlas_obj = nil end
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LSpriteSheet:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LSpriteSheet:typeOf
--   local ok_s, sprite_sheet_obj = pcall(lurek.sprite.newSheet, "sprites/sheet.png", 32, 32)
--   if not ok_s then sprite_sheet_obj = nil end

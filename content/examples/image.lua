...
-- if false then -- lurek.image.newImageData
...
-- if false then -- lurek.image.newCompressedData
...
-- if false then -- lurek.image.isCompressed
...
-- if false then -- lurek.image.newLayeredImage
...
-- if false then -- lurek.image.saveImage
...
-- if false then -- lurek.image.savePNG
...
-- if false then -- lurek.image.loadImage
...
-- if false then -- lurek.image.loadLayered
--   pcall(function()
...
-- if false then -- lurek.image.newPaletteLut
...
-- if false then -- lurek.image.newProvinceGrid
...
-- if false then -- ProvinceGrid:getWidth
--   local ok_grid, grid = pcall(lurek.image.newProvinceGrid, "assets/world_provinces.png")
--   if not ok_grid then return end
...
-- if false then -- ProvinceGrid:getHeight
--   local ok_grid, grid = pcall(lurek.image.newProvinceGrid, "assets/world_provinces.png")
--   if not ok_grid then return end
...
-- if false then -- ProvinceGrid:getAt
--   local ok_grid, grid = pcall(lurek.image.newProvinceGrid, "assets/world_provinces.png")
--   if not ok_grid then return end
...
-- if false then -- ProvinceGrid:provinceCount
--   local ok_grid, grid = pcall(lurek.image.newProvinceGrid, "assets/world_provinces.png")
--   if not ok_grid then return end
--   local count = (grid and grid:provinceCount() or 0)
--   local owners = {}
...
-- if false then -- ProvinceGrid:adjacencies
--   local ok_grid, grid = pcall(lurek.image.newProvinceGrid, "assets/world_provinces.png")
--   if not ok_grid then return end
...
-- if false then -- LayeredImage:getWidth
...
-- if false then -- LayeredImage:getHeight
--   local doc = lurek.image.newLayeredImage(256, 128)
...
-- if false then -- LayeredImage:layerCount
...
-- if false then -- LayeredImage:addLayer
...
-- if false then -- LayeredImage:removeLayer
--   local doc = lurek.image.newLayeredImage(64, 64)
...
-- if false then -- LayeredImage:getLayer
--   local doc = lurek.image.newLayeredImage(64, 64)
--   doc:addLayer("base")
...
-- if false then -- LayeredImage:getOpacity
--   local doc = lurek.image.newLayeredImage(64, 64)
--   doc:addLayer("ink")
...
-- if false then -- LayeredImage:setOpacity
--   local doc = lurek.image.newLayeredImage(64, 64)
...
-- if false then -- LayeredImage:isVisible
--   local doc = lurek.image.newLayeredImage(64, 64)
--   doc:addLayer("ink")
...
-- if false then -- LayeredImage:setVisible
--   local doc = lurek.image.newLayeredImage(64, 64)
...
-- if false then -- LayeredImage:getName
--   local doc = lurek.image.newLayeredImage(64, 64)
...
-- if false then -- LayeredImage:setName
--   local doc = lurek.image.newLayeredImage(64, 64)
...
-- if false then -- LayeredImage:swapLayers
--   local doc = lurek.image.newLayeredImage(64, 64)
--   doc:addLayer("a")
--   doc:addLayer("b")
--   doc:swapLayers(1, 2)
...
-- if false then -- LayeredImage:merge
--   local doc = lurek.image.newLayeredImage(64, 64)
--   doc:addLayer("base")
...
-- if false then -- LayeredImage:save
--   local doc = lurek.image.newLayeredImage(128, 128)
--   doc:addLayer("background")
...
-- if false then -- CompressedImageData:getWidth
--   local ok_cd, cd = pcall(lurek.image.newCompressedData, "assets/terrain_bc1.dds")
--   if not ok_cd then return end
...
-- if false then -- CompressedImageData:getHeight
--   local ok_cd, cd = pcall(lurek.image.newCompressedData, "assets/terrain_bc1.dds")
--   if not ok_cd then return end
...
-- if false then -- CompressedImageData:getDimensions
--   local ok_cd, cd = pcall(lurek.image.newCompressedData, "assets/terrain_bc1.dds")
--   if not ok_cd then return end
--   local w = cd and cd:getWidth() or 0
--   local h = cd and cd:getHeight() or 0
...
-- if false then -- CompressedImageData:getMipmapCount
--   local ok_cd, cd = pcall(lurek.image.newCompressedData, "assets/terrain_bc1.dds")
--   if not ok_cd then return end
--   local mips = (cd and cd:getMipmapCount() or 0)
--   if mips > 1 then
...
-- if false then -- CompressedImageData:getFormat
--   local ok_cd, cd = pcall(lurek.image.newCompressedData, "assets/terrain_bc1.dds")
--   if not ok_cd then return end
...
-- if false then -- mlua:getDimensions
--   local img = lurek.image.newImageData(64, 64)
...
-- if false then -- mlua:flipHorizontal
--   local img = lurek.image.newImageData(64, 64)
--   img:flipHorizontal()
...
-- if false then -- mlua:flipVertical
--   local img = lurek.image.newImageData(64, 64)
--   img:flipVertical()
...
-- if false then -- mlua:resizeNearest
--   local img = lurek.image.newImageData(64, 64)
...
-- if false then -- mlua:applyPaletteLut
--   local img = lurek.image.newImageData(64, 64)
--   local lut = lurek.image.newPaletteLut()
...
-- if false then -- PaletteLUT:getColorCount
--   local lut = lurek.image.newPaletteLut()
...
-- if false then -- PaletteLUT:clear
--   local lut = lurek.image.newPaletteLut()
--   lut:clear()
...
-- if false then -- mlua (ImageData):blit
...
-- if false then -- mlua (ImageData):convolve
--   local img = lurek.image.newImageData(32, 32)
--   img:fill(1, 1, 1, 1)
...
-- if false then -- mlua (ImageData):drawCircle
--   local img = lurek.image.newImageData(64, 64)
--   img:fill(0, 0, 0, 1)
...
-- if false then -- mlua (ImageData):drawLine
--   local img = lurek.image.newImageData(64, 64)
--   img:fill(0, 0, 0, 1)
...
-- if false then -- mlua (ImageData):drawRect
--   local img = lurek.image.newImageData(64, 64)
--   img:fill(0, 0, 0, 1)
...
-- if false then -- mlua (ImageData):getRegion
--   local img = lurek.image.newImageData(64, 64)
--   img:fill(1, 0, 0, 1)
...
-- if false then -- LayeredImage:moveLayer
...
-- if false then -- mlua (ImageData):paste
...
-- if false then -- PaletteLUT:setColor
--   local lut = lurek.image.newPaletteLut()
...
-- if false then -- LayeredImage:setLayer
...
-- if false then -- mlua (ImageData):setPixel
...
-- if false then -- mlua (ImageData):tint
--   local img = lurek.image.newImageData(32, 32)
--   img:fill(1, 1, 1, 1)
...
-- if false then -- LCompressedImageData:type
...
-- if false then -- LCompressedImageData:typeOf
--   local compressed_image_data_obj = lurek.image.newCompressedData(nil)
...
-- if false then -- LImageData:getWidth
...
-- if false then -- LImageData:getHeight
--   local img = lurek.image.newImageData(64, 32)
...
-- if false then -- LImageData:getDimensions
...
-- if false then -- LImageData:getPixel
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:setPixel
--   local img = lurek.image.newImageData(16, 16)
...
-- if false then -- LImageData:encode
--   local img = lurek.image.newImageData(16, 16)
...
-- if false then -- LImageData:getString
...
-- if false then -- LImageData:mapPixel
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:brightness
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:contrast
--   local img = lurek.image.newImageData(8, 8)
--   img:fill(128, 128, 128, 255)
...
-- if false then -- LImageData:saturation
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:gamma
--   local img = lurek.image.newImageData(8, 8)
--   img:fill(100, 100, 100, 255)
...
-- if false then -- LImageData:grayscale
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:sepia
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:invert
--   local img = lurek.image.newImageData(8, 8)
--   img:fill(100, 150, 200, 255)
--   img:invert()
--   local r, g, b = img:getPixel(0, 0)
...
-- if false then -- LImageData:threshold
--   local img = lurek.image.newImageData(8, 8)
--   img:fill(200, 200, 200, 255)
...
-- if false then -- LImageData:posterize
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:noise
--   local img = lurek.image.newImageData(16, 16)
--   img:fill(128, 128, 128, 255)
...
-- if false then -- LImageData:alphaMask
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:flipHorizontal
--   local img = lurek.image.newImageData(4, 4)
--   img:setPixel(0, 0, 255, 0, 0, 255)   -- red at top-left
--   img:flipHorizontal()
...
-- if false then -- LImageData:flipVertical
--   local img = lurek.image.newImageData(4, 4)
...
-- if false then -- LImageData:rotate90cw
...
-- if false then -- LImageData:resizeNearest
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:sharpen
--   local img = lurek.image.newImageData(16, 16)
...
-- if false then -- LImageData:drawRect
--   local img = lurek.image.newImageData(32, 32)
--   img:fill(0, 0, 0, 255)
...
-- if false then -- LImageData:drawCircle
--   local img = lurek.image.newImageData(32, 32)
--   img:fill(0, 0, 0, 255)
...
-- if false then -- LImageData:drawLine
--   local img = lurek.image.newImageData(32, 32)
--   img:fill(0, 0, 0, 255)
...
-- if false then -- LImageData:resize
--   local img = lurek.image.newImageData(64, 64)
...
-- if false then -- LImageData:getRegion
--   local img = lurek.image.newImageData(64, 64)
...
-- if false then -- LImageData:mapPixels
--   local img = lurek.image.newImageData(8, 8)
--   img:fill(255, 0, 0, 255)
...
-- if false then -- LImageData:convolve
--   local img = lurek.image.newImageData(16, 16)
--   img:fill(180, 180, 180, 255)
...
-- if false then -- LImageData:applyPaletteLut
--   local img = lurek.image.newImageData(8, 8)
...
-- if false then -- LImageData:setRawData
--   local img = lurek.image.newImageData(2, 2)
...
-- if false then -- LImageData:paste
...
-- if false then -- LLayeredImage:getWidth
...
-- if false then -- LLayeredImage:getHeight
--   local li = lurek.image.newLayeredImage(128, 64)
...
-- if false then -- LLayeredImage:layerCount
--   local li = lurek.image.newLayeredImage(64, 64)
--   li:addLayer()
--   li:addLayer()
...
-- if false then -- LLayeredImage:addLayer
--   local li = lurek.image.newLayeredImage(64, 64)
...
-- if false then -- LLayeredImage:removeLayer
--   local li = lurek.image.newLayeredImage(64, 64)
--   li:addLayer()
--   li:addLayer()
...
-- if false then -- LLayeredImage:getLayer
--   local li = lurek.image.newLayeredImage(32, 32)
--   local idx = li:addLayer()
...
-- if false then -- LLayeredImage:setLayer
--   local li = lurek.image.newLayeredImage(32, 32)
--   local idx = li:addLayer()
--   local src = lurek.image.newImageData(32, 32)
...
-- if false then -- LLayeredImage:getOpacity
--   local li = lurek.image.newLayeredImage(32, 32)
--   local idx = li:addLayer()
...
-- if false then -- LLayeredImage:setOpacity
--   local li = lurek.image.newLayeredImage(32, 32)
--   local idx = li:addLayer()
...
-- if false then -- LLayeredImage:isVisible
--   local li = lurek.image.newLayeredImage(32, 32)
--   local idx = li:addLayer()
...
-- if false then -- LLayeredImage:setVisible
--   local li = lurek.image.newLayeredImage(32, 32)
--   local idx = li:addLayer()
--   li:setVisible(idx, false)
...
-- if false then -- LLayeredImage:getName
--   local li = lurek.image.newLayeredImage(32, 32)
--   local idx = li:addLayer()
--   li:setName(idx, "body")
...
-- if false then -- LLayeredImage:setName
--   local li = lurek.image.newLayeredImage(32, 32)
--   local idx = li:addLayer()
...
-- if false then -- LLayeredImage:swapLayers
--   local li = lurek.image.newLayeredImage(32, 32)
...
-- if false then -- LLayeredImage:moveLayer
--   local li = lurek.image.newLayeredImage(32, 32)
...
-- if false then -- LLayeredImage:merge
--   local li = lurek.image.newLayeredImage(32, 32)
...
-- if false then -- LLayeredImage:save
--   local li = lurek.image.newLayeredImage(32, 32)
--   local idx = li:addLayer()
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LLayeredImage:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LLayeredImage:typeOf
--   local layered_image_obj = lurek.image.newLayeredImage(32, 32)
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LPaletteLUT:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LPaletteLUT:typeOf
--   local palette_l_u_t_obj = lurek.image.newPaletteLut()
...
-- Returns the type name of this object.
-- Useful for runtime type inspection.
-- if false then -- LProvinceGrid:type
...
-- Returns true if this object is of the given type.
-- Use for runtime type checks.
-- if false then -- LProvinceGrid:typeOf
--   local province_grid_obj = lurek.image.newProvinceGrid(nil)
...
-- =============================================================================
-- STUBS: 6 uncovered lurek.image API item(s)
-- Generated by tools/audit/example_add_missing.py
-- REQUIRED: replace every --@api-stub: block below with a real scenario.
-- Run .github/prompts/flesh-out-example.prompt.md for instructions.
-- The final committed file must contain ZERO --@api-stub: lines.
...
-- ---- Stub: lurek.image.newImageDataFromBytes -----------------------------
...
-- if false then -- raw bytes + resize filter
...
-- if false then -- lurek.image.fromScreen
...
-- if false then -- ProvinceGrid geometry helpers

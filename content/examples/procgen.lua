...
local W, H = 80, 50
local cave_map = lurek.procgen.cellularAutomata(W, H, {
    fill       = 0.45,
    iterations = 5,
    birth      = 5,
    survive    = 4,
...
local reachable = lurek.procgen.floodFill(cave_map, W, H, 40, 25, 128, false)
...
for y = 0, 63 do
    for x = 0, 63 do
        ...
        local n = 0
        n = n + 1.0 * lurek.procgen.perlinNoise(x * 0.05, y * 0.05, 64 * 0.05, 64 * 0.05)
        n = n + 0.5 * lurek.procgen.perlinNoise(x * 0.10, y * 0.10, 64 * 0.10, 64 * 0.10)
        n = n + 0.25 * lurek.procgen.perlinNoise(x * 0.20, y * 0.20, 64 * 0.20, 64 * 0.20)
...
local tree_points = lurek.procgen.poissonDisk(map_w, map_h, 64.0, 30, 42)
...
local regions, dist1, dist2 = lurek.procgen.voronoi(W, H, seeds, {
    warp_scale    = 0.1,
    warp_strength = 20.0,
    seed          = 7,
})
...
local function make_island(gw, gh, seed)
    ...
    for y = 0, gh-1 do
        for x = 0, gw-1 do
            local n = lurek.procgen.perlinNoise(x*0.04, y*0.04, gw*0.04, gh*0.04)
                    + 0.5 * lurek.procgen.perlinNoise(x*0.08, y*0.08, gw*0.08, gh*0.08)
            local h = (n / 1.5 + 1) * 0.5
            -- fade edges to ocean
            local fx = (x/gw - 0.5) * 2
            local fy = (y/gh - 0.5) * 2
    ...
    local reach = lurek.procgen.floodFill(landmask, gw, gh, gw//2, gh//2, 1, true)

    -- 4. Poisson trees on grass cells only
    local trees = lurek.procgen.poissonDisk(gw, gh, 4.0, 20, seed)
    local final_trees = {}

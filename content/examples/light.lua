...
-- Useful for budget warnings â€” getMaxLights caps how many actually render per frame.
...
-- Returns a list of normal-map lighting hints for plugin renderers.
...
-- Distinct from intensity â€” energy affects falloff strength, useful for sun-like contributors.
do -- Light:setEnergy
...
-- Inspect when debugging "why does this light look harsh?" â€” likely 'linear' on a small radius.
do -- Light:getFalloff
  local lamp = lurek.light.newLight(0, 0, 100)
...
-- Returns the penumbra softness multiplier.
...
-- A zero result means the light never casts shadows â€” promote to 0xFFFF to enable everything.
...
-- Switching to 'directional' makes a light behave like the sun â€” direction matters, position does not.
...
-- Pass min/max intensity multipliers (e.g. 0.8, 1.2 = Â±20%) and frequency in Hz; engine converts internally.
do -- Light:addFlicker
  local torch = lurek.light.newLight(0, 0, 120)
...
-- Sets the normal-map texture path hint for plugin renderers.
...
-- Returns the normal-map texture path hint, or nil.

...
- [ ] **B-04** — No business logic in closures: '<closure@175>' (20 LOC, line 175) — extract body to src/event/ | '<closure@205>' has if/match/for — extract to src/event/ | '<closure@258>' has if/match/for — extract to src/event/
...
|-------|---------|---------|
| **S-01** lib.rs registration | ✅ PASS | Registered in lib.rs + lua_api (event_api) |
...
| **B-04** No business logic in closures | ⚠️ WARNING | '<closure@175>' (20 LOC, line 175) — extract body to src/event/ \| '<closure@205>' has if/match/for — extract to src/event/ \| '<closure@258>' has if/match/for — extract to src/event/ |
...
| Check | Verdict | Details |
|-------|---------|---------|
| **T-01** Rust test file | ✅ PASS | Found: tests\rust\unit\event_tests.rs |
| **T-02** Lua test file | ✅ PASS | tests/lua/unit/test_event.lua registered in harness |
| **T-03** Test naming | ✅ PASS | Test names follow convention |
| **T-04** Float comparisons | ✅ PASS | No float assert_eq! found |
| **T-05** Test adequacy | ✅ PASS | 19 tests / 18 pub methods (106%) |
| **T-06** Golden tests | 🔵 MANUAL | Check if module qualifies for golden/snapshot tests |
| **T-07** Tests pass | 🔵 MANUAL | Run: cargo test --test event_tests -- --nocapture |
...
| **Q-04** Error handling | ⚠️ WARNING | .unwrap() calls: event_queue:245, event_queue:246, event_queue:275 |
...
```powershell
python tools/audit/audit_module.py event --docs-quality
```

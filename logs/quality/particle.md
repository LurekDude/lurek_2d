...
- [ ] **B-02** — Registration-only: extra pub fn (move to src/particle/): from_lua_opts

### 🟡 Warnings — Should Fix

- [ ] **B-04** — No business logic in closures: '<closure@1097>' has if/match/for — extract to src/particle/ | '<closure@1146>' has if/match/for — extract to src/particle/
...
|-------|---------|---------|
| **S-01** lib.rs registration | ✅ PASS | Registered in lib.rs + lua_api (particle_api) |
...
| **B-02** Registration-only | ❌ ERROR | extra pub fn (move to src/particle/): from_lua_opts |
| **B-03** impl LuaUserData placement | ✅ PASS | All impl LuaUserData blocks are in lua_api (correct) |
| **B-04** No business logic in closures | ⚠️ WARNING | '<closure@1097>' has if/match/for — extract to src/particle/ \| '<closure@1146>' has if/match/for — extract to src/particle/ |
...
| **T-05** Test adequacy | ✅ PASS | 84 tests / 40 pub methods (210%) |
...
```powershell
python tools/audit/audit_module.py particle --docs-quality
```

...
- [ ] **B-04** — No business logic in closures: '<closure@287>' (25 LOC, line 287) — extract body to src/input/ | '<closure@417>' has if/match/for — extract to src/input/ | '<closure@670>' has if/match/for — extract to src/input/
...
|-------|---------|---------|
| **S-01** lib.rs registration | ✅ PASS | Registered in lib.rs + lua_api (input_api) |
...
| **B-04** No business logic in closures | ⚠️ WARNING | '<closure@287>' (25 LOC, line 287) — extract body to src/input/ \| '<closure@417>' has if/match/for — extract to src/input/ \| '<closure@670>' has if/match/for — extract to src/input/ |
...
```powershell
python tools/audit/audit_module.py input --docs-quality
```

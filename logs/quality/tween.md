...
- [ ] **B-04** — No business logic in closures: '<closure@132>' (20 LOC, line 132) — extract body to src/tween/ | '<closure@182>' has if/match/for — extract to src/tween/ | '<closure@199>' has if/match/for — extract to src/tween/
...
|-------|---------|---------|
| **S-01** lib.rs registration | ✅ PASS | Registered in lib.rs + lua_api (tween_api) |
...
| **B-04** No business logic in closures | ⚠️ WARNING | '<closure@132>' (20 LOC, line 132) — extract body to src/tween/ \| '<closure@182>' has if/match/for — extract to src/tween/ \| '<closure@199>' has if/match/for — extract to src/tween/ |
...
| Check | Verdict | Details |
|-------|---------|---------|
| **T-01** Rust test file | ✅ PASS | Found: tests\rust\unit\tween_tests.rs |
...
```powershell
python tools/audit/audit_module.py tween --docs-quality
```

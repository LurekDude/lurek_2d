...
> **Status**: 🔴 FAIL  |  **Date**: 2026-04-12  |  **Score**: 38 ✅ / 1 ⚠️ / 2 ❌ / 19 🔵
...
- [ ] **B-02** — Registration-only: extra pub fn (move to src/procgen/): from_lua_table
- [ ] **R-02** — Dependency direction: cellular: Foundations imports runtime(Core Runtime); render: Foundations imports image(Platform Services); render: Foundations imports render(Platform Services); voronoi: Foundations imports runtime(Core Runtime)

### 🟡 Warnings — Should Fix

- [ ] **B-04** — No business logic in closures: '<closure@126>' (21 LOC, line 126) — extract body to src/procgen/ | '<closure@30>' has if/match/for — extract to src/procgen/ | '<closure@102>' has if/match/for — extract to src/procgen/
...
|-------|---------|---------|
| **S-01** lib.rs registration | ✅ PASS | Registered in lib.rs + lua_api (procgen_api) |
...
| **B-02** Registration-only | ❌ ERROR | extra pub fn (move to src/procgen/): from_lua_table |
| **B-03** impl LuaUserData placement | ✅ PASS | All impl LuaUserData blocks are in lua_api (correct) |
| **B-04** No business logic in closures | ⚠️ WARNING | '<closure@126>' (21 LOC, line 126) — extract body to src/procgen/ \| '<closure@30>' has if/match/for — extract to src/procgen/ \| '<closure@102>' has if/match/for — extract to src/procgen/ |
...
| **R-02** Dependency direction | ❌ ERROR | cellular: Foundations imports runtime(Core Runtime); render: Foundations imports image(Platform Services); render: Foundations imports render(Platform Services); voronoi: Foundations imports runtime(Core Runtime) |
...
| Check | Verdict | Details |
|-------|---------|---------|
| **T-01** Rust test file | ✅ PASS | Found: tests\rust\unit\procgen_tests.rs |
...
| **T-05** Test adequacy | ✅ PASS | 12 tests / 3 pub methods (400%) |
...
| Check | Verdict | Details |
|-------|---------|---------|
| **W-01** Example file exists | ✅ PASS | content/examples/procgen.lua present |
| **W-02** API surface coverage | ✅ PASS | All 5 bound functions in example |
| **W-03** Example comments | 🔵 MANUAL | Verify content/examples/procgen.lua has realistic one-line comments per call |
| **W-04** Example–spec sync | ✅ PASS | All 5 functions consistent across spec and example |
| **W-05** Wiki page | ✅ PASS | docs\wiki\Procgen-API.md |
...
```powershell
python tools/audit/audit_module.py procgen --docs-quality
```

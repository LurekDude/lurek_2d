...
distance-field goal maps â€” the runtime queries that turn `lurek.tilemap`
into a real roguelike. Each subsystem is independent.

## Public API

### `Fov`
- `M.newFov(opts?)` â€” `{algorithm="shadowcast", range=8, light_walls=true}`
...
- **`lurek.tilemap`** â€” `:attachTilemap` consumes the user's tilemap. The
  binding is duck-typed: any object with `:getTile(layer, x, y)` works, as
  do plain `tilemap[layer][y][x]` Lua tables.
- **`lurek.pathfind`** â€” `GoalMap:bake` prefers `lurek.pathfind.dijkstra`
  when it exists; otherwise falls back to an in-Lua BFS.
- **`lurek.math.bresenham`** â€” preferred backend for `M.bresenham`; same
  fallback strategy.

## Status

`full` â€” recursive shadowcasting FOV, energy scheduler, BFS-fallback Dijkstra
...
See `library/roguelike/example.lua` for a small map with a player,
two monsters, and a goal-map-driven hunt loop.

## Notes

- **FOV is symmetric**: `Fov:isVisible(a,b)` â†” `Fov:isVisible(b,a)` for the
  same origin (within recursive-shadowcasting precision).
- **Scheduler is action-cost based**, not Î”t: actors accumulate energy at
...
- **GoalMap fallback** uses 4-neighbour BFS â€” fine for square maps up to ~120Â˛

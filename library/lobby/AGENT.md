# `lobby` â€” Agent Reference (Lunasome)
...
(MessagePack â€” the canonical ENet payload format). For human-readable
...
```
M.new(host?, channel?) â†’ Lobby
  â”śâ”€â”€ _host         (lurek.network host or nil)
  â”śâ”€â”€ _channel      (ENet channel, default 0)
  â”śâ”€â”€ _rooms        ({ name = Room })
  â”śâ”€â”€ _peer_rooms   ({ peer_id = room_name } reverse map)
  â”śâ”€â”€ _my_room      (current room name on the local peer)
  â”śâ”€â”€ _my_name      (display name; setPlayerName(name))
  â”śâ”€â”€ _on_event     (single legacy callback)
  â””â”€â”€ _event_bus    (optional lurek.patterns.newEventBus())

Lobby methods:
  setPlayerName(name)
  onEvent(fn)              | getEventBus() â†’ EventBus|nil
  createRoom(name, opts?)  â†’ ok, err?    (server-side)
  removeRoom(name)
  joinRoom(name, peer?, player_name?, password?) â†’ ok, err?
  leaveRoom(peer?) â†’ ok, err?
  setReady(ready, peer?)
  isAllReady() â†’ boolean   (â‰Ą2 players, all ready)
  listRooms() â†’ { {name, players, maxPlayers, hasPassword}, ... }
  getPlayers(name?) â†’ array of {peer_id, name, ready}
  getCurrentRoom() â†’ string|nil
  getHost(name?) â†’ peer_id|nil
  getRoomCount() â†’ number
  poll() â†’ array of processed events  (call once per frame)
...
| File                     | Purpose                                                   |
| ------------------------ | --------------------------------------------------------- |
| `library/lobby/init.lua` | Full implementation â€” Room internal type + Lobby manager. |
...
- Host election is **deterministic** by join order â€” the earliest-joined
  remaining player becomes the new host on disconnect.
- `isAllReady()` requires â‰Ą2 players to return true (single-player rooms can't
  start).
- Empty rooms are auto-removed.
- `joinRoom` accepts an optional `password`; rooms without a password ignore
  the parameter.
- Wire-format layer (`lurek.network.pack`) is binary MessagePack â€” do not mix
  with `lurek.serial.toJson` payloads on the same channel.

## Lua API Reference

See LDoc-generated page: `docs/library/lib_lobby.md` (regenerated by

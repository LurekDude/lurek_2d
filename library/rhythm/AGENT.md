...
beat/bar/swing/judgement abstractions on top of audio sources and Î”t timers.

## Public API

### Beat clock
- `M.newClock(bpm, opts?)` â€” `{subdivision=4, swing=0, latency_ms=0}`
- `M.fromAudio(source, bpm, opts?)` â€” anchor to an audio source's playhead
- `clock:setBpm(bpm) / rampBpm(target, seconds) / getBpm()`
- `clock:setSwing(amount)`
- `clock:start() / stop() / isRunning()`
- `clock:update(dt)`
- `clock:syncToAudio(source)` â€” re-anchor after a Source seek
...
- **`lurek.audio` Source** â€” when constructed via `M.fromAudio`, the clock
...
- **`lurek.timer.getMicroTime`** â€” default `M.judge` hit time. Falls back to
  `os.clock()` when missing.
- **`lurek.event`** â€” emits `rhythm.beat`, `rhythm.bar`, and `rhythm.miss`
  events when present; entirely optional.

## Status

`full` â€” clock, BPM ramp, swing knob, audio sync, every/at/pattern sequencer,
and judge scoring all implemented and tested.

## Examples

See `library/rhythm/example.lua` for an audio-driven beat clock with
`every`, `pattern`, and `judge` exercised on a small input timing demo.

## Notes

- `clock:at(beat, fn)` raises if the requested beat is in the past â€” use
  `clock:every` for periodic firing.
- `clock:pattern("x..x", fn)` interprets the string as one full bar â€” string

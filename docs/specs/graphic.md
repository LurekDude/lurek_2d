...
- **`GpuRenderer`** (struct): Processes `RenderCommand` queues via wgpu. Manages all GPU-side resources (textures, canvases, font atlases, shader pipelines), vertex buffers, depth/stencil targets, and the per-frame render pass.
...
- **`RenderCommand`** (enum): Central deferred draw operation enum with 45+ variants covering shapes, images, text, transforms, state changes, stencil, canvas, mesh, nine-slice, compound shape, particle system, and post-fx.
...
- **`ShapeCommand`** (enum): Restricted subset of `RenderCommand` for object-space primitives — `SetColor`, `SetLineWidth`, `Rectangle`, `RoundedRectangle`, `Circle`, `Ellipse`, `Triangle`, `Polygon`, `Line`, `Polyline`, `Arc`.
- **`CompoundShape`** (struct): Accumulates `ShapeCommand` entries in local object space. Replayed via `RenderCommand::DrawShape` with a per-call affine transform.
...
Accumulates `ShapeCommand` entries in local object space and replays them as a unified draw call via `RenderCommand::DrawShape` with a per-call affine transform.
...
GPU-accelerated renderer that processes `RenderCommand` queues via wgpu. Manages all GPU-side resources, vertex/index buffers, the depth/stencil target, pipeline cache, and the per-frame render pass. Entry point: `render_frame()`.
...
#### `graphics::renderer::RenderCommand`
...
Restricted subset of `RenderCommand` for object-space primitives stored inside a `CompoundShape` — `SetColor`, `SetLineWidth`, `Rectangle`, `RoundedRectangle`, `Circle`, `Ellipse`, `Triangle`, `Polygon`, `Line`, `Polyline`, `Arc`.
...
Exposed under `lurek.graphic.*` by `src/lua_api/graphic_api.rs` (2,407 lines). The API provides 66 namespace functions and 7 UserData types.
...
| `image`     | Related       | `image` provides CPU pixel buffers (`ImageData`); `graphic` uploads them to GPU via `Texture::from_rgba` |
...
| `lua_api`   | Imported by   | `src/lua_api/graphic_api.rs` registers all `lurek.graphic.*` bindings |

**Similar modules and differentiation:**
- `image` vs `graphic`: `image` owns CPU-side pixel buffers (`ImageData`) for manipulation; `graphic` owns GPU-side textures and rendering.
- `camera` vs `graphic`: `camera` was extracted from `graphic`; it provides `Camera2D`, `Viewport`, and `ViewportScale` types that feed the view matrix into `GpuRenderer`.
...
- **Adding new `RenderCommand` variants**: Both `renderer.rs` (enum definition) and `gpu_renderer/render_pass.rs` (execution arm) must be updated together.
- **Orphaned `color.rs`**: `src/graphic/color.rs` defines a `Color` struct but is not declared in `mod.rs`. The active `Color` type lives in `src/math/color.rs`.
...
- **Transform stack**: Implemented as `RenderCommand` variants (`PushTransform`, `PopTransform`, `Translate`, `Rotate`, `Scale`, `Shear`, `Origin`, `ApplyTransform`). The renderer maintains a `Mat3` stack internally.

...
*21 libraries, 1409 functions, 0 module fields.*
...
- [`library.rhythm`](#libraryrhythm) — 26 fn
- [`library.roguelike`](#libraryroguelike) — 32 fn
...
*38 functions, 0 module fields documented.*

See: [`lurek.tween`](../api/lurek.md#lurektween) — backbone for `track:tween` clips, [`lurek.camera`](../api/lurek.md#lurekcamera) — consumed by `track:cameraTo` / `track:shake`, [`lurek.audio`](../api/lurek.md#lurekaudio) — consumed by `track:audio` (one-shot fire), [`lurek.event`](../api/lurek.md#lurekevent) — consumed by `track:signal`, [`lurek.filesystem`](../api/lurek.md#lurekfilesystem) — timeline TOML loader (`fromToml`), [`lurek.serial`](../api/lurek.md#lurekserial) — JSON serialisation for `tl:export`, [`lurek.save`](../api/lurek.md#lureksave) — collector wiring for export/restore
...
- `delta` *number* — Signed seconds to advance (negative = rewind).
...
- `label` *string* — Label name registered with `Timeline:label`.
...
- `fn` *function* — callback `fn(timeline)`

**Returns**

- *table* — handle

#### `onTrackEnter(name, fn)`
...
- *table* — `{t, scale, duration, playing, finished}`
...
See: [`lurek.physics`](../api/lurek.md#lurekphysics), [`lurek.math`](../api/lurek.md#lurekmath)
...
See: [`lurek.patterns.newEventBus`](../api/lurek.md#lurekpatternsneweventbus) — optional event bus mirror returned by `seq:getEventBus()`, [`lurek.event.newSignal`](../api/lurek.md#lurekeventnewsignal) — alternative scoped pub/sub backbone, [`lurek.i18n.t`](../api/lurek.md#lureki18nt) — translate `say`/`choice` text fields before passing them in, [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson) — serialise/deserialise script node arrays for persistence
...
*65 functions, 0 module fields documented.*

See: [`lurek.render`](../api/lurek.md#lurekrender) — caller-side renderer that consumes `getDrawList()` entries, [`lurek.image`](../api/lurek.md#lurekimage) — image/texture loader for `Part:setTexture()`, [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson) — serialise template + part state for persistence
...
- `tex` *any* — texture handle from `lurek.image`
...
- `ox` *number* — horizontal origin
- `oy` *number* — vertical origin
...
- `fx` *boolean* — flip horizontally
- `fy` *boolean* — flip vertically
...
#### `setFollowsRotation(f)`

Set whether this part inherits the socket's rotation.

**Parameters**

- `f` *boolean* — true to inherit socket rotation
...
- `socketName` *string* — socket to remove
...
- `socketName` *string* — socket to look up

**Returns**

- *table|nil* — socket definition copy `{name, acceptType, x, y, rotation, drawOrder}`
...
- *table* — array of socket name strings
...
#### `getPartAt(socketName)`

Return the Part attached at `socketName`, or nil.

**Parameters**

- `socketName` *string* — socket to query
...
- `part` *Part* — part to find

**Returns**

- *string|nil* — socket name
...
See: [`lurek.render`](../api/lurek.md#lurekrender)
...
See: [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson) — serialise inventory snapshots for save round-trip, [`lurek.serial.fromJson`](../api/lurek.md#lurekserialfromjson) — restore inventory snapshots, [`lurek.save.SaveManager`](../api/lurek.md#lureksavesavemanager) — register inventory state via a SaveManager collector, [`lurek.patterns.newEventBus`](../api/lurek.md#lurekpatternsneweventbus) — optional change-event bus from `inv:getEventBus()`, [`lurek.data.deepCopy`](../api/lurek.md#lurekdatadeepcopy) — P4 lift candidate — `item:clone()` will delegate when available
...
See: [`lurek.data.deepCopy`](../api/lurek.md#lurekdatadeepcopy)
...
See: [`lurek.math`](../api/lurek.md#lurekmath), [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson)
...
See: [`lurek.network`](../api/lurek.md#lureknetwork), [`lurek.patterns.newEventBus`](../api/lurek.md#lurekpatternsneweventbus), [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson)
...
See: [`lurek.math.newRandomGenerator`](../api/lurek.md#lurekmathnewrandomgenerator) — default RNG source for sampling, [`lurek.serial.fromToml`](../api/lurek.md#lurekserialfromtoml) — designer-authored loot tables (`fromToml`), [`lurek.filesystem.read`](../api/lurek.md#lurekfilesystemread) — sandboxed file load for `fromToml`, [`lurek.save`](../api/lurek.md#lureksave) — `Pity:save`/`restore` collector wiring
...
See: [`lurek.filesystem.read`](../api/lurek.md#lurekfilesystemread) — load `.ink` files, [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson) — precompile / save state serialisation, [`lurek.save`](../api/lurek.md#lureksave) — wire `story:save`/`resume` into a SaveManager, [`lurek.i18n.t`](../api/lurek.md#lureki18nt) — used by `M.localiseStory` for {loc:key} markers, [`lurek.event`](../api/lurek.md#lurekevent) — optional trace event sink
...
See: [`lurek.network`](../api/lurek.md#lureknetwork), [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson), [`lurek.timer.Scheduler`](../api/lurek.md#lurektimerscheduler)
...
See: [`lurek.data.hash`](../api/lurek.md#lurekdatahash)
...
See: [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson)
...
See: [`lurek.timer.getTime`](../api/lurek.md#lurektimergettime)
...
See: [`lurek.patterns.newEventBus`](../api/lurek.md#lurekpatternsneweventbus), [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson), [`lurek.serial.fromJson`](../api/lurek.md#lurekserialfromjson), [`lurek.save.SaveManager`](../api/lurek.md#lureksavesavemanager), [`lurek.timer.Scheduler`](../api/lurek.md#lurektimerscheduler)
...
See: [`lurek.serial.fromJson`](../api/lurek.md#lurekserialfromjson)
...
See: [`lurek.audio`](../api/lurek.md#lurekaudio) — Source playhead drives `M.fromAudio` and `:syncToAudio`, [`lurek.timer`](../api/lurek.md#lurektimer) — `getMicroTime` powers default `M.judge` hit time, [`lurek.event`](../api/lurek.md#lurekevent) — optional emit on `rhythm.bar`/`rhythm.beat`/`rhythm.miss`, [`lurek.save`](../api/lurek.md#lureksave) — `clock:dump` collector wiring
...
- `bpm` *number* — New BPM value (must be > 0).

**Returns**

- *Clock* — self

#### `rampBpm(target, seconds)`

Smoothly ramp BPM from the current value to `target` over `seconds`.

**Parameters**

- `target` *number* — Target BPM (must be > 0).
- `seconds` *number* — Ramp duration in seconds (must be > 0).
...
- `amount` *number* — Swing factor (clamped by caller convention).
...
- `dt` *number* — Elapsed time in seconds since the last frame.
...
- `source` *table* — lurek.audio Source (must support `:getPosition()`).
...
- `division` *integer?* — Step size in subdivisions (defaults to clock subdivision).

**Returns**

- *number* — phase in [0, 1)

#### `beatTimeRemaining(division)`

Seconds until the next step boundary at the given subdivision.

**Parameters**

- `division` *integer?* — Beat subdivision (defaults to clock subdivision).

**Returns**

- *number* — seconds remaining until the next step

#### `isOnBeat(division, tolerance)`
...
- `division` *integer?* — Beat subdivision (defaults to clock subdivision).
- `tolerance` *number?* — Phase tolerance (default 0.05).

**Returns**

- *boolean*

#### `nearestBeat(division)`
...
- *number* — nearest beat index
- *number* — signed error in seconds (negative = early)
...
- `division` *integer* — Step frequency in subdivisions.
- `fn` *function* — Callback `fn(step_index)`.

**Returns**

- *table* — schedule handle
...
- `beat` *number* — Target beat position (must be in the future).
- `fn` *function* — Callback `fn(beat)`.
...
- `str` *string* — Pattern string (`'x'` = trigger, others = rest).
- `fn` *function* — Callback `fn(step_index)`.
...
- `handle` *table* — Handle returned by `:every`, `:at`, or `:pattern`.

**Returns**

- *boolean* — true if the handle was found and removed
...
- *table* — `{bpm, beat, bar, phase, running}`

#### `setJudgementWindows(w)`
...
- `w` *table* — `{perfect=number, great=number, good=number}`.
...
- *table* — `{perfect, great, good}` in seconds
...
See: [`lurek.tilemap`](../api/lurek.md#lurektilemap) — attach FOV/GoalMap blockers via `:attachTilemap`, [`lurek.pathfind`](../api/lurek.md#lurekpathfind) — preferred Dijkstra backend; in-Lua fallback used otherwise, [`lurek.math.bresenham`](../api/lurek.md#lurekmathbresenham) — line-of-sight helper (re-exported as `M.bresenham`), [`lurek.save`](../api/lurek.md#lureksave) — scheduler/FOV state collectors
...
- `fn` *function* — callback receiving integer `x, y`
...
- *table* — array of `{x=integer, y=integer}`
...
- *table* — `{range, origin, visible, explored}`
...
- `actor` *any* — opaque actor reference
- `speed` *number* — Energy gain per tick (must be > 0).
...
- `actor` *any* — actor reference previously added with `:add`
...
- `speed` *number* — New speed value (must be > 0).
...
- *table* — `{clock, actors}` where `actors` contains per-actor speed/energy
...
- `blob` *table* — Blob produced by `:save()`.
...
- `fn` *function|nil* — returns true for impassable cells (nil = allow all)
...
- `tilemap` *table|userdata* — tilemap with `:getTile(layer, x, y)` or table indexing
- `layer` *integer?* — layer index (default 1)
- `blocker_ids` *table?* — array of tile IDs to treat as walls (default {1})

**Returns**

- *GoalMap* — self

#### `setSources(positions)`

Replace the current source list with the given positions. Each entry may be `{x, y}`, `{x, y, weight}`, or `{x=, y=, weight=}`.

**Parameters**

- `positions` *table* — array of source position entries
...
- `x` *integer* — source x coordinate
- `y` *integer* — source y coordinate
- `w` *number?* — initial distance weight (default 0)
...
See: [`lurek.network`](../api/lurek.md#lureknetwork), [`lurek.serial.toJson`](../api/lurek.md#lurekserialtojson), [`lurek.serial.fromJson`](../api/lurek.md#lurekserialfromjson)
...
See: [`lurek.timer.Scheduler`](../api/lurek.md#lurektimerscheduler)
...
See: [`lurek.timer.getDelta`](../api/lurek.md#lurektimergetdelta)

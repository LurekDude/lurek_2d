...
- **P4+P6 â€” Architecture restructure.** New `docs/architecture/README.md` navigational index. `engine-architecture.md` â†” `render-command-architecture.md` boundary cleaned (T-01..T-08 single-homed in `philosophy.md`; Module Internal File Structure single-homed in `engine-architecture.md`); TOCs added to `vscode-architecture.md` and `cag-system.md`. `philosophy.md` adds proposed binding constraint **A-05** (core binary â‰¤ 15 MB stripped, plugins add on top) and fixes naming drift (`core` â†’ `runtime`, `scripting` â†’ `lua_api`).

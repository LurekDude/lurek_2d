...
│   ├── codeLens.ts       CodeLens — function ref counts + lurek callback labels
│   ├── color.ts          Color picker provider for lurek.renders.setColor calls
...
| **Definition**      | `providers/definition.ts`      | Go-to-definition for lurek.* names opens a virtual `lurek-api://` document; for local Lua symbols delegates to `SymbolIndex`. Implements `TextDocumentContentProvider` for the `lurek-api` URI scheme.                                                                                                                                                                                                                                                       |
...
| **Color**           | `providers/color.ts`           | Inline colour swatch for `lurek.renders.setColor(r, g, b)` calls.                                                                                                                                                                                                                                                                                                                                                                                         |
...
| **Type Inference**  | `providers/typeInference.ts`   | Type inference engine for user Lua variables. Tracks 25+ factory return types (`Image`, `Canvas`, `Font`, `Shader`, `Entity`, `Timer`, `Tween`, `World`, `Body`, `ParticleSystem`, etc.), OOP class instances via `setmetatable`, module aliases (`local gfx = lurek.renders`), and variable re-assignments. Provides dot-access completion (fields + methods), colon-access completion (methods), and hover information showing type and factory origin. |
...
| **System Monitor**        | `providers/systemMonitor.ts` | Rolling 120-sample history of CPU %, RAM, lurek process CPU + RAM, GPU %, VRAM, disk I/O rates, and network rates. Collects via PowerShell on Windows, `ps`/`vmstat` on Unix.                    |
...
The extension reads from the `lurek` workspace configuration namespace:

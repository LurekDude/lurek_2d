...
The engine-wide module file-structure standard (required files, optional
files, the thin-`mod.rs` rule, Lua binding separation, anti-patterns) is
defined in
[engine-architecture.md § Module Internal File Structure Standard](engine-architecture.md#module-internal-file-structure-standard).

Render-module specifics — where `RenderCommand`, `GpuRenderer`, `Shader`,
and the GPU-coupled files live — are covered immediately below in
§ [Render Module Refactoring Plan](#render-module-refactoring-plan).

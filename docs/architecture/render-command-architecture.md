...
**Step 1: Color → src/math/**

| # | Action |
|---|---|
| 1 | Copy `src/render/color.rs` to `src/math/color.rs` |
| 2 | Add `pub mod color;` and `pub use color::Color;` to `src/math/mod.rs` |
| 3 | Remove `color.rs` from `src/render/` and its re-export from `src/render/mod.rs` |
| 4 | Global search-replace: `crate::render::Color` → `crate::math::Color` and `use crate::render::color` → `use crate::math::color` |
| 5 | `cargo check` — fix any remaining imports |

**Step 2: Texture + TextureAtlas → src/image/**

| # | Action |
|---|---|
| 1 | Copy `src/render/texture.rs` and `texture_atlas.rs` to `src/image/` |
| 2 | Add `pub mod texture; pub mod texture_atlas;` to `src/image/mod.rs` |
| 3 | Remove both from `src/render/` |
| 4 | Global search-replace import paths |
| 5 | `gpu_renderer.rs` imports `TextureData` from `crate::image::texture` |

**Step 3: Sprite types → src/sprite/ (new)**

| # | Action |
|---|---|
| 1 | Create `src/sprite/` directory with standard `mod.rs` |
| 2 | Move 4 files from `src/render/` to `src/sprite/` |
| 3 | Add `pub mod sprite;` to `src/lib.rs` |
| 4 | Global search-replace import paths |
| 5 | Create `src/sprite/AGENT.md` |
| 6 | Create `docs/specs/sprite.md` |
| 7 | Create or update `src/lua_api/sprite_api.rs` if sprite-specific Lua APIs exist |

**Order matters**: Do Color first (most broadly used), then Texture (fewer
dependents), then Sprite (new module). Each step should pass `cargo check`
before proceeding to the next.

### Why NOT Extract (Reasoning for KEEP Decisions)

| File | Why it stays in render |
|---|---|
| `renderer.rs` | `RenderCommand` is the render contract. Domain modules depend on it to describe what they want drawn. It belongs where the consumer lives. |
| `font.rs` | Glyph atlas lifecycle is managed by `gpu_renderer.rs`. No module outside rendering reads glyph data. Creating `src/font/` would be a weak module (Rule 14). |
| `mesh.rs` | `MeshVertex` format matches the GPU vertex buffer layout. Changing the vertex format requires changing the shader, so they co-evolve. |
| `canvas.rs` / `decal_surface.rs` | Thin descriptor metadata (width, height) for GPU-managed render targets. Extracting ≤20-line files creates noise, not clarity. |
| `draw_layer.rs` | Z-ordering of `RenderCommand` items is a rendering concern. No domain module uses draw layers independently. |
| `shape.rs` | Vector shapes feed directly into `gpu_renderer` tessellation. The tessellation code and the shape types co-evolve. |
| `image_effect.rs` | `ShaderPassDescriptor` configures specific render passes. It is consumed exclusively by `gpu_renderer`. |
...
| Component | Status |
|---|---|
| `RenderCommand` enum (46+ variants) | ✅ `src/render/renderer.rs` |
| `GpuRenderer::render_frame()` | ✅ `src/render/gpu_renderer.rs` |
| App collects Lua-pushed commands via `SharedState.render_commands` | ✅ |
| Viewport transform wrapping (`PushTransform/Scale/PopTransform`) | ✅ |
| `TextureKey`, `FontKey`, `CanvasKey`, `ShaderKey`, `MeshKey` handle system | ✅ |
| Separate `src/lua_api/` binding module, one `*_api.rs` per domain | ✅ |
| Debug overlay: `build_render_commands()` pattern | ✅ `src/app/debug_overlay.rs` |
| Error screen: `build_render_commands()` pattern | ✅ `src/app/error_screen.rs` |
| Raycaster DDA math, `RaycasterScene`, `RayHit` types | ✅ `src/raycaster/dda.rs` + `scene.rs` (planned) |
| Raycaster `draw_to_image()` testing functions (model-level) | ✅ `src/raycaster/dda.rs` (should move to `draw.rs`) |
| Particle `build_render_commands()` → `DrawParticleSystem` | ✅ `src/particle/emitter.rs` |
| `Light2D` descriptor with shadow config, occluders, falloff | ✅ `src/render/light/light2d.rs` |
| `PostFxEffect` with 16+ built-in types + custom shader | ✅ `src/render/effect/` |
| `WidgetBase` with `x, y, width, height, flex_grow, flex_shrink` | ✅ `src/ui/widget.rs` |
| `Minimap` data model (terrain, fog, objects, pings, markers) | ✅ `src/minimap/` |

### What is missing ❌

| Gap | Description |
|---|---|
| `WidgetBase::computed_rect` | ❌ No computed layout rect on widgets. |
| `GuiContext::run_layout_pass()` | ❌ Flexbox resolver not implemented. |
| `GuiContext::generate_render_commands()` | ❌ UI does not self-render. |
| `TileMap::generate_render_commands()` | ❌ Tiles drawn manually from Lua. |
| `Parallax::generate_render_commands()` | ❌ Parallax drawn manually from Lua. |
| GPU raycaster column renderer | ❌ No instanced column rendering path in `src/render/`. |
| GPU light pass in `render_frame()` | ❌ Light data exists but renderer does not consume it in a dedicated pass. |
| App auto-collect loop | ❌ App does not call `generate_render_commands()` on domain modules. |
| `render_frame()` light/postfx args | ❌ Signature does not accept light/occluder/postfx data separately. |
| Raycaster `draw.rs` file split | ❌ Debug draw functions are in `dda.rs` instead of separate file. |
| Color extracted to src/math/ | ❌ `Color` still lives in `src/render/color.rs`. |
| Sprite types extracted to src/sprite/ | ❌ `Sprite`, `SpriteSheet`, `SpriteBatch`, `NineSlice` still in `src/render/`. |
| Texture/TextureAtlas extracted to src/image/ | ❌ `Texture`, `TextureAtlas` still in `src/render/`. |
| Module file structure standardised | ❌ Modules follow inconsistent internal patterns. |

### Why the gap exists

Lua scripts currently call `lurek.graphics.drawRect()` etc. per frame, pushing
commands synchronously. This works but means domain modules do not self-render.
The gap is architectural incompleteness, not a bug. Filling it module-by-module
does not break existing Lua scripts — both paths coexist.

---

## Implementation Checklist

Priority order — phases are independent unless noted.

**Phase 0 — Render Module Refactoring (prerequisite for clean imports)**
- [ ] Extract `Color` from `src/render/` to `src/math/color.rs`
- [ ] Extract `Texture`, `TextureAtlas` from `src/render/` to `src/image/`
- [ ] Create `src/sprite/` module; move `Sprite`, `SpriteSheet`, `SpriteBatch`, `NineSlice`
- [ ] Update all import paths, verify with `cargo check`
- [ ] Create `src/sprite/AGENT.md` and `docs/specs/sprite.md`

**Phase 1 — UI Layout**
- [ ] Add `computed_rect: Rect` and `is_visible: bool` to `WidgetBase`
- [ ] Implement `GuiContext::run_layout_pass()` — flexbox for Layout containers
- [ ] Implement `GuiContext::emit_commands()` — walk widgets, emit draw primitives
- [ ] Wire `GuiContext::generate_render_commands()` as the public entry point
- [ ] Add `widget:getRect()` to Lua API — returns computed rect

**Phase 2 — Tile and Parallax auto-render**
- [ ] `TileMap::generate_render_commands()` — frustum-cull + `DrawQuad` per tile
- [ ] `Parallax::generate_render_commands()` — `DrawImageEx` per layer
- [ ] Wire both in App auto-collect loop

**Phase 3 — Raycaster GPU path**
- [ ] Move `draw_*_to_image()` from `dda.rs` to `draw.rs` (separate file)
- [ ] Define raycaster instance buffer format in `src/render/`
- [ ] Implement GPU column renderer (instanced vertical strips)
- [ ] Wire raycaster output → `render_frame()` via App

**Phase 4 — Lighting GPU pass**
- [ ] Expand `render_frame()` signature to accept `&[Light2D]`, `&[Occluder]`
- [ ] Implement shadow map generation pass in `src/render/light/`
- [ ] Implement light accumulation + compositing pass
- [ ] Wire App to pass light data to renderer each frame

**Phase 5 — App wiring**
- [ ] After `render_ui` callback, auto-collect from all domain modules:
  ```rust
  commands.extend(state.borrow().ui_ctx.generate_render_commands());
  commands.extend(state.borrow().particle_sys.generate_render_commands());
  commands.extend(state.borrow().tilemap.generate_render_commands());
  commands.extend(state.borrow().parallax.generate_render_commands());
  ```
- [ ] Pass light/occluder/postfx data as separate args to `render_frame()`
- [ ] Ensure existing Lua-pushed commands still work alongside auto-collect

**Phase 6 — Module File Structure Standardisation**
- [ ] Audit every `src/<module>/mod.rs` — enforce ≤30 lines, declarations only
- [ ] Ensure every module has `AGENT.md`
- [ ] Ensure every module has matching `docs/specs/<module>.md`
- [ ] Run `python tools/docs/collect_docs.py --report-missing` — zero missing docstrings
- [ ] Verify no `impl LuaUserData` in any `src/<module>/` — all must be in `lua_api/`
- [ ] Verify no `use wgpu::` in any domain module (only in `src/render/`)

**Reference implementation**: `src/particle/emitter.rs::build_render_commands()`
and `src/app/debug_overlay.rs::build_render_commands()`. These already follow
the correct pattern.

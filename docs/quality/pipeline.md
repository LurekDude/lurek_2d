...
- [ ] **B-04** — No business logic in closures: '<closure@1054>' (60 LOC, line 1054) — extract body to src/pipeline/ | '<closure@1026>' has if/match/for — extract to src/pipeline/
...
|-------|---------|---------|
| **S-01** lib.rs registration | ✅ PASS | Registered in lib.rs + lua_api (pipeline_api) |
...
| **B-04** No business logic in closures | ⚠️ WARNING | '<closure@1054>' (60 LOC, line 1054) — extract body to src/pipeline/ \| '<closure@1026>' has if/match/for — extract to src/pipeline/ |
...
| **W-05** Wiki page | ✅ PASS | docs\wiki\Pipeline-API.md |
...
```powershell
python tools/audit/audit_module.py pipeline --docs-quality
```

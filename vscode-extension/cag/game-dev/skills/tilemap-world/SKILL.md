...
local function load_map(path)
    ...
    map.tileset = luna.graphics.newImage("tileset.png")
end
```
...
local function get_tile_quad(tile_id, tileset_w)
    ...
    return luna.graphics.newQuad(col * ts, row * ts, ts, ts, tileset_w, tileset_w)
end
```
    ...
    for row = start_row, end_row do
        for col = start_col, end_col do
            ...
            if tile_id and tile_id > 0 then
                ...
                luna.graphics.draw(map.tileset, quad, col * ts, row * ts)
            end
        end
    end
end

...
local function save_game(slot)
    ...
    luna.filesystem.write(path, toml_str)
end
```
...
local function load_game(slot)
    local path = "saves/slot" .. slot .. ".toml"
    if not luna.filesystem.exists(path) then return nil, "no save" end
...
- **Forgetting luna.filesystem sandbox** — saves go to the game's sandboxed directory, not an absolute path.

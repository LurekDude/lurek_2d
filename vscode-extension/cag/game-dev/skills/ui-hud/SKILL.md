...
local function draw_health_bar(x, y, w, h, hp, max_hp)
    ...
    luna.graphics.setColor(r, g, 0, 1)
...
local function draw_floaters()
    for _, f in ipairs(floaters) do
        ...
        luna.graphics.setColor(1, 0.2, 0.2, alpha)
        luna.graphics.print(f.text, f.x, f.y)
    end
    luna.graphics.setColor(1, 1, 1, 1)
end
```
...
local function draw_cooldown(x, y, size, remaining, total)
    ...
    luna.graphics.rectangle("fill", x, y, size, size * pct)
    luna.graphics.setColor(1, 1, 1, 1)
...
local function draw_tooltip(mx, my, text)
    ...
    luna.graphics.rectangle("fill", mx + 12, my, tw, th)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print(text, mx + 20, my + 4)
end
```
...
local function draw_inventory_grid(inv, ox, oy)
    for i = 1, inv.size do
        ...
        luna.graphics.setColor(0.3, 0.3, 0.3, 0.9)
        luna.graphics.rectangle("fill", sx, sy, SLOT_SIZE, SLOT_SIZE)
        if inv.slots[i] then
            luna.graphics.setColor(1, 1, 1, 1)
            luna.graphics.print(inv.slots[i].id, sx + 2, sy + 2)
            luna.graphics.print(inv.slots[i].count, sx + 20, sy + 20)
        end
        luna.graphics.setColor(0.6, 0.6, 0.6, 1)
        luna.graphics.rectangle("line", sx, sy, SLOT_SIZE, SLOT_SIZE)
    end
    luna.graphics.setColor(1, 1, 1, 1)
end
```
...
- **Color leak** — always reset `luna.graphics.setColor(1,1,1,1)` after drawing colored HUD elements.

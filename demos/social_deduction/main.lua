...
local function draw_task_phase()
    ...
    for _, t in ipairs(tasks) do
        if not t.done then
            ...
            if visible then
                luna.graphics.setColor(1, 1, 0.3, 0.8)
                luna.graphics.rectangle("fill", t.x - 10, t.y - 10, 20, 20)
                luna.graphics.setColor(0, 0, 0, 1)
                luna.graphics.print("!", t.x - 3, t.y - 8, 1)
            end
        else
            luna.graphics.setColor(0.2, 0.5, 0.2, 0.5)
            luna.graphics.rectangle("line", t.x - 10, t.y - 10, 20, 20)
        end
    end
    ...
    for i = 1, 6 do
        ...
        if p.alive then
            ...
            if visible then
                ...
                luna.graphics.circle("fill", p.x, p.y, 14)
                luna.graphics.setColor(1, 1, 1, 0.9)
                luna.graphics.print(NAMES[i], p.x - 10, p.y - 28, 0.7)
            end
        end
    end

    -- vision circle
    luna.graphics.setColor(1, 1, 1, 0.08)
    luna.graphics.circle("line", me.x, me.y, VISION_RADIUS)

    -- HUD
    luna.graphics.setColor(0.2, 0.2, 0.3, 0.8)
    luna.graphics.rectangle("fill", 0, 0, W, 36)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Tasks: " .. progress .. "/" .. TASK_COUNT, 10, 8, 1)
    -- progress bar
    luna.graphics.setColor(0.3, 0.3, 0.3, 1)
    luna.graphics.rectangle("fill", 200, 10, 200, 16)
    luna.graphics.setColor(0.2, 0.9, 0.3, 1)
    luna.graphics.rectangle("fill", 200, 10, 200 * (progress / TASK_COUNT), 16)

    luna.graphics.setColor(1, 1, 1, 1)
    local role = (player_id == traitor_id) and "TRAITOR" or "CREW"
    luna.graphics.print("Role: " .. role, 500, 8, 1)
    luna.graphics.print("M=Meeting  WASD=Move", 10, H - 22, 0.8)
    if player_id == traitor_id then
        luna.graphics.setColor(1, 0.3, 0.3, 1)
        luna.graphics.print("S=Sabotage", 680, 8, 1)
    end
end

local function draw_meeting()
    luna.graphics.setColor(0.15, 0.15, 0.25, 1)
    luna.graphics.rectangle("fill", 50, 50, W - 100, H - 100)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("EMERGENCY MEETING", W / 2 - 90, 70, 1.5)
    luna.graphics.print("Time: " .. math.floor(meeting_timer), W / 2 - 20, 100, 1)
    luna.graphics.print("Click a name to vote", W / 2 - 65, 120, 0.9)
    ...
    for i = 1, 6 do
        ...
        if p.alive then
            luna.graphics.setColor(c[1], c[2], c[3], 1)
            luna.graphics.circle("fill", 120, y + 10, 12)
            luna.graphics.setColor(1, 1, 1, 1)
            luna.graphics.print(NAMES[i], 145, y, 1.2)
            if meeting_votes[player_id] == i then
                luna.graphics.setColor(1, 1, 0, 1)
                luna.graphics.print(" <-- YOUR VOTE", 260, y, 1)
            end
        else
            luna.graphics.setColor(0.4, 0.4, 0.4, 1)
            luna.graphics.print(NAMES[i] .. " (eliminated)", 145, y, 1)
        end
        y = y + 45
    end
end
...
function luna.draw()
    if game_over then
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(game_result, W / 2 - 140, H / 2 - 20, 1.3)
        luna.graphics.print("Press R to restart", W / 2 - 60, H / 2 + 20, 1)
        return
    end
    if phase == "task" then
        draw_task_phase()
    ...
    if message and msg_timer > 0 then
        luna.graphics.setColor(1, 1, 0.5, clamp(msg_timer, 0, 1))
        luna.graphics.print(message, W / 2 - 120, H / 2, 1)
    end
end

...
function luna.draw()
    luna.graphics.setBackgroundColor(0.12, 0.14, 0.1)
    ...
    for y = 1, MAP_H do
        for x = 1, MAP_W do
            ...
            local t = terrain[y][x]
            if t == 0 then
                luna.graphics.setColor(0.25, 0.3, 0.2, 1)
            elseif t == 1 then
                luna.graphics.setColor(0.15, 0.35, 0.15, 1)
            elseif t == 2 then
                luna.graphics.setColor(0.35, 0.3, 0.2, 1)
            end
            luna.graphics.rectangle("fill", px, py, GRID - 1, GRID - 1)
            if t == 1 then
                luna.graphics.setColor(0.1, 0.5, 0.1, 0.5)
                luna.graphics.print("F", px + 14, py + 12)
            elseif t == 2 then
                luna.graphics.setColor(0.6, 0.5, 0.3, 0.5)
                luna.graphics.print("H", px + 14, py + 12)
            end
        end
    end

    -- Move range highlight
    luna.graphics.setColor(0.3, 0.6, 1, 0.3)
    for _, m in ipairs(moveRange) do
        luna.graphics.rectangle("fill", OX + (m[1]-1)*GRID, OY + (m[2]-1)*GRID, GRID-1, GRID-1)
    end

    -- Attack target highlight
    for _, t in ipairs(attackTargets) do
        luna.graphics.setColor(1, 0.3, 0.3, 0.4)
        luna.graphics.rectangle("fill", OX + (t.gx-1)*GRID, OY + (t.gy-1)*GRID, GRID-1, GRID-1)
    end
    ...
    for i, u in ipairs(units) do
        if u.hp > 0 then
            ...
            -- Team color
            if u.team == "blue" then
                luna.graphics.setColor(0.2, 0.4, 0.9, 1)
            else
                luna.graphics.setColor(0.9, 0.25, 0.2, 1)
            end
            luna.graphics.circle("fill", px + GRID/2, py + GRID/2, GRID/2 - 4)

            -- Selected indicator
            if i == selected then
                luna.graphics.setColor(1, 1, 0, 1)
                luna.graphics.setLineWidth(2)
                luna.graphics.circle("line", px + GRID/2, py + GRID/2, GRID/2 - 2)
                luna.graphics.setLineWidth(1)
            end

            -- Icon
            luna.graphics.setColor(1, 1, 1, 1)
            luna.graphics.print(u.icon, px + GRID/2 - 4, py + GRID/2 - 6)

            -- HP bar
            luna.graphics.setColor(0.2, 0.2, 0.2, 1)
            luna.graphics.rectangle("fill", px + 4, py + GRID - 8, GRID - 8, 4)
            luna.graphics.setColor(0.2, 0.9, 0.2, 1)
            luna.graphics.rectangle("fill", px + 4, py + GRID - 8, (GRID - 8) * (u.hp / u.maxHp), 4)
        end
    end

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.85)
    luna.graphics.rectangle("fill", 0, 0, 800, 46)

    local turnColor = turn == "blue" and {0.4, 0.6, 1} or {1, 0.4, 0.3}
    luna.graphics.setColor(turnColor[1], turnColor[2], turnColor[3], 1)
    luna.graphics.print(turn:upper() .. " TURN", 10, 5, 1.2)

    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("CP: " .. cp .. "/" .. MAX_CP, 150, 8)
    luna.graphics.print("Blue: " .. morale("blue") .. " units", 280, 8)
    luna.graphics.print("Red: " .. morale("red") .. " units", 420, 8)
    luna.graphics.print("[Space] End Turn  [Esc] Deselect", 10, 28)
    luna.graphics.print("LClick=Move  RClick=Attack", 400, 28)

    -- Combat log
    luna.graphics.setColor(0, 0, 0, 0.7)
    luna.graphics.rectangle("fill", 0, 530, 800, 70)
    luna.graphics.setColor(0.8, 0.8, 0.6, 1)
    for i, log in ipairs(combatLog) do
        luna.graphics.print(log, 10, 530 + (i - 1) * 13)
    end

    -- Game over
    if gameOver then
        luna.graphics.setColor(0, 0, 0, 0.75)
        luna.graphics.rectangle("fill", 200, 220, 400, 100)
        luna.graphics.setColor(1, 1, 0.3, 1)
        luna.graphics.print(winner .. " WINS!", 330, 240, 1.8)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("Press R to restart", 330, 290)
    end
end

...
function luna.load()
    luna.graphics.setBackgroundColor(0.2, 0.55, 0.15)
    reset()
end
...
function luna.draw()
    -- Pitch
    luna.graphics.setColor(0.16, 0.52, 0.12)
    luna.graphics.rectangle("fill", PX, PY, PW, PH)
    -- Striped grass
    for i = 0, 9 do
        if i % 2 == 0 then
            luna.graphics.setColor(0.2, 0.56, 0.15, 0.4)
            luna.graphics.rectangle("fill", PX + i * (PW/10), PY, PW/10, PH)
        end
    end
    -- Lines
    luna.graphics.setColor(1, 1, 1, 0.8)
    luna.graphics.rectangle("line", PX, PY, PW, PH)
    luna.graphics.line(PX + PW/2, PY, PX + PW/2, PY + PH)
    luna.graphics.circle("line", PX + PW/2, PY + PH/2, 50)
    -- Goals
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.rectangle("line", PX - GOAL_W, PY + PH/2 - GOAL_H/2, GOAL_W, GOAL_H)
    luna.graphics.rectangle("line", PX + PW, PY + PH/2 - GOAL_H/2, GOAL_W, GOAL_H)
    -- Penalty areas
    luna.graphics.rectangle("line", PX, PY + PH/2 - 90, 70, 180)
    luna.graphics.rectangle("line", PX + PW - 70, PY + PH/2 - 90, 70, 180)

    -- CPU team (red)
    for i, cp in ipairs(cpu) do
        luna.graphics.setColor(0.85, 0.15, 0.15)
        luna.graphics.circle("fill", cp.x, cp.y, 9)
        luna.graphics.setColor(0, 0, 0)
        luna.graphics.print(tostring(i), cp.x - 4, cp.y - 6, 1.1)
    end

    -- Player team (blue)
    for i, p in ipairs(team) do
        local sel = i == controlled
        luna.graphics.setColor(sel and 0.9 or 0.2, sel and 0.9 or 0.4, sel and 0.2 or 0.9)
        luna.graphics.circle("fill", p.x, p.y, 9)
        if sel then
            luna.graphics.setColor(1, 1, 0)
            luna.graphics.circle("line", p.x, p.y, 12)
        end
        luna.graphics.setColor(0, 0, 0)
        luna.graphics.print(tostring(i), p.x - 4, p.y - 6, 1.1)
    end

    -- Ball
    local bs = 0.6 + 0.4 * math.sin(anim * 10)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.circle("fill", ball.x, ball.y, 7)
    luna.graphics.setColor(0, 0, 0)
    luna.graphics.circle("line", ball.x, ball.y, 7)
    -- Spinning pattern
    luna.graphics.setColor(0.2, 0.2, 0.2)
    luna.graphics.rectangle("fill", ball.x - 2, ball.y - 2, 4, 4)

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.7)
    luna.graphics.rectangle("fill", 0, 0, W, 42)
    luna.graphics.setColor(0.4, 0.6, 1)
    luna.graphics.print("YOU", 20, 5, 2)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print(scores.player .. " — " .. scores.cpu, W/2 - 30, 5, 2.5)
    luna.graphics.setColor(1, 0.4, 0.4)
    luna.graphics.print("CPU", W - 70, 5, 2)
    local mins = math.floor(time_left / 60)
    local secs = math.floor(time_left % 60)
    luna.graphics.setColor(1, 0.9, 0.3)
    luna.graphics.print(string.format("%d:%02d", mins, secs), W/2 - 20, H - 25, 1.8)

    luna.graphics.setColor(0.6, 0.8, 0.6, 0.6)
    luna.graphics.print("[WASD/Arrows] Move  [Space] Kick  [Tab] Switch player", 10, H - 20, 1.3)

    -- Kickoff flash
    if kickoff_timer > 0 then
        luna.graphics.setColor(1, 1, 0.3, kickoff_timer)
        luna.graphics.print("GOAL!", W/2 - 35, H/2 - 15, 4)
    end
    ...
    if game_state == "gameover" then
        ...
        luna.graphics.setColor(col[1], col[2], col[3])
        luna.graphics.print(msg, W/2 - #msg * 14, H/2 - 25, 4)
        luna.graphics.setColor(1, 1, 1)
        luna.graphics.print(scores.player .. " — " .. scores.cpu, W/2 - 30, H/2 + 20, 3)
        luna.graphics.setColor(0.6, 0.6, 0.6)
        luna.graphics.print("Press R to restart", W/2 - 100, H/2 + 65, 2)
    end
end

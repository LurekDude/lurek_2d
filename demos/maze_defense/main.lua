...
function luna.draw()
    luna.graphics.setBackgroundColor(0.08, 0.1, 0.08)
    ...
    for y = 1, GRID_H do
        for x = 1, GRID_W do
            ...
            if grid[y][x] == 0 then
                luna.graphics.setColor(0.15, 0.18, 0.15, 1)
            elseif grid[y][x] == 1 then
                luna.graphics.setColor(0.4, 0.35, 0.25, 1)
            elseif grid[y][x] == 2 then
                luna.graphics.setColor(0.2, 0.3, 0.6, 1)
            end
            luna.graphics.rectangle("fill", px, py, CELL - 1, CELL - 1)
        end
    end

    -- Spawn and exit markers
    luna.graphics.setColor(0, 1, 0, 1)
    luna.graphics.rectangle("fill", OX + (SPAWN[1]-1)*CELL, OY + (SPAWN[2]-1)*CELL, CELL-1, CELL-1)
    luna.graphics.setColor(1, 0, 0, 1)
    luna.graphics.rectangle("fill", OX + (EXIT[1]-1)*CELL, OY + (EXIT[2]-1)*CELL, CELL-1, CELL-1)

    -- Path visualization
    luna.graphics.setColor(0.3, 0.8, 0.3, 0.3)
    for _, p in ipairs(path) do
        if grid[p[2]][p[1]] == 0 then
            luna.graphics.rectangle("fill", OX + (p[1]-1)*CELL + 8, OY + (p[2]-1)*CELL + 8, CELL - 17, CELL - 17)
        end
    end

    -- Tower range indicators
    luna.graphics.setColor(0.3, 0.4, 0.8, 0.15)
    for y = 1, GRID_H do
        for x = 1, GRID_W do
            if grid[y][x] == 2 then
                local tx = (x - 0.5) * CELL + OX
                local ty = (y - 0.5) * CELL + OY
                luna.graphics.circle("fill", tx, ty, CELL * 3)
            end
        end
    end
    ...
    for _, e in ipairs(enemies) do
        ...
        luna.graphics.rectangle("fill", e.x - 8, e.y - 14, 16, 3)
        luna.graphics.setColor(1, 0.2, 0.2, 1)
        luna.graphics.rectangle("fill", e.x - 8, e.y - 14, 16 * (e.hp / e.maxHp), 3)
        -- Body
        luna.graphics.setColor(0.9, 0.3, 0.3, 1)
        luna.graphics.circle("fill", e.x, e.y, 6)
    end
    ...
    luna.graphics.rectangle("fill", 0, 0, 800, 18)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Wave: " .. wave .. "  Gold: " .. gold .. "  Lives: " .. lives .. "  Score: " .. score, 10, 2)
    ...
    luna.graphics.print(modeText .. modeText2 .. "  RClick=Remove", 400, 2)

    if gameOver then
        luna.graphics.setColor(0, 0, 0, 0.7)
        luna.graphics.rectangle("fill", 250, 250, 300, 80)
        luna.graphics.setColor(1, 0.3, 0.3, 1)
        luna.graphics.print("GAME OVER", 330, 265, 1.5)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("Score: " .. score .. "  Wave: " .. wave, 320, 300)
        luna.graphics.print("Press R to restart", 320, 318)
    end
end

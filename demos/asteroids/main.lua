...
function luna.load()
    luna.graphics.setBackgroundColor(0, 0, 0.02)
...
local function draw_ship(sx, sy, angle)
    ...
    luna.graphics.line(x2, y2, x3, y3)
    luna.graphics.line(x3, y3, x1, y1)
end
...
function luna.draw()
    ...
    for i = 1, 90 do
        ...
        luna.graphics.setColor(alpha, alpha, alpha)
    ...
    luna.graphics.print("ASTEROIDS", W/2 - 55, 5, 2)
    ...
    luna.graphics.setColor(0.5, 0.6, 0.8)
    luna.graphics.print("Wave " .. wave, W - 70, H - 20, 1.5)

    -- Particles
    for _, p in ipairs(particles) do
        local t = p.life / p.max_life
        luna.graphics.setColor(1, 0.6 * t + 0.2, 0.1, t)
        luna.graphics.circle("fill", p.x, p.y, p.size * t)
    end

    -- Asteroids
    luna.graphics.setColor(0.8, 0.75, 0.65)
    for _, a in ipairs(asteroids) do
    ...
    if game_state == "playing" and (invincible <= 0 or math.floor(invincible * 8) % 2 == 0) then
        luna.graphics.setColor(0.4, 0.9, 1.0)
        ...
        if ship.thrusting then
            ...
            luna.graphics.setColor(1, 0.5, 0.1, 0.8)
            luna.graphics.circle("fill", fx, fy, 5 + math.random() * 4)
        end
    end

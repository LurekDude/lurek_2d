...
local function draw_waveform()
    ...
    luna.graphics.rectangle("fill", wx, wy, ww, wh)

    local time = luna.timer.getTime()
    for t = 1, 3 do
        if not track_muted[t] then
            local tc = track_colors[t]
            luna.graphics.setColor(tc[1], tc[2], tc[3], 0.6)
...
function luna.draw()
    ...
    luna.graphics.print("Music Composer", 10, 5, 1.2)
    luna.graphics.setColor(0.7, 0.7, 0.8, 1)
    luna.graphics.print("Track: " .. track_names[current_track], 200, 8, 0.85)
    luna.graphics.print("BPM: " .. bpm, 370, 8, 0.85)
    luna.graphics.print(playing and "PLAYING" or "STOPPED", 460, 8, 0.85)
    luna.graphics.print(looping and "[LOOP]" or "", 560, 8, 0.85)

    -- Controls
    luna.graphics.setColor(0.5, 0.5, 0.55, 1)
    luna.graphics.print("Space=play/stop | 1/2/3=track | Up/Down=BPM | L=loop | C=clear | M=mute | E=export | Esc=quit", 10, 30, 0.55)
    luna.graphics.print("Left-click=toggle note | Right-click=long note (2-4 beats)", 10, 42, 0.55)
    ...
    for r = 1, GRID_ROWS do
        ...
        if sharp then
            luna.graphics.setColor(0.6, 0.5, 0.7, 1)
        else
            luna.graphics.setColor(0.7, 0.7, 0.75, 1)
        end
        luna.graphics.print(label, OX - 40, OY + (r - 1) * CELL_H + 1, 0.55)
    end
    ...
    for c = 1, GRID_COLS do
        ...
        luna.graphics.print(tostring(c), OX + (c - 1) * CELL_W + 4, OY - 14, 0.5)
    end
    ...
    for r = 1, GRID_ROWS do
        for c = 1, GRID_COLS do
            ...
            if sharp then
                luna.graphics.setColor(0.12, 0.12, 0.16, 1)
            else
                luna.graphics.setColor(0.14, 0.14, 0.18, 1)
            end
            luna.graphics.rectangle("fill", gx, gy, CELL_W - 1, CELL_H - 1)

            -- Measure lines
            if is_measure_line then
                luna.graphics.setColor(0.3, 0.3, 0.35, 0.6)
                luna.graphics.line(gx, gy, gx, gy + CELL_H)
            end
            ...
            for t = 1, 3 do
                if tracks[t][r][c] then
                    ...
                    luna.graphics.setColor(tc[1], tc[2], tc[3], alpha)
                    luna.graphics.rectangle("fill", gx + 1, gy + 1, CELL_W - 3, CELL_H - 3)
                end
            end
        end
    end
    ...
    if playing then
        ...
        luna.graphics.line(cx, OY, cx, OY + GRID_ROWS * CELL_H)
        luna.graphics.setLineWidth(1)
    ...
    luna.graphics.setColor(0.8, 0.75, 0.9, 1)
    luna.graphics.print("Tracks:", px, OY, 0.85)
    for t = 1, 3 do
        local ty = OY + 22 + (t - 1) * 30
        local tc = track_colors[t]
        if t == current_track then
            luna.graphics.setColor(tc[1], tc[2], tc[3], 1)
            luna.graphics.rectangle("fill", px, ty, 10, 14)
            luna.graphics.setColor(1, 1, 1, 1)
        else
            luna.graphics.setColor(tc[1], tc[2], tc[3], 0.5)
            luna.graphics.rectangle("fill", px, ty, 10, 14)
            luna.graphics.setColor(0.6, 0.6, 0.6, 1)
        end
        local mute_txt = track_muted[t] and " [M]" or ""
        luna.graphics.print(t .. " " .. track_names[t] .. mute_txt, px + 14, ty, 0.7)
    end

    -- Time signature and info
    luna.graphics.setColor(0.6, 0.6, 0.7, 1)
    luna.graphics.print("4/4 time", px, OY + 120, 0.7)
    luna.graphics.print(GRID_COLS .. " beats", px, OY + 138, 0.7)
    luna.graphics.print(math.floor(GRID_COLS / 4) .. " measures", px, OY + 156, 0.7)
    ...
    if show_export then
        ...
        luna.graphics.rectangle("fill", 40, 40, 700, 400)
        luna.graphics.setColor(0.4, 0.4, 0.6, 1)
        luna.graphics.rectangle("line", 40, 40, 700, 400)
        luna.graphics.setColor(1, 0.9, 0.5, 1)
        luna.graphics.print("Note Export (press E or Esc to close)", 60, 50, 1)
        luna.graphics.setColor(0.85, 0.85, 0.9, 1)
        -- Print export text line by line
        local line_y = 80
        for line in export_text:gmatch("[^\n]+") do
            luna.graphics.print(line, 60, line_y, 0.65)
            line_y = line_y + 16

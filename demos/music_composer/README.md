...
- `luna.graphics.rectangle()` — piano-roll grid cells with per-track color and sharp-key shading
- `luna.graphics.setColor()` — active/inactive note state and muted-track greying
- `luna.graphics.print()` — note labels (C3, D#4 etc.), beat numbers, BPM counter, and track names
- `luna.graphics.line()` — playback cursor scrolling across the grid

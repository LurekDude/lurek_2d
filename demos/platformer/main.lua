...
function luna.draw()
    luna.graphics.push()
    luna.graphics.translate(-camera_x, 0)

    -- Draw sky gradient (simple horizontal bands)
    for i = 0, 5 do
        local shade = 0.05 + i * 0.02
        luna.graphics.setColor(shade * 0.5, shade * 0.7, shade * 1.5)
        luna.graphics.rectangle("fill", camera_x, i * 100, 800, 100)
    end

    -- Draw platforms
    for _, p in ipairs(platforms) do
        luna.graphics.setColor(p.color[1], p.color[2], p.color[3])
        luna.graphics.rectangle("fill", p.x, p.y, p.w, p.h)
        -- Platform top highlight
        luna.graphics.setColor(p.color[1] + 0.1, p.color[2] + 0.1, p.color[3] + 0.1)
        luna.graphics.rectangle("fill", p.x, p.y, p.w, 3)
    end
    ...
    for _, coin in ipairs(coins) do
        if not coin.collected then
            ...
            luna.graphics.circle("fill", coin.x - 2, coin.y + bob_y - 2, 3)
        end
    end
    ...
    for _, p in ipairs(particles) do
        ...
        luna.graphics.setColor(p.r, p.g, p.b, alpha)
        luna.graphics.rectangle("fill", p.x - p.size / 2, p.y - p.size / 2, p.size, p.size)
    end

    -- Draw player
    if player.alive then
        -- Body
        luna.graphics.setColor(0.3, 0.6, 1.0)
        ...
        luna.graphics.rectangle("fill", eye_x, player.y + 8, 6, 6)
        luna.graphics.setColor(0, 0, 0)
        luna.graphics.rectangle("fill", eye_x + (player.facing > 0 and 2 or 0), player.y + 10, 3, 3)
    end
    ...
    if not player.alive then
        ...
        luna.graphics.print("GAME OVER - Press R to restart", 200, 280, 2.5)
    end

    luna.graphics.setColor(0.4, 0.4, 0.4)
    luna.graphics.print("Arrows/WASD + SPACE to jump", 10, 575, 1.5)
end

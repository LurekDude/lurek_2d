...
function luna.load()
    luna.graphics.setBackgroundColor(0, 0, 0.05)
    reset()
end
...
function luna.draw()
    ...
    for i = 1, 100 do
        ...
        luna.graphics.setColor(blink, blink, blink, 0.5)
        luna.graphics.circle("fill", math.random(W), math.random(H), 1)
    end
    math.randomseed(os.time())

    -- HUD
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("GALAGA", W/2 - 35, 5, 2)
    luna.graphics.setColor(0.8, 0.9, 1)
    luna.graphics.print("Score: " .. score, 8, 8, 1.5)
    luna.graphics.setColor(1, 0.4, 0.4)
    luna.graphics.print("Lives: " .. lives, W - 100, 8, 1.5)
    luna.graphics.setColor(0.6, 0.6, 0.8)
    luna.graphics.print("Wave " .. wave, W - 80, H - 20, 1.5)

    -- Player ship
    luna.graphics.setColor(0.4, 0.8, 1.0)
    -- Fuselage
    luna.graphics.rectangle("fill", player.x + 10, player.y, 12, player.h)
    -- Wings
    luna.graphics.rectangle("fill", player.x, player.y + 10, player.w, 10)
    -- Cockpit
    luna.graphics.setColor(0.8, 1.0, 1.0)
    luna.graphics.circle("fill", player.x + player.w/2, player.y + 6, 5)

    -- Enemy bullets
    luna.graphics.setColor(1, 0.5, 0.1)
    for _, eb in ipairs(enemy_bullets) do
        luna.graphics.rectangle("fill", eb.x, eb.y, eb.w, eb.h)
    end
    ...
    for _, e in ipairs(enemies) do
        if e.alive then
            ...
            if e.row == 1 then
                luna.graphics.setColor(0.9 * pulsate, 0.2, 0.8 * pulsate)
            elseif e.row == 2 then
                luna.graphics.setColor(0.1, 0.8 * pulsate, 0.9 * pulsate)
            else
                luna.graphics.setColor(0.8 * pulsate, 0.8 * pulsate, 0.1)
            end
            -- Wing/body shape
            luna.graphics.rectangle("fill", e.x + 4, e.y + 6, ENEMY_W - 8, ENEMY_H - 12)
            luna.graphics.rectangle("fill", e.x, e.y + 14, ENEMY_W, 10)
            -- Antennae
            luna.graphics.line(e.x + ENEMY_W/2 - 6, e.y + 6, e.x + ENEMY_W/2 - 12, e.y)
            luna.graphics.line(e.x + ENEMY_W/2 + 6, e.y + 6, e.x + ENEMY_W/2 + 12, e.y)
        end
    end

    -- Player bullets
    luna.graphics.setColor(0.9, 1, 0.5)
    for _, b in ipairs(bullets) do
        luna.graphics.rectangle("fill", b.x, b.y, 4, 14)
    end
    ...
    if game_state == "gameover" then
        ...
        luna.graphics.print("GAME OVER", W/2 - 80, H/2 - 30, 3)
        luna.graphics.setColor(1, 1, 1)
        luna.graphics.print("Score: " .. score, W/2 - 50, H/2 + 10, 2)

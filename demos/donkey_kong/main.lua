...
function luna.draw()
    -- Platforms
    luna.graphics.setColor(0.5, 0.35, 0.2)
    for _, p in ipairs(PLATFORMS) do
        ...
        for i = 0, steps - 1 do
            ...
            luna.graphics.rectangle("fill", lx, ly, rw + 1, FLOOR_H)
        end
    end

    -- Ladders
    luna.graphics.setColor(0.8, 0.65, 0.2)
    for _, lad in ipairs(LADDERS) do
        luna.graphics.rectangle("fill", lad.x - 3, lad.y_top, 6, lad.y_bot - lad.y_top)
        -- Rungs
        local num_rungs = math.floor((lad.y_bot - lad.y_top) / 20)
        for r = 0, num_rungs do
            local ry = lad.y_top + r * 20
            luna.graphics.rectangle("fill", lad.x - 12, ry, 24, 4)
        end
    end

    -- Pauline (goal) at top
    luna.graphics.setColor(1, 0.6, 0.6)
    luna.graphics.rectangle("fill", W - 200, H - 440 - 40, 20, 40)
    luna.graphics.circle("fill", W - 190, H - 440 - 48, 12)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("PAULINE!", W - 230, H - 440 - 60, 1.4)

    -- Donkey Kong at top
    luna.graphics.setColor(0.45, 0.25, 0.1)
    luna.graphics.rectangle("fill", W - 300, H - 440 - 50, 50, 50)
    luna.graphics.circle("fill", W - 275, H - 440 - 60, 18)
    -- DK arms
    luna.graphics.rectangle("fill", W - 330, H - 440 - 40, 30, 14)
    luna.graphics.rectangle("fill", W - 250, H - 440 - 40, 30, 14)

    -- Barrels
    for _, b in ipairs(barrels) do
        local roll = math.sin(b.roll_anim) * 5
        luna.graphics.setColor(0.6, 0.35, 0.1)
        luna.graphics.circle("fill", b.x + b.w/2, b.y + b.h/2, b.w/2)
        luna.graphics.setColor(0.8, 0.55, 0.2)
        luna.graphics.line(
    ...
    luna.graphics.rectangle("fill", player.x, player.y + player.h * 0.4, player.w, player.h * 0.6)
    luna.graphics.setColor(0.9, 0.7, 0.4)
    luna.graphics.circle("fill", player.x + player.w/2, player.y + 12, 12)
    -- Hat
    luna.graphics.setColor(0.6, 0.15, 0.1)
    luna.graphics.rectangle("fill", player.x + 2, player.y, player.w - 4, 10)
    luna.graphics.rectangle("fill", player.x - 2, player.y + 6, player.w + 4, 5)

    -- HUD
    luna.graphics.setColor(1, 1, 0)
    luna.graphics.print("DONKEY KONG", 10, 8, 2)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("Score: " .. score, 10, 30, 1.5)
    luna.graphics.setColor(1, 0.3, 0.3)
    luna.graphics.print("Lives: " .. lives, W/2 - 50, 8, 1.5)
    luna.graphics.setColor(0.6, 0.8, 1)
    luna.graphics.print("Level " .. level, W - 90, 8, 1.5)

    -- Win overlay
    if game_state == "win" then
        luna.graphics.setColor(0, 0, 0, 0.5)
        luna.graphics.rectangle("fill", 0, 0, W, H)
        luna.graphics.setColor(1, 1, 0, math.abs(math.sin(win_flash * 4)))
        luna.graphics.print("YOU SAVED PAULINE!", W/2 - 140, H/2 - 20, 3)
    end
    -- Game over overlay

...
function luna.load()
    luna.graphics.setBackgroundColor(0.4, 0.6, 0.9)
    build_terrain()
...
function luna.draw()
    ...
    luna.graphics.setColor(0.45, 0.65, 0.95)
    ...
    for x = 1, COLS do
        for y = 1, ROWS do
            if terrain[x] and terrain[x][y] then
                local wx, wy = t2w(x, y)
                local ratio = y / ROWS
                luna.graphics.setColor(0.35 - ratio*0.1, 0.55 - ratio*0.1, 0.15)
                luna.graphics.rectangle("fill", wx, wy, TILE, TILE)
                -- Top edge highlight
                if not (terrain[x] and terrain[x][y-1]) then
                    luna.graphics.setColor(0.5, 0.8, 0.25)
                    luna.graphics.rectangle("fill", wx, wy, TILE, 3)
                end
            end
        end
    end
    ...
    -- Exit door
    luna.graphics.setColor(0.9, 0.7, 0.1)
    luna.graphics.rectangle("fill", EXIT_X, EXIT_Y - 20, TILE * 2, TILE * 2 + 20)
    luna.graphics.setColor(0, 0, 0)
    luna.graphics.print("EXIT", EXIT_X + 3, EXIT_Y - 16, 1.1)

    -- Spawn hatch
    luna.graphics.setColor(0.8, 0.5, 0.2)
    luna.graphics.rectangle("fill", SPAWN_X - 5, SPAWN_Y - 14, TILE * 2 + 10, TILE)
    local bounce = 0.5 + 0.5 * math.sin(anim * 8)
    luna.graphics.setColor(1, 0.8, 0.1)
    luna.graphics.rectangle("fill", SPAWN_X + 2, SPAWN_Y - 12 - bounce * 3, TILE + 6, 6)

    -- Lemmings
    for _, lem in ipairs(lemmings) do
        if lem.alive then
            local jc = JOB_COLORS[lem.job] or JOB_COLORS.none
            luna.graphics.setColor(jc[1], jc[2], jc[3])
            luna.graphics.rectangle("fill", lem.x, lem.y, LSIZE, LSIZE)
            -- Head
            luna.graphics.setColor(0.9, 0.75, 0.6)
            luna.graphics.circle("fill", lem.x + LSIZE/2, lem.y - 3, 5)
            -- Blue hat
            luna.graphics.setColor(0.2, 0.2, 0.9)
            luna.graphics.rectangle("fill", lem.x + 1, lem.y - 8, LSIZE - 2, 6)
            -- Walking legs
            local leg = math.floor(anim * 8) % 2 == 0
            if lem.on_ground and lem.job == "none" then
                luna.graphics.setColor(jc[1] * 0.7, jc[2] * 0.7, jc[3] * 0.7)
                luna.graphics.rectangle("fill", lem.x + (leg and 1 or 5), lem.y + LSIZE, 3, 4)
                luna.graphics.rectangle("fill", lem.x + (leg and 5 or 1), lem.y + LSIZE, 3, 4)
            end
        end
    end

    -- HUD panel
    luna.graphics.setColor(0.1, 0.1, 0.15, 0.85)
    luna.graphics.rectangle("fill", 0, H - 42, W, 42)
    luna.graphics.setColor(0.5, 1, 0.3)
    luna.graphics.print("Saved: " .. saved .. "/" .. LEVEL_NEED, 8, H - 37, 1.5)
    luna.graphics.setColor(1, 0.4, 0.4)
    luna.graphics.print("Dead: " .. dead, 170, H - 37, 1.5)
    luna.graphics.setColor(1, 0.8, 0.2)
    luna.graphics.print("Left: " .. (LEVEL_TOTAL - spawned), 290, H - 37, 1.5)
    ...
    for i, j in ipairs(jobs_row) do
        ...
        luna.graphics.setColor(sel and 0.3 or 0.2, sel and 0.5 or 0.3, sel and 0.9 or 0.5)
        luna.graphics.rectangle("fill", bx, H - 40, 94, 36)
        if sel then
            luna.graphics.setColor(1, 1, 0.2)
            luna.graphics.rectangle("line", bx, H - 40, 94, 36)
        end
        local jc = JOB_COLORS[j]
        luna.graphics.setColor(jc[1], jc[2], jc[3])
        luna.graphics.print(j:upper() .. " " .. (skill_counts[j] or 0), bx + 4, H - 30, 1.2)
    end
    ...
    if game_state ~= "playing" then
        ...
        if game_state == "win" then
            ...
            luna.graphics.print("LEVEL COMPLETE!", W/2 - 110, H/2 - 25, 3)
            luna.graphics.setColor(1, 1, 1)
            luna.graphics.print("Saved: " .. saved .. " / " .. LEVEL_NEED, W/2 - 90, H/2 + 20, 2)
        else
            luna.graphics.setColor(1, 0.2, 0.2)
            luna.graphics.print("LEVEL FAILED", W/2 - 94, H/2 - 25, 3)
            luna.graphics.setColor(1, 1, 1)
            luna.graphics.print("Saved: " .. saved .. " / " .. LEVEL_NEED, W/2 - 90, H/2 + 20, 2)
        end
        luna.graphics.setColor(0.6, 0.6, 0.6)
        luna.graphics.print("Press R to retry", W/2 - 88, H/2 + 58, 2)
    end
end

...
function luna.draw()
    ...
    for r = 1, ROWS do
        for c = 1, COLS do
            ...
            if map[r][c] == 0 then
                luna.graphics.setColor(0.08 * vis, 0.08 * vis, 0.12 * vis, 1)
                luna.graphics.rectangle("fill", x, y, TILE, TILE)
            elseif map[r][c] == 2 then
                luna.graphics.setColor(0.1 * vis, 0.4 * vis, 0.5 * vis, 1)
                luna.graphics.rectangle("fill", x, y, TILE, TILE)
            elseif map[r][c] == 3 then
                local g = keysFound >= KEYS_NEEDED and 0.8 or 0.3
                luna.graphics.setColor(g * vis, 0.2 * vis, 0.2 * vis, 1)
                luna.graphics.rectangle("fill", x, y, TILE, TILE)
            else
                luna.graphics.setColor(0.25 * vis, 0.22 * vis, 0.2 * vis, 1)
                luna.graphics.rectangle("fill", x, y, TILE, TILE)
            end
        end
    end
    ...
    for _, f in ipairs(footsteps) do
        ...
        luna.graphics.setColor(0.5, 0.5, 0.5, a)
        luna.graphics.circle("line", f.x + shx, f.y + shy, r)
    end
    ...
    for _, k in ipairs(keys) do
        if not k.found then
            ...
            luna.graphics.setColor(1 * vis, 0.9 * vis, 0.2 * vis, 1)
            luna.graphics.circle("fill", k.x + shx, k.y + shy, 5)
        end
    end
    ...
    for _, n in ipairs(notes) do
        if not n.read then
            ...
            luna.graphics.setColor(0.9 * vis, 0.9 * vis, 0.8 * vis, 1)
            luna.graphics.rectangle("fill", n.x - 5 + shx, n.y - 5 + shy, 10, 10)
        end
    end
    ...
    for _, ev in ipairs(events) do
        ...
        luna.graphics.setColor(0.5 * vis, 0, 0, ev.life)
        luna.graphics.circle("fill", ev.x + shx, ev.y + shy, 15)
    end

    -- Enemy
    local eVis = visibility(enemy.x, enemy.y)
    if eVis > 0.05 then
        luna.graphics.setColor(0.8 * eVis, 0.1, 0.1, eVis)
        luna.graphics.circle("fill", enemy.x + shx, enemy.y + shy, 12)
        -- Vision cone
        luna.graphics.setColor(0.5, 0, 0, 0.15 * eVis)
        ...
        luna.graphics.polygon("fill", {enemy.x + shx, enemy.y + shy, ex1 + shx, ey1 + shy, ex2 + shx, ey2 + shy})
    end

    -- Player
    luna.graphics.setColor(0.9, 0.8, 0.6, 1)
    luna.graphics.circle("fill", player.x + shx, player.y + shy, 8)
    ...
    if flashlight.on and flashlight.battery > 0 then
        ...
        luna.graphics.setColor(1, 1, 0.8, intensity)
        luna.graphics.polygon("fill", {player.x + shx, player.y + shy, fx1 + shx, fy1 + shy, fx2 + shx, fy2 + shy})
    end

    -- Distortion overlay (low sanity)
    if distortion > 0.3 then
        local a = (distortion - 0.3) * 0.5
        local r = 0.3 + math.sin(luna.timer.getTime() * 5) * 0.2
        luna.graphics.setColor(r, 0, 0.1, a)
    ...
    luna.graphics.rectangle("fill", 10, 5, 100, 12)
    local bc = flashlight.battery / flashlight.maxBattery
    luna.graphics.setColor(bc, bc, 0.2, 1)
    luna.graphics.rectangle("fill", 10, 5, bc * 100, 12)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Battery", 115, 4, 0.7)
    -- Sanity bar
    luna.graphics.setColor(0.3, 0.3, 0.3, 1)
    luna.graphics.rectangle("fill", 200, 5, 100, 12)
    local sc = sanity.value / sanity.max
    luna.graphics.setColor(0.2, sc * 0.8, sc, 1)
    luna.graphics.rectangle("fill", 200, 5, sc * 100, 12)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Sanity", 305, 4, 0.7)
    -- Keys
    luna.graphics.setColor(1, 0.9, 0.2, 1)
    luna.graphics.print("Keys: " .. keysFound .. "/" .. KEYS_NEEDED, 400, 5, 0.9)
    luna.graphics.setColor(0.6, 0.6, 0.6, 1)
    luna.graphics.print("WASD move | F flashlight | Find 5 keys, reach exit", 520, 6, 0.65)

    -- Note display
    if noteDisplay and noteTimer > 0 then
        luna.graphics.setColor(0, 0, 0, 0.85)
        luna.graphics.rectangle("fill", W / 2 - 200, H / 2 - 40, 400, 80)
        luna.graphics.setColor(0.9, 0.85, 0.7, 1)
        luna.graphics.print(noteDisplay, W / 2 - 180, H / 2 - 20, 0.9)
    end

    -- Game over / win
    if gameState == "dead" then
        luna.graphics.setColor(0.5, 0, 0, 0.8)
        luna.graphics.rectangle("fill", 0, 0, W, H)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("YOUR MIND IS LOST", W / 2 - 120, H / 2 - 20, 1.5)
        luna.graphics.print("Press R to retry", W / 2 - 60, H / 2 + 20, 0.9)
    elseif gameState == "won" then
        luna.graphics.setColor(0, 0.2, 0, 0.8)
        luna.graphics.rectangle("fill", 0, 0, W, H)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("YOU ESCAPED!", W / 2 - 80, H / 2 - 20, 1.5)
        luna.graphics.print("Press R to play again", W / 2 - 70, H / 2 + 20, 0.9)
    end
end

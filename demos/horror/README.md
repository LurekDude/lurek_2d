...
- `luna.graphics.rectangle()` — tile map, sanity bar, battery bar, and key HUD
- `luna.graphics.setColor()` — darkness overlay tinted by sanity level; distortion tint when sanity is low
- `luna.graphics.polygon()` — flashlight cone rendered as a triangle fan in world space
- `luna.graphics.print()` — key counter, note text pop-ups, win/lose overlays
- `luna.graphics.setBackgroundColor()` — full black to simulate total darkness

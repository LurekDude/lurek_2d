...
function luna.load()
    luna.graphics.setBackgroundColor(0.05, 0.02, 0.15)
    reset()
end
...
function luna.draw()
    -- Sky
    luna.graphics.setColor(LAYERS[1].r, LAYERS[1].g, LAYERS[1].b)
    luna.graphics.rectangle("fill", 0, 0, W, H)

    -- Moon
    luna.graphics.setColor(0.9, 0.85, 0.65)
    luna.graphics.circle("fill", MOON.x, MOON.y, MOON.r)
    luna.graphics.setColor(LAYERS[1].r, LAYERS[1].g + 0.01, LAYERS[1].b + 0.02)
    luna.graphics.circle("fill", MOON.x + 18, MOON.y - 10, MOON.r * 0.85)
    ...
    for li = 2, 4 do
        ...
        if layer_trees[li] then
            ...
            for _, t in ipairs(layer_trees[li]) do
                local dark = li / 5
                luna.graphics.setColor(lc.r, lc.g, lc.b)
                luna.graphics.rectangle("fill", t.x, horizon, t.w, H - horizon)
                luna.graphics.rectangle("fill", t.x + t.w/2 - t.w * 0.3, horizon - t.h, t.w * 0.6, t.h)
            end
        end
    end

    -- Ground
    luna.graphics.setColor(0.12, 0.08, 0.05)
    luna.graphics.rectangle("fill", 0, GROUND_Y, W, H - GROUND_Y)
    luna.graphics.setColor(0.25, 0.14, 0.28)
    luna.graphics.rectangle("fill", 0, GROUND_Y, W, 6)
    ...
    for _, e in ipairs(enemies) do
        if e.alive then
            ...
            local gx = e.x + e.w/2
            -- Body
            luna.graphics.setColor(0.5 * sz, 0.2, 0.05)
            luna.graphics.rectangle("fill", e.x, e.y, e.w, e.h)
            -- Head
            luna.graphics.setColor(0.35, 0.15, 0.04)
            luna.graphics.circle("fill", gx, e.y + e.h * 0.25, e.w * 0.4)
            -- Eyes
            luna.graphics.setColor(1, 0.1, 0)
            luna.graphics.circle("fill", gx - 4, e.y + e.h * 0.22, 4)
            luna.graphics.circle("fill", gx + 4, e.y + e.h * 0.22, 4)
            -- HP bar
            if e.boss then
                luna.graphics.setColor(0.4, 0, 0)
                luna.graphics.rectangle("fill", e.x, e.y - 10, e.w, 6)
                luna.graphics.setColor(0.9, 0.1, 0.1)
                luna.graphics.rectangle("fill", e.x, e.y - 10, e.w * (e.hp / 8), 6)
            end
        end
    end
    ...
    for _, a in ipairs(attacks) do
        local t = a.life / 0.4
        luna.graphics.setColor(1, 0.8, 0.2, t)
        luna.graphics.circle("fill", a.x, a.y, 10 * t)
    end

    -- Player (beast man)
    if player.attacking then
        luna.graphics.setColor(0.9, 0.7, 0.2, 0.5)
        luna.graphics.circle("fill", player.x + player.w/2 + player.facing * 25, player.y + 15, 18)
    end
    -- Body
    luna.graphics.setColor(0.55, 0.42, 0.28)
    luna.graphics.rectangle("fill", player.x + 4, player.y + 16, player.w - 8, player.h - 16)
    -- Head
    luna.graphics.setColor(0.6, 0.45, 0.3)
    luna.graphics.circle("fill", player.x + player.w/2, player.y + 13, 13)
    -- Horn / beast features
    luna.graphics.setColor(0.3, 0.2, 0.1)
    luna.graphics.rectangle("fill", player.x + player.w/2 - 2, player.y, 4, 10)
    -- Eyes
    luna.graphics.setColor(0.9, 0.5, 0.1)
    local ex2 = player.x + (player.facing > 0 and player.w - 7 or 7)
    luna.graphics.circle("fill", ex2, player.y + 11, 3)

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.65)
    luna.graphics.rectangle("fill", 0, 0, W, 28)
    luna.graphics.setColor(0.8, 0.4, 1)
    luna.graphics.print("SHADOW OF THE BEAST", 8, 4, 1.8)
    luna.graphics.setColor(1, 0.8, 0.2)
    luna.graphics.print("Score: " .. score, W/2 - 50, 4, 1.6)
    -- Health orbs
    for i = 1, HEALTH_MAX do
        local hx = W - 24 * i - 5
        luna.graphics.setColor(i <= health and 0.9 or 0.3, i <= health and 0.2 or 0.2, i <= health and 0.1 or 0.2)
        luna.graphics.circle("fill", hx + 10, 14, 9)
    end
    luna.graphics.setColor(0.6, 0.4, 0.9)
    luna.graphics.print("Stage " .. stage .. "/3", W/2 + 70, 4, 1.6)

    luna.graphics.setColor(0.5, 0.4, 0.65, 0.65)
    luna.graphics.print("[A/D] Walk  [Space/W] Jump  [X] Attack  Defeat all beasts each stage!", 8, H - 20, 1.3)
    ...
    if game_state == "gameover" then
        ...
        luna.graphics.print("THE BEAST CLAIMS YOU", W/2 - 150, H/2 - 25, 2.5)
        luna.graphics.setColor(1, 0.8, 0.2)
    ...
    elseif game_state == "win" then
        luna.graphics.setColor(0, 0, 0, 0.78)
        luna.graphics.rectangle("fill", 0, 0, W, H)
        luna.graphics.setColor(0.7, 0.3, 1)
        luna.graphics.print("CURSE BROKEN!", W/2 - 110, H/2 - 30, 3)
        luna.graphics.setColor(1, 0.9, 0.5)
        luna.graphics.print("Score: " .. score, W/2 - 50, H/2 + 20, 2)
        luna.graphics.setColor(0.6, 0.6, 0.6)
        luna.graphics.print("Press R to play again", W/2 - 110, H/2 + 55, 2)
    end
end

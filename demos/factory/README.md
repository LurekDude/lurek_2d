...
- `luna.graphics.rectangle()` — grid cell backgrounds, building footprints, and item dots
- `luna.graphics.polygon()` — directional arrows drawn on each machine to show output direction
- `luna.graphics.line()` — grid line overlay over the entire play area
- `luna.graphics.setColor()` — distinct colours per machine type and item type
- `luna.graphics.print()` — product and ingot counters, placement HUD, and input queue display

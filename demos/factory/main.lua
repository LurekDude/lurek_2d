...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.12, 0.14, 0.12)
    grid = {}
    for r = 1, ROWS do
        grid[r] = {}
...
function luna.draw()
    -- grid lines
    luna.graphics.setColor(0.2, 0.22, 0.2, 0.5)
    for r = 0, ROWS do
        luna.graphics.line(0, r * TILE, W, r * TILE)
    end
    for c = 0, COLS do
        luna.graphics.line(c * TILE, 0, c * TILE, H)
    end
    ...
    for key, _ in pairs(ore_patches) do
        ...
        local pc = key - pr * 100
        luna.graphics.setColor(0.4, 0.3, 0.15, 0.5)
        luna.graphics.rectangle("fill", (pc - 1) * TILE + 2, (pr - 1) * TILE + 2, TILE - 4, TILE - 4)
    end
    ...
    for r = 1, ROWS do
        for c = 1, COLS do
            ...
            if cell then
                ...
                luna.graphics.setColor(col[1], col[2], col[3], 0.85)
                luna.graphics.rectangle("fill", (c - 1) * TILE + 1, (r - 1) * TILE + 1, TILE - 2, TILE - 2)
                -- direction arrow
                ...
                if cell.input_count and cell.input_count > 0 then
                    ...
                    luna.graphics.print(tostring(cell.input_count), (c - 1) * TILE + 2, (r - 1) * TILE + 1, 0.7)
                end
            end
        end
    end
    ...
    for _, it in ipairs(items) do
        ...
        luna.graphics.setColor(ic[1], ic[2], ic[3], 1)
        luna.graphics.rectangle("fill", bx - 4, by - 4, 8, 8)
    end
    ...
    if in_bounds(gc, gr) then
        ...
        luna.graphics.setColor(col[1], col[2], col[3], 0.35)
        luna.graphics.rectangle("fill", (gc - 1) * TILE, (gr - 1) * TILE, TILE, TILE)
    ...
    luna.graphics.rectangle("fill", 0, H, W, 30)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Placing: " .. TYPE_NAMES[place_type] .. "  Dir: " .. ({"R","D","L","U"})[place_dir + 1]

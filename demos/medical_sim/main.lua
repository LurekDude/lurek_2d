...
local function draw_vitals(p)
    ...
    luna.graphics.print("Heart Rate: " .. math.floor(p.heart_rate), 20, 420, 1)
    local hr_pct = clamp((p.heart_rate - 30) / 70, 0, 1)
    luna.graphics.setColor(0.3,0.3,0.3,1)
    luna.graphics.rectangle("fill", 20, 440, 160, 14)
    luna.graphics.setColor(1 - hr_pct, hr_pct, 0.2, 1)
    luna.graphics.rectangle("fill", 20, 440, 160 * hr_pct, 14)

    luna.graphics.setColor(1,1,1,1)
    luna.graphics.print("Blood Pressure: " .. math.floor(p.blood_pressure), 20, 465, 1)
    local bp_pct = clamp((p.blood_pressure - 60) / 80, 0, 1)
    luna.graphics.setColor(0.3,0.3,0.3,1)
    luna.graphics.rectangle("fill", 20, 485, 160, 14)
    luna.graphics.setColor(1 - bp_pct, bp_pct, 0.2, 1)
    luna.graphics.rectangle("fill", 20, 485, 160 * bp_pct, 14)
end

local function draw_patient(p)
    -- table
    luna.graphics.setColor(0.4,0.35,0.3,1)
    luna.graphics.rectangle("fill", 200, 360, 400, 40)
    -- body outline
    luna.graphics.setColor(0.9,0.75,0.65,1)
    if p.layers.skin then
        luna.graphics.rectangle("fill", 250, 200, 300, 160)
    end
    if not p.layers.skin and p.layers.muscle then
        luna.graphics.setColor(0.7,0.25,0.2,1)
        luna.graphics.rectangle("fill", 250, 200, 300, 160)
    end
    if not p.layers.skin and not p.layers.muscle then
        luna.graphics.setColor(0.8,0.4,0.4,1)
        luna.graphics.rectangle("fill", 250, 200, 300, 160)
        -- problem zone
        if not p.repaired then
            luna.graphics.setColor(1,0.1,0.1, 0.5 + 0.3 * math.sin(luna.timer.getTime() * 5))
            luna.graphics.circle("fill", p.problem_x, p.problem_y, 20)
        else
            luna.graphics.setColor(0.2,1,0.2,0.6)
            luna.graphics.circle("fill", p.problem_x, p.problem_y, 20)
        end
    end
    -- body outline
    luna.graphics.setColor(0.2,0.2,0.2,1)
    luna.graphics.rectangle("line", 250, 200, 300, 160)
    -- step label
    ...
    luna.graphics.print("Step: " .. (steps[p.step] or "Done!"), 250, 185, 1)
end

function luna.draw()
    luna.graphics.setBackgroundColor(0.15,0.18,0.22)

    if game_over then
        luna.graphics.setColor(1,1,1,1)
        luna.graphics.print("ALL PATIENTS COMPLETE", 260, 220, 1.5)
        luna.graphics.print("Score: " .. score, 320, 270, 1.4)
        luna.graphics.print("Time: " .. math.floor(total_time) .. "s", 320, 310, 1)
        luna.graphics.print("Press R to restart", 300, 370, 1)
        return
    end

    luna.graphics.setColor(1,1,1,1)
    luna.graphics.print("MEDICAL SIM — Patient " .. current_patient .. "/5", 20, 10, 1.2)
    luna.graphics.print("Score: " .. score, 20, 40, 1)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), 700, 10, 0.8)
    ...
    luna.graphics.rectangle("fill", 630, 170, 160, 320)
    luna.graphics.setColor(1,1,1,1)
    luna.graphics.print("TOOLS", 680, 155, 1)
    for i, t in ipairs(tools) do
        local ty = 180 + (i-1) * 60
        local c = tool_colors[t]
        if selected_tool == t then
            luna.graphics.setColor(1,1,0.3,0.4)
            luna.graphics.rectangle("fill", 640, ty, 140, 50)
        end
        luna.graphics.setColor(c[1],c[2],c[3],1)
        luna.graphics.rectangle("fill", 645, ty + 5, 40, 40)
        luna.graphics.setColor(1,1,1,1)
        luna.graphics.print(t, 690, ty + 15, 0.9)
    end

    -- message
    if message and msg_timer > 0 then
        luna.graphics.setColor(1,1,0.5,1)
        luna.graphics.print(message, 220, 520, 1)
    end

    luna.graphics.setColor(0.6,0.6,0.6,1)
    luna.graphics.print("[R] Restart   [ESC] Quit", 20, 570, 0.8)
end

...
function luna.draw()
    ...
    luna.graphics.print(format_num(coins) .. " coins", 60, 20, 2.5)
    luna.graphics.setColor(0.7, 0.7, 0.7, 1)
    luna.graphics.print(format_num(get_cps()) .. " per second", 60, 65)
    luna.graphics.print("Click power: " .. format_num(click_power * prestige_mult), 60, 85)
    ...
    luna.graphics.setColor(0.9, 0.7, 0.1, 1)
    luna.graphics.circle("fill", bx, by, hw)
    luna.graphics.setColor(1, 0.85, 0.2, 1)
    luna.graphics.circle("fill", bx, by, hw - 6)
    luna.graphics.setColor(0.3, 0.2, 0, 1)
    luna.graphics.print("CLICK", bx - 30, by - 10, 1.5)

    -- Click particles
    for _, p in ipairs(click_particles) do
        luna.graphics.setColor(1, 1, 0.3, p.life)
        luna.graphics.print(p.text, p.x, p.y, 1.2)
    end

    -- Generator panel
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("GENERATORS", 350, 95, 1.2)
    for i, g in ipairs(generators) do
        local by = 120 + (i - 1) * 52
        local affordable = coins >= g.cost
        if affordable then
            luna.graphics.setColor(0.15, 0.3, 0.15, 1)
        else
            luna.graphics.setColor(0.2, 0.15, 0.15, 1)
        end
        luna.graphics.rectangle("fill", 320, by, 240, 44)
        luna.graphics.setColor(0.4, 0.4, 0.4, 1)
        luna.graphics.rectangle("line", 320, by, 240, 44)
        luna.graphics.setColor(1, 1, 1, affordable and 1 or 0.4)
        luna.graphics.print(g.name .. " (" .. g.count .. ")", 328, by + 4)
        luna.graphics.setColor(0.7, 0.7, 0.5, 1)
        luna.graphics.print("Cost: " .. format_num(g.cost) .. "  +" .. format_num(g.cps * prestige_mult) .. "/s", 328, by + 22)
    end

    -- Upgrades panel
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("UPGRADES", 640, 95, 1.2)
    for i, u in ipairs(upgrades) do
        local by = 120 + (i - 1) * 42
        if u.bought then
            luna.graphics.setColor(0.15, 0.15, 0.25, 1)
        elseif coins >= u.cost then
            luna.graphics.setColor(0.15, 0.25, 0.35, 1)
        else
            luna.graphics.setColor(0.15, 0.12, 0.12, 1)
        end
        luna.graphics.rectangle("fill", 590, by, 200, 34)
        luna.graphics.setColor(0.4, 0.4, 0.4, 1)
        luna.graphics.rectangle("line", 590, by, 200, 34)
        if u.bought then
            luna.graphics.setColor(0.4, 0.7, 0.4, 1)
            luna.graphics.print(u.name .. " [OWNED]", 598, by + 4)
        else
            luna.graphics.setColor(1, 1, 1, coins >= u.cost and 1 or 0.4)
            luna.graphics.print(u.name .. " - " .. format_num(u.cost), 598, by + 4)
        end
        luna.graphics.setColor(0.6, 0.6, 0.6, 1)
        luna.graphics.print(u.desc, 598, by + 18)
    end

    -- Prestige button
    luna.graphics.setColor(0.5, 0.2, 0.6, total_earned >= 10000 and 1 or 0.3)
    luna.graphics.rectangle("fill", 60, 500, 200, 40)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("PRESTIGE (10K req)", 75, 508)

    -- Stats
    luna.graphics.setColor(0.6, 0.6, 0.6, 1)
    luna.graphics.print("Total earned: " .. format_num(total_earned), 60, 400)
    luna.graphics.print("Total clicks: " .. total_clicked, 60, 418)
    luna.graphics.print("Prestige: x" .. prestige_mult .. " (" .. prestige_count .. ")", 60, 436)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), 60, 460)
end

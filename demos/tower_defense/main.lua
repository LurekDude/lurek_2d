...
function luna.draw()
    ...
    for gx = 0, COLS - 1 do
        for gy = 0, ROWS - 1 do
            ...
            if cell and cell.path then
                luna.graphics.setColor(0.3, 0.28, 0.2, 1)
            else
                luna.graphics.setColor(0.18, 0.22, 0.14, 1)
            end
            luna.graphics.rectangle("fill", px, py, TILE, TILE)
            luna.graphics.setColor(0.1, 0.12, 0.08, 0.3)
    ...
    for _, tw in ipairs(towers) do
        if tw.kind == "basic" then
            luna.graphics.setColor(0.3, 0.7, 0.9, 1)
        else
            luna.graphics.setColor(0.8, 0.4, 0.2, 1)
        end
        luna.graphics.rectangle("fill", tw.x - 12, tw.y - 12, 24, 24)
        luna.graphics.setColor(0.1, 0.1, 0.1, 1)
        luna.graphics.rectangle("line", tw.x - 12, tw.y - 12, 24, 24)
        -- Range indicator on hover
        local mx, my = luna.mouse.getPosition()
        if distXY(mx, my, tw.x, tw.y) < 18 then
            luna.graphics.setColor(1, 1, 1, 0.1)
            luna.graphics.circle("line", tw.x, tw.y, tw.range)
        end
    end
    ...
    for _, e in ipairs(enemies) do
        ...
        luna.graphics.rectangle("fill", e.x - 10, e.y - 14, 20, 4)
        luna.graphics.setColor(0.1, 0.9, 0.1, 1)
        luna.graphics.rectangle("fill", e.x - 10, e.y - 14, 20 * (e.hp / e.maxHp), 4)
    end
    ...
    for _, p in ipairs(particles) do
        luna.graphics.setColor(p.r, p.g, p.b, p.life * 2.5)
        luna.graphics.circle("fill", p.x, p.y, 3)
    end
    ...
    luna.graphics.print("Gold: " .. cash, 10, H - 24)
    luna.graphics.setColor(1, 0.3, 0.3, 1)
    luna.graphics.print("Lives: " .. lives, 120, H - 24)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Wave: " .. wave, 230, H - 24)
    local tInfo = towerType == "basic" and "[1]Basic(20g)" or "[2]Cannon(40g)"
    luna.graphics.print(tInfo .. "  [N]NextWave", 340, H - 24)
    luna.graphics.print("FPS:" .. luna.timer.getFPS(), W - 70, H - 24)

    if gameOver then
        luna.graphics.setColor(0, 0, 0, 0.6)
        luna.graphics.rectangle("fill", 250, 260, 300, 60)
        luna.graphics.setColor(1, 0.2, 0.2, 1)
        luna.graphics.print("GAME OVER — Wave " .. wave, 280, 275, 1.5)
    end
end

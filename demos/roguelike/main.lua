...
function luna.draw()
    ...
    for y = 1, MAP_H do
        for x = 1, MAP_W do
            ...
            if revealed[y][x] then
                ...
                local t = map[y][x]
                if t == WALL then
                    luna.graphics.setColor(0.25 * bright, 0.22 * bright, 0.3 * bright)
                    luna.graphics.rectangle("fill", offsetX + (x - 1) * TILE_SIZE, offsetY + (y - 1) * TILE_SIZE, TILE_SIZE - 1, TILE_SIZE - 1)
                elseif t == FLOOR then
                    luna.graphics.setColor(0.12 * bright, 0.12 * bright, 0.15 * bright)
                    luna.graphics.rectangle("fill", offsetX + (x - 1) * TILE_SIZE, offsetY + (y - 1) * TILE_SIZE, TILE_SIZE - 1, TILE_SIZE - 1)
                elseif t == STAIRS and visible then
                    luna.graphics.setColor(0.1, 0.1, 0.15)
                    luna.graphics.rectangle("fill", offsetX + (x - 1) * TILE_SIZE, offsetY + (y - 1) * TILE_SIZE, TILE_SIZE - 1, TILE_SIZE - 1)
                    luna.graphics.setColor(1, 0.9, 0.3)
                    luna.graphics.print(">", offsetX + (x - 1) * TILE_SIZE + 6, offsetY + (y - 1) * TILE_SIZE + 4)
                end
                -- Pickups
                if visible then
                    for _, p in ipairs(pickups) do
                        if p.x == x and p.y == y then
                            ...
                            luna.graphics.circle("fill", offsetX + (x - 1) * TILE_SIZE + TILE_SIZE / 2, offsetY + (y - 1) * TILE_SIZE + TILE_SIZE / 2, 5)
                        end
                    end
                end
            end
    ...
    for _, e in ipairs(enemies) do
        ...
        if math.sqrt(dx * dx + dy * dy) <= VIEW_RADIUS then
            luna.graphics.setColor(0.9, 0.2, 0.2)
            luna.graphics.rectangle("fill", offsetX + (e.x - 1) * TILE_SIZE + 3, offsetY + (e.y - 1) * TILE_SIZE + 3, TILE_SIZE - 7, TILE_SIZE - 7)
            luna.graphics.setColor(1, 1, 1)
            luna.graphics.print("E", offsetX + (e.x - 1) * TILE_SIZE + 6, offsetY + (e.y - 1) * TILE_SIZE + 4)
        end
    end
    -- Draw player
    luna.graphics.setColor(0.3, 0.7, 1)
    luna.graphics.rectangle("fill", offsetX + (player.x - 1) * TILE_SIZE + 2, offsetY + (player.y - 1) * TILE_SIZE + 2, TILE_SIZE - 5, TILE_SIZE - 5)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("@", offsetX + (player.x - 1) * TILE_SIZE + 6, offsetY + (player.y - 1) * TILE_SIZE + 4)
    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.7); luna.graphics.rectangle("fill", 0, 0, 800, 30)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("HP: " .. player.hp .. "/" .. player.maxHp .. "  Floor: " .. floor_num .. "  Kills: " .. player.kills .. "  Turn: " .. turnCount, 10, 8)
    -- HP bar
    luna.graphics.setColor(0.3, 0.3, 0.3); luna.graphics.rectangle("fill", 600, 8, 120, 14)
    luna.graphics.setColor(0.8, 0.2, 0.2); luna.graphics.rectangle("fill", 600, 8, 120 * (player.hp / player.maxHp), 14)
    -- Messages
    for i, msg in ipairs(messages) do
        luna.graphics.setColor(1, 1, 0.8, 1.0 - (i - 1) * 0.2)
        luna.graphics.print(msg, 10, 560 - (i - 1) * 16)
    end
    if gameOver then
        luna.graphics.setColor(0, 0, 0, 0.75); luna.graphics.rectangle("fill", 200, 220, 400, 120)
        luna.graphics.setColor(1, 0.2, 0.2); luna.graphics.print("YOU DIED", 330, 240, 2)
        luna.graphics.setColor(1, 1, 1); luna.graphics.print("Floor " .. floor_num .. " | Kills: " .. player.kills .. " | Turns: " .. turnCount, 280, 290)
        luna.graphics.print("Press R to restart", 330, 315)
    end
end

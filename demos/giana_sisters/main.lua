...
function luna.load()
    luna.graphics.setBackgroundColor(0.3, 0.5, 0.9)
...
function luna.draw()
    ...
    for row = 1, map_rows do
        for col = first_col, last_col do
            ...
            local sy = (row - 1) * TILE
            if ch == "1" then
                luna.graphics.setColor(0.55, 0.38, 0.15)
                luna.graphics.rectangle("fill", sx, sy, TILE, TILE)
                luna.graphics.setColor(0.7, 0.5, 0.25)
    ...
    for _, g in ipairs(gems) do
        if g.alive then
            luna.graphics.setColor(1, 0.2, 0.8)
            luna.graphics.circle("fill", g.x - cam + 8, g.y + 8, 8)
            luna.graphics.setColor(1, 0.7, 1)
            luna.graphics.circle("fill", g.x - cam + 5, g.y + 5, 3)
        end
    end
    ...
    for _, e in ipairs(enemies) do
        if e.alive then
            local sx = e.x - cam
            luna.graphics.setColor(0.9, 0.3, 0.1)
            luna.graphics.rectangle("fill", sx + 2, e.y + 2, e.w - 4, e.h - 4)
            luna.graphics.setColor(1, 1, 1)
            luna.graphics.circle("fill", sx + (e.vx > 0 and e.w - 8 or 8), e.y + 8, 5)
        end
    end

    -- Player
    local px = player.x - cam
    luna.graphics.setColor(0.2, 0.2, 0.8)
    luna.graphics.rectangle("fill", px + 2, player.y + player.h/3, player.w - 4, player.h * 2/3)
    luna.graphics.setColor(0.9, 0.7, 0.5)
    luna.graphics.circle("fill", px + player.w/2, player.y + 14, 12)
    -- Hair / hat
    luna.graphics.setColor(1, 0.8, 0.1)
    luna.graphics.rectangle("fill", px + 2, player.y - 4, player.w - 4, 10)

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.5)
    luna.graphics.rectangle("fill", 0, 0, W, 28)
    luna.graphics.setColor(1, 1, 0)
    luna.graphics.print("GIANA  Score: " .. score, 8, 5, 1.5)
    luna.graphics.setColor(1, 0.5, 0.5)
    luna.graphics.print("Lives: " .. lives, W - 100, 5, 1.5)
    luna.graphics.setColor(0.5, 1, 0.5)
    luna.graphics.print("Level " .. level, W/2 - 30, 5, 1.5)

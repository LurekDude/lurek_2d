...
- `luna.graphics.polygon()` — camera vision-cone rendered as a filled triangle fan
- `luna.graphics.rectangle()` — tile map, door overlays, alert bar, gadget HUD
- `luna.graphics.setColor()` — alert-level colour interpolation from green → yellow → red
- `luna.graphics.print()` — mission timer countdown, door labels, and win/caught overlays

...
function luna.load()
    luna.graphics.setBackgroundColor(0.05, 0.05, 0.28)
    score = 0
    load_scene(1)
end
...
function luna.draw()
    ...
    luna.graphics.setColor(COLORS.sky_top[1], COLORS.sky_top[2], COLORS.sky_top[3])
    luna.graphics.rectangle("fill", 0, 0, W, H / 2)
    luna.graphics.setColor(COLORS.sky_bottom[1], COLORS.sky_bottom[2], COLORS.sky_bottom[3])
    luna.graphics.rectangle("fill", 0, H/2, W, H/2)

    -- Background silhouette shapes
    luna.graphics.setColor(0.12, 0.07, 0.22)
    luna.graphics.rectangle("fill", 0, H/2 - 40, W, 80)
    for i = 0, 6 do
        local h2 = 60 + math.sin(i * 2.7) * 30
        luna.graphics.rectangle("fill", i * 120 - 20, H/2 - 40 - h2, 90, h2)
    end

    -- Platforms
    for _, p in ipairs(sc.platforms) do
        luna.graphics.setColor(COLORS.ground[1], COLORS.ground[2], COLORS.ground[3])
        luna.graphics.rectangle("fill", p[1], p[2], p[3], p[4])
        luna.graphics.setColor(COLORS.ground_hi[1], COLORS.ground_hi[2], COLORS.ground_hi[3])
        luna.graphics.rectangle("fill", p[1], p[2], p[3], 4)
    end
    ...
    for _, ex in ipairs(sc.exits) do
        ...
        if show then
            luna.graphics.setColor(0.1, 0.8, 0.9, 0.5 + 0.5 * math.sin(anim * 3))
            luna.graphics.rectangle("fill", ex.x, ex.y, ex.w, ex.h)
        end
    end
    ...
    if enemy.alive then
        -- Shield glow
        if enemy.shield_active then
            luna.graphics.setColor(0.8, 0.3, 0.1, 0.4)
            luna.graphics.circle("fill", enemy.x + enemy.w/2, enemy.y + enemy.h/2, 30)
        end
        luna.graphics.setColor(0.3, 0.6, 0.35)
        luna.graphics.rectangle("fill", enemy.x + 4, enemy.y + 14, enemy.w - 8, enemy.h - 14)
        luna.graphics.setColor(0.2, 0.45, 0.25)
        luna.graphics.circle("fill", enemy.x + enemy.w/2, enemy.y + 12, 14)
        -- Eyes
        local ex2 = enemy.facing > 0 and enemy.x + enemy.w - 10 or enemy.x + 6
        luna.graphics.setColor(1, 0.3, 0)
        luna.graphics.circle("fill", ex2, enemy.y + 10, 4)
        -- HP
        luna.graphics.setColor(0.8, 0.2, 0.2)
        luna.graphics.rectangle("fill", enemy.x, enemy.y - 8, enemy.w, 5)
        luna.graphics.setColor(0.1, 0.9, 0.2)
        luna.graphics.rectangle("fill", enemy.x, enemy.y - 8, enemy.w * (enemy.hp / 3), 5)
    end

    -- Bullets
    for _, b in ipairs(bullets) do
        if b.owner == "player" then
            luna.graphics.setColor(0.3, 0.9, 1)
        else
            luna.graphics.setColor(0.9, 0.4, 0.1)
        end
    ...
    -- Shield
    if player.shield_active then
        luna.graphics.setColor(0.3, 0.7, 1, 0.35)
        luna.graphics.circle("fill", player.x + player.w/2, player.y + player.h/2, 28)
    end
    luna.graphics.setColor(0.7, 0.6, 0.4)
    luna.graphics.rectangle("fill", player.x + 3, player.y + 16, player.w - 6, player.h - 16)
    luna.graphics.setColor(0.8, 0.7, 0.55)
    luna.graphics.circle("fill", player.x + player.w/2, player.y + 14, 13)
    -- Gun arm
    local garm_x = player.x + (player.facing > 0 and player.w or 0)
    luna.graphics.setColor(0.6, 0.5, 0.3)
    luna.graphics.rectangle("fill", garm_x - 3, player.y + 18, 12, 6)

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.6)
    luna.graphics.rectangle("fill", 0, 0, W, 28)
    luna.graphics.setColor(0.4, 0.8, 1)
    luna.graphics.print("ANOTHER WORLD", 8, 4, 1.8)
    luna.graphics.setColor(1, 0.8, 0.3)
    luna.graphics.print("Score: " .. score, W/2 - 50, 4, 1.6)
    -- Shield charges
    for i = 1, MAX_SHIELD do
        local sx = W - 28 * i - 6
        luna.graphics.setColor(i <= player.shield_count and 0.3 or 0.25, i <= player.shield_count and 0.7 or 0.25, i <= player.shield_count and 1 or 0.25)
        luna.graphics.rectangle("fill", sx, 4, 22, 20)
    end

    -- Scene caption
    if caption_timer > 0 then
        local alpha = math.min(1, caption_timer)
        luna.graphics.setColor(1, 1, 0.8, alpha)
        luna.graphics.print(sc.caption, W/2 - #sc.caption * 6, H - 45, 1.6)
    end

    luna.graphics.setColor(0.5, 0.6, 0.7, 0.65)
    luna.graphics.print("[A/D] Walk  [Space/W] Jump  [X] Shoot  [Z] Shield  Defeat enemy→reach exit", 8, H - 20, 1.2)
    ...
    if game_state == "gameover" then
        ...
        luna.graphics.setColor(0.4, 0.8, 1)
        luna.graphics.print("YOU ARE DEAD", W/2 - 92, H/2 - 25, 3)
        luna.graphics.setColor(1, 0.9, 0.5)

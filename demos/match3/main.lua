...
function luna.draw()
    luna.graphics.setBackgroundColor(0.12, 0.1, 0.18)

    -- Title and score
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("MATCH-3", GRID_X, 10, 2)
    luna.graphics.print("Score: " .. score, GRID_X + 250, 15, 1.5)
    luna.graphics.print("Moves: " .. moves, GRID_X + 250, 38)
    if combo > 1 then
        luna.graphics.setColor(1, 0.8, 0.1, 1)
        luna.graphics.print("COMBO x" .. combo, GRID_X + 420, 15, 1.5)
    end

    -- Grid background
    luna.graphics.setColor(0.18, 0.16, 0.22, 1)
    luna.graphics.rectangle("fill", GRID_X - 4, GRID_Y - 4, GRID_SIZE * CELL + 8, GRID_SIZE * CELL + 8)
    ...
    for r = 1, GRID_SIZE do
        for c = 1, GRID_SIZE do
            ...
            if g then
                ...
                -- Cell background
                luna.graphics.setColor(0.14, 0.12, 0.18, 1)
                luna.graphics.rectangle("fill", x, y, CELL, CELL)

                -- Gem
                local clr = gem_colors[g.color]
                luna.graphics.setColor(clr[1], clr[2], clr[3], 1)
                luna.graphics.circle("fill", x + CELL / 2, y + CELL / 2, CELL / 2 - pad)

                -- Highlight for specials
                if g.special == "bomb" then
                    luna.graphics.setColor(1, 1, 1, 0.5)
                    luna.graphics.circle("fill", x + CELL / 2, y + CELL / 2, 8)
                elseif g.special == "wiper" then
                    luna.graphics.setColor(1, 1, 1, 0.4)
                    luna.graphics.circle("line", x + CELL / 2, y + CELL / 2, CELL / 2 - pad - 3)
                    luna.graphics.circle("line", x + CELL / 2, y + CELL / 2, CELL / 2 - pad - 6)
                end

                -- Shine
                luna.graphics.setColor(1, 1, 1, 0.15)
                luna.graphics.circle("fill", x + CELL / 2 - 6, y + CELL / 2 - 8, 6)
            end
        end
    end
    ...
    if selected then
        ...
        luna.graphics.rectangle("line", sx, sy, CELL, CELL)
        luna.graphics.setLineWidth(1)
    end

    -- Controls
    luna.graphics.setColor(0.5, 0.5, 0.5, 1)
    luna.graphics.print("Click gems to swap | R: restart | ESC: quit", GRID_X, SCREEN_H - 28)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), SCREEN_W - 90, 10)
end

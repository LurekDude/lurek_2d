...
- `luna.graphics.rectangle()` — drawing the grid cells, UI panels, and gem backgrounds
- `luna.graphics.circle()` — rendering circular gem shapes with highlight rings
- `luna.graphics.print()` — score, combo counter, and move display
- `luna.mousepressed()` — selecting and swapping adjacent gems
- `luna.graphics.setColor()` — six distinct gem colours plus special-gem overlays
- `luna.timer.getTime()` — swap and fall animation timing

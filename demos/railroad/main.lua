...
function luna.load()
    luna.graphics.setBackgroundColor(0.15, 0.2, 0.15)
    initGrid()
...
local function drawTrack(col, row, t)
    ...
    if t == 1 then
        luna.graphics.line(x, cy, x + TILE, cy)
    elseif t == 2 then
        luna.graphics.line(cx, y, cx, y + TILE)
    elseif t == 3 then
        luna.graphics.line(cx, cy, x + TILE, cy)
        luna.graphics.line(cx, cy, cx, y)
    elseif t == 4 then
        luna.graphics.line(cx, cy, x + TILE, cy)
        luna.graphics.line(cx, cy, cx, y + TILE)
    elseif t == 5 then
        luna.graphics.line(cx, cy, x, cy)
        luna.graphics.line(cx, cy, cx, y + TILE)
    elseif t == 6 then
        luna.graphics.line(cx, cy, x, cy)
        luna.graphics.line(cx, cy, cx, y)
...
function luna.draw()
    -- Grid
    luna.graphics.setColor(0.2, 0.25, 0.2, 1)
    for r = 1, ROWS do
        for c = 1, COLS do
            luna.graphics.rectangle("line", (c - 1) * TILE, (r - 1) * TILE, TILE, TILE)
        end
    end
    ...
    for _, st in ipairs(stations) do
        ...
        luna.graphics.rectangle("fill", x + 2, y + 2, TILE - 4, TILE - 4)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(st.name, x, y - 14, 0.8)
        luna.graphics.print("Stock:" .. st.stock, x, y + TILE + 2, 0.7)
    end
    ...
    for _, tr in ipairs(trains) do
        ...
        luna.graphics.setColor(clr[1], clr[2], clr[3], 1)
        luna.graphics.rectangle("fill", tr.px - 8, tr.py - 8, 16, 16)
        luna.graphics.setColor(0, 0, 0, 1)
        luna.graphics.rectangle("line", tr.px - 8, tr.py - 8, 16, 16)
    end

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.7)
    luna.graphics.rectangle("fill", 0, H - 60, W, 60)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Day " .. day .. "  Revenue: $" .. revenue .. "  Track: " .. TRACK_NAMES[selectedTrack] .. " (1-6/Tab)", 10, H - 55, 1)
    luna.graphics.print("Left-click: place track | Right-click: cycle/remove | Trains auto-spawn from stations", 10, H - 35, 0.8)
    ...
    -- Message
    if msgTimer > 0 then
        luna.graphics.setColor(0, 1, 0.5, msgTimer / 2)
        luna.graphics.print(message, W / 2 - 100, 20, 1)
    end
    ...
    if hc >= 1 and hc <= COLS and hr >= 1 and hr <= ROWS and grid[hr] and grid[hr][hc] == 0 then
        ...
        luna.graphics.rectangle("fill", (hc - 1) * TILE, (hr - 1) * TILE, TILE, TILE)
    end
end

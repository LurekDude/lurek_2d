...
function luna.draw()
    ...
    for y = sy + 1, ey do
        for x = sx + 1, ex do
            if biomes[y] and biomes[y][x] then
                ...
                luna.graphics.setColor(c[1]*visibility, c[2]*visibility, c[3]*visibility, 1)
                luna.graphics.rectangle("fill", (x-1)*TILE - ox, (y-1)*TILE - oy, TILE, TILE)
            end
        end
    end
    ...
    for _, a in ipairs(animals) do
        ...
        if ax > -20 and ax < W + 20 and ay > -20 and ay < H + 20 then
            ...
            luna.graphics.setColor(c[1]*visibility, c[2]*visibility, c[3]*visibility, alpha)
            luna.graphics.circle("fill", ax, ay, a.species.size)
            -- state indicator
            if a.state == "sleep" then
                luna.graphics.setColor(1,1,1,0.5*visibility)
                luna.graphics.print("z", ax+6, ay-10, 0.7)
            elseif a.state == "flee" then
                luna.graphics.setColor(1,0.3,0.3,0.7)
                luna.graphics.print("!", ax+6, ay-10, 0.7)
            end
        end
    end
    ...
    luna.graphics.setColor(0.9, 0.85, 0.3, 1)
    luna.graphics.circle("fill", px, py, pr)
    luna.graphics.setColor(0.2, 0.2, 0.2, 1)
    luna.graphics.circle("line", px, py, pr)

    -- camera mode crosshair
    if camera_mode then
        local mx, my = luna.mouse.getPosition()
        luna.graphics.setColor(1, 1, 1, 0.8)
        luna.graphics.line(mx - 15, my, mx + 15, my)
        luna.graphics.line(mx, my - 15, mx, my + 15)
        luna.graphics.circle("line", mx, my, 25)
        luna.graphics.setColor(1,0.3,0.3,0.6)
        luna.graphics.print("CAMERA", mx + 20, my - 8, 0.8)
    end

    -- night overlay
    if visibility < 0.9 then
        luna.graphics.setColor(0, 0, 0.05, 1 - visibility)
    ...
    luna.graphics.print("Photos: " .. #photos .. "  Best: " .. best_score
        .. "  Species: " .. discovered .. "/" .. #SPECIES, 10, 5, 0.9)
    luna.graphics.print(camera_mode and "[CAMERA ON]" or "[C] Camera", 500, 5, 0.9)
    luna.graphics.print(player.crouching and "Crouching" or "Standing", 630, 5, 0.9)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), 740, 5, 0.7)

    -- noise meter
    luna.graphics.setColor(0.3,0.3,0.3,0.7)
    luna.graphics.rectangle("fill", 10, H-22, 80, 12)
    local np = noise_level / 100
    luna.graphics.setColor(np, 1-np, 0, 0.8)
    luna.graphics.rectangle("fill", 10, H-22, 80*np, 12)
    luna.graphics.setColor(1,1,1,0.7)
    luna.graphics.print("Noise", 14, H-24, 0.7)

    -- photo log (last 5)
    if #photos > 0 then
        luna.graphics.setColor(0,0,0,0.5)
        luna.graphics.rectangle("fill", W-160, H-110, 155, 105)
        luna.graphics.setColor(1,1,1,0.9)
        luna.graphics.print("Recent Photos:", W-155, H-108, 0.7)
        local start = clamp(#photos - 4, 1, #photos)
        for i = start, #photos do
            local p = photos[i]
            local yi = (i - start) * 16 + (H - 92)
            luna.graphics.print(p.species .. ": " .. p.score .. "pts", W-150, yi, 0.7)
        end
    end

    -- message
    if message and msg_timer > 0 then
        luna.graphics.setColor(1,1,0.4,1)
        luna.graphics.print(message, 200, 50, 1.1)
    end

    luna.graphics.setColor(0.6,0.6,0.6,0.7)
    luna.graphics.print("[WASD] Move  [Shift] Crouch  [C] Camera  [Space] Photo  [R] Restart", 100, H-14, 0.65)
end

...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.1, 0.12, 0.1)
...
function luna.draw()
    -- hide spots
    for _, hs in ipairs(hideSpots) do
        luna.graphics.setColor(0.05, 0.08, 0.05, 1)
        luna.graphics.rectangle("fill", hs.x, hs.y, hs.w, hs.h)
        luna.graphics.setColor(0.2, 0.3, 0.2, 1)
        luna.graphics.rectangle("line", hs.x, hs.y, hs.w, hs.h)
    end

    -- walls
    for _, w in ipairs(walls) do
        luna.graphics.setColor(0.35, 0.3, 0.25, 1)
        luna.graphics.rectangle("fill", w.x, w.y, w.w, w.h)
    end

    -- exit zone
    luna.graphics.setColor(0.2, 0.8, 0.2, 0.5)
    luna.graphics.rectangle("fill", exitZone.x, exitZone.y, exitZone.w, exitZone.h)
    luna.graphics.setColor(0.2, 1, 0.2, 1)
    luna.graphics.rectangle("line", exitZone.x, exitZone.y, exitZone.w, exitZone.h)
    luna.graphics.print("EXIT", exitZone.x + 8, exitZone.y + 20)
    ...
    for _, g in ipairs(guards) do
        ...
        luna.graphics.setColor(r, gr, b, a)
        for i = 0, segments - 1 do
            ...
            luna.graphics.polygon("fill", verts)
        end
    end

    -- noise ripples
    for _, nr in ipairs(noiseRipples) do
        luna.graphics.setColor(1, 1, 0.5, nr.alpha * 0.3)
        luna.graphics.circle("line", nr.x, nr.y, nr.r)
    end

    -- guards
    for _, g in ipairs(guards) do
        if g.state == "chase" then
            luna.graphics.setColor(1, 0.1, 0.1, 1)
        elseif g.suspicion > 0.5 then
            luna.graphics.setColor(1, 0.6, 0.1, 1)
        else
            luna.graphics.setColor(0.8, 0.8, 0.2, 1)
        end
        luna.graphics.circle("fill", g.x, g.y, 10)
        -- direction indicator
        luna.graphics.setColor(1, 1, 1, 0.8)
        luna.graphics.line(g.x, g.y, g.x + math.cos(g.angle) * 14, g.y + math.sin(g.angle) * 14)
    end

    -- player
    local pa = player.hidden and 0.4 or 1
    local pr = player.crouching and 5 or player.r
    luna.graphics.setColor(0.2, 0.6, 1, pa)
    luna.graphics.circle("fill", player.x, player.y, pr)
    if player.crouching then
        luna.graphics.setColor(0.4, 0.8, 1, 0.3)
        luna.graphics.circle("line", player.x, player.y, 12)
    end

    -- HUD
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print(player.crouching and "CROUCHING" or "STANDING", 10, 10)
    if player.hidden then
        luna.graphics.setColor(0.3, 1, 0.3, 1)
        luna.graphics.print("HIDDEN", 10, 30)
    end
    luna.graphics.setColor(1, 1, 1, 0.5)
    luna.graphics.print("WASD: Move  |  LShift: Crouch  |  R: Reset", 10, 575)

    -- game over / win
    if player.caught then
        luna.graphics.setColor(0, 0, 0, 0.7)
        luna.graphics.rectangle("fill", 250, 250, 300, 80)
        luna.graphics.setColor(1, 0.2, 0.2, 1)
        luna.graphics.print("CAUGHT! Press R to retry", 290, 280, 1.2)
    end
    if player.won then
        luna.graphics.setColor(0, 0, 0, 0.7)
        luna.graphics.rectangle("fill", 250, 250, 300, 80)
        luna.graphics.setColor(0.2, 1, 0.2, 1)
        luna.graphics.print("ESCAPED! Press R to replay", 285, 280, 1.2)
    end
end

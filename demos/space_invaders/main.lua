...
function luna.draw()
    -- Stars background
    math.randomseed(42)
    luna.graphics.setColor(0.9, 0.9, 0.9, 0.4)
    for i = 1, 80 do
    ...
    -- HUD
    luna.graphics.setColor(0.8, 1, 0.8)
    luna.graphics.print("SCORE: " .. score, 10, 8, 1.5)
    luna.graphics.print("WAVE: " .. wave, W/2 - 40, 8, 1.5)
    luna.graphics.setColor(1, 0.3, 0.3)
    luna.graphics.print("LIVES: " .. lives, W - 100, 8, 1.5)

    -- Ground line
    luna.graphics.setColor(0.4, 0.8, 0.4)
    luna.graphics.line(0, H - 30, W, H - 30)

    -- Barriers
    for _, blk in ipairs(barriers) do
        if blk.hp > 0 then
            local alpha = blk.hp / 3
            luna.graphics.setColor(0.2, 0.9, 0.2, alpha)
            luna.graphics.rectangle("fill", blk.x, blk.y, BARRIER_CELL, BARRIER_CELL)
        end
    end

    -- Player ship (simple polygon)
    luna.graphics.setColor(0.4, 0.8, 1.0)
    luna.graphics.rectangle("fill", player.x + player.w/2 - 4, player.y - 10, 8, 10)
    ...
    for _, inv in ipairs(invaders) do
        if inv.alive then
            ...
            luna.graphics.setColor(0.2 + row_hue * 0.8, 1.0 - row_hue * 0.5, 0.2)
            local ix = inv.x + (frame == 1 and 3 or 0)
            -- Body
            luna.graphics.rectangle("fill", ix + 5, inv.y + 4, INVADER_W - 10, INVADER_H - 10)
            -- Head bumps
            luna.graphics.rectangle("fill", ix + 8, inv.y, INVADER_W - 16, 8)
            -- Legs (alternate frame)
            if frame == 0 then
                luna.graphics.rectangle("fill", ix + 2, inv.y + INVADER_H - 8, 6, 8)
                luna.graphics.rectangle("fill", ix + INVADER_W - 8, inv.y + INVADER_H - 8, 6, 8)
            else
                luna.graphics.rectangle("fill", ix + 6, inv.y + INVADER_H - 8, 6, 8)
                luna.graphics.rectangle("fill", ix + INVADER_W - 12, inv.y + INVADER_H - 8, 6, 8)
            end
            -- Eyes
            luna.graphics.setColor(0, 0, 0)
            luna.graphics.circle("fill", ix + INVADER_W/2 - 5, inv.y + 8, 3)
            luna.graphics.circle("fill", ix + INVADER_W/2 + 5, inv.y + 8, 3)
        end
    end

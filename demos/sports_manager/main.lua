...
function luna.keypressed(key)
    if key == "escape" then
        if state == "menu" then luna.event.quit()
...
function luna.draw()
    luna.graphics.setBackgroundColor(0.06, 0.08, 0.12)

    -- Header
    luna.graphics.setColor(0.3, 0.8, 0.4, 1)
    luna.graphics.print(team_name .. " — Sports Manager", 10, 5, 1.2)
    luna.graphics.setColor(0.8, 0.8, 0.6, 1)
    luna.graphics.print("Budget: " .. budget .. "  |  Match " .. clamp(season_match, 1, TOTAL_MATCHES) .. "/" .. TOTAL_MATCHES .. "  |  Formation: " .. formation, 10, 28, 0.75)

    local Y = 55

    if state == "menu" then
        luna.graphics.setColor(1, 0.9, 0.5, 1)
        luna.graphics.print("Main Menu", 30, Y, 1.2)
        for i, item in ipairs(menu_items) do
            local iy = Y + 30 + (i - 1) * 28
            if i == selected then
                luna.graphics.setColor(0.2, 0.3, 0.5, 1)
                luna.graphics.rectangle("fill", 25, iy - 2, 300, 24)
                luna.graphics.setColor(1, 1, 0.6, 1)
            else
                luna.graphics.setColor(0.8, 0.8, 0.8, 1)
            end
            luna.graphics.print("> " .. item, 35, iy, 1)
        end
        luna.graphics.setColor(0.5, 0.5, 0.5, 1)
        luna.graphics.print("Up/Down=select  Enter=confirm  Esc=back/quit", 30, 400, 0.7)

    elseif state == "roster" then
        luna.graphics.setColor(1, 0.9, 0.5, 1)
        luna.graphics.print("Roster  (OVR: " .. team_overall(players) .. ")", 30, Y, 1)
        luna.graphics.setColor(0.6, 0.6, 0.6, 1)
        luna.graphics.print("#   Name               SPD  PWR  STA  SKL  MOR  Status", 30, Y + 25, 0.65)
        for i, p in ipairs(players) do
            local py = Y + 42 + (i - 1) * 20
            if i == selected then
                luna.graphics.setColor(0.2, 0.3, 0.5, 1)
                luna.graphics.rectangle("fill", 25, py - 1, 720, 18)
            end
            ...
            luna.graphics.print(line, 30, py, 0.6)
        end

    elseif state == "formation" then
        luna.graphics.setColor(1, 0.9, 0.5, 1)
        luna.graphics.print("Select Formation", 30, Y, 1.2)
        for i, f in ipairs(formations) do
            local fy = Y + 40 + (i - 1) * 35
            if i == form_idx then
                luna.graphics.setColor(0.2, 0.4, 0.6, 1)
                luna.graphics.rectangle("fill", 30, fy - 2, 200, 28)
                luna.graphics.setColor(1, 1, 0.6, 1)
            else
                luna.graphics.setColor(0.7, 0.7, 0.7, 1)
            end
            luna.graphics.print(f, 40, fy, 1.2)
        end
        luna.graphics.setColor(0.5, 0.5, 0.5, 1)
        luna.graphics.print("Up/Down=select  Enter=confirm", 30, 250, 0.75)

    elseif state == "training" then
        luna.graphics.setColor(1, 0.9, 0.5, 1)
        luna.graphics.print("Training (10 budget per session)", 30, Y, 1)
        for i, p in ipairs(players) do
            local py = Y + 30 + (i - 1) * 20
            if i == selected then
                luna.graphics.setColor(0.2, 0.3, 0.5, 1)
                luna.graphics.rectangle("fill", 25, py - 1, 500, 18)
                luna.graphics.setColor(1, 1, 0.6, 1)
            else
                luna.graphics.setColor(0.7, 0.8, 0.7, 1)
            end
            luna.graphics.print(string.format("%-18s SPD:%d PWR:%d STA:%d SKL:%d", p.name, p.speed, p.power, p.stamina, p.skill), 30, py, 0.65)
        end
        luna.graphics.setColor(0.5, 0.5, 0.5, 1)
        luna.graphics.print("Enter=train selected player (random stat +1-5)", 30, 400, 0.7)

    elseif state == "transfer" then
        luna.graphics.setColor(1, 0.9, 0.5, 1)
        luna.graphics.print("Transfer Market", 30, Y, 1)
        if #transfer_list == 0 then
            luna.graphics.setColor(0.7, 0.7, 0.7, 1)
            luna.graphics.print("No players available.", 30, Y + 30, 0.9)
        end
        for i, p in ipairs(transfer_list) do
            local py = Y + 30 + (i - 1) * 22
            if i == selected then
                luna.graphics.setColor(0.2, 0.3, 0.5, 1)
                luna.graphics.rectangle("fill", 25, py - 1, 600, 20)
                luna.graphics.setColor(1, 1, 0.6, 1)
            else
                luna.graphics.setColor(0.7, 0.7, 0.8, 1)
            end
            luna.graphics.print(string.format("%-18s SPD:%d PWR:%d STA:%d SKL:%d  Cost:%d", p.name, p.speed, p.power, p.stamina, p.skill, p.value), 30, py, 0.65)
        end
        luna.graphics.setColor(0.5, 0.5, 0.5, 1)
        luna.graphics.print("Enter=buy  S=sell last player  Esc=back", 30, 400, 0.7)

    elseif state == "match_log" then
        luna.graphics.setColor(1, 0.9, 0.5, 1)
        luna.graphics.print("Match Result: " .. match_result, 30, Y, 1.2)
        local max_lines = 20
        for i = 1, max_lines do
            local idx = i + scroll
            if idx <= #match_log then
                luna.graphics.setColor(0.8, 0.85, 0.9, 1)
                luna.graphics.print(match_log[idx], 30, Y + 25 + (i - 1) * 18, 0.65)
            end
        end
        luna.graphics.setColor(0.5, 0.5, 0.5, 1)
        luna.graphics.print("Up/Down=scroll  Enter=continue", 30, 430, 0.7)

    elseif state == "table" then
        luna.graphics.setColor(1, 0.9, 0.5, 1)
        luna.graphics.print("League Table", 30, Y, 1.2)
        -- Sort by points
        table.sort(league, function(a, b) return a.pts > b.pts end)
        luna.graphics.setColor(0.6, 0.6, 0.6, 1)
        luna.graphics.print("#   Team               W   D   L   GF  GA  PTS", 30, Y + 30, 0.7)
        for i, e in ipairs(league) do
            local ey = Y + 50 + (i - 1) * 22
            if e.name == team_name then luna.graphics.setColor(0.4, 1, 0.5, 1)
            else luna.graphics.setColor(0.8, 0.8, 0.8, 1) end
            luna.graphics.print(string.format("%-3d %-18s %2d  %2d  %2d  %2d  %2d  %3d", i, e.name, e.w, e.d, e.l, e.gf, e.ga, e.pts), 30, ey, 0.65)
        end
        luna.graphics.setColor(0.5, 0.5, 0.5, 1)
        luna.graphics.print("Enter=back", 30, 400, 0.7)

    elseif state == "season_end" then
        table.sort(league, function(a, b) return a.pts > b.pts end)
        luna.graphics.setColor(1, 0.85, 0.3, 1)
        luna.graphics.print("Season Complete!", 200, 100, 2)
        local pos = 1
        for i, e in ipairs(league) do
            if e.name == team_name then pos = i; break end
        end
        luna.graphics.setColor(0.9, 0.9, 0.9, 1)
        luna.graphics.print(team_name .. " finished in position #" .. pos, 180, 200, 1.2)
        luna.graphics.print("W:" .. league[1].w .. " D:" .. league[1].d .. " L:" .. league[1].l, 250, 240, 1)
        luna.graphics.setColor(0.5, 1, 0.5, 1)
        luna.graphics.print("Press ENTER", 300, 340, 1)
    end

    luna.graphics.setColor(0.4, 0.4, 0.4, 1)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), 730, 5, 0.6)
end

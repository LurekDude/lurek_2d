...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.08, 0.05, 0.15)
    resetRound()
end
...
local function drawFighter(f, r, g, b)
    ...
    luna.graphics.rectangle("fill", bx, by, f.w, f.h)

    -- blocking indicator
    if f.blocking then
        luna.graphics.setColor(0.5, 0.5, 1, 0.4)
        luna.graphics.rectangle("fill", bx - 4, by - 4, f.w + 8, f.h + 8)
    end
    ...
    if f.attackBox and f.stateTimer > 0 then
        ...
        luna.graphics.rectangle("fill", ax, ay, ab.w, ab.h)
    end
end

local function drawHealthBar(x, y, w, hp, maxHp, r, g, b)
    luna.graphics.setColor(0.2, 0.2, 0.2, 1)
    luna.graphics.rectangle("fill", x, y, w, 18)
    local ratio = hp / maxHp
    luna.graphics.setColor(r, g, b, 1)
    luna.graphics.rectangle("fill", x, y, w * ratio, 18)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.rectangle("line", x, y, w, 18)
end

local function drawSuperBar(x, y, w, s, maxS)
    luna.graphics.setColor(0.15, 0.15, 0.15, 1)
    luna.graphics.rectangle("fill", x, y, w, 8)
    local ratio = s / maxS
    luna.graphics.setColor(1, 1, 0, 1)
    luna.graphics.rectangle("fill", x, y, w * ratio, 8)
end

function luna.draw()
    -- stage floor
    luna.graphics.setColor(0.25, 0.2, 0.3, 1)
    luna.graphics.rectangle("fill", 0, stageFloor + shakeY, 800, 80)
    ...
    luna.graphics.setColor(1, 1, 1, 1)
    for i = 1, P1.wins do
        luna.graphics.circle("fill", 340 - i * 20, 28, 6)
    end
    for i = 1, P2.wins do
        luna.graphics.circle("fill", 460 + i * 20, 28, 6)
    end

    -- labels
    luna.graphics.setColor(0.5, 0.7, 1, 1)
    luna.graphics.print("P1", 30, 5)
    luna.graphics.setColor(1, 0.5, 0.4, 1)
    luna.graphics.print("AI", 740, 5)
    luna.graphics.setColor(1, 1, 1, 0.6)
    luna.graphics.print("VS", 390, 5)

    -- combo texts
    for _, ct in ipairs(comboTexts) do
        luna.graphics.setColor(1, 1, 0, clamp(ct.timer, 0, 1))
        luna.graphics.print(ct.text, ct.x - 20 + shakeX, ct.y + shakeY, 1.3)
    end

    -- HUD
    luna.graphics.setColor(1, 1, 1, 0.5)
    luna.graphics.print("WASD: Move | F: Punch | G: Kick | H: Block | V: Super (when full)", 120, 575)
    ...
    if roundDelay > 0 then
        ...
        luna.graphics.setColor(1, 1, 1, 1)
        if P1.wins >= roundsToWin then
            luna.graphics.print("P1 WINS THE MATCH!", 300, 270, 1.3)
            luna.graphics.print("Press R to rematch", 320, 300)
        elseif P2.wins >= roundsToWin then
            luna.graphics.print("AI WINS THE MATCH!", 300, 270, 1.3)
            luna.graphics.print("Press R to rematch", 320, 300)
        else
            luna.graphics.print("ROUND " .. (P1.wins + P2.wins + 1), 340, 275, 1.5)
        end
    end
end

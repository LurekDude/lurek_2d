...
function luna.draw()
    luna.graphics.setBackgroundColor(0.1, 0.12, 0.08)

    -- UI header
    luna.graphics.setColor(1, 0.9, 0.4, 1)
    luna.graphics.print("Zoo Tycoon", 10, 5, 1.2)
    luna.graphics.setColor(1, 1, 0.6, 1)
    luna.graphics.print("Gold: " .. gold, 160, 8, 0.9)
    luna.graphics.print("Day: " .. day, 300, 8, 0.9)
    luna.graphics.print("Guests: " .. #guests, 400, 8, 0.9)
    luna.graphics.print("Happy: " .. math.floor(happiness) .. "%", 520, 8, 0.9)

    -- Tool bar
    luna.graphics.setColor(0.7, 0.7, 0.6, 1)
    luna.graphics.print("Tool: " .. selected_tool, 10, 30, 0.75)
    luna.graphics.print("Tab=cycle | 1-4=animals | Click=place", 150, 30, 0.65)
    ...
    for y = 1, GRID_H do
        for x = 1, GRID_W do
            ...
            luna.graphics.rectangle("fill", OX + (x - 1) * CELL, OY + (y - 1) * CELL, CELL - 1, CELL - 1)
        end
    end

    -- Gate label
    luna.graphics.setColor(0, 0, 0, 1)
    luna.graphics.print("G", OX + 6, OY + (GRID_H - 1) * CELL + 6, 0.7)
    ...
    for _, a in ipairs(animals) do
        ...
        if a.happy then
            ...
            luna.graphics.setColor(0.8, 0.2, 0.2, 0.4)
        end
        luna.graphics.rectangle("fill", ax, ay, CELL - 3, CELL - 3)
        -- Icon
        luna.graphics.setColor(at.color[1], at.color[2], at.color[3], 1)
        luna.graphics.print(at.icon, ax + 6, ay + 4, 1)
    end

    -- Guests
    luna.graphics.setColor(1, 0.9, 0.7, 1)
    for _, g in ipairs(guests) do
        luna.graphics.circle("fill", g.x, g.y, 4)
    end

    -- Day timer bar
    luna.graphics.setColor(0.2, 0.2, 0.2, 1)
    luna.graphics.rectangle("fill", OX, OY + GRID_H * CELL + 5, GRID_W * CELL, 8)
    luna.graphics.setColor(0.4, 0.7, 1, 1)
    luna.graphics.rectangle("fill", OX, OY + GRID_H * CELL + 5, GRID_W * CELL * (day_timer / DAY_LENGTH), 8)

    -- Legend
    local lx = OX + GRID_W * CELL + 10
    luna.graphics.setColor(0.9, 0.85, 0.6, 1)
    luna.graphics.print("Legend:", lx, OY, 0.8)
    ...
    for i, l in ipairs(legend) do
        luna.graphics.setColor(l[2], l[3], l[4], 1)
        luna.graphics.rectangle("fill", lx, OY + 18 + (i - 1) * 20, 12, 12)
        luna.graphics.setColor(0.8, 0.8, 0.8, 1)
        luna.graphics.print(l[1], lx + 16, OY + 16 + (i - 1) * 20, 0.65)
    end

    -- Animal key
    luna.graphics.setColor(0.9, 0.85, 0.6, 1)
    luna.graphics.print("Animals:", lx, OY + 150, 0.8)
    for i, at in ipairs(animal_types) do
        luna.graphics.setColor(at.color[1], at.color[2], at.color[3], 1)
        luna.graphics.print(i .. "=" .. at.name, lx, OY + 170 + (i - 1) * 18, 0.65)
    end

    -- Message
    if msg_timer > 0 then
        luna.graphics.setColor(1, 1, 0.7, clamp(msg_timer, 0, 1))
        luna.graphics.print(message, OX, OY + GRID_H * CELL + 20, 0.85)
    end

    -- Day report overlay
    if show_report then
        luna.graphics.setColor(0, 0, 0, 0.75)
        luna.graphics.rectangle("fill", 100, 100, 400, 220)
        luna.graphics.setColor(1, 0.9, 0.4, 1)
        luna.graphics.print(last_report, 120, 120, 1)
        luna.graphics.setColor(0.5, 1, 0.5, 1)
        luna.graphics.print("Press ENTER to continue", 180, 290, 0.9)
    end

...
local function drawCard(c, x, y, hovered)
    ...
    luna.graphics.rectangle("fill", x, y, CARD_W, CARD_H)
    luna.graphics.setColor(0.9, 0.9, 0.8, 1)
    luna.graphics.rectangle("line", x, y, CARD_W, CARD_H)
    -- Name
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print(c.name, x + 4, y + 6)
    -- Cost
    luna.graphics.setColor(0.3, 0.5, 1, 1)
    luna.graphics.circle("fill", x + CARD_W - 14, y + 14, 12)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print(c.cost, x + CARD_W - 18, y + 7)
    -- Stats
    luna.graphics.setColor(1, 0.9, 0.7, 1)
    if c.kind == "attack" or c.kind == "drain" then
        luna.graphics.print("ATK:" .. c.atk, x + 6, y + 50)
    end
    luna.graphics.setColor(0.7, 0.7, 0.7, 1)
    luna.graphics.print(c.kind, x + 6, y + 80)
end
...
function luna.draw()
    ...
    -- Enemy area
    luna.graphics.setColor(0.15, 0.1, 0.1, 1)
    luna.graphics.rectangle("fill", 50, 20, 700, 80)
    luna.graphics.setColor(0.9, 0.3, 0.3, 1)
    luna.graphics.print("ENEMY", 60, 26)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("HP: " .. enemy.hp .. "/" .. enemy.maxHp, 60, 46)
    luna.graphics.print("Mana: " .. enemy.mana .. "/" .. enemy.maxMana, 60, 66)
    luna.graphics.print("Shield: " .. enemy.shield, 200, 46)
    luna.graphics.print("Cards: " .. #enemy.hand, 200, 66)
    -- Enemy HP bar
    luna.graphics.setColor(0.3, 0.1, 0.1, 1)
    luna.graphics.rectangle("fill", 340, 40, 200, 16)
    luna.graphics.setColor(0.9, 0.2, 0.2, 1)
    luna.graphics.rectangle("fill", 340, 40, 200 * (enemy.hp / enemy.maxHp), 16)

    -- Player area
    luna.graphics.setColor(0.1, 0.1, 0.15, 1)
    luna.graphics.rectangle("fill", 50, 110, 700, 80)
    luna.graphics.setColor(0.3, 0.5, 1, 1)
    luna.graphics.print("YOU", 60, 116)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("HP: " .. player.hp .. "/" .. player.maxHp, 60, 136)
    luna.graphics.print("Mana: " .. player.mana .. "/" .. player.maxMana, 60, 156)
    luna.graphics.print("Shield: " .. player.shield, 200, 136)
    luna.graphics.print("Deck: " .. #player.deck, 200, 156)
    -- Player HP bar
    luna.graphics.setColor(0.1, 0.1, 0.3, 1)
    luna.graphics.rectangle("fill", 340, 130, 200, 16)
    luna.graphics.setColor(0.2, 0.5, 1, 1)
    luna.graphics.rectangle("fill", 340, 130, 200 * (player.hp / player.maxHp), 16)

    -- Log
    luna.graphics.setColor(0, 0, 0, 0.5)
    luna.graphics.rectangle("fill", 50, 200, 300, 120)
    for i, msg in ipairs(log) do
        luna.graphics.setColor(0.8, 0.8, 0.7, clamp(1.2 - i * 0.15, 0.3, 1))
        luna.graphics.print(msg, 58, 200 + (i - 1) * 18)
    end
    ...
    for i, c in ipairs(player.hand) do
        ...
        if not canPlay then
            luna.graphics.setColor(0.4, 0.4, 0.4, 0.4)
    ...
    -- Phase info
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Turn " .. turnNum .. "  [Enter] End Turn", 400, 200)

    if gameOver then
        luna.graphics.setColor(0, 0, 0, 0.6)
        luna.graphics.rectangle("fill", 200, 250, 400, 80)
        luna.graphics.setColor(1, 1, 0.3, 1)
        local msg = enemy.hp <= 0 and "YOU WIN!" or "YOU LOSE!"
        luna.graphics.print(msg, 330, 270, 2)
    end
end

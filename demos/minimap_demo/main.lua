...
function luna.draw()
    ...
    luna.graphics.print("Game World", 10, 10)
    luna.graphics.print(string.format("Player: %.0f, %.0f", playerX, playerY), 10, 30)

    -- Draw player indicator in game world
    local wx = playerX * 5
    local wy = playerY * 4 + 60
    luna.graphics.setColor(0, 1, 0)
    luna.graphics.circle("fill", wx, wy, 6)

    -- Draw minimap background (top-right corner)
    local mx, my = 580, 10
    luna.graphics.setColor(0.1, 0.1, 0.1, 0.8)
    luna.graphics.rectangle("fill", mx - 2, my - 2, 204, 204)

    -- Draw minimap border
    luna.graphics.setColor(0.5, 0.5, 0.5)
    luna.graphics.rectangle("line", mx - 2, my - 2, 204, 204)

    -- Draw HUD text
    luna.graphics.setColor(1, 1, 1, 0.8)
    luna.graphics.print("Controls:", 10, 520)
    luna.graphics.print("Arrows=Move  P=Ping  M=Mode  F=Fog  Z=Zoom", 10, 540)
    luna.graphics.print(string.format("Mode: %s | Fog: %s | Zoom: %.1fx | Pings: %d",
        minimap:getColorMode(),
        minimap:isFogEnabled() and "ON" or "OFF",
        minimap:getZoom(),
        minimap:getPingCount()),
        10, 560)
    luna.graphics.print(string.format("Objects: %d | Markers: %d",

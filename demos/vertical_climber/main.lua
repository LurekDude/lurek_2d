...
function luna.draw()
    luna.graphics.setBackgroundColor(0.12, 0.12, 0.2)
    ...
    for _, p in ipairs(platforms) do
        if p.alive then
            ...
            luna.graphics.rectangle("fill", p.x, p.y - camera_y, PLAT_W, PLAT_H)
        end
    end
    ...
    for _, s in ipairs(springs) do
        if s.plat.alive then
            ...
            luna.graphics.rectangle("fill", s.x, s.y - camera_y, s.w, s.h)
        end
    end
    ...
    for _, e in ipairs(enemies) do
        ...
        luna.graphics.rectangle("fill", e.x, e.y - camera_y, e.w, e.h)
        -- Eyes
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.circle("fill", e.x + 5, e.y + 6 - camera_y, 3)
        luna.graphics.circle("fill", e.x + 15, e.y + 6 - camera_y, 3)
    end

    -- Player
    luna.graphics.setColor(1, 0.85, 0.2, 1)
    luna.graphics.rectangle("fill", player.x, player.y - camera_y, PLAYER_W, PLAYER_H)
    -- Eyes
    luna.graphics.setColor(0, 0, 0, 1)
    luna.graphics.circle("fill", player.x + 7, player.y + 8 - camera_y, 3)
    luna.graphics.circle("fill", player.x + 17, player.y + 8 - camera_y, 3)

    -- HUD
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Score: " .. score, 10, 10, 1.2)
    luna.graphics.print("High: " .. high_score, 10, 32)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), SCREEN_W - 90, 10)

    if game_over then
        luna.graphics.setColor(0, 0, 0, 0.6)
        luna.graphics.rectangle("fill", 0, SCREEN_H / 2 - 60, SCREEN_W, 120)
        luna.graphics.setColor(1, 0.3, 0.3, 1)
        luna.graphics.print("GAME OVER", SCREEN_W / 2 - 80, SCREEN_H / 2 - 40, 2)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("Score: " .. score .. "  High: " .. high_score,
            SCREEN_W / 2 - 90, SCREEN_H / 2 + 10, 1.2)
        luna.graphics.print("Press SPACE to restart", SCREEN_W / 2 - 90, SCREEN_H / 2 + 40)
    end
end

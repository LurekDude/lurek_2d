...
- `luna.graphics.rectangle()` — tile map, soil plots, crop stems, and HUD panel rendering
- `luna.graphics.circle()` — ripe crop icons and growth-stage seedling indicators
- `luna.graphics.setColor()` — dynamic day/night factor applied to every colour each frame
- `luna.graphics.setBackgroundColor()` — season-tinted background that updates in `luna.draw()`
- `luna.graphics.print()` — day, season, money, seed inventory, and harvest count HUD

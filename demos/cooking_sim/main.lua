...
function luna.load()
    luna.graphics.setBackgroundColor(0.2, 0.18, 0.15)
    newOrder()
    newOrder()
end
...
local function drawStation(s)
    ...
    luna.graphics.rectangle("line", s.x, s.y, s.w, s.h)
    luna.graphics.print(s.label, s.x + 5, s.y + s.h + 4, 0.8)
end
...
function luna.draw()
    ...
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Ingredients (1-" .. #INGREDIENTS .. "):", 10, 220, 0.8)
    for i, name in ipairs(INGREDIENTS) do
        local c = INGR_COLORS[name]
        if i == shelfSelect then
            luna.graphics.setColor(1, 1, 0, 1)
            luna.graphics.print("> ", 10, 240 + (i - 1) * 18, 0.8)
        end
        luna.graphics.setColor(c[1], c[2], c[3], 1)
        luna.graphics.print(i .. ". " .. name, 25, 240 + (i - 1) * 18, 0.8)
    end
    ...
    if chopProgress > 0 then
        ...
        luna.graphics.setColor(0, 1, 0, 1)
        luna.graphics.rectangle("fill", cx, stations[2].y - 10, chopProgress * 80, 6)
    end

    -- Plate contents
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Plate:", stations[4].x, stations[4].y + stations[4].h + 20, 0.7)
    for i, item in ipairs(plate) do
        luna.graphics.print("- " .. item, stations[4].x, stations[4].y + stations[4].h + 35 + (i - 1) * 14, 0.65)
    end

    -- Chef
    luna.graphics.setColor(1, 0.9, 0.7, 1)
    luna.graphics.circle("fill", chef.x, chef.y, chef.size)
    luna.graphics.setColor(0.3, 0.3, 0.8, 1)
    luna.graphics.rectangle("fill", chef.x - 12, chef.y - 5, 24, 20)
    -- Holding indicator
    if chef.holding then
        local c = INGR_COLORS[chef.holding.name] or {1, 1, 1}
        luna.graphics.setColor(c[1], c[2], c[3], 1)
        luna.graphics.circle("fill", chef.x, chef.y - 30, 8)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(chef.holding.name .. "(" .. chef.holding.state .. ")", chef.x - 30, chef.y - 48, 0.6)
    end

    -- Orders
    luna.graphics.setColor(0, 0, 0, 0.8)
    luna.graphics.rectangle("fill", 0, 0, W, 70)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("ORDERS:", 10, 5, 1)
    for i, order in ipairs(orders) do
        local ox = 10 + (i - 1) * 195
        local urgency = order.timer < 10 and {1, 0.3, 0.3} or {1, 1, 1}
        luna.graphics.setColor(urgency[1], urgency[2], urgency[3], 1)
        luna.graphics.print(order.recipe.name .. " ($" .. order.recipe.reward .. ")", ox, 25, 0.8)
        luna.graphics.setColor(0.7, 0.7, 0.7, 1)
        local ingStr = table.concat(order.recipe.ingredients, ", ")
        luna.graphics.print(ingStr, ox, 40, 0.55)
        luna.graphics.setColor(1, 0.8, 0, 1)
        luna.graphics.print(math.floor(order.timer) .. "s", ox + 150, 25, 0.7)
    end

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.7)
    luna.graphics.rectangle("fill", 0, H - 30, W, 30)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Score: " .. score .. "  Money: $" .. money .. "  |  WASD move, Space/E interact, 1-6 select ingredient", 10, H - 25, 0.8)

    -- Message
    if msgTimer > 0 then
        luna.graphics.setColor(1, 1, 0, clamp(msgTimer, 0, 1))
        luna.graphics.print(message, W / 2 - 80, H / 2, 1.2)
    end
end

...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.4, 0.7, 0.95)
    reset_build()
end
...
function luna.draw()
    -- water
    luna.graphics.setColor(0.15, 0.3, 0.6, 1)
    luna.graphics.rectangle("fill", 0, H - 80, W, 80)

    -- cliffs
    luna.graphics.setColor(0.35, 0.25, 0.15, 1)
    luna.graphics.rectangle("fill", 0, GAP_Y, LEFT_EDGE, H - GAP_Y)
    luna.graphics.rectangle("fill", RIGHT_EDGE, GAP_Y, W - RIGHT_EDGE, H - GAP_Y)
    -- grass
    luna.graphics.setColor(0.2, 0.6, 0.2, 1)
    luna.graphics.rectangle("fill", 0, GAP_Y - 8, LEFT_EDGE, 8)
    luna.graphics.rectangle("fill", RIGHT_EDGE, GAP_Y - 8, W - RIGHT_EDGE, 8)
    ...
    for _, beam in ipairs(beams) do
        ...
        local n2 = nodes[beam.b]
        if beam.broken then
            luna.graphics.setColor(0.3, 0.3, 0.3, 0.4)
        elseif mode == "test" or mode == "result" then
            ...
            local mat = MAT[beam.mat]
            luna.graphics.setColor(mat.r, mat.g, mat.b, 1)
        end
        luna.graphics.setLineWidth(beam.mat == 2 and 4 or 3)
        luna.graphics.line(n1.x, n1.y, n2.x, n2.y)
    end
    ...
    for i, n in ipairs(nodes) do
        ...
        luna.graphics.circle("fill", n.x, n.y, 6)
    end

    -- vehicle
    if vehicle and (mode == "test" or mode == "result") then
        luna.graphics.setColor(0.8, 0.2, 0.2, 1)
        luna.graphics.rectangle("fill", vehicle.x, vehicle.y, vehicle.w, vehicle.h)
        -- wheels
        luna.graphics.setColor(0.2, 0.2, 0.2, 1)
        luna.graphics.circle("fill", vehicle.x + 8, vehicle.y + vehicle.h, 6)
        luna.graphics.circle("fill", vehicle.x + vehicle.w - 8, vehicle.y + vehicle.h, 6)
    end
    ...
    luna.graphics.print("Material: " .. mat.name .. " ($" .. mat.cost .. ")  |  Budget: $" .. budget, 10, 10, 1)
    luna.graphics.print("1/2=Material  T=Test  R=Reset", W - 280, 10, 0.9)

    if mode == "build" then
        luna.graphics.setColor(0.8, 0.8, 0.2, 0.7)
        luna.graphics.print("Click to place nodes. Click two nodes to connect. T to test.", 10, H - 25, 0.9)
    end

    if result_text then
        luna.graphics.setColor(0, 0, 0, 0.7)
        luna.graphics.rectangle("fill", W / 2 - 200, H / 2 - 30, 400, 60)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(result_text, W / 2 - 180, H / 2 - 15, 0.9)
    end
end

...
function luna.load()
    luna.graphics.setBackgroundColor(0.08, 0.18, 0.05)
...
function luna.draw()
    ...
    for _, t in ipairs(tiles) do
        ...
        luna.graphics.setColor(t.c[1], t.c[2], t.c[3])
        luna.graphics.rectangle("fill", t.x, sy, t.w, t.h)
    end

    -- Road stripe
    luna.graphics.setColor(0.4, 0.35, 0.2, 0.3)
    local roffset = world_y % 60
    for ry = -roffset, H, 60 do
        luna.graphics.rectangle("fill", W/2 - 20, ry, 40, 30)
    end
    ...
    for _, p in ipairs(pows) do
        if p.alive then
            ...
            luna.graphics.rectangle("fill", p.x, p.y, p.w, p.h)
            luna.graphics.setColor(0, 0, 0)
            luna.graphics.print("POW", p.x + 1, p.y + 6, 1.1)
        end
    end

    -- Enemies
    for _, e in ipairs(enemies) do
        luna.graphics.setColor(0.6, 0.2, 0.1)
        luna.graphics.rectangle("fill", e.x + 2, e.y + 8, e.w - 4, e.h - 8)
        luna.graphics.setColor(0.75, 0.55, 0.35)
        luna.graphics.circle("fill", e.x + e.w/2, e.y + 8, 9)
        luna.graphics.setColor(0.4, 0.2, 0)
        luna.graphics.rectangle("fill", e.x, e.y, e.w, 7)
    end

    -- Enemy bullets
    luna.graphics.setColor(1, 0.4, 0.1)
    for _, b in ipairs(bullets) do
        if b.enemy then
            luna.graphics.rectangle("fill", b.x - 2, b.y - 4, b.w, b.h)
        end
    end

    -- Player
    luna.graphics.setColor(0.2, 0.35, 0.7)
    luna.graphics.rectangle("fill", player.x + 4, player.y + 10, player.w - 8, player.h - 10)
    luna.graphics.setColor(0.85, 0.65, 0.4)
    luna.graphics.circle("fill", player.x + player.w/2, player.y + 10, 11)
    luna.graphics.setColor(0.3, 0.3, 0.1)
    luna.graphics.rectangle("fill", player.x, player.y, player.w, 8)
    -- Gun
    luna.graphics.setColor(0.4, 0.4, 0.4)
    luna.graphics.rectangle("fill", player.x + player.w/2 - 2, player.y - 10, 4, 14)

    -- Player bullets
    luna.graphics.setColor(1, 1, 0.4)
    for _, b in ipairs(bullets) do
        if not b.enemy then
    ...
    for _, g in ipairs(grenades) do
        luna.graphics.circle("fill", g.x, g.y, 8)
    end

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.55)
    luna.graphics.rectangle("fill", 0, 0, W, 28)
    luna.graphics.setColor(0.9, 0.8, 0.2)
    luna.graphics.print("COMMANDO", 8, 5, 1.8)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("Score: " .. score, W/2 - 50, 5, 1.6)
    luna.graphics.setColor(1, 0.4, 0.4)
    luna.graphics.print("Lives: " .. lives, W - 100, 5, 1.5)
    luna.graphics.setColor(0.4, 1, 0.4)
    luna.graphics.print("Grenades: " .. grenades_left, W/2 + 60, 5, 1.5)

    -- Overlay
    if game_state == "gameover" then
        luna.graphics.setColor(0, 0, 0, 0.72)
        luna.graphics.rectangle("fill", 0, 0, W, H)
        luna.graphics.setColor(1, 0.2, 0.2)
        luna.graphics.print("MISSION FAILED", W/2 - 110, H/2 - 25, 3)
        luna.graphics.setColor(1, 1, 1)

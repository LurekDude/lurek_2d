...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.05, 0.02, 0.1)
    player.x = W / 2
    player.y = H - 60
    player.w = 20
    player.h = 24
...
function luna.draw()
    -- Stars background
    luna.graphics.setColor(0.3, 0.3, 0.4, 0.6)
    for i = 1, 60 do
        local sx = (i * 137) % W
        local sy = ((i * 251) + luna.timer.getTime() * (20 + i % 3 * 10)) % H
        luna.graphics.rectangle("fill", sx, sy, 2, 2)
    end

    -- Player
    if not gameOver then
        luna.graphics.setColor(0.3, 0.7, 1, 1)
    ...
    -- Player bullets
    luna.graphics.setColor(0.4, 1, 0.4, 1)
    for _, b in ipairs(bullets) do
        luna.graphics.rectangle("fill", b.x - 2, b.y, 4, 10)
    end

    -- Enemies
    luna.graphics.setColor(1, 0.3, 0.3, 1)
    for _, e in ipairs(enemies) do
        luna.graphics.rectangle("fill", e.x - e.w / 2, e.y - e.h / 2, e.w, e.h)
        luna.graphics.setColor(1, 0.6, 0, 1)
        luna.graphics.rectangle("fill", e.x - 4, e.y - 4, 8, 8)
        luna.graphics.setColor(1, 0.3, 0.3, 1)
    end
    ...
    for _, b in ipairs(enemyBullets) do
        luna.graphics.circle("fill", b.x, b.y, 4)
    end
    ...
    luna.graphics.print("Lives: " .. lives, 10, 30)
    luna.graphics.print("Wave: " .. wave, 10, 50)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), W - 90, 10)

    if gameOver then
        luna.graphics.setColor(1, 0.2, 0.2, 1)
        luna.graphics.print("GAME OVER", W / 2 - 60, H / 2 - 20, 2)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("Final Score: " .. score, W / 2 - 55, H / 2 + 20)
    end
end

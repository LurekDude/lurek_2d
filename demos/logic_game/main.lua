...
local function draw_grid()
    for r = 1, ROWS do
        for c = 1, COLS do
            ...
            if grid[r][c] == 1 then
                ...
                luna.graphics.setColor(0.18, 0.18, 0.22, 1)
                luna.graphics.rectangle("fill", x, y, TILE, TILE)
            end
            luna.graphics.setColor(0.25, 0.25, 0.3, 1)
            luna.graphics.rectangle("line", x, y, TILE, TILE)
        end
    end
    ...
    luna.graphics.circle("fill", fx, fy, 10)
    luna.graphics.setColor(0, 0, 0, 1)
    luna.graphics.print("F", fx - 4, fy - 7, 0.9)
end
...
local function draw_robot()
    ...
    -- body
    luna.graphics.setColor(0.2, 0.8, 0.9, 1)
    luna.graphics.circle("fill", rx, ry, 14)

    -- direction indicator
    local d = DIR[robot.dir]
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.circle("fill", rx + d[1] * 10, ry + d[2] * 10, 4)
end
...
local function draw_command_panel()
    ...
    luna.graphics.rectangle("fill", PANEL_X, 50, PANEL_W, H - 100)
    luna.graphics.setColor(0.3, 0.3, 0.4, 1)
    luna.graphics.rectangle("line", PANEL_X, 50, PANEL_W, H - 100)

    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("COMMANDS", PANEL_X + 10, 58, 1.1)
    ...
    for i, cmd in ipairs(CMD_LIST) do
        ...
        luna.graphics.setColor(c[1], c[2], c[3], 0.8)
        luna.graphics.rectangle("fill", bx, by, 62, 28)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(cmd, bx + 5, by + 5, 0.85)
    end

    -- program list
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("PROGRAM (" .. #program .. "/" .. MAX_PROGRAM .. ")", PANEL_X + 10, 135, 1)
    ...
    for i, cmd in ipairs(program) do
        ...
        if executing and i == exec_index then
            ...
            luna.graphics.rectangle("fill", PANEL_X + 8, py - 2, PANEL_W - 16, 26)
        end
        luna.graphics.setColor(c[1], c[2], c[3], 1)
        luna.graphics.rectangle("fill", PANEL_X + 12, py, 18, 18)
        luna.graphics.setColor(1, 1, 1, 1)
        local pointer = (executing and i == exec_index) and " >> " or ("  " .. i .. ". ")
        luna.graphics.print(pointer .. cmd, PANEL_X + 35, py, 0.9)
    end

    -- controls
    luna.graphics.setColor(0.6, 0.6, 0.6, 0.8)
    luna.graphics.print("ENTER = Run", PANEL_X + 10, H - 90, 0.85)
    luna.graphics.print("C = Clear   R = Reset", PANEL_X + 10, H - 70, 0.85)
end

function luna.draw()
    if all_done then
        luna.graphics.setColor(1, 1, 0.5, 1)
        luna.graphics.print("ALL LEVELS COMPLETE!", W / 2 - 100, H / 2 - 20, 1.5)
    ...
    -- level name
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print(levels[current_level].name, 10, 10, 1.2)
    ...
    if level_complete then
        ...
        luna.graphics.rectangle("fill", W / 2 - 150, H / 2 - 30, 300, 60)
        luna.graphics.setColor(0.2, 1, 0.3, 1)
        luna.graphics.print("LEVEL COMPLETE!", W / 2 - 70, H / 2 - 20, 1.4)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("Press N for next level", W / 2 - 65, H / 2 + 10, 0.9)
    end
end

...
- `luna.graphics.rectangle()` — witness stand, evidence panel, jury progress bar
- `luna.graphics.print()` — animated typewriter-style testimony text rendering
- `luna.graphics.setColor()` — flash banners for successful objections and contradiction reveals
- `luna.keyboard.isPressed()` — state-machine navigation across title, intro, testimony, and verdict screens
- `luna.event.quit()` — clean exit triggered from the title screen
- `luna.graphics.setBackgroundColor()` — courtroom backdrop theming per state

...
local function draw_cell(x, y, color, alpha)
    ...
    luna.graphics.setColor(color[1], color[2], color[3], alpha or 1)
    luna.graphics.rectangle("fill", px+1, py+1, CELL-2, CELL-2)
    luna.graphics.setColor(color[1]*1.4, color[2]*1.4, color[3]*1.4, alpha or 1)
    luna.graphics.rectangle("line", px+1, py+1, CELL-2, CELL-2)
end

function luna.draw()
    -- Board border
    luna.graphics.setColor(0.3, 0.3, 0.5)
    luna.graphics.rectangle("line", BOARD_X - 1, BOARD_Y - 1, COLS * CELL + 2, ROWS * CELL + 2)

    -- Grid ghost
    luna.graphics.setColor(0.12, 0.12, 0.18)
    for y = 0, ROWS - 1 do
        for x = 0, COLS - 1 do
            luna.graphics.rectangle("line", BOARD_X + x*CELL + 1, BOARD_Y + y*CELL + 1, CELL - 2, CELL - 2)
        end
    end
    ...
    luna.graphics.print("SCORE",  sx, BOARD_Y + 10, 1.5)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print(tostring(score), sx, BOARD_Y + 28, 1.8)
    luna.graphics.setColor(0.7, 0.7, 0.9)
    luna.graphics.print("LEVEL", sx, BOARD_Y + 65, 1.5)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print(tostring(level), sx, BOARD_Y + 82, 2)
    luna.graphics.setColor(0.7, 0.7, 0.9)
    luna.graphics.print("LINES", sx, BOARD_Y + 115, 1.5)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print(tostring(lines_cleared), sx, BOARD_Y + 132, 2)
    luna.graphics.setColor(0.7, 0.7, 0.9)
    luna.graphics.print("NEXT",  sx, BOARD_Y + 165, 1.5)
    if next_piece then
        ...
        for _, c in ipairs(next_piece.cells) do
            ...
            luna.graphics.setColor(next_piece.color[1], next_piece.color[2], next_piece.color[3])
            luna.graphics.rectangle("fill", px2+1, py2+1, CELL-2, CELL-2)
        end
    end

    -- Controls
    luna.graphics.setColor(0.4, 0.4, 0.5)
    luna.graphics.print("←→  Move",   sx, H - 110, 1.2)
    luna.graphics.print("↑   Rotate", sx, H - 95,  1.2)
    luna.graphics.print("↓   Soft",   sx, H - 80,  1.2)
    luna.graphics.print("SPC Hard",   sx, H - 65,  1.2)
    luna.graphics.print("ESC Quit",   sx, H - 50,  1.2)
    ...
    if game_over then
        ...
        luna.graphics.print("Press R to restart", W/2 - 100, H/2 + 50, 2)
    end
end

...
function luna.draw()
    luna.graphics.setBackgroundColor(0.08, 0.08, 0.14)
    ...
    for _, brick in ipairs(bricks) do
        if brick.alive then
            ...
            luna.graphics.rectangle("fill", brick.x, brick.y, brick.w, brick.h)
            luna.graphics.setColor(r * 0.6, g * 0.6, b * 0.6, 1)
            luna.graphics.rectangle("line", brick.x, brick.y, brick.w, brick.h)
        end
    end
    ...
    for _, pu in ipairs(powerups) do
        if pu.type == "wide" then luna.graphics.setColor(0.2, 0.6, 1, 1)
        elseif pu.type == "multi" then luna.graphics.setColor(1, 0.4, 1, 1)
        else luna.graphics.setColor(0.4, 1, 0.4, 1) end
        luna.graphics.rectangle("fill", pu.x - pu.w / 2, pu.y, pu.w, pu.h)
        luna.graphics.setColor(1, 1, 1, 0.8)
        local label = pu.type:sub(1, 1):upper()
        luna.graphics.print(label, pu.x - 3, pu.y)
    end
    ...
    if state.wide_timer > 0 then
        luna.graphics.setColor(0.5, 0.8, 1, 0.4)
        luna.graphics.rectangle("fill", paddle.x - 2, paddle.y - 2, paddle.w + 4, paddle.h + 4)
    end
    ...
    for _, ball in ipairs(balls) do
        ...
        if state.slow_timer > 0 then
            luna.graphics.setColor(0.4, 1, 0.4, 0.3)
            luna.graphics.circle("fill", ball.x, ball.y, ball.radius + 3)
        end
    end

    -- HUD
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Score: " .. state.score, 10, 10)
    luna.graphics.print("Level: " .. state.level, 10, 30)
    luna.graphics.print("Combo: x" .. state.combo, W / 2 - 30, 10)

    -- lives
    luna.graphics.setColor(1, 0.3, 0.3, 1)
    for i = 1, state.lives do
        luna.graphics.circle("fill", W - 20 * i, 20, 7)
    end

    -- serving prompt
    if state.serving and not state.game_over then
        luna.graphics.setColor(1, 1, 0.5, 0.7 + math.sin(luna.timer.getTime() * 5) * 0.3)
        luna.graphics.print("Click or press Space to launch!", W / 2 - 100, H / 2)
    end

    -- active effects
    if state.wide_timer > 0 then
        luna.graphics.setColor(0.2, 0.6, 1, 0.8)
        luna.graphics.print("Wide: " .. math.floor(state.wide_timer) .. "s", W - 100, 40)
    end
    if state.slow_timer > 0 then
        luna.graphics.setColor(0.4, 1, 0.4, 0.8)
        luna.graphics.print("Slow: " .. math.floor(state.slow_timer) .. "s", W - 100, 55)
    end
    ...
    if state.game_over then
        ...
        luna.graphics.setColor(1, 0.3, 0.3, 1)
        luna.graphics.print("GAME OVER", W / 2 - 60, H / 2 - 25, 2)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("Score: " .. state.score .. "  |  R to restart", W / 2 - 80, H / 2 + 15)
    end

    luna.graphics.setColor(0.5, 0.5, 0.5, 0.5)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), W - 70, H - 20)
end

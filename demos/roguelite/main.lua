...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.06, 0.05, 0.1)
    startRoom()
end
...
function luna.draw()
    -- Arena
    luna.graphics.setColor(0.1, 0.1, 0.15)
    luna.graphics.rectangle("fill", ARENA_X, ARENA_Y, ARENA_W, ARENA_H)
    luna.graphics.setColor(0.3, 0.25, 0.4)
    luna.graphics.rectangle("line", ARENA_X, ARENA_Y, ARENA_W, ARENA_H)
    -- Door indicator
    if roomCleared then
        luna.graphics.setColor(0.2, 0.8, 0.3, 0.7 + 0.3 * math.sin(luna.timer.getTime() * 4))
        luna.graphics.rectangle("fill", ARENA_X + ARENA_W / 2 - 15, ARENA_Y - 8, 30, 10)
    end
    -- Enemies
    for _, e in ipairs(enemies) do
        if e.hitTimer > 0 then luna.graphics.setColor(1, 1, 1)
        elseif e.kind == "boss" then luna.graphics.setColor(0.8, 0.1, 0.6)
        elseif e.kind == "fast" then luna.graphics.setColor(1, 0.6, 0.1)
        else luna.graphics.setColor(0.9, 0.2, 0.2) end
        luna.graphics.rectangle("fill", e.x, e.y, e.w, e.h)
        -- HP bar
        luna.graphics.setColor(0.3, 0.3, 0.3); luna.graphics.rectangle("fill", e.x, e.y - 6, e.w, 3)
        luna.graphics.setColor(0.9, 0.2, 0.2); luna.graphics.rectangle("fill", e.x, e.y - 6, e.w * (e.hp / e.maxHp), 3)
    end
    -- Player
    local blink = player.iframes > 0 and math.sin(luna.timer.getTime() * 25) > 0
    if not blink then
        if player.dashing then luna.graphics.setColor(0.5, 0.8, 1, 0.7)
        else luna.graphics.setColor(0.3, 0.8, 1) end
        luna.graphics.rectangle("fill", player.x, player.y, player.w, player.h)
    end
    -- Attack arc
    if player.atkDuration > 0 then
        luna.graphics.setColor(1, 1, 0.5, 0.6)
        local ax = player.x + player.w / 2 + math.cos(player.atkAngle) * 20
        local ay = player.y + player.h / 2 + math.sin(player.atkAngle) * 20
        luna.graphics.circle("fill", ax, ay, player.atkRange * 0.6)
    end
    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.7); luna.graphics.rectangle("fill", 0, 0, 800, 28)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("HP: " .. math.floor(player.hp) .. "/" .. player.maxHp .. "  Room: " .. roomNum .. "  Score: " .. score .. "  Kills: " .. killCount, 10, 6)
    -- HP bar
    luna.graphics.setColor(0.2, 0.2, 0.2); luna.graphics.rectangle("fill", 580, 6, 150, 16)
    luna.graphics.setColor(0.1, 0.7, 0.3); luna.graphics.rectangle("fill", 580, 6, 150 * clamp(player.hp / player.maxHp, 0, 1), 16)
    -- Room info
    if bossRoom and #enemies > 0 then
        luna.graphics.setColor(1, 0.3, 0.6); luna.graphics.print("!! BOSS !!", 370, 35, 1.5)
    end
    -- Perk selection
    if state == "perkSelect" then
        luna.graphics.setColor(0, 0, 0, 0.8); luna.graphics.rectangle("fill", 150, 180, 500, 220)
        luna.graphics.setColor(1, 0.9, 0.3); luna.graphics.print("ROOM CLEARED! Choose a perk:", 250, 200, 1.2)
        for i, p in ipairs(perkOptions) do
            local bx, by = 180, 230 + (i - 1) * 50
            luna.graphics.setColor(0.2, 0.2, 0.3); luna.graphics.rectangle("fill", bx, by, 440, 40)
            luna.graphics.setColor(0.3, 0.4, 0.6); luna.graphics.rectangle("line", bx, by, 440, 40)
            luna.graphics.setColor(1, 1, 1); luna.graphics.print("[" .. i .. "] " .. p.name, bx + 15, by + 12)
        end
    end
    -- Game over
    if state == "gameOver" then
        luna.graphics.setColor(0, 0, 0, 0.85); luna.graphics.rectangle("fill", 0, 0, 800, 600)
        luna.graphics.setColor(1, 0.2, 0.2); luna.graphics.print("GAME OVER", 300, 220, 2.5)
        luna.graphics.setColor(1, 1, 1); luna.graphics.print("Score: " .. score .. "  |  Best: " .. bestScore .. "  |  Rooms: " .. roomNum, 240, 310, 1.2)
        luna.graphics.print("Press R to restart", 320, 360)
    end

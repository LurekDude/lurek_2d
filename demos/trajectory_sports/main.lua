...
function luna.update(dt)
    ...
    -- charging power
    if luna.keyboard.isDown("space") and not ball.moving then
        state.charging = true
...
function luna.draw()
    luna.graphics.setBackgroundColor(0.15, 0.2, 0.35)

    -- sky gradient bands
    for i = 0, 5 do
        local t = i / 5
        luna.graphics.setColor(0.15 + t * 0.1, 0.2 + t * 0.15, 0.35 + t * 0.15, 1)
        luna.graphics.rectangle("fill", 0, i * 60, W, 60)
    end

    -- terrain fill
    luna.graphics.setColor(0.2, 0.5, 0.2, 1)
    for i = 1, #terrain - 1 do
        local a, b = terrain[i], terrain[i + 1]
    ...
    -- terrain outline
    luna.graphics.setColor(0.3, 0.65, 0.3, 1)
    luna.graphics.setLineWidth(2)
    for i = 1, #terrain - 1 do
        luna.graphics.line(terrain[i].x, terrain[i].y, terrain[i + 1].x, terrain[i + 1].y)
    end

    -- hole/target
    local hole = holes[state.current_hole]
    if hole then
        luna.graphics.setColor(0.1, 0.1, 0.1, 1)
        luna.graphics.circle("fill", hole.x, hole.y, hole.radius)
        luna.graphics.setColor(1, 0.8, 0, 1)
        luna.graphics.circle("line", hole.x, hole.y, hole.radius)
        -- flag
        luna.graphics.setColor(1, 0.2, 0.2, 1)
        luna.graphics.line(hole.x, hole.y - 30, hole.x, hole.y)
        luna.graphics.polygon("fill", { hole.x, hole.y - 30, hole.x + 15, hole.y - 22, hole.x, hole.y - 14 })
    end

    -- skid marks
    luna.graphics.setColor(0.15, 0.35, 0.15, 0.6)
    ...
    if not state.hole_complete then
        ...
        luna.graphics.circle("fill", ball.x, ball.y, ball.radius)
        luna.graphics.setColor(0.8, 0.8, 0.8, 1)
        luna.graphics.circle("line", ball.x, ball.y, ball.radius)
    end
    ...
    if not ball.moving and not state.hole_complete and not state.game_over then
        ...
        luna.graphics.line(ball.x, ball.y, mx, my)
    end

    -- power bar
    luna.graphics.setColor(0.2, 0.2, 0.2, 0.8)
    luna.graphics.rectangle("fill", 20, 20, 160, 20)
    local pf = state.power / 500
    local pr = clamp(pf * 2, 0, 1)
    local pg = clamp(2 - pf * 2, 0, 1)
    luna.graphics.setColor(pr, pg, 0, 1)
    luna.graphics.rectangle("fill", 22, 22, 156 * pf, 16)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Power", 70, 22)

    -- wind arrow
    local wind_str = "Wind: "
    if state.wind > 0 then wind_str = wind_str .. ">>>" else wind_str = wind_str .. "<<<" end
    luna.graphics.setColor(0.7, 0.9, 1, 1)
    luna.graphics.print(wind_str .. " " .. math.floor(math.abs(state.wind)), 20, 50)

    -- HUD
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Hole: " .. state.current_hole .. "/" .. state.total_holes, 20, 75)
    luna.graphics.print("Strokes: " .. state.hole_strokes, 20, 95)
    luna.graphics.print("Total: " .. state.strokes, 20, 115)

    -- distance to hole
    if hole and not state.hole_complete then
        local dist = math.floor(math.abs(ball.x - hole.x))
        luna.graphics.print("Distance: " .. dist .. "px", 20, 135)
    end

    -- message
    if state.message ~= "" and (state.hole_complete or state.game_over) then
        luna.graphics.setColor(1, 1, 0.3, 1)
        luna.graphics.print(state.message, W / 2 - 100, H / 2 - 20, 1.5)
    end

    -- controls
    luna.graphics.setColor(0.6, 0.6, 0.6, 0.6)
    luna.graphics.print("Aim: Mouse | Power: Hold Space | R: Restart", 200, H - 25)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), W - 80, 10)
end

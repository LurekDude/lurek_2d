...
- `luna.graphics.setColor()` / `luna.graphics.rectangle()` — underwater scene, tension bar, and HUD
- `luna.graphics.circle()` — fish sprites and bobber rendering
- `luna.timer.getTime()` — sinusoidal fish-pull oscillation during the reel phase
...
- Wave animation on the water surface uses a `waveOffset` driven by `luna.timer.getTime()`

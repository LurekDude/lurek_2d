...
function luna.load()
    luna.graphics.setBackgroundColor(0.4, 0.7, 1)
...
function luna.update(dt)
    ...
    if state == "hooked" and hookedFish then
        ...
        tensionSpeed = pull * (math.sin(luna.timer.getTime() * 3) * 0.5 + 0.5)
...
function luna.draw()
    ...
    luna.graphics.setColor(0.3, 0.55, 0.9, 1)
    luna.graphics.rectangle("fill", 0, 0, W, WATER_Y / 2)
    luna.graphics.setColor(0.5, 0.7, 1, 1)
    luna.graphics.rectangle("fill", 0, WATER_Y / 2, W, WATER_Y / 2)

    -- Sun
    luna.graphics.setColor(1, 0.9, 0.3, 1)
    luna.graphics.circle("fill", 650, 60, 35)

    -- Dock
    luna.graphics.setColor(0.5, 0.35, 0.15, 1)
    luna.graphics.rectangle("fill", playerX - 60, DOCK_Y, 120, 15)
    luna.graphics.rectangle("fill", playerX - 40, DOCK_Y + 15, 8, 30)
    luna.graphics.rectangle("fill", playerX + 32, DOCK_Y + 15, 8, 30)

    -- Player (stick figure on dock)
    luna.graphics.setColor(0.9, 0.8, 0.6, 1)
    luna.graphics.circle("fill", playerX, DOCK_Y - 20, 10)
    luna.graphics.setColor(0.2, 0.3, 0.6, 1)
    luna.graphics.rectangle("fill", playerX - 6, DOCK_Y - 10, 12, 20)

    -- Rod
    luna.graphics.setColor(0.5, 0.3, 0.1, 1)
    luna.graphics.setLineWidth(2)
    luna.graphics.line(playerX + 5, DOCK_Y - 15, playerX + 30, DOCK_Y - 45)

    -- Fishing line
    if lineOut then
        luna.graphics.setColor(0.8, 0.8, 0.8, 0.7)
        luna.graphics.setLineWidth(1)
        luna.graphics.line(playerX + 30, DOCK_Y - 45, lineX, lineY)
        -- Bobber
        luna.graphics.setColor(1, 0.2, 0, 1)
        luna.graphics.circle("fill", lineX, WATER_Y + math.sin(waveOffset + lineX * 0.1) * 3, 4)
    end

    -- Water surface with waves
    luna.graphics.setColor(0.1, 0.3, 0.7, 0.9)
    luna.graphics.rectangle("fill", 0, WATER_Y, W, H - WATER_Y)
    -- Wave lines
    luna.graphics.setColor(0.2, 0.4, 0.8, 0.4)
    luna.graphics.setLineWidth(1)
    for wy = WATER_Y, WATER_Y + 30, 8 do
        for wx = 0, W - 20, 20 do
            local yo = math.sin(waveOffset + wx * 0.08 + wy * 0.2) * 3
            luna.graphics.line(wx, wy + yo, wx + 15, wy + yo + 1)
        end
    end

    -- Depth markers
    luna.graphics.setColor(0.5, 0.6, 0.8, 0.3)
    for d = 50, maxDepth, 50 do
        local dy = WATER_Y + d
        if dy < H then
            luna.graphics.line(0, dy, W, dy)
            luna.graphics.print(d .. "m", 5, dy - 12, 0.6)
        end
    end
    ...
    for _, f in ipairs(fish) do
        if f.y < H + 20 then
            ...
            luna.graphics.setColor(c[1] * depth_fade, c[2] * depth_fade, c[3] * depth_fade, 0.8)
            -- Fish body (triangle-ish)
            local dir = f.vx >= 0 and 1 or -1
            local s = f.size
            luna.graphics.polygon("fill", {
                f.x + dir * s, f.y,
                f.x - dir * s, f.y - s * 0.5,
                f.x - dir * s, f.y + s * 0.5,
            })
    ...
    if casting then
        ...
        luna.graphics.rectangle("fill", playerX - 25, DOCK_Y - 70, 50, 10)
        luna.graphics.setColor(1, 1 - castPower / 100, 0, 1)
        luna.graphics.rectangle("fill", playerX - 25, DOCK_Y - 70, castPower / 2, 10)
    end

    -- Tension meter (when hooked)
    if state == "hooked" then
        luna.graphics.setColor(0, 0, 0, 0.8)
        luna.graphics.rectangle("fill", W / 2 - 110, 50, 220, 70)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("TENSION", W / 2 - 30, 55, 0.9)

        -- Tension bar
        luna.graphics.setColor(0.3, 0.3, 0.3, 1)
        luna.graphics.rectangle("fill", W / 2 - 95, 75, 190, 16)
        -- Sweet spot zone
        luna.graphics.setColor(0, 0.6, 0, 0.4)
        local zoneX = W / 2 - 95 + tensionMin / 100 * 190
        local zoneW = (tensionMax - tensionMin) / 100 * 190
        luna.graphics.rectangle("fill", zoneX, 75, zoneW, 16)
        -- Current tension
        local tColor = (tension > tensionMax or tension < tensionMin) and {1, 0, 0} or {0, 1, 0}
        luna.graphics.setColor(tColor[1], tColor[2], tColor[3], 1)
        local tx = W / 2 - 95 + tension / 100 * 190
        luna.graphics.rectangle("fill", tx - 3, 73, 6, 20)

        -- Reel progress
        luna.graphics.setColor(0.3, 0.3, 0.3, 1)
        luna.graphics.rectangle("fill", W / 2 - 95, 97, 190, 10)
        luna.graphics.setColor(0, 0.7, 1, 1)
        luna.graphics.rectangle("fill", W / 2 - 95, 97, reelProgress / 100 * 190, 10)
        luna.graphics.setColor(1, 1, 1, 0.7)
        luna.graphics.print("Up/Down keys", W / 2 - 45, 110, 0.6)
    end

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.75)
    luna.graphics.rectangle("fill", 0, 0, W, 40)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Day " .. day, 10, 5, 1)
    local mins = math.floor(dayTimer / 60)
    local secs = math.floor(dayTimer % 60)
    luna.graphics.print("Time: " .. mins .. ":" .. (secs < 10 and "0" or "") .. secs, 80, 8, 0.8)
    luna.graphics.print("Money: $" .. money, 200, 8, 0.8)

    -- Bait selector
    for i, b in ipairs(BAITS) do
        local sel = i == currentBait
        luna.graphics.setColor(b.color[1], b.color[2], b.color[3], sel and 1 or 0.4)
        luna.graphics.rectangle("fill", 320 + (i - 1) * 80, 5, 70, 20)
        luna.graphics.setColor(1, 1, 1, sel and 1 or 0.5)
        luna.graphics.print(i .. ":" .. b.name, 325 + (i - 1) * 80, 8, 0.7)
    end

    -- Catch log
    luna.graphics.setColor(0.6, 0.6, 0.6, 1)
    luna.graphics.print("Catches: " .. #catches, 600, 8, 0.75)
    if #catches > 0 then
        local logY = 45
        luna.graphics.setColor(0, 0, 0, 0.5)
        local logH = clamp(#catches, 1, 8) * 14 + 5
        luna.graphics.rectangle("fill", W - 160, logY, 155, logH)
        for i = clamp(#catches - 7, 1, #catches), #catches do
            local c = catches[i]
            luna.graphics.setColor(0.8, 0.9, 1, 1)
            luna.graphics.print(c.name .. " (" .. c.size .. ") $" .. c.value, W - 155, logY + (i - clamp(#catches - 7, 1, #catches)) * 14, 0.6)
        end
    end

    -- Bottom controls
    luna.graphics.setColor(0, 0, 0, 0.6)
    luna.graphics.rectangle("fill", 0, H - 22, W, 22)
    luna.graphics.setColor(0.7, 0.7, 0.7, 1)
    luna.graphics.print("A/D move | Space hold+release to cast | Up/Down reel tension | 1/2/3 bait", 10, H - 19, 0.7)

    -- Message
    if msgTimer > 0 then
        luna.graphics.setColor(1, 1, 0.5, clamp(msgTimer, 0, 1))
        luna.graphics.print(message, W / 2 - 80, H / 2 - 50, 1.1)
    end
end

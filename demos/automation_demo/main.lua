...
function luna.draw()
    ...
    luna.graphics.print("Automation Demo", 20, 20)

    -- Status
    luna.graphics.setColor(0.6, 1, 0.6)
    luna.graphics.print("Status: " .. status, 20, 50)

    -- Scripts loaded
    luna.graphics.setColor(0.8, 0.8, 0.8)
    local scripts = luna.simulator.getScripts()
    luna.graphics.print("Scripts loaded: " .. #scripts, 20, 80)
    for i, name in ipairs(scripts) do
        luna.graphics.print("  " .. i .. ". " .. name, 30, 80 + i * 20)
    end

    -- Controls
    luna.graphics.setColor(0.6, 0.8, 1)
    local y = 180
    luna.graphics.print("Controls:", 20, y)
    luna.graphics.print("  1 = Play demo_sequence", 30, y + 20)
    luna.graphics.print("  2 = Play toml_script", 30, y + 40)
    luna.graphics.print("  P = Pause / Resume", 30, y + 60)
    luna.graphics.print("  S = Stop", 30, y + 80)

    -- Log
    luna.graphics.setColor(0.7, 0.7, 0.7)
    local logY = y + 120
    luna.graphics.print("Log:", 20, logY)
    for i, entry in ipairs(log) do
        luna.graphics.print(entry, 30, logY + i * 18)
    end
end

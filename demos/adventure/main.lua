...
function luna.draw()
    ...
    -- room background
    luna.graphics.setBackgroundColor(room.bg[1], room.bg[2], room.bg[3])

    -- floor
    luna.graphics.setColor(room.bg[1] * 0.7, room.bg[2] * 0.7, room.bg[3] * 0.7, 1)
    luna.graphics.rectangle("fill", 0, 460, 800, 40)
    ...
    for _, obj in ipairs(room.objects) do
        ...
        if hover then
            luna.graphics.setColor(c[1] + 0.2, c[2] + 0.2, c[3] + 0.2, 1)
        else
            luna.graphics.setColor(c[1], c[2], c[3], 1)
        end
        luna.graphics.rectangle("fill", obj.x, obj.y, obj.w, obj.h)
        if hover then
            luna.graphics.setColor(1, 1, 0.5, 1)
            luna.graphics.rectangle("line", obj.x - 1, obj.y - 1, obj.w + 2, obj.h + 2)
        end
    end
    ...
    if hoverObj then
        ...
        luna.graphics.rectangle("fill", mx + 10, my - 22, #hoverObj.label * 8 + 10, 20)
        luna.graphics.setColor(1, 1, 0.8, 1)
        luna.graphics.print(hoverObj.label, mx + 15, my - 20)
    end

    -- inventory bar
    luna.graphics.setColor(0.1, 0.1, 0.12, 0.9)
    luna.graphics.rectangle("fill", 0, 520, 800, 80)
    luna.graphics.setColor(0.3, 0.3, 0.35, 1)
    luna.graphics.line(0, 520, 800, 520)
    luna.graphics.setColor(0.7, 0.7, 0.7, 1)
    luna.graphics.print("Inventory:", 10, 530)
    for i, item in ipairs(inventory) do
        local ix = 20 + (i - 1) * 90
        luna.graphics.setColor(0.25, 0.25, 0.3, 1)
        luna.graphics.rectangle("fill", ix, 550, 80, 30)
        luna.graphics.setColor(0.8, 0.8, 0.5, 1)
        luna.graphics.rectangle("line", ix, 550, 80, 30)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(item, ix + 8, 557)
    end

    -- dialog box
    if #dialogText > 0 then
        luna.graphics.setColor(0, 0, 0, 0.85)
        luna.graphics.rectangle("fill", 50, 460, 700, 50)
        luna.graphics.setColor(0.8, 0.8, 0.6, 1)
        luna.graphics.rectangle("line", 50, 460, 700, 50)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(dialogText, 65, 475)
    end

    -- room name
    luna.graphics.setColor(1, 1, 1, 0.4)
    luna.graphics.print(string.upper(currentRoom), 700, 10)

    -- cursor
    local mx, my = luna.mouse.getPosition()
    luna.graphics.setColor(1, 1, 1, 0.6)
    luna.graphics.circle("line", mx, my, 6)
end

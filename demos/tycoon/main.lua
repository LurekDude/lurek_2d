...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.12, 0.1, 0.08)
    initGrid()
end
...
function luna.draw()
    ...
    for y = 1, ROWS do
        for x = 1, COLS do
            ...
            elseif t == "door" then
                luna.graphics.setColor(0.5, 0.4, 0.15, 1)
            elseif t == "counter" then
                luna.graphics.setColor(0.7, 0.5, 0.2, 1)
            elseif t == "table" then
                luna.graphics.setColor(0.5, 0.35, 0.2, 1)
            elseif t == "kitchen" then
                luna.graphics.setColor(0.6, 0.2, 0.2, 1)
            else
                luna.graphics.setColor(0.2, 0.18, 0.15, 1)
            end
    ...
    for _, c in ipairs(customers) do
        ...
        elseif c.state == "eat" then
            ...
            luna.graphics.setColor(0.3, 0.6, 1, 1)
        end
        luna.graphics.circle("fill", c.x, c.y, 10)
    end
    ...
    if buildMode then
        ...
        if gx >= 2 and gx <= COLS - 1 and gy >= 2 and gy <= ROWS - 1 then
            ...
            luna.graphics.setColor(b.color[1], b.color[2], b.color[3], 0.4)
            luna.graphics.rectangle("fill", (gx - 1) * TILE, (gy - 1) * TILE, TILE - 1, TILE - 1)
        end
    end
    ...
    luna.graphics.rectangle("fill", COLS * TILE, 0, 200, ROWS * TILE)

    luna.graphics.setColor(1, 0.85, 0.2, 1)
    luna.graphics.print("$" .. money, px, 10, 1.3)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Day " .. day, px, 40)
    luna.graphics.print("Served: " .. customersServed, px, 60)
    luna.graphics.print("Satisfaction: " .. math.floor(satisfaction) .. "%", px, 80)
    luna.graphics.print("Cooks: " .. staff.cooks .. " ($" .. staffCost.cook .. "/d)", px, 110)
    luna.graphics.print("Waiters: " .. staff.waiters .. " ($" .. staffCost.waiter .. "/d)", px, 130)

    -- build panel
    luna.graphics.setColor(0.6, 0.8, 1, 1)
    luna.graphics.print("BUILD (B toggle):", px, 170)
    for i, b in ipairs(buildings) do
        local sel = (buildMode and i == buildType) and "> " or "  "
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(sel .. i .. ") " .. b.name .. " $" .. b.cost, px, 190 + i * 18)
    end

    luna.graphics.setColor(0.7, 0.7, 0.7, 1)
    luna.graphics.print("H=Hire Cook", px, 280)
    luna.graphics.print("J=Hire Waiter", px, 298)
    luna.graphics.print("Day: " .. math.floor(dayTimer) .. "/" .. dayLength, px, 330)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), px, 360)
end

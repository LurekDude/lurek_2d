...
function luna.draw()
    ...
    local sw = luna.graphics.getWidth()
    local sh = luna.graphics.getHeight()
    ...
    luna.graphics.push()
    luna.graphics.scale(scale, scale)

    -- Draw province colour buffer
    local ids = map:getProvinceIds()
    for _, pid in ipairs(ids) do
        local r, g, b = map:getProvinceColor(pid)
        luna.graphics.setColor(r / 255, g / 255, b / 255)
        ...
        luna.graphics.rectangle("fill", cx - side / 2, cy - side / 2, side, side)
    end

    -- Highlight selected province
    if selected then
        luna.graphics.setColor(1, 1, 0, 0.5)
        local cx, cy = map:getCentroid(selected)
        luna.graphics.circle("fill", cx, cy, 5)
    end
    ...
    luna.graphics.print("Province Demo -- click to select, right-click to path", 10, 10)
    luna.graphics.print("Mode: " .. mode_names[mode_idx] .. "  [M] to cycle", 10, 30)
    luna.graphics.print("Provinces: " .. map:getProvinceCount(), 10, 50)
    if selected then
        local terrain = data:getProperty(selected, "terrain") or "unknown"
        local owner = data:getProperty(selected, "owner") or 0
        luna.graphics.print("Selected: " .. selected .. "  terrain=" .. tostring(terrain) .. "  owner=" .. tostring(owner), 10, 70)
    end
end

...
local function draw_player(p, label)
    if p.stocks <= 0 then return end
    local alpha = (p.invuln > 0 and math.sin(luna.timer.getTime() * 20) > 0) and 0.3 or 1.0
    luna.graphics.setColor(p.cr, p.cg, p.cb, alpha)
    luna.graphics.rectangle("fill", p.x, p.y, p.w, p.h)
    -- eyes
    luna.graphics.setColor(1, 1, 1, alpha)
    local ex = (p.dir == 1) and (p.x + 18) or (p.x + 6)
    luna.graphics.rectangle("fill", ex, p.y + 10, 6, 6)
    -- attack flash
    if p.attacking then
        luna.graphics.setColor(1, 1, 0, 0.6)
        local reach = (p.atk_type == "smash") and 50 or 35
        local ax = (p.dir == 1) and (p.x + p.w) or (p.x - reach)
        luna.graphics.rectangle("fill", ax, p.y + 5, reach, p.h - 10)
    end
end

local function draw_hud(p, x_base, label)
    luna.graphics.setColor(p.cr, p.cg, p.cb, 1)
    luna.graphics.print(label, x_base, 10, 1.2)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print(tostring(math.floor(p.damage)) .. "%", x_base, 35, 1.8)
    for i = 1, 3 do
        if i <= p.stocks then
            luna.graphics.setColor(p.cr, p.cg, p.cb, 1)
            luna.graphics.circle("fill", x_base + (i - 1) * 20 + 8, 75, 7)
        else
            luna.graphics.setColor(0.3, 0.3, 0.3, 1)
            luna.graphics.circle("line", x_base + (i - 1) * 20 + 8, 75, 7)
        end
    end
end
...
function luna.draw()
    -- ground
    luna.graphics.setColor(0.25, 0.22, 0.3, 1)
    luna.graphics.rectangle("fill", 0, GROUND_Y, W, H - GROUND_Y)
    -- platforms
    luna.graphics.setColor(0.35, 0.3, 0.45, 1)
    for _, plat in ipairs(platforms) do
        luna.graphics.rectangle("fill", plat.x, plat.y, plat.w, plat.h)
    end
    draw_player(p1, "P1")
    draw_player(p2, "P2")
    draw_hud(p1, 30, "P1")
    ...
    if match_over then
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("PLAYER " .. winner .. " WINS!", W / 2 - 80, H / 2 - 20, 2)
        luna.graphics.print("Press R to rematch", W / 2 - 70, H / 2 + 20, 1)
    end
end

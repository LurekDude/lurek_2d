...
function luna.draw()
    ...
    for y = clamp(start_row, 1, GRID_H), clamp(end_row, 1, GRID_H) do
        for x = 1, GRID_W do
            ...
            if t ~= EMPTY then
                ...
                luna.graphics.setColor(c[1] * light, c[2] * light, c[3] * light, 1)
    ...
    if mining.active then
        ...
        luna.graphics.setColor(1, 1, 0, 0.8)
        luna.graphics.rectangle("fill", mx, my - 4, TILE_SIZE * pct, 3)
    end
    ...
    luna.graphics.rectangle("fill", sx, sy, TILE_SIZE, TILE_SIZE)

    -- HUD
    luna.graphics.setColor(1, 1, 1, 1)
    local depth = math.floor(player.y - SURFACE_Y)
    if depth < 0 then depth = 0 end
    luna.graphics.print("Depth: " .. depth .. "m", 10, 10)
    luna.graphics.print("Dirt:" .. inventory.dirt .. " Stone:" .. inventory.stone ..
        " Ore:" .. inventory.ore .. " Gem:" .. inventory.gem, 10, 28)
    luna.graphics.print("WASD:move  Click:mine  L:ladder", 10, 46)

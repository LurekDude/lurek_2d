...
- `luna.graphics.rectangle()` — tile map rendering with per-tile colour coding
- `luna.graphics.circle()` — creature sprites and player avatar on the overworld
- `luna.graphics.print()` — battle log, HP values, party summary HUD
- `luna.graphics.setColor()` — type-coloured creature indicators and HP bar gradient
...
- `luna.graphics.setBackgroundColor()` — distinct overworld vs. battle scene backgrounds

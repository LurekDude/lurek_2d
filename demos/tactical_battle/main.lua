...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.15, 0.12, 0.1)
    -- Player units (blue)
...
function luna.draw()
    ...
    for gx = 0, GRID - 1 do
        for gy = 0, GRID - 1 do
            ...
            if (gx + gy) % 2 == 0 then
                luna.graphics.setColor(0.25, 0.22, 0.18, 1)
            else
                luna.graphics.setColor(0.3, 0.27, 0.22, 1)
            end
            luna.graphics.rectangle("fill", px, py, TILE, TILE)
            luna.graphics.setColor(0.15, 0.12, 0.1, 1)
            luna.graphics.rectangle("line", px, py, TILE, TILE)
        end
    end

    -- Reachable tiles
    luna.graphics.setColor(0.2, 0.5, 0.9, 0.25)
    for _, r in ipairs(reachable) do
        luna.graphics.rectangle("fill", OX + r.x * TILE, OY + r.y * TILE, TILE, TILE)
    end

    -- Attackable highlights
    luna.graphics.setColor(0.9, 0.2, 0.2, 0.3)
    for _, a in ipairs(attackable) do
        luna.graphics.rectangle("fill", OX + a.gx * TILE, OY + a.gy * TILE, TILE, TILE)
    end
    ...
    for i, u in ipairs(units) do
        if u.hp > 0 then
            ...
            if u.class == "knight" then
                luna.graphics.rectangle("fill", px - 14, py - 14, 28, 28)
            else
                luna.graphics.circle("fill", px, py, 14)
            end
            -- Selection ring
            if selected == i then
                luna.graphics.setColor(1, 1, 0, 1)
                luna.graphics.setLineWidth(2)
                luna.graphics.circle("line", px, py, 20)
                luna.graphics.setLineWidth(1)
            end
            -- HP bar
            local hpFrac = u.hp / u.maxHp
            luna.graphics.setColor(0.2, 0.2, 0.2, 1)
            luna.graphics.rectangle("fill", px - 14, py + 18, 28, 4)
            luna.graphics.setColor(0.1, 0.9, 0.2, 1)
            luna.graphics.rectangle("fill", px - 14, py + 18, 28 * hpFrac, 4)
            -- Label
            luna.graphics.setColor(1, 1, 1, 1)
            local label = u.class == "knight" and "K" or "A"
            luna.graphics.print(label, px - 4, py - 6)
            -- Moved indicator
            if u.moved then
    ...
    -- HUD
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Turn: " .. turn, 10, 10)
    luna.graphics.print("[Enter] End Turn", 10, 30)
    luna.graphics.print("K=Knight(melee)  A=Archer(ranged)", 10, 560)

    -- Message
    if msgTimer > 0 then
        luna.graphics.setColor(1, 1, 0.5, clamp(msgTimer, 0, 1))
        luna.graphics.print(gameMessage, 240, 10, 1.2)
    end

    if gameOver then
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(gameMessage, 300, 280, 2)
    end
end

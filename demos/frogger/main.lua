...
function luna.draw()
    ...
    for _, lane in ipairs(LANES) do
        ...
        if lane.type == "safe" then
            luna.graphics.setColor(0.15, 0.3, 0.1)
        elseif lane.type == "road" then
            luna.graphics.setColor(0.2, 0.2, 0.2)
        else -- water
            luna.graphics.setColor(0.05, 0.15, 0.45)
        end
        luna.graphics.rectangle("fill", 0, ly, W, CELL)
    end

    -- Lane dividers (road)
    luna.graphics.setColor(0.5, 0.5, 0.1, 0.4)
    for _, lane in ipairs(LANES) do
        if lane.type == "road" then
            local ly = lane.row * CELL + CELL/2
            for x = 0, W, 30 do
                luna.graphics.rectangle("fill", x, ly - 2, 16, 4)
            end
        end
    end
    ...
    for i = 1, HOME_SLOTS do
        ...
        local hy = 4
        if homes[i] then
            luna.graphics.setColor(0.1, 0.8, 0.2)
        else
            luna.graphics.setColor(0.1, 0.4, 0.15)
        end
        luna.graphics.circle("fill", (i - 1) * slot_w + slot_w/2, CELL/2, 20)
    end
        ...
        if lane.type ~= "safe" then
            ...
            if objs then
                for _, obj in ipairs(objs) do
                    ...
                    luna.graphics.setColor(lane.color[1], lane.color[2], lane.color[3])
                    luna.graphics.rectangle("fill", obj.x, ly, obj.w, CELL - 8)
                    -- Highlight
                    if lane.type == "road" then
                        luna.graphics.setColor(1, 1, 0.5, 0.4)
                        luna.graphics.circle("fill", obj.x + 12, ly + 8, 6)
                        luna.graphics.circle("fill", obj.x + obj.w - 12, ly + 8, 6)
                    end
                end
            end
        end
    ...
    luna.graphics.rectangle("fill", fx + fw/4, fy, fw/2, fh)
    luna.graphics.rectangle("fill", fx, fy + fh/3, fw, fh/3)
    -- Eyes
    luna.graphics.setColor(1, 1, 0)
    luna.graphics.circle("fill", fx + fw/4, fy + 5, 4)
    luna.graphics.circle("fill", fx + fw*3/4, fy + 5, 4)

    -- HUD
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("Score: " .. score, 8, H - 20, 1.5)
    luna.graphics.setColor(1, 0.4, 0.4)
    local life_str = ""
    for i = 1, lives do life_str = life_str .. "🐸" end
    luna.graphics.print("Lives: " .. lives, W/2 - 50, H - 20, 1.5)
    luna.graphics.setColor(0.5, 0.8, 0.5)
    luna.graphics.print("Level " .. level, W - 90, H - 20, 1.5)

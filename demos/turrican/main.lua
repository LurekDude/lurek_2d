...
function luna.load()
    luna.graphics.setBackgroundColor(0.06, 0.04, 0.14)
...
function luna.update(dt)
    ...
    -- Beam weapon
    player.beam_active = luna.input.isKeyDown("z")
...
function luna.draw()
    ...
    for row = 1, MAP_H do
        for col = fc, lc do
            if tile_at(col, row) == "1" then
                ...
                luna.graphics.setColor(0.22, 0.14, 0.38)
                luna.graphics.rectangle("fill", sx, sy, TILE, TILE)
                luna.graphics.setColor(0.4, 0.25, 0.6)
                luna.graphics.rectangle("line", sx, sy, TILE, TILE)
            end
        end
    end
    ...
    if exit_tile.x then
        ...
        luna.graphics.setColor(0, 0.8, 1)
        luna.graphics.rectangle("fill", sx, exit_tile.y, exit_tile.w, exit_tile.h)
        luna.graphics.setColor(0, 0, 0)
        luna.graphics.print("EXIT", sx + 3, exit_tile.y + 8, 1.2)
    end
    ...
    for _, p in ipairs(powerups) do
        if p.alive then
            luna.graphics.setColor(0.9 * pulse, 0.5, 1)
            luna.graphics.circle("fill", p.x - cam + p.w/2, p.y + p.h/2, 10)
            luna.graphics.setColor(1, 1, 1)
            luna.graphics.print("+", p.x - cam + 6, p.y + 4, 1.4)
        end
    end
    ...
    for _, e in ipairs(enemies) do
        if e.alive then
            local sx = e.x - cam
            luna.graphics.setColor(0.7, 0.2, 0.4)
            luna.graphics.rectangle("fill", sx + 3, e.y + 6, e.w - 6, e.h - 6)
            luna.graphics.setColor(0.9, 0.5, 0.2)
            luna.graphics.circle("fill", sx + e.w/2, e.y + 7, 8)
            -- HP bar
            luna.graphics.setColor(0.2, 0, 0)
            luna.graphics.rectangle("fill", sx, e.y - 7, e.w, 5)
            luna.graphics.setColor(0.8, 0.1, 0.1)
            luna.graphics.rectangle("fill", sx, e.y - 7, e.w * (e.hp / 2), 5)
        end
    end
    ...
    for _, b in ipairs(bullets) do
        if not b.enemy then luna.graphics.rectangle("fill", b.x - cam - 2, b.y, 5, 8) end
    end
    luna.graphics.setColor(1, 0.3, 0.1)
    for _, b in ipairs(bullets) do
        if b.enemy then luna.graphics.rectangle("fill", b.x - cam - 2, b.y, 5, 5) end
    end

    -- Beam weapon
    if player.beam_active then
        local bx = player.x - cam + (player.facing > 0 and player.w or -BEAM_DIST)
        luna.graphics.setColor(0.1, 0.8, 1, 0.7)
        luna.graphics.rectangle("fill", bx, player.y + 10, BEAM_DIST, 12)
    end

    -- Player
    local px = player.x - cam
    luna.graphics.setColor(0.1, 0.5, 0.9)
    luna.graphics.rectangle("fill", px + 3, player.y + 10, player.w - 6, player.h - 10)
    luna.graphics.setColor(0.85, 0.75, 0.6)
    luna.graphics.circle("fill", px + player.w/2, player.y + 10, 11)
    -- Helmet
    luna.graphics.setColor(0.15, 0.15, 0.5)
    luna.graphics.rectangle("fill", px + 2, player.y, player.w - 4, 12)

    -- HUD
    luna.graphics.setColor(0, 0, 0, 0.6)
    luna.graphics.rectangle("fill", 0, 0, W, 30)
    luna.graphics.setColor(0.5, 0.3, 1)
    luna.graphics.print("TURRICAN", 8, 4, 1.8)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("Score: " .. score, W/2 - 50, 4, 1.6)
    -- Health bar
    for i = 1, MAX_HEALTH do
        local hx = W - 22 * i - 8
        luna.graphics.setColor(i <= health and 0.9 or 0.25, 0.2, 0.4)
        luna.graphics.rectangle("fill", hx, 5, 18, 20)
    end
    luna.graphics.setColor(0.4, 0.8, 1)
    luna.graphics.print("Lv " .. level, W/2 + 70, 4, 1.6)

    -- Controls hint
    luna.graphics.setColor(0.5, 0.5, 0.7, 0.7)
    luna.graphics.print("[A/D] Move  [W/Up/Space] Jump  [X] Shoot  [Z] Beam", 10, H - 22, 1.3)

    -- Overlay
    if game_state == "gameover" then
        luna.graphics.setColor(0, 0, 0, 0.75)
        luna.graphics.rectangle("fill", 0, 0, W, H)
        luna.graphics.setColor(0.6, 0.2, 1)
        luna.graphics.print("TURRICAN FALLS", W/2 - 115, H/2 - 25, 3)
        luna.graphics.setColor(1, 1, 1)

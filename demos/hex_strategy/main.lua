...
local function drawHex(cx, cy, size, mode)
    ...
    luna.graphics.polygon(mode, verts)
end
...
function luna.draw()
    ...
    for _, h in pairs(hexes) do
        ...
        local t = TERRAIN[h.terrain]
        luna.graphics.setColor(t.r, t.g, t.b, 1)
        drawHex(px, py, HEX_SIZE - 1, "fill")
        luna.graphics.setColor(0.1, 0.1, 0.1, 0.5)
        drawHex(px, py, HEX_SIZE - 1, "line")

        -- City marker
        if h.hasCity then
            luna.graphics.setColor(1, 0.85, 0.2, 1)
            luna.graphics.rectangle("fill", px - 8, py - 10, 16, 14)
            luna.graphics.setColor(0.9, 0.6, 0.1, 1)
            luna.graphics.polygon("fill", { px - 10, py - 10, px, py - 18, px + 10, py - 10 })
            luna.graphics.setColor(1, 1, 1, 1)
            luna.graphics.print("C", px - 4, py - 8)
        end
    end
    ...
    luna.graphics.rectangle("fill", 0, 0, 220, 130)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Turn: " .. turnNum, 10, 10)
    luna.graphics.setColor(1, 0.85, 0.2, 1)
    luna.graphics.print("Gold: " .. resources.gold, 10, 30)
    luna.graphics.setColor(0.5, 0.35, 0.15, 1)
    luna.graphics.print("Wood: " .. resources.wood, 10, 50)
    luna.graphics.setColor(0.4, 0.85, 0.3, 1)
    luna.graphics.print("Food: " .. resources.food, 10, 70)
    luna.graphics.setColor(0.8, 0.8, 0.8, 1)
    luna.graphics.print("Cities: " .. #cities, 10, 90)
    luna.graphics.print("[N] Next Turn  [C] Place City", 10, 110)
    ...
    if selected then
        ...
        if h then
            local t = TERRAIN[h.terrain]
            luna.graphics.setColor(0, 0, 0, 0.7)
            luna.graphics.rectangle("fill", 580, 0, 220, 110)
            luna.graphics.setColor(1, 1, 1, 1)
            luna.graphics.print("Hex (" .. h.q .. "," .. h.r .. ")", 590, 10)
            luna.graphics.print("Terrain: " .. h.terrain, 590, 30)
            luna.graphics.print("Gold/turn: " .. t.gold, 590, 50)
            luna.graphics.print("Wood/turn: " .. t.wood, 590, 70)
            luna.graphics.print("Food/turn: " .. t.food, 590, 90)
        end
    end
    ...
    if infoText ~= "" then
        ...
        luna.graphics.print(infoText, 240, 570)
    end
end

...
function luna.draw()
    ...
    for y = 1, ROWS do
        for x = 1, COLS do
            ...
            luna.graphics.setColor(c[1]*bright, c[2]*bright, c[3]*bright, 1)
    ...
    -- prey (deer)
    luna.graphics.setColor(0.6,0.45,0.2,1)
    for _, pr in ipairs(prey) do luna.graphics.circle("fill", pr.x, pr.y, 4) end

    -- predators (wolves)
    luna.graphics.setColor(0.5,0.1,0.1,1)
    for _, p in ipairs(predators) do luna.graphics.circle("fill", p.x, p.y, 5) end

    -- tribes
    for _, t in ipairs(tribes) do
        local g = t.hp / 100
        luna.graphics.setColor(0.2, 0.2 + g * 0.6, 1, 1)
        luna.graphics.circle("fill", t.x, t.y, 5)
    end

    -- storms
    for _, s in ipairs(storms) do
        luna.graphics.setColor(0.7,0.7,1, 0.3)
        luna.graphics.circle("fill", s.x, s.y, s.r)
        luna.graphics.setColor(1,1,0.5,0.8)
        luna.graphics.line(s.x, s.y - 20, s.x + 5, s.y + 10)
    end

    -- night overlay
    if is_night then
        luna.graphics.setColor(0, 0, 0.1, 0.35)
    ...
    luna.graphics.print("Day " .. day_count .. (is_night and " (Night)" or ""), 10, 5, 0.9)
    luna.graphics.print("Faith: " .. math.floor(faith), 150, 5, 0.9)
    luna.graphics.print("Food: " .. math.floor(food_supply), 290, 5, 0.9)
    luna.graphics.print("Pop: " .. #tribes, 400, 5, 0.9)
    luna.graphics.print("Wolves: " .. #predators .. "  Deer: " .. #prey, 490, 5, 0.9)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), 720, 5, 0.8)

    -- miracle bar
    luna.graphics.setColor(0, 0, 0, 0.7)
    luna.graphics.rectangle("fill", 0, H - 40, W, 40)
    for i, m in ipairs(MIRACLES) do
        local bx = 10 + (i-1) * 100
        if i == selected_miracle then
            luna.graphics.setColor(1,1,0.3,0.4)
            luna.graphics.rectangle("fill", bx, H-35, 90, 30)
        end
        luna.graphics.setColor(1,1,1,1)
        luna.graphics.print(m .. "(" .. MIRACLE_COST[m] .. ")", bx+5, H-28, 0.8)
    end
    luna.graphics.setColor(0.7,0.7,0.7,1)
    luna.graphics.print("[T+click] Build Temple(30)  [1-4] Select  [R] Restart", 420, H-28, 0.7)

    -- message
    if message and msg_timer > 0 then
        luna.graphics.setColor(1,1,0.5,1)
        luna.graphics.print(message, 300, 50, 1.1)
    end
end

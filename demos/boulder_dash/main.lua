...
function luna.load()
    luna.graphics.setBackgroundColor(0.04, 0.03, 0.01)
...
function luna.draw()
    ...
    luna.graphics.setColor(0.9, 0.7, 0.2)
    luna.graphics.print("BOULDER DASH", 8, 6, 1.6)
    luna.graphics.setColor(0.3, 0.8, 1)
    luna.graphics.print("Dia: " .. diamonds_got .. "/" .. diamonds_needed, W/2 - 60, 8, 1.5)
    luna.graphics.setColor(1, 0.4, 0.4)
    local tsec = math.max(0, math.floor(time_left))
    luna.graphics.print("Time: " .. tsec, W - 120, 8, 1.5)
    luna.graphics.setColor(0.7, 0.9, 0.7)
    luna.graphics.print("Score: " .. score, W/2 + 80, 8, 1.5)
    ...
    for y = 0, MAP_H - 1 do
        for x = 0, MAP_W - 1 do
            ...
            if c ~= EMPTY then
                ...
                if c == BOULDER then
                    luna.graphics.circle("fill", px + CELL/2, py + CELL/2, CELL/2 - 1)
                elseif c == DIAMOND then
                    -- Diamond shape
                    luna.graphics.circle("fill", px + CELL/2, py + CELL/2, CELL/2 - 2)
                    luna.graphics.setColor(0.7, 1, 1)
                    luna.graphics.circle("fill", px + CELL/2 - 2, py + CELL/2 - 2, 3)
                elseif c == EXIT then
                    local open = diamonds_got >= diamonds_needed
                    luna.graphics.setColor(open and 0.1 or 0.5, open and 0.9 or 0.5, open and 0.3 or 0.5)
                    luna.graphics.rectangle("fill", px + 2, py + 2, CELL - 4, CELL - 4)
                else
    ...
    luna.graphics.circle("fill", px + CELL/2, py + CELL/2, CELL/2 - 1)
    luna.graphics.setColor(0.2, 0.1, 0)
    luna.graphics.circle("fill", px + CELL/2 - 3, py + CELL/2 - 2, 2)
    luna.graphics.circle("fill", px + CELL/2 + 3, py + CELL/2 - 2, 2)
    ...
    if game_state == "gameover" then
        ...
        luna.graphics.print("Score: " .. score, W/2 - 55, H/2 + 15, 2)

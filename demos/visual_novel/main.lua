...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.1, 0.15, 0.1)
    build_scenes()
    go_to_scene("intro")
end
...
local function draw_portrait(speaker, side)
    ...
    local py = H - 260
    -- body
    luna.graphics.setColor(c[1], c[2], c[3], 0.9)
    luna.graphics.rectangle("fill", px, py + 40, 60, 80)
    -- head
    luna.graphics.circle("fill", px + 30, py + 20, 25)
    -- eyes
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.circle("fill", px + 22, py + 16, 4)
    luna.graphics.circle("fill", px + 38, py + 16, 4)
    luna.graphics.setColor(0, 0, 0, 1)
    luna.graphics.circle("fill", px + 23, py + 17, 2)
    luna.graphics.circle("fill", px + 39, py + 17, 2)
end

function luna.draw()
    -- background
    local bg = get_current_bg()
    luna.graphics.setColor(bg[1], bg[2], bg[3], 1)
    luna.graphics.rectangle("fill", 0, 0, W, H)

    if ending_text then
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print(ending_text, W / 2 - 180, H / 2 - 40, 1.1)
        luna.graphics.setColor(0.7, 0.7, 0.7, 1)
        luna.graphics.print("Press R to replay", W / 2 - 60, H / 2 + 60, 1)
        return
    end
    ...
    luna.graphics.rectangle("fill", 30, H - 160, W - 60, 130)
    luna.graphics.setColor(0.5, 0.5, 0.7, 1)
    luna.graphics.rectangle("line", 30, H - 160, W - 60, 130)

    -- speaker name
    local name = CHAR_NAMES[entry.speaker] or ""
    if name ~= "" then
        local nc = CHAR_COLORS[entry.speaker] or { 1, 1, 1 }
        luna.graphics.setColor(nc[1], nc[2], nc[3], 1)
        luna.graphics.print(name, 50, H - 155, 1.1)
    end

    -- text (typewriter)
    local shown = string.sub(entry.text, 1, char_index)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print(shown, 50, H - 130, 1)
    ...
    if choices_visible and entry.choices then
        for i, ch in ipairs(entry.choices) do
            ...
            if hover then
                luna.graphics.setColor(0.3, 0.3, 0.5, 0.9)
            else
                luna.graphics.setColor(0.15, 0.15, 0.25, 0.9)
            end
            luna.graphics.rectangle("fill", 60, cy, 440, 32)
            luna.graphics.setColor(1, 1, 0.8, 1)
            luna.graphics.print(i .. ". " .. ch.text, 75, cy + 6, 1)
        end
    end

    -- controls hint
    luna.graphics.setColor(0.5, 0.5, 0.5, 0.6)
    luna.graphics.print("Space=Advance  Tab=Skip  Click=Choose", 10, 10, 0.8)
    ...
    for _, key in ipairs({"luna_char", "sol", "nova"}) do
        if affection[key] > 0 then
            ...
            luna.graphics.setColor(nc[1], nc[2], nc[3], 0.7)
            local hearts = ""
            for h = 1, clamp(affection[key], 0, 5) do hearts = hearts .. "<3 " end
            luna.graphics.print(CHAR_NAMES[key] .. ": " .. hearts, W - 200, ay, 0.8)
            ay = ay + 18
        end
    end
end

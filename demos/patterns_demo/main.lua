...
function luna.draw()
    ...
    luna.graphics.print("Patterns Demo", 20, 20)
    luna.graphics.print("State: " .. (fsm:getCurrent() or "none"), 20, 40)
    luna.graphics.print("[1] Spawn enemy  [2] Acquire bullet  [3] Move +10", 20, 60)
    luna.graphics.print("[U] Undo  [R] Redo  [SPACE] Toggle play/menu  [G] Game over", 20, 78)

    local score = loc:locate("score")
    luna.graphics.print("Score: " .. (score and score.value or 0), 20, 100)
    luna.graphics.print("x = " .. x, 200, 100)
    luna.graphics.print("Pool: " .. pool:getAvailableCount() .. " avail / " .. pool:getActiveCount() .. " active", 20, 118)
    luna.graphics.print("History: " .. cmds:getHistorySize() .. " cmds", 300, 118)

    luna.graphics.setColor(0.7, 0.85, 1)
    for i, msg in ipairs(log) do
        luna.graphics.print(msg, 30, 140 + (i - 1) * 18)
    end
end

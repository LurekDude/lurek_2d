...
- `luna.graphics.polygon()` — tessellated track outer edge and car body rendering
- `luna.graphics.rectangle()` — boost zone highlight and HUD panels
- `luna.graphics.setColor()` — per-car colour differentiation and skid-mark fade
- `luna.graphics.print()` — lap counter, race time, best lap, and position HUD
- `luna.keyboard.isDown()` — analogue-style throttle, brake, and steering input each frame
- `luna.graphics.line()` — skid mark trail drawn as point history
- `luna.graphics.circle()` — checkpoint and boost zone radius indicators
- `luna.graphics.setBackgroundColor()` — dark asphalt track background

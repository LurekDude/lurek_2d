...
local function draw_car(c)
    ...
    luna.graphics.setColor(c.r, c.g, c.b, 1)
    luna.graphics.polygon("fill", corners)
    luna.graphics.setColor(1, 1, 1, 0.5)
    luna.graphics.polygon("line", corners)
end

function luna.draw()
    luna.graphics.setBackgroundColor(0.15, 0.18, 0.12)
    ...
    for i = 1, #track_outer - 1 do
        ...
        luna.graphics.polygon("fill", { a.x, a.y, b.x, b.y, c.x, c.y, d.x, d.y })
    end
    ...
    for i = 1, #track_outer - 1 do
        luna.graphics.line(track_outer[i].x, track_outer[i].y, track_outer[i + 1].x, track_outer[i + 1].y)
        luna.graphics.line(track_inner[i].x, track_inner[i].y, track_inner[i + 1].x, track_inner[i + 1].y)
    end

    -- boost zones
    for _, bz in ipairs(boost_zones) do
        if bz.active then
            luna.graphics.setColor(0, 1, 1, 0.4)
            luna.graphics.circle("fill", bz.x, bz.y, bz.radius)
            luna.graphics.setColor(0, 1, 1, 0.8)
            luna.graphics.circle("line", bz.x, bz.y, bz.radius)
        end
    end

    -- checkpoints
    for i, cp in ipairs(checkpoints) do
        luna.graphics.setColor(1, 1, 0, 0.3)
        luna.graphics.circle("fill", cp.x, cp.y, 10)
        luna.graphics.setColor(1, 1, 0, 0.7)
        luna.graphics.print(tostring(i), cp.x - 3, cp.y - 6)
    end

    -- skid marks
    luna.graphics.setColor(0.15, 0.15, 0.15, 0.5)
    for _, s in ipairs(skid_marks) do
        luna.graphics.circle("fill", s.x, s.y, 2)
    end
    ...
    draw_car(car)
    if car.boost > 0 then
        luna.graphics.setColor(0, 1, 1, 0.6)
        luna.graphics.circle("fill", car.x, car.y, 15)
    end

    -- HUD
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("Speed: " .. math.floor(car.speed), 10, 10)
    luna.graphics.print("Lap: " .. car.lap .. "/" .. state.laps_to_win, 10, 30)
    local lt = math.floor(car.lap_time * 10) / 10
    luna.graphics.print("Lap Time: " .. lt .. "s", 10, 50)
    luna.graphics.print("Drift: " .. math.floor(car.drift), 10, 70)

    if car.boost > 0 then
        luna.graphics.setColor(0, 1, 1, 1)
        luna.graphics.print("BOOST!", 10, 90)
    end

    -- mini map
    local mx, my, ms = W - 110, 10, 0.12
    luna.graphics.setColor(0, 0, 0, 0.5)
    luna.graphics.rectangle("fill", mx - 5, my - 5, 110, 85)
    luna.graphics.setColor(0.4, 0.4, 0.4, 0.8)
    for i = 1, #track_pts - 1 do
        local a, b = track_pts[i], track_pts[i + 1]
        luna.graphics.line(mx + a.x * ms, my + a.y * ms, mx + b.x * ms, my + b.y * ms)
    end
    luna.graphics.setColor(0, 0.8, 1, 1)
    luna.graphics.circle("fill", mx + car.x * ms, my + car.y * ms, 3)
    for _, ai in ipairs(ai_cars) do
        luna.graphics.setColor(ai.r, ai.g, ai.b, 1)
        luna.graphics.circle("fill", mx + ai.x * ms, my + ai.y * ms, 2)
    end

    -- finished message
    if state.finished then
        luna.graphics.setColor(1, 1, 0.3, 1)
        luna.graphics.print(state.message, W / 2 - 120, H / 2 - 15, 1.5)
    end

    luna.graphics.setColor(0.6, 0.6, 0.6, 0.5)
    luna.graphics.print("WASD: Drive | R: Restart", W / 2 - 80, H - 20)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), W - 80, H - 20)
end

...
function luna.draw()
    if mode == "build" then
        ...
        luna.graphics.setColor(0.3, 0.5, 0.3, 1)
        luna.graphics.rectangle("fill", 0, 280, 800, 20)
        luna.graphics.setColor(0.6, 0.6, 0.6, 1)
        luna.graphics.print("TRACK PREVIEW (press T to test)", 280, 140)

        -- Ramps in preview
        luna.graphics.setColor(0.5, 0.4, 0.2, 1)
        for _, r in ipairs(ramps) do
            local rx = r.x * 0.2 + 50
            if rx < 800 then
                luna.graphics.rectangle("fill", rx, 280 - r.h * 0.5, r.w * 0.3, r.h * 0.5)
            end
        end
        ...
        for gy = 0, BUILD_H - 1 do
            for gx = 0, BUILD_W - 1 do
                luna.graphics.rectangle("line", BUILD_OX + gx * GRID, BUILD_OY + gy * GRID, GRID, GRID)
            end
        end
        ...
        for _, p in ipairs(parts) do
            ...
            if p.type == "wheel" then
                luna.graphics.circle("fill", BUILD_OX + p.gx * GRID + GRID/2, BUILD_OY + p.gy * GRID + GRID/2, GRID/2 - 1)
            else
                luna.graphics.rectangle("fill", BUILD_OX + p.gx * GRID + 1, BUILD_OY + p.gy * GRID + 1, GRID - 2, GRID - 2)
            end
            -- Label
            luna.graphics.setColor(1, 1, 1, 0.7)
            local label = p.type:sub(1, 1):upper()
            luna.graphics.print(label, BUILD_OX + p.gx * GRID + 5, BUILD_OY + p.gy * GRID + 3)
        end
        ...
        if #parts > 0 then
            ...
            luna.graphics.circle("fill", BUILD_OX + cx * GRID + GRID/2, BUILD_OY + cy * GRID + GRID/2, 4)
            luna.graphics.print("CoM", BUILD_OX + cx * GRID + GRID/2 + 6, BUILD_OY + cy * GRID + GRID/2 - 6)
        end
        ...
        luna.graphics.print("Budget: $" .. (budget - getTotalCost()) .. "/" .. budget, 10, 568)
        luna.graphics.print("[1]Chassis($20) [2]Wheel($50) [3]Engine($80) [4]Wing($40)", 200, 568)
        luna.graphics.setColor(0.5, 1, 0.5, 1)
        luna.graphics.print("Selected: " .. selectedPart, 10, 548)
        luna.graphics.print("[T]Test [C]Clear [RClick]Remove", 500, 548)

    else
        -- Test mode
        luna.graphics.setBackgroundColor(0.3, 0.5, 0.8)

        local sx = -trackScroll

        -- Ground
        luna.graphics.setColor(0.3, 0.5, 0.3, 1)
        luna.graphics.rectangle("fill", sx, 280, 8000, 40)

        -- Ramps
        luna.graphics.setColor(0.6, 0.5, 0.2, 1)
        for _, r in ipairs(ramps) do
            luna.graphics.rectangle("fill", r.x + sx, 280 - r.h, r.w, r.h)
        end

        -- Obstacles
        luna.graphics.setColor(0.5, 0.3, 0.3, 1)
        for _, o in ipairs(obstacles) do
            luna.graphics.rectangle("fill", o.x + sx, 290 - o.h, o.w, o.h)
        end
        ...
        for _, p in ipairs(parts) do
            ...
            if p.type == "wheel" then
                luna.graphics.circle("fill", px + GRID/2, py + GRID/2, GRID/2)
            else
                luna.graphics.rectangle("fill", px, py, GRID, GRID)
            end
        end

        -- HUD
        luna.graphics.setColor(0, 0, 0, 0.7)
        luna.graphics.rectangle("fill", 0, 0, 800, 30)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("Distance: " .. testScore .. "m   [R] Return to build   [Left/Right] Brake/Accel", 10, 6)
    end

    -- Messages
    if testMsgTimer > 0 then
        luna.graphics.setColor(1, 1, 0.5, 1)
        luna.graphics.print(testMsg, 250, 150, 1.3)
    end
end

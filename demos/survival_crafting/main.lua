...
function luna.draw()
    ...
    for _, e in ipairs(enemies) do
        luna.graphics.rectangle("fill", (e.gx - 1) * TILE + 4, (e.gy - 1) * TILE + 4, TILE - 8, TILE - 8)
    end
    ...
    if mining.active then
        ...
        luna.graphics.rectangle("fill", bx, by, TILE, 6)
        luna.graphics.setColor(0, 1, 0.3, 1)
        luna.graphics.rectangle("fill", bx, by, TILE * (mining.progress / mining.needed), 6)
    end

    -- night overlay
    if nightAlpha > 0 then
        luna.graphics.setColor(0, 0, 0.05, nightAlpha)
    ...
    luna.graphics.print(dayStr .. " " .. dayNum .. "  HP:" .. math.floor(player.hp) .. "  Hunger:" .. math.floor(player.hunger), 10, ROWS * TILE + 5)
    luna.graphics.print("Wood:" .. inventory.wood .. " Stone:" .. inventory.stone .. " Berry:" .. inventory.berry .. " Pick:" .. inventory.pickaxe .. " Wall:" .. inventory.wall, 10, ROWS * TILE + 22)

    -- craft menu
    if craftOpen then
        luna.graphics.setColor(0, 0, 0, 0.8)
        luna.graphics.rectangle("fill", 250, 150, 300, 200)
        luna.graphics.setColor(1, 1, 0.6, 1)
        luna.graphics.print("CRAFTING (1-2 to craft, C to close)", 260, 160)
        for i, r in ipairs(recipes) do
            local parts = {}
            for k, v in pairs(r.needs) do parts[#parts + 1] = k .. "x" .. v end
            luna.graphics.setColor(1, 1, 1, 1)
            luna.graphics.print(i .. ") " .. r.name .. " = " .. table.concat(parts, " + "), 270, 185 + i * 22)
        end
    end
end

...
function luna.load()
    ...
    luna.graphics.setBackgroundColor(0.12, 0.18, 0.1)
    -- Player base
...
function luna.draw()
    -- Resources
    for _, r in ipairs(resources) do
        luna.graphics.setColor(1, 0.85, 0.2, 0.8)
        luna.graphics.circle("fill", r.x, r.y, r.r)
        luna.graphics.setColor(1, 1, 1, 0.7)
        luna.graphics.print(r.amount, r.x - 8, r.y - 6)
    end

    -- Buildings
    for _, b in ipairs(buildings) do
        if b.team == "player" then luna.graphics.setColor(0.2, 0.5, 1, 1)
        else luna.graphics.setColor(1, 0.3, 0.2, 1) end
        luna.graphics.rectangle("fill", b.x - 25, b.y - 25, 50, 50)
        luna.graphics.setColor(1, 1, 1, 1)
        luna.graphics.print("BASE", b.x - 16, b.y - 6)
        -- HP bar
        luna.graphics.setColor(0.2, 0.2, 0.2, 1)
        luna.graphics.rectangle("fill", b.x - 25, b.y - 32, 50, 5)
        luna.graphics.setColor(0.1, 0.9, 0.1, 1)
        luna.graphics.rectangle("fill", b.x - 25, b.y - 32, 50 * (b.hp / b.maxHp), 5)
    end
    ...
    for _, u in ipairs(units) do
        ...
        luna.graphics.circle("fill", u.x, u.y, sz)
        -- Worker gold indicator
        if u.kind == "worker" and u.carrying > 0 then
            luna.graphics.setColor(1, 0.9, 0.2, 1)
            luna.graphics.circle("fill", u.x, u.y - sz - 3, 3)
        end
        -- Selected ring
        for _, sid in ipairs(selected) do
            if sid == u.id then
                luna.graphics.setColor(1, 1, 0, 1)
                luna.graphics.circle("line", u.x, u.y, sz + 3)
            end
        end
        -- HP bar
        if u.hp < u.maxHp then
            luna.graphics.setColor(0.9, 0.1, 0.1, 1)
            luna.graphics.rectangle("fill", u.x - 8, u.y + sz + 2, 16 * (u.hp / u.maxHp), 2)
        end
    end
    ...
    luna.graphics.print("Gold: " .. gold, 10, 6)
    luna.graphics.setColor(1, 1, 1, 1)
    luna.graphics.print("[B] Worker (25g)  [S] Soldier (25g)  LClick:Select  RClick:Move", 180, 6)
    luna.graphics.print("FPS: " .. luna.timer.getFPS(), W - 80, 6)

    if gameOver then
        luna.graphics.setColor(0, 0, 0, 0.5)
        luna.graphics.rectangle("fill", 250, 260, 300, 60)
        luna.graphics.setColor(1, 1, 0.3, 1)
        luna.graphics.print(gameMsg, 330, 275, 2)
    end
end

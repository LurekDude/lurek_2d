...
function luna.load()
    luna.graphics.setBackgroundColor(0.04, 0.06, 0.04)
    reset()
end
...
function luna.draw()
    -- Header
    luna.graphics.setColor(0.08, 0.12, 0.08)
    luna.graphics.rectangle("fill", 0, 0, W, 40)
    luna.graphics.setColor(0.4, 0.9, 0.4)
    luna.graphics.print("SNAKE", 8, 8, 2)
    luna.graphics.setColor(1, 1, 1)
    luna.graphics.print("Score: " .. score, W/2 - 50, 10, 1.8)
    luna.graphics.setColor(0.6, 0.8, 0.6)
    luna.graphics.print("Best: " .. high_score, W - 100, 10, 1.5)

    -- Grid background
    luna.graphics.setColor(0.06, 0.08, 0.06)
    luna.graphics.rectangle("fill", 0, 40, W, ROWS * CELL)

    -- Subtle grid lines
    luna.graphics.setColor(0.09, 0.12, 0.09)
    for gx = 0, COLS do
        luna.graphics.line(gx * CELL, 40, gx * CELL, 40 + ROWS * CELL)
    end
    for gy = 0, ROWS do
        luna.graphics.line(0, 40 + gy * CELL, W, 40 + gy * CELL)
    end
    ...
    for _, f in ipairs(food) do
        ...
        luna.graphics.circle("fill", f[1] * CELL + CELL/2, f[2] * CELL + 40 + CELL/2, CELL/2 - 3)
        luna.graphics.setColor(0.2, 0.8, 0.2)
        luna.graphics.rectangle("fill", f[1] * CELL + CELL/2 - 1, f[2] * CELL + 40 + 2, 3, 5)
    end
    ...
    for i, seg in ipairs(snake) do
        ...
        if i == #snake then
            -- Head: brighter
            luna.graphics.setColor(0.4, 1.0, 0.4)
            luna.graphics.rectangle("fill", seg[1]*CELL + 1, seg[2]*CELL + 40 + 1, CELL - 2, CELL - 2)
            -- Eyes
            ...
            luna.graphics.circle("fill", ex, ey, 2)
        else
            luna.graphics.setColor(gr, gg, gb)
            luna.graphics.rectangle("fill", seg[1]*CELL + 2, seg[2]*CELL + 40 + 2, CELL - 4, CELL - 4)
        end
    end
    ...
    if game_state == "dead" then
        ...
        luna.graphics.setColor(1, 1, 1)
        luna.graphics.print("Score: " .. score, W/2 - 45, H/2 + 10, 2)
        luna.graphics.setColor(0.7, 0.7, 0.7)
        luna.graphics.print("Press R to restart", W/2 - 100, H/2 + 40, 2)
    end
end
